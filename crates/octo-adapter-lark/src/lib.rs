...
impl std::fmt::Debug for LarkConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LarkConfig").field("app_id", &self.app_id).field("app_secret", &"***").field("region", &self.region).field("groups", &self.groups).finish()
    }
}
...
impl LarkAdapter {
    ...
    pub fn max_payload_bytes() -> usize { 30_000 }
    pub fn rate_limit_per_second() -> u32 { 50 }
    ...
    async fn get_tenant_token(&self) -> Result<String, PlatformAdapterError> {
        ...
        let body = serde_json::json!({"app_id": self.config.app_id, "app_secret": self.config.app_secret});
        let resp = self.client.post(&url).json(&body).send().await
            .map_err(|e| transport_err(format!("Token failed: {e}")))?
            .json::<serde_json::Value>().await
            .map_err(|e| transport_err(format!("Token parse: {e}")))?;
        let token = resp["tenant_access_token"].as_str().ok_or_else(|| transport_err("Missing token"))?.to_string();
        let expire = resp["expire"].as_u64().unwrap_or(7200);
        *self.token_cache.lock() = Some((token.clone(), chrono::Utc::now().timestamp() as u64 + expire));
        Ok(token)
    }

    async fn send_message(&self, chat_id: &str, text: &str) -> Result<String, PlatformAdapterError> {
        ...
        let msg_id = resp["data"]["message_id"].as_str().unwrap_or("unknown").to_string();
        Ok(msg_id)
    }
}

fn transport_err(msg: impl Into<String>) -> PlatformAdapterError {
    PlatformAdapterError::Unreachable { platform: "lark".into(), reason: msg.into() }
}
...
impl PlatformAdapter for LarkAdapter {
    async fn send_envelope(&self, domain: &BroadcastDomainId, envelope: &DeterministicEnvelope) -> Result<DeliveryReceipt, PlatformAdapterError> {
        ...
        let chat_id = self.config.groups.iter()
    ...
    fn capabilities(&self) -> CapabilityReport {
        CapabilityReport {
            ...
            media_capabilities: Some(MediaCapabilities { max_upload_bytes: 52_428_800, supported_mime_types: vec!["image/jpeg".into(), "image/png".into(), "application/pdf".into()] }),
        }
    }

    fn domain_id(&self, pid: &str) -> BroadcastDomainId { BroadcastDomainId::new(PlatformType::Lark, pid) }
    fn platform_type(&self) -> PlatformType { PlatformType::Lark }
    fn self_handle(&self) -> Option<String> { None }

    async fn shutdown(&self) -> Result<(), PlatformAdapterError> { *self.token_cache.lock() = None; Ok(()) }
    async fn health_check(&self) -> Result<(), PlatformAdapterError> { self.get_tenant_token().await.map(|_| ()) }
}

#[no_mangle] pub extern "C" fn adapter_version() -> u32 { 1 }
#[no_mangle] pub extern "C" fn platform_type() -> u16 { 0x0013 }
...
#[no_mangle] pub unsafe extern "C" fn destroy_adapter(ptr: *mut ()) {
    if !ptr.is_null() { let _ = Box::from_raw(ptr as *mut LarkAdapter); }
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn test_domain_hash() { assert_eq!(LarkAdapter::domain_hash("oc123"), LarkAdapter::domain_hash("oc123")); assert_ne!(LarkAdapter::domain_hash("oc123"), LarkAdapter::domain_hash("oc456")); }
    #[test] fn test_encode_decode() { let d = b"test"; let e = LarkAdapter::encode_envelope(d); assert!(e.starts_with("DOT/1/")); assert_eq!(LarkAdapter::decode_envelope(&e).unwrap(), d); }
    #[test] fn test_platform_type() { assert_eq!(LarkAdapter::PLATFORM_TYPE, 0x0013); }
    #[test] fn test_abi() { assert_eq!(adapter_version(), 1); assert_eq!(platform_type(), 0x0013); }
    #[test] fn test_capabilities() { let a = LarkAdapter::new(LarkConfig { app_id: "".into(), app_secret: "".into(), region: LarkRegion::International, groups: vec![] }); assert_eq!(a.capabilities().max_payload_bytes, 30_000); assert!(!a.capabilities().supports_fragmentation); assert!(a.capabilities().media_capabilities.is_some()); }
    #[test] fn test_region_api_base() { let intl = LarkAdapter::new(LarkConfig { app_id: "".into(), app_secret: "".into(), region: LarkRegion::International, groups: vec![] }); assert!(intl.api_base().contains("larksuite")); let cn = LarkAdapter::new(LarkConfig { app_id: "".into(), app_secret: "".into(), region: LarkRegion::China, groups: vec![] }); assert!(cn.api_base().contains("feishu")); }
    #[test] fn test_self_handle_none() { assert!(LarkAdapter::new(LarkConfig { app_id: "".into(), app_secret: "".into(), region: LarkRegion::International, groups: vec![] }).self_handle().is_none()); }
    #[test] fn test_decode_missing_prefix() { assert!(LarkAdapter::decode_envelope("hello").is_err()); }
    #[test] fn test_config_from_json() { let a = LarkAdapter::from_config_bytes(serde_json::to_vec(&serde_json::json!({"app_id":"cli_123","app_secret":"s","region":"china","groups":["oc_1"]})).unwrap().as_slice()).unwrap(); assert_eq!(a.config.groups.len(), 1); }
}
