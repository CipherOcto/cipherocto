...
impl SlackAdapter {
    pub fn new(config: SlackConfig) -> Self {
        Self { config, client: reqwest::Client::new(), last_ts: Mutex::new(BTreeMap::new()), self_id: tokio::sync::Mutex::new(None) }
    }

    pub fn from_config_bytes(config: &[u8]) -> Result<Self, String> {
        let config: SlackConfig = serde_json::from_slice(config).map_err(|e| format!("Invalid config: {e}"))?;
        Ok(Self::new(config))
    }

    fn api_base() -> &'static str { "https://slack.com/api" }
    ...
    pub fn max_payload_bytes() -> usize { 40_000 }
    ...
    async fn send_message(&self, channel: &str, text: &str) -> Result<String, String> {
        ...
        let resp = self.client.post(format!("{}/chat.postMessage", Self::api_base()))
        ...
        if data["ok"] == true {
            ...
            Err(format!("Slack error: {}", data["error"].as_str().unwrap_or("unknown")))
        }
    }

    async fn get_messages(&self, channel: &str) -> Result<Vec<SlackMessage>, String> {
        let mut url = format!("{}/conversations.history?channel={}&limit=20", Self::api_base(), channel);
        ...
        let data: serde_json::Value = resp.json().await.map_err(|e| format!("JSON: {e}"))?;
        if data["ok"] != true { return Err(format!("Slack error: {}", data["error"].as_str().unwrap_or(""))); }
...
#[derive(Debug, Clone)]
struct SlackMessage { ts: String, text: String, user: String }

fn transport_err(msg: impl Into<String>) -> PlatformAdapterError {
    PlatformAdapterError::Unreachable { platform: "slack".into(), reason: msg.into() }
}
...
impl PlatformAdapter for SlackAdapter {
    async fn send_envelope(&self, domain: &BroadcastDomainId, envelope: &DeterministicEnvelope) -> Result<DeliveryReceipt, PlatformAdapterError> {
        ...
        for attempt in 0..=retry.max_retries {
            match self.send_message(channel, &encoded).await {
                Ok(ts) => return Ok(DeliveryReceipt { platform_message_id: ts, delivered_at: 0 }),
                Err(e) => {
                    if e.contains("rate_limited") && retry.should_retry(attempt) {
                        tokio::time::sleep(retry.delay_for_attempt(attempt)).await; continue;
                    }
    ...
    async fn receive_messages(&self, _domain: &BroadcastDomainId) -> Result<Vec<RawPlatformMessage>, PlatformAdapterError> {
        let mut messages = Vec::new();
            ...
            if let Ok(slack_msgs) = self.get_messages(ch).await {
                for m in slack_msgs {
                    if let Ok(payload) = Self::decode_envelope(&m.text) {
                        ...
                        messages.push(RawPlatformMessage { platform_id: m.ts.clone(), payload, metadata: meta });
                    }
                }
            }
        }
    ...
    fn canonicalize(&self, raw: &RawPlatformMessage) -> Result<DeterministicEnvelope, PlatformAdapterError> {
        ...
        DeterministicEnvelope::from_wire_bytes(&raw.payload).map_err(|e| PlatformAdapterError::ApiError { code: 400, message: format!("canonicalize: {e}") })
    }
    ...
    fn platform_type(&self) -> PlatformType { PlatformType::Slack }
    ...
    async fn health_check(&self) -> Result<(), PlatformAdapterError> {
        ...
        match tokio::time::timeout(timeout, self.client.get(&url).header("Authorization", format!("Bearer {}", self.config.bot_token)).send()).await {
            Ok(Ok(resp)) => {
...
#[no_mangle] pub extern "C" fn platform_type() -> u16 { 0x0007 }
...
#[no_mangle] pub unsafe extern "C" fn destroy_adapter(ptr: *mut ()) {
    if !ptr.is_null() { let _ = Box::from_raw(ptr as *mut SlackAdapter); }
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn test_domain_hash() { assert_eq!(SlackAdapter::domain_hash("C123"), SlackAdapter::domain_hash("C123")); assert_ne!(SlackAdapter::domain_hash("C123"), SlackAdapter::domain_hash("C456")); }
    #[test] fn test_encode_decode() { let d = b"test"; let e = SlackAdapter::encode_envelope(d); assert!(e.starts_with("DOT/1/")); assert_eq!(SlackAdapter::decode_envelope(&e).unwrap(), d); }
    #[test] fn test_platform_type() { assert_eq!(SlackAdapter::PLATFORM_TYPE, 0x0007); }
