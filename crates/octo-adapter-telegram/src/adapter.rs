...
impl<C: TelegramClient + Send + Sync> PlatformAdapter for TelegramAdapter<C> {
    ...
    async fn send_message(
        ...
        // PERF-H3: allocate wire buffer with known capacity (282 bytes typical).
        // L4: removed thread_local buffer — the previous implementation cloned
        // the buffer on every call, defeating the reuse intent. Direct
        // allocation is simpler and equally fast for this size.
        let mut wire = Vec::with_capacity(282);
        envelope_obj.write_wire_bytes(&mut wire);
