...
impl WeChatAdapter {
    ...
    async fn get_access_token(&self) -> Result<String, PlatformAdapterError> {
        ...
        *self.access_token.lock() = Some((
            token.clone(),
            (chrono::Utc::now().timestamp() as u64 + expires_in) * 1000,
        ));
        Ok(token)
    }
    ...
    async fn send_text(&self, openid: &str, text: &str) -> Result<String, PlatformAdapterError> {
        ...
        let url = format!(
            "{}/message/custom/send?access_token={}",
            Self::api_base(),
            token
        );
        let body =
            serde_json::json!({"touser": openid, "msgtype": "text", "text": {"content": text}});
        let resp = self
            .client
            .post(&url)
            .json(&body)
        ...
        if resp["errcode"].as_i64().unwrap_or(0) != 0 {
            return Err(transport_err(format!(
                "WeChat: {}",
                resp["errmsg"].as_str().unwrap_or("err")
            )));
        }
        Ok("ok".to_string())
    }
...
impl PlatformAdapter for WeChatAdapter {
    async fn send_envelope(
        ...
        Ok(DeliveryReceipt {
            platform_message_id: "wechat".to_string(),
    ...
    fn domain_id(&self, pid: &str) -> BroadcastDomainId {
        BroadcastDomainId::new(PlatformType::WeChat, pid)
    }
...
pub unsafe extern "C" fn destroy_adapter(ptr: *mut ()) {
    if !ptr.is_null() {
        let _ = Box::from_raw(ptr as *mut WeChatAdapter);
    }
}
...
mod tests {
    ...
    fn test_domain_hash() {
        assert_eq!(
            WeChatAdapter::domain_hash("g1"),
            WeChatAdapter::domain_hash("g1")
        );
        assert_ne!(
            WeChatAdapter::domain_hash("g1"),
            WeChatAdapter::domain_hash("g2")
        );
    }
    #[test]
    fn test_encode_decode() {
        let d = b"test";
        let e = WeChatAdapter::encode_envelope(d);
        assert!(e.starts_with("DOT/1/"));
        assert_eq!(WeChatAdapter::decode_envelope(&e).unwrap(), d);
    }
    #[test]
    fn test_platform_type() {
        assert_eq!(WeChatAdapter::PLATFORM_TYPE, 0x0011);
    }
    #[test]
    fn test_abi() {
        assert_eq!(adapter_version(), 1);
        assert_eq!(platform_type(), 0x0011);
    }
    #[test]
    fn test_capabilities() {
        let a = WeChatAdapter::new(WeChatConfig {
            app_id: "".into(),
            app_secret: "".into(),
            token: "".into(),
        ...
        assert_eq!(a.capabilities().max_payload_bytes, 2048);
        assert!(a.capabilities().supports_fragmentation);
    }
    #[test]
    fn test_self_handle_none() {
        assert!(WeChatAdapter::new(WeChatConfig {
            app_id: "".into(),
            app_secret: "".into(),
            token: "".into(),
            encoding_aes_key: None,
    ...
    fn test_decode_missing_prefix() {
        assert!(WeChatAdapter::decode_envelope("hello").is_err());
    }
    #[test]
    fn test_config_from_json() {
        let a = WeChatAdapter::from_config_bytes(
            serde_json::to_vec(
                &serde_json::json!({"app_id":"wx123","app_secret":"s","token":"t","groups":["g1"]}),
            )
            .unwrap()
            .as_slice(),
        )
        .unwrap();
        assert_eq!(a.config.groups, vec!["g1"]);
    }
}
