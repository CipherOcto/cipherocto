...
fn dom_adapter_atomic_observation_runtime() {
    ...
    use octo_audit::{compute_chain_hash, AppendOnlyAuditSink, AuditEvent, AuditEventKind};
    ...
    let mut event = AuditEvent {
        ...
        cap_root_hash: [0u8; CHAIN_HASH_SIZE],
        at_millis_unix: 1_000,
        prev_chain_hash: [0u8; CHAIN_HASH_SIZE],
        chain_hash: [0u8; CHAIN_HASH_SIZE],
    };
...
fn dom_adapter_storage_module_wiring() {
    ...
    assert!(
        storage_mod.contains("stoolap") || storage_mod.contains("pub mod stoolap"),
