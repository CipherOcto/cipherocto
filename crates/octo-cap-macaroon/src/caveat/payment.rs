...
//!
//! Attenuation invariant (RFC-0957 §Attenuation Invariant): `attenuate` only NARROWS —
...
/// Canonical OCTO-W asset_id (RFC-0105 v3.5 §3.1).
///
...
pub const OCTO_W_ASSET_ID_BYTES: [u8; 32] = [
    ...
    // at the substrate boundary. Per RFC-0105 v3.5 §3.1 with the
...
/// `DqaEncoding` wire form (per RFC-0862 §Substrate types).
///
/// **Mission E field additions** (RFC-0965 v2.1 §2.1):
...
pub struct PaymentCaveat {
    ...
    /// Asset bound to this caveat (RFC-0965 v2.1 §2.1). `AssetId`
...
impl PaymentCaveat {
    ...
    /// 4-arg attenuate per RFC-0965 v2.1 §2.2.
    ///
    /// Gates:
    ...
    ///
    /// Gates (RFC-0965 v2.1 §2.3, 8 gates):
    ...
    /// RFC-0105 v3.5 §3.13 + RFC-0965 v2.1 §2.3.
    pub fn validate(
        &self,
        current_epoch: Epoch,
        registry: &dyn AssetRegistry,
...
/// Reason a paid-query was rejected (RFC-0965 v2.1 §2.3).
///
...
/// Legacy `PaidQueryDecision` alias (RFC-0965 v2.1 §2.3).
#[deprecated(note = "use PaymentDecision per RFC-0965 v2.1 §2.3")]
pub type PaidQueryDecision = PaymentDecision;

/// Legacy `PaidQueryRejectionReason` alias (RFC-0965 v2.1 §2.3).
