...
pub(crate) struct AgentRecord {
    #[allow(dead_code)]
    // Deferred cleanup: separate mission scope.
...
pub fn transition_agent(
    ...
    let audit_result: Result<[u8; 32], WalletError> = Err(WalletError::AuditUnavailable(
        "octo-audit-internal feature not enabled (RFC-0015-a §6.5 paired-acceptance bridge)"
            .to_string(),
    ));
...
pub trait AgentRegistry: Send + Sync + std::fmt::Debug {
    ...
    ///   pattern (per RFC-0015-a §6.5 paired-acceptance bridge).
    ///   The gate is the compile-time cfg, not a runtime
    ///   registration step. Append failure triggers fail-CLOSED
    ///   rollback (never silently swallow the append).
    fn transition_agent(
        &self,
        caller_did: &Did,
        agent_id: Uuid,
...
mod tests {
    ...
    fn agent_registry_kind_returns_canonical_string() {
        // Per-impl diagnostic identity (RFC-0011-c §F.7.5); Arc<dyn> dispatch pins object-safety.
