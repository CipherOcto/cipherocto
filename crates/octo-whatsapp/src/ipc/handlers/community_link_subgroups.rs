//! `community.link_subgroups` — link existing groups as subgroups of a community.
...
#[derive(Debug)]
pub struct CommunityLinkSubgroups;

#[async_trait::async_trait]
impl RpcHandler for CommunityLinkSubgroups {
    ...
    async fn call(&self, h: DaemonHandle, params: Value) -> Result<Value, RpcError> {
        ...
        let result = adapter
            .community_link_subgroups(&p.community_jid, &p.subgroup_jids)
            .await
            .map_err(|e| RpcError {
                code: RpcErrorCode::InternalError.as_i32(),
                message: format!("community.link_subgroups failed: {e}"),
                data: None,
            })?;
        Ok(json!({
            "status": "linked",
...
mod tests {
    ...
    async fn empty_community_jid_rejected() {
        ...
        let err = CommunityLinkSubgroups
            .call(
                h,
                serde_json::json!({
                    "community_jid": "   ",
    ...
    async fn success_path_with_mock() {
        ...
        assert_eq!(mock.call_count("community_link_subgroups"), 1);
    }
}
