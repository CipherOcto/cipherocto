//! `labels.remove_chat_label` — detach a label from a chat.
...
#[derive(Debug)]
pub struct LabelsRemoveChatLabel;

#[async_trait::async_trait]
impl RpcHandler for LabelsRemoveChatLabel {
    ...
    async fn call(&self, h: DaemonHandle, params: Value) -> Result<Value, RpcError> {
        ...
        adapter
            .remove_chat_label(&p.label_id, &chat_jid)
            .await
            .map_err(|e| RpcError {
                code: RpcErrorCode::InternalError.as_i32(),
                message: format!("labels.remove_chat_label failed: {e}"),
