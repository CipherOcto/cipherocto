//! `newsletter.set_follower_mute` — mute / unmute a newsletter as a follower.
...
#[derive(Debug)]
pub struct NewsletterSetFollowerMute;

#[async_trait::async_trait]
impl RpcHandler for NewsletterSetFollowerMute {
    ...
    async fn call(&self, h: DaemonHandle, params: Value) -> Result<Value, RpcError> {
        ...
        adapter
            .set_follower_mute(&p.jid, p.muted)
            .await
            .map_err(|e| RpcError {
                code: RpcErrorCode::InternalError.as_i32(),
                message: format!("adapter set_follower_mute failed: {e}"),
                data: None,
            })?;
...
mod tests {
    ...
    async fn not_connected_returns_minus_32012() {
        let err = NewsletterSetFollowerMute
            .call(
                handle(),
                serde_json::json!({"jid": "120363012345678901@newsletter", "muted": true}),
            )
            .await
            .unwrap_err();
    ...
    async fn success_path_with_mock() {
        ...
        let r = NewsletterSetFollowerMute
            .call(
                h,
                serde_json::json!({"jid": "120363012345678901@newsletter", "muted": true}),
            )
        ...
        assert_eq!(r["muted"], true);
        assert_eq!(mock.call_count("set_follower_mute"), 1);
    }
}
