...
impl RpcHandler for SendAudio {
    ...
    async fn call(&self, h: DaemonHandle, params: Value) -> Result<Value, RpcError> {
        ...
        let (id, token) = adapter
            .send_audio_checked(&jid, &p.file, kind.max_bytes())
            .await
            .map_err(|e| RpcError {
                code: RpcErrorCode::NotConnected.as_i32(),
        ...
        let outbound_event = crate::events::InboundEvent::from_outbound_media(
            ...
            p.peer.clone(),
            jid.clone(),
            crate::events::MessageKind::Audio,
            None,
            Some(token.clone()),
            ts_unix_ms,
            ts_mono_ns,
...
mod tests {
    ...
    async fn not_connected_returns_minus_32012() {
        ...
        let f = tmp.path().join("audio.bin");
        std::fs::write(&f, b"hello-audio").unwrap();
        let err = SendAudio
            .call(
                handle(),
                serde_json::json!({
    ...
    async fn success_path_with_mock() {
        ...
        assert_eq!(r["message_id"], "fake-aud-msg-id");
        assert_eq!(r["media_ref_token"], "fake-aud-token");
