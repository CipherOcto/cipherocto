//! `tctoken.get_all_jids` — list all JIDs that have a stored
//! privacy token locally.
...
#[derive(Debug)]
pub struct TcTokenGetAllJids;

#[async_trait::async_trait]
impl RpcHandler for TcTokenGetAllJids {
    ...
    async fn call(&self, h: DaemonHandle, _params: Value) -> Result<Value, RpcError> {
        ...
        let jids = adapter
            ...
            .map_err(|e| RpcError {
                ...
                message: format!("adapter get_all_tc_token_jids failed: {e}"),
                data: None,
            })?;
        Ok(json!({
            "status": "ok",
...
mod tests {
    ...
    async fn success_path_with_mock() {
        let r = TcTokenGetAllJids
            .call(handle_with_mock(), serde_json::json!({}))
            .await
            .unwrap();
        assert_eq!(r["status"], "ok");
        assert_eq!(r["count"], 0);
        assert!(r["jids"].is_array());
    }
}
