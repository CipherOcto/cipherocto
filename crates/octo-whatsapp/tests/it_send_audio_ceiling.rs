//! Integration test for `send.audio` 16 MiB ceiling.
...
async fn send_audio_one_byte_over_ceiling_is_rejected() {
    ...
    let media_file = tmp.path().join("over.mp3");
    let bytes = vec![0u8; MediaKind::Audio.max_bytes() + 1];
    ...
    assert_eq!(err["data"]["max_bytes"], MediaKind::Audio.max_bytes());
    assert_eq!(err["data"]["kind"], "audio");
}
