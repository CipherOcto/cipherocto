//! Integration test for `send.video` 16 MiB ceiling.
...
async fn send_video_one_byte_over_ceiling_is_rejected() {
    ...
    let media_file = tmp.path().join("over.mp4");
    let bytes = vec![0u8; MediaKind::Video.max_bytes() + 1];
    ...
    assert_eq!(err["data"]["max_bytes"], MediaKind::Video.max_bytes());
    assert_eq!(err["data"]["kind"], "video");
}
