//! Integration test for `send.voice` 16 MiB ceiling.
...
async fn send_voice_one_byte_over_ceiling_is_rejected() {
    ...
    let media_file = tmp.path().join("over.ogg");
    let bytes = vec![0u8; MediaKind::Voice.max_bytes() + 1];
    ...
    assert_eq!(err["data"]["max_bytes"], MediaKind::Voice.max_bytes());
    assert_eq!(err["data"]["kind"], "voice");
}
