...
mod tests {
    ...
    async fn completion_success() {
        ...
        let mut r = req("mistral-large-latest");
        r.api_base = Some(s.base_url());
        let resp = MistralProvider::new().completion(&r, Some("k")).await.unwrap();
    ...
    async fn completion_auth_401() {
        ...
        r.api_base = Some(s.base_url());
        assert!(matches!(
            MistralProvider::new().completion(&r, Some("k")).await.unwrap_err(),
    ...
    async fn streaming_completion_auth_error() {
        ...
        let mut r = req("m");
        r.api_base = Some(s.base_url());
        assert!(MistralProvider::new().streaming_completion(&r, Some("k")).await.is_err());
    }
    ...
    async fn streaming_completion_success() {
        ...
        let mut r = req("m");
        r.api_base = Some(s.base_url());
        let mut resp = MistralProvider::new().streaming_completion(&r, Some("k")).await.unwrap();
