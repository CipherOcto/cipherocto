// cerebras — Cerebras Python SDK via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the official Cerebras Python SDK via PyO3.
...
/// Cerebras provider via official Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl CerebrasProvider {
    ...
    pub fn completion(
        ...
        let api_base = self
            .api_base
            .as_deref()
            .unwrap_or("https://api.cerebras.ai");

        Python::with_gil(|py| {
            // Import Cerebras SDK
            let cerebras = PyModule::import(py, "cerebras")
                .map_err(|e| PyBridgeError::PyError(format!("Failed to import cerebras: {}", e)))?;

            let cerebras_class = cerebras.getattr("Cerebras").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to get Cerebras class: {}", e))
            })?;
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for CerebrasProvider {
    fn name(&self) -> &str {
        "cerebras"
    }
