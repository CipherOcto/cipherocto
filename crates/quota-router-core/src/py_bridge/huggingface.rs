// huggingface — Hugging Face InferenceClient via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the Hugging Face Python SDK via PyO3.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// Per RFC-0917 lines 220-221:
// "Hugging Face | `huggingface_hub` Python SDK | Official Hugging Face SDK"
...
/// Hugging Face provider via official Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl HuggingFaceProvider {
    ...
    /// Call Hugging Face completion via Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        let api_base = self
            .api_base
            .as_deref()
            .unwrap_or("https://api.huggingface.co/v1");

        Python::with_gil(|py| {
            // Import Hugging Face SDK
            let hf = PyModule::import(py, "huggingface_hub").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to import huggingface_hub: {}", e))
            })?;

            let inference_class = hf.getattr("InferenceClient").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to get InferenceClient class: {}", e))
            })?;

            // Create client with api_key and base_url
            ...
            // Build messages string for chat method
            // HuggingFace InferenceClient.chat expects messages as list of dicts
            ...
            // Call client.chat.chat(model, messages)
            let chat = client
                .getattr("chat")
...
/// Convert Python HuggingFace response to Rust ChatCompletion
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
fn convert_response(
    py_obj: &PyAny,
    _py: Python<'_>,
) -> Result<crate::types::ChatCompletion, PyBridgeError> {
    // HuggingFace InferenceClient.chat returns: { id, model, choices: [{message: {role, content}, finish_reason}], usage, created }
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for HuggingFaceProvider {
    fn name(&self) -> &str {
        "huggingface"
    }
