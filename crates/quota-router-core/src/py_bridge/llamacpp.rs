// llamacpp — llama.cpp server via OpenAI-compatible API (INTERNAL boundary #1 per RFC-0917)
//
// This module calls a llama.cpp server using the OpenAI Python SDK with a custom base_url.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// llama.cpp servers expose an OpenAI-compatible API at localhost:8080 by default.
...
impl LlamaCppProvider {
    ...
    /// Call llama.cpp completion via OpenAI Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        Python::with_gil(|py| {
            ...
            // Create client with custom base_url for llama.cpp server
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for LlamaCppProvider {
    fn name(&self) -> &str {
        "llamacpp"
    }
