// llamafile — llamafile server via OpenAI-compatible API (INTERNAL boundary #1 per RFC-0917)
//
// This module calls a llamafile server using the OpenAI Python SDK with a custom base_url.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// llamafile servers expose an OpenAI-compatible API at localhost:8080 by default.
...
/// Llamafile provider via OpenAI-compatible API
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl LlamaFileProvider {
    ...
    /// Call llamafile completion via OpenAI Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        Python::with_gil(|py| {
            ...
            // Create client with custom base_url for llamafile server
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for LlamaFileProvider {
    fn name(&self) -> &str {
        "llamafile"
    }
