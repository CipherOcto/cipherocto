// lmstudio — LM Studio server via OpenAI-compatible API (INTERNAL boundary #1 per RFC-0917)
//
// This module calls an LM Studio server using the OpenAI Python SDK with a custom base_url.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// LM Studio servers expose an OpenAI-compatible API at localhost:1234/v1 by default.
...
impl LMStudioProvider {
    ...
    /// Call LM Studio completion via OpenAI Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        Python::with_gil(|py| {
            ...
            // Create client with custom base_url for LM Studio server
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for LMStudioProvider {
    fn name(&self) -> &str {
        "lmstudio"
    }
