// mistral — Mistral Python SDK via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the official Mistral Python SDK via PyO3.
...
/// Mistral provider via official Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl MistralProvider {
    ...
    /// Call Mistral completion via Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        Python::with_gil(|py| {
            ...
            let mistral = PyModule::import(py, "mistralai").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to import mistralai: {}", e))
            })?;

            let mistral_class = mistral.getattr("Mistral").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to get Mistral class: {}", e))
            })?;
...
/// Convert Python Mistral response to Rust ChatCompletion
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
fn convert_response(
    py_obj: &PyAny,
    _py: Python<'_>,
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for MistralProvider {
    fn name(&self) -> &str {
        "mistral"
    }
