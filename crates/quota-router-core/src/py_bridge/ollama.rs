// ollama — Ollama via Python SDK or OpenAI-compatible API (INTERNAL boundary #1 per RFC-0917)
//
// This module calls Ollama via the `ollama` Python SDK or OpenAI-compatible interface.
...
/// Ollama provider via official Python SDK or OpenAI-compatible interface
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl OllamaProvider {
    ...
    pub fn completion(
        ...
        // Ollama typically uses "ollama" as API key or no auth
        let api_key = self.api_key.clone().unwrap_or_else(|| "ollama".to_string());

        Python::with_gil(|py| {
            // Try importing the official ollama SDK first, fall back to openai with base_url
            let result = PyModule::import(py, "ollama").and_then(|ollama| ollama.getattr("Ollama"));
            ...
            match result {
                ...
                Err(_) => {
                    // Fall back to OpenAI SDK with custom base_url for Ollama's OpenAI-compatible endpoint
                    ...
                    // Create client with custom base_url for Ollama
                    let kwargs = PyDict::new(py);
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for OllamaProvider {
    fn name(&self) -> &str {
        "ollama"
    }
