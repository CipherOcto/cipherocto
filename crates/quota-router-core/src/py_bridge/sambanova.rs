// sambanova — SambaNova via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the SambaNova API via PyO3 using OpenAI SDK.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// Per RFC-0917 lines 220-221:
// "SambaNova | `openai` Python SDK with base_url | OpenAI-compatible API"
...
/// SambaNova provider via OpenAI-compatible Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl SambaNovaProvider {
    ...
    /// Call SambaNova completion via Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        let api_base = self
            .api_base
            .as_deref()
            .unwrap_or("https://api.sambanova.solutions/api/v1");

        Python::with_gil(|py| {
            // Import OpenAI SDK (SambaNova uses OpenAI-compatible API)
            ...
            // Create client with SambaNova base_url
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for SambaNovaProvider {
    fn name(&self) -> &str {
        "sambanova"
    }
