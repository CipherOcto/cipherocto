// together — Together AI Python SDK via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the official Together AI Python SDK via PyO3.
...
/// Together AI provider via official Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl TogetherProvider {
    ...
    pub fn completion(
        ...
        Python::with_gil(|py| {
            ...
            let client_class = openai.getattr("OpenAI").map_err(|e| {
...
fn convert_response(
    ...
    let model_str: String = py_obj
        ...
        .extract()
        .unwrap_or_else(|_| "together".to_string());
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl crate::py_bridge::openai::PyBridgeProvider for TogetherProvider {
    fn name(&self) -> &str {
        "together"
    }
