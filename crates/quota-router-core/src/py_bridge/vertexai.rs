// vertexai — Google Vertex AI via PyO3 (INTERNAL boundary #1 per RFC-0917)
//
// This module calls the Google Vertex AI Python SDK via PyO3.
...
/// Vertex AI provider via official Python SDK
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl VertexAIProvider {
    ...
    /// Call Vertex AI completion via Python SDK
    pub fn completion(
        &self,
        model: &str,
        ...
        let api_base = self
            .api_base
            .as_deref()
            .unwrap_or("https://aiplatform.googleapis.com/v1");

        Python::with_gil(|py| {
            // Import Vertex AI SDK
            let vertexai = PyModule::import(py, "vertexai")
                .map_err(|e| PyBridgeError::PyError(format!("Failed to import vertexai: {}", e)))?;

            let vertexai_class = vertexai.getattr("VertexAI").map_err(|e| {
                PyBridgeError::PyError(format!("Failed to get VertexAI class: {}", e))
            })?;

            // Create client
            let kwargs = PyDict::new(py);
            kwargs.set_item("project", api_key).unwrap(); // project_id as api_key
            kwargs.set_item("location", "us-central1").unwrap();
            kwargs.set_item("base_url", api_base).unwrap();

            let client = vertexai_class
                .call((), Some(kwargs))
