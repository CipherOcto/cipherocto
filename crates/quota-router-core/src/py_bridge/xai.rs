// xai — xAI via Python SDK or OpenAI-compatible API (INTERNAL boundary #1 per RFC-0917)
//
// This module calls xAI via the `xai` Python SDK or OpenAI-compatible interface.
// It is called by python_sdk_entry (EXTERNAL boundary #2).
//
// Per RFC-0917 lines 220-221:
// "xAI | `xai` Python SDK | Official xAI SDK"
...
/// xAI provider via official Python SDK or OpenAI-compatible interface
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
...
impl XaiProvider {
    ...
    /// Call xAI completion via Python SDK or OpenAI-compatible API
    pub fn completion(
        &self,
        model: &str,
        ...
        let api_base = self.api_base.as_deref().unwrap_or("https://api.x.ai/v1");

        Python::with_gil(|py| {
            // Try importing the official xai SDK first, fall back to openai with base_url
            let result = PyModule::import(py, "xai").and_then(|xai| xai.getattr("xAI"));
            ...
            match result {
                Ok(xai_class) => {
                    ...
                    let client = xai_class.call((), Some(kwargs)).map_err(|e| {
                ...
                Err(_) => {
                    // Fall back to OpenAI SDK with custom base_url for xAI's OpenAI-compatible endpoint
                    ...
                    // Create client with custom base_url for xAI
                    let kwargs = PyDict::new(py);
                    kwargs.set_item("api_key", api_key).unwrap();
                    kwargs.set_item("base_url", api_base).unwrap();
...
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
#[cfg(any(feature = "any-llm-mode", feature = "full"))]
impl PyBridgeProvider for XaiProvider {
    fn name(&self) -> &str {
        "xai"
    }
