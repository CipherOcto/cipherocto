...
async fn l2_inbound_ttl_exceeded_via_route() {
    ...
    // Seed node 0's gossip cache with node 1's gpt-4o capability.
    {
        ...
        node.gossip_cache.lock().unwrap().merge(
            RouterNodeId([2u8; 32]),
            vec![quota_router_core::node::provider::ProviderCapacity {
                provider_id: quota_router_core::node::provider::ProviderId([2u8; 32]),
                provider_name: "peer1".into(),
                router_node_id: RouterNodeId([2u8; 32]),
                models: vec!["gpt-4o".into()],
                requests_remaining: 100,
                pricing: vec![quota_router_core::node::provider::ModelPricing {
                    model: "gpt-4o".into(),
    ...
    let ctx = quota_router_integration_tests::make_request("gpt-4o");
