    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.azure.com/".to_string(),
...
impl LLMProvider for AZUREProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "azure") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("azure package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("azure: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
