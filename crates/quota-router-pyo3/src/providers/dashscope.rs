    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.dashscope.com/".to_string(),
                env_api_key: "DASHSCOPE_API_KEY".to_string(),
                env_api_base: Some("DASHSCOPE_BASE_URL".to_string()),
                api_base: None,
...
impl LLMProvider for DASHSCOPEProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| {
            match PyModule::import(py, "dashscope") {
                Ok(_) => Ok(()),
    ...
    fn embedding(&self, _input: &[String], _model: &str) -> Result<EmbeddingsResponse, ProviderError> {
        Err(ProviderError::new(
            "dashscope does not support embeddings",
            "dashscope",
        ))
    }
