    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.databricks.com/".to_string(),
...
impl LLMProvider for DATABRICKSProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "databricks") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("databricks package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("databricks: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
