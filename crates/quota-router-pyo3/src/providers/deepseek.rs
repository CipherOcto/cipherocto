    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.deepseek.com/".to_string(),
...
impl LLMProvider for DEEPSEEKProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "deepseek") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("deepseek package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("deepseek: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
