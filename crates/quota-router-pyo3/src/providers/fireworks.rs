    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.fireworks.com/".to_string(),
...
impl LLMProvider for FIREWORKSProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "fireworks") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("fireworks package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("fireworks: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
