...
// Calls gateway API via PyO3 (OpenAI-compatible)
...
impl GATEWAYProvider {
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                env_api_key: "GATEWAY_API_KEY".to_string(),
                env_api_base: Some("GATEWAY_BASE_URL".to_string()),
                api_base: None,
    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, ProviderError> {
        let mut client_guard = self
            .client
            .lock()
            .map_err(|e| ProviderError::new(format!("Lock error: {}", e), "gateway"))?;
        ...
        Python::with_gil(|py| {
            let openai = PyModule::import(py, "openai").map_err(|e| {
                ProviderError::new(format!("Failed to import openai: {}", e), "gateway")
            })?;

            let openai_class = openai.getattr("OpenAI").map_err(|e| {
                ProviderError::new(format!("Failed to get OpenAI: {}", e), "gateway")
            })?;

            let kwargs = PyDict::new(py);

            // Check for platform mode via GATEWAY_PLATFORM_TOKEN env var
            let platform_token: Option<String> = std::env::var("GATEWAY_PLATFORM_TOKEN").ok();
            ...
            if let Some(k) = resolved_key {
                kwargs.set_item("api_key", k.as_str()).unwrap();

                // If using platform token, set the X-AnyLLM-Key header
                if platform_token.is_some() && key.is_none() {
                    let headers = PyDict::new(py);
                    headers
                        .set_item("X-AnyLLM-Key", format!("Bearer {}", k))
                        .unwrap();
                    kwargs.set_item("default_headers", headers).unwrap();
                } else if let Some(k) = key {
                    // Regular mode: set Bearer token in X-AnyLLM-Key header
                    let headers = PyDict::new(py);
                    headers
                        .set_item("X-AnyLLM-Key", format!("Bearer {}", k))
                        .unwrap();
            ...
            // Get base_url from stored value or env var
            let base_url: Option<String> = api_base
                .clone()
                .or_else(|| std::env::var("GATEWAY_BASE_URL").ok());

            if let Some(b) = base_url {
                kwargs.set_item("base_url", b.as_str()).unwrap();
            }

            let client = openai_class.call((), Some(kwargs)).map_err(|e| {
                ProviderError::new(format!("Failed to create client: {}", e), "gateway")
            })?;
...
impl LLMProvider for GATEWAYProvider {
    ...
    fn completion(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let chat = client_obj
                .getattr("chat")
                .map_err(|e| ProviderError::new(format!("Failed to get chat: {}", e), "gateway"))?;
            let completions = chat.getattr("completions").map_err(|e| {
                ProviderError::new(format!("Failed to get completions: {}", e), "gateway")
            })?;
            let create = completions.getattr("create").map_err(|e| {
                ProviderError::new(format!("Failed to get create: {}", e), "gateway")
            })?;
            ...
            create
                .call((), Some(kwargs))
                .map_err(|e| ProviderError::new(format!("SDK call failed: {}", e), "gateway"))
                .map(|obj| obj.into())
        })?;
    ...
    fn embedding(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let embed = client_obj.getattr("embeddings").map_err(|e| {
                ProviderError::new(format!("Failed to get embeddings: {}", e), "gateway")
            })?;
...
fn convert_py_openai_response(
    ...
    let id: String = py_obj
        .get_item("id")
        .map_err(|e| ProviderError::new(format!("Failed to get id: {}", e), "gateway"))?
        .extract()
        .unwrap_or_else(|_| format!("chatcmpl-{}", uuid::Uuid::new_v4()));

    let model_str: String = py_obj
        .get_item("model")
        .map_err(|e| ProviderError::new(format!("Failed to get model: {}", e), "gateway"))?
        .extract()
        .unwrap_or_else(|_| model.to_string());

    let py_choices = py_obj
        .get_item("choices")
        .map_err(|e| ProviderError::new(format!("Failed to get choices: {}", e), "gateway"))?;
    ...
    let choices: Vec<Choice> = if let Ok(list) = py_choices.downcast::<pyo3::types::PyList>() {
        ...
        for i in 0..list.len() {
            ...
            let message_obj = choice_obj.get_item("message").map_err(|e| {
                ProviderError::new(format!("Failed to get message: {}", e), "gateway")
            })?;
            let role: String = message_obj
                .get_item("role")
                .map_err(|e| ProviderError::new(format!("Failed to get role: {}", e), "gateway"))?
                .extract()
                .unwrap_or_else(|_| "assistant".to_string());
            let content: String = message_obj
                .get_item("content")
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get content: {}", e), "gateway")
                })?
                .extract()
                .unwrap_or_default();
            ...
            let finish_reason: String = choice_obj
                ...
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get finish_reason: {}", e), "gateway")
                })?
                .extract()
        ...
        return Err(ProviderError::new("choices is not a list", "gateway"));
    };

    let usage_obj = py_obj
        .get_item("usage")
        .map_err(|e| ProviderError::new(format!("Failed to get usage: {}", e), "gateway"))?;

    let prompt_tokens: u32 = usage_obj
        .get_item("prompt_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get prompt_tokens: {}", e), "gateway"))?
        .extract()
        .unwrap_or(0);
    let completion_tokens: u32 = usage_obj
        .get_item("completion_tokens")
        .map_err(|e| {
            ProviderError::new(format!("Failed to get completion_tokens: {}", e), "gateway")
        })?
        .extract()
        .unwrap_or(0);
    let total_tokens: u32 = usage_obj
        .get_item("total_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get total_tokens: {}", e), "gateway"))?
        .extract()
        .unwrap_or(0);
...
fn convert_py_embedding_response(
    ...
    let data = py_obj
        .get_item("data")
        .map_err(|e| ProviderError::new(format!("Failed to get data: {}", e), "gateway"))?
        .downcast::<pyo3::types::PyList>()
        .map_err(|_| ProviderError::new("data is not a list", "gateway"))?;
    ...
    for i in 0..data.len() {
        ...
        let embedding_vec = item
            .get_item("embedding")
            .map_err(|e| ProviderError::new(format!("Failed to get embedding: {}", e), "gateway"))?
            .extract::<Vec<f32>>()
            .unwrap_or_default();
