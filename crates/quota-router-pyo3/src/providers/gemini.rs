    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.gemini.com/".to_string(),
                env_api_key: "GEMINI_API_KEY".to_string(),
                env_api_base: Some("GEMINI_BASE_URL".to_string()),
                api_base: None,
...
impl LLMProvider for GEMINIProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "gemini") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("gemini package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("gemini: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
