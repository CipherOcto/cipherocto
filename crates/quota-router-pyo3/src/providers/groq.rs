    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, PyErr> {
        ...
        Python::with_gil(|py| {
            let groq = PyModule::import(py, "groq")
                .map_err(|e| ProviderError::new_err(format!("Failed to import groq: {}", e)))?;

            let groq_class = groq
                .getattr("Groq")
                .map_err(|e| ProviderError::new_err(format!("Failed to get Groq: {}", e)))?;
...
impl LLMProvider for GROQProvider {
    ...
    fn embedding(&self, _input: &[String], _model: &str) -> Result<EmbeddingsResponse, PyErr> {
        Err(ProviderError::new_err("groq does not support embeddings"))
    }
...
fn convert_py_groq_response(py_obj: &PyAny, model: &str) -> Result<ChatCompletion, PyErr> {
