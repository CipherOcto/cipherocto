    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.huggingface.com/".to_string(),
...
impl LLMProvider for HUGGINGFACEProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "huggingface") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("huggingface package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("huggingface: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
    ...
    fn embedding(
        ...
        Err(ProviderError::new(
            "huggingface does not support embeddings",
            "huggingface",
        ))
    }
