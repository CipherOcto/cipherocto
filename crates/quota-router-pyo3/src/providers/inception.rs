...
impl LLMProvider for INCEPTIONProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "inception") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("inception package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("inception: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
