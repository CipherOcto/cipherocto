...
impl LLMProvider for LLAMAProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "llama") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("llama package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("llama: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
