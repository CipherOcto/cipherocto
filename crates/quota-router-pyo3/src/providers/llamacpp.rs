...
impl LLMProvider for LLAMACPPProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "llamacpp") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("llamacpp package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("llamacpp: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
