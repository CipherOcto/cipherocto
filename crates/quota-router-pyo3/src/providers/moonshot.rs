    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.moonshot.com/".to_string(),
...
impl LLMProvider for MOONSHOTProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "moonshot") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("moonshot package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("moonshot: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
