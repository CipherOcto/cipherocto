...
impl LLMProvider for MZAIProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "mzai") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("mzai package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("mzai: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
