// mzai provider implementation
// Calls Mzai API via PyO3
...
impl MZAIProvider {
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                name: "mzai".to_string(),
                documentation_url: "https://docs.mzai.com/".to_string(),
                env_api_key: "MZAI_API_KEY".to_string(),
                env_api_base: Some("MZAI_BASE_URL".to_string()),
                api_base: None,
    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, ProviderError> {
        let mut client_guard = self
            .client
            .lock()
            .map_err(|e| ProviderError::new(format!("Lock error: {}", e), "mzai"))?;
        ...
        Python::with_gil(|py| {
            let openai = PyModule::import(py, "openai").map_err(|e| {
                ProviderError::new(format!("Failed to import openai: {}", e), "mzai")
            })?;

            let openai_class = openai
                .getattr("OpenAI")
                .map_err(|e| ProviderError::new(format!("Failed to get OpenAI: {}", e), "mzai"))?;

            let key = api_key
                .as_ref()
                .ok_or_else(|| ProviderError::new("No API key set", "mzai"))?;
            ...
            let client = openai_class.call((), Some(kwargs)).map_err(|e| {
                ProviderError::new(format!("Failed to create client: {}", e), "mzai")
            })?;
...
impl LLMProvider for MZAIProvider {
    ...
    fn completion(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let chat = client_obj
                .getattr("chat")
                .map_err(|e| ProviderError::new(format!("Failed to get chat: {}", e), "mzai"))?;
            let completions = chat.getattr("completions").map_err(|e| {
                ProviderError::new(format!("Failed to get completions: {}", e), "mzai")
            })?;
            let create = completions
                .getattr("create")
                .map_err(|e| ProviderError::new(format!("Failed to get create: {}", e), "mzai"))?;
            ...
            create
                .call((), Some(kwargs))
                .map_err(|e| ProviderError::new(format!("SDK call failed: {}", e), "mzai"))
                .map(|obj| obj.into())
        })?;
    ...
    fn embedding(
        ...
        Err(ProviderError::new(
            "mzai does not support embeddings",
            "mzai",
        ))
    }
...
fn convert_py_openai_response(
    ...
    let id: String = py_obj
        .get_item("id")
        .map_err(|e| ProviderError::new(format!("Failed to get id: {}", e), "mzai"))?
        .extract()
        .unwrap_or_else(|_| format!("chatcmpl-{}", uuid::Uuid::new_v4()));

    let model_str: String = py_obj
        .get_item("model")
        .map_err(|e| ProviderError::new(format!("Failed to get model: {}", e), "mzai"))?
        .extract()
        .unwrap_or_else(|_| model.to_string());

    let py_choices = py_obj
        .get_item("choices")
        .map_err(|e| ProviderError::new(format!("Failed to get choices: {}", e), "mzai"))?;
    ...
    let choices: Vec<Choice> = if let Ok(list) = py_choices.downcast::<pyo3::types::PyList>() {
        ...
        for i in 0..list.len() {
            ...
            let message_obj = choice_obj
                .get_item("message")
                .map_err(|e| ProviderError::new(format!("Failed to get message: {}", e), "mzai"))?;
            let role: String = message_obj
                .get_item("role")
                .map_err(|e| ProviderError::new(format!("Failed to get role: {}", e), "mzai"))?
                .extract()
                .unwrap_or_else(|_| "assistant".to_string());
            let content: String = message_obj
                .get_item("content")
                .map_err(|e| ProviderError::new(format!("Failed to get content: {}", e), "mzai"))?
                .extract()
                .unwrap_or_default();
            ...
            let finish_reason: String = choice_obj
                ...
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get finish_reason: {}", e), "mzai")
                })?
                .extract()
        ...
        return Err(ProviderError::new("choices is not a list", "mzai"));
    };

    let usage_obj = py_obj
        .get_item("usage")
        .map_err(|e| ProviderError::new(format!("Failed to get usage: {}", e), "mzai"))?;

    let prompt_tokens: u32 = usage_obj
        .get_item("prompt_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get prompt_tokens: {}", e), "mzai"))?
        .extract()
        .unwrap_or(0);
    let completion_tokens: u32 = usage_obj
        .get_item("completion_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get completion_tokens: {}", e), "mzai"))?
        .extract()
        .unwrap_or(0);
    let total_tokens: u32 = usage_obj
        .get_item("total_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get total_tokens: {}", e), "mzai"))?
        .extract()
        .unwrap_or(0);
