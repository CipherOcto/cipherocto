...
impl LLMProvider for NEBIUSProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "nebius") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("nebius package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("nebius: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
