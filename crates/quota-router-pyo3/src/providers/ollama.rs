...
impl OLLAMAProvider {
    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, PyErr> {
        ...
        Python::with_gil(|py| {
            let ollama = PyModule::import(py, "ollama")
                .map_err(|e| ProviderError::new_err(format!("Failed to import ollama: {}", e)))?;

            let ollama_class = ollama
                .getattr("Client")
...
impl LLMProvider for OLLAMAProvider {
    ...
    fn embedding(&self, _input: &[String], _model: &str) -> Result<EmbeddingsResponse, PyErr> {
        Err(ProviderError::new_err("ollama does not support embeddings"))
    }
...
fn convert_py_ollama_response(py_obj: &PyAny, model: &str) -> Result<ChatCompletion, PyErr> {
