    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.openrouter.com/".to_string(),
...
impl LLMProvider for OPENROUTERProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "openrouter") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("openrouter package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("openrouter: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
