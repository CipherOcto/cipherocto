    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.platform.com/".to_string(),
                env_api_key: "PLATFORM_API_KEY".to_string(),
                env_api_base: Some("PLATFORM_BASE_URL".to_string()),
                api_base: None,
...
impl LLMProvider for PLATFORMProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "platform") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("platform package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("platform: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
    ...
    fn embedding(
        ...
        Err(ProviderError::new(
            "platform does not support embeddings",
            "platform",
        ))
    }
