...
impl LLMProvider for PORTKEYProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "portkey") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("portkey package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("portkey: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
