...
impl LLMProvider for SAMBANOVAProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "sambanova") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("sambanova package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("sambanova: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
