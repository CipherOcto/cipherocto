...
impl SAMBANOVAProvider {
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.sambanova.com/".to_string(),
                env_api_key: "SAMBANOVA_API_KEY".to_string(),
                env_api_base: Some("SAMBANOVA_BASE_URL".to_string()),
                api_base: None,
    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, ProviderError> {
        let mut client_guard = self
            .client
            .lock()
            .map_err(|e| ProviderError::new(format!("Lock error: {}", e), "sambanova"))?;
        ...
        Python::with_gil(|py| {
            let openai = PyModule::import(py, "openai").map_err(|e| {
                ProviderError::new(format!("Failed to import openai: {}", e), "sambanova")
            })?;

            let openai_class = openai.getattr("OpenAI").map_err(|e| {
                ProviderError::new(format!("Failed to get OpenAI: {}", e), "sambanova")
            })?;

            let key = api_key
                .as_ref()
                .ok_or_else(|| ProviderError::new("No API key set", "sambanova"))?;
            ...
            if let Some(base) = api_base.as_ref() {
                ...
                kwargs
                    .set_item("base_url", "https://api.sambanova.ai/api/v1")
                    .unwrap();
            }

            let client = openai_class.call((), Some(kwargs)).map_err(|e| {
                ProviderError::new(format!("Failed to create client: {}", e), "sambanova")
            })?;
...
impl LLMProvider for SAMBANOVAProvider {
    ...
    fn completion(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let chat = client_obj.getattr("chat").map_err(|e| {
                ProviderError::new(format!("Failed to get chat: {}", e), "sambanova")
            })?;
            let completions = chat.getattr("completions").map_err(|e| {
                ProviderError::new(format!("Failed to get completions: {}", e), "sambanova")
            })?;
            let create = completions.getattr("create").map_err(|e| {
                ProviderError::new(format!("Failed to get create: {}", e), "sambanova")
            })?;
            ...
            create
                .call((), Some(kwargs))
                .map_err(|e| ProviderError::new(format!("SDK call failed: {}", e), "sambanova"))
                .map(|obj| obj.into())
        })?;
    ...
    fn embedding(
        ...
        Err(ProviderError::new(
            "sambanova does not support embeddings",
            "sambanova",
        ))
    }
...
fn convert_py_openai_response(
    ...
    let id: String = py_obj
        .get_item("id")
        .map_err(|e| ProviderError::new(format!("Failed to get id: {}", e), "sambanova"))?
        .extract()
        .unwrap_or_else(|_| format!("chatcmpl-{}", uuid::Uuid::new_v4()));

    let model_str: String = py_obj
        .get_item("model")
        .map_err(|e| ProviderError::new(format!("Failed to get model: {}", e), "sambanova"))?
        .extract()
        .unwrap_or_else(|_| model.to_string());

    let py_choices = py_obj
        .get_item("choices")
        .map_err(|e| ProviderError::new(format!("Failed to get choices: {}", e), "sambanova"))?;
    ...
    let choices: Vec<Choice> = if let Ok(list) = py_choices.downcast::<pyo3::types::PyList>() {
        ...
        for i in 0..list.len() {
            ...
            let message_obj = choice_obj.get_item("message").map_err(|e| {
                ProviderError::new(format!("Failed to get message: {}", e), "sambanova")
            })?;
            let role: String = message_obj
                .get_item("role")
                .map_err(|e| ProviderError::new(format!("Failed to get role: {}", e), "sambanova"))?
                .extract()
                .unwrap_or_else(|_| "assistant".to_string());
            let content: String = message_obj
                .get_item("content")
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get content: {}", e), "sambanova")
                })?
                .extract()
                .unwrap_or_default();
            ...
            let finish_reason: String = choice_obj
                ...
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get finish_reason: {}", e), "sambanova")
                })?
                .extract()
        ...
        return Err(ProviderError::new("choices is not a list", "sambanova"));
    };

    let usage_obj = py_obj
        .get_item("usage")
        .map_err(|e| ProviderError::new(format!("Failed to get usage: {}", e), "sambanova"))?;

    let prompt_tokens: u32 = usage_obj
        .get_item("prompt_tokens")
        .map_err(|e| {
            ProviderError::new(format!("Failed to get prompt_tokens: {}", e), "sambanova")
        })?
        .extract()
        .unwrap_or(0);
    ...
    let total_tokens: u32 = usage_obj
        ...
        .map_err(|e| ProviderError::new(format!("Failed to get total_tokens: {}", e), "sambanova"))?
        .extract()
        .unwrap_or(0);
