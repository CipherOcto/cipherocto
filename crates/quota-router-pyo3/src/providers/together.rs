    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, ProviderError> {
        ...
        Python::with_gil(|py| {
            ...
            let client = openai_class.call((), Some(kwargs)).map_err(|e| {
                ProviderError::new(format!("Failed to create client: {}", e), "together")
            })?;
    ...
    fn completion(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let chat = client_obj.getattr("chat").map_err(|e| {
                ProviderError::new(format!("Failed to get chat: {}", e), "together")
            })?;
            let completions = chat.getattr("completions").map_err(|e| {
                ProviderError::new(format!("Failed to get completions: {}", e), "together")
            })?;
            let create = completions.getattr("create").map_err(|e| {
                ProviderError::new(format!("Failed to get create: {}", e), "together")
            })?;
    ...
    fn embedding(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let embeddings = client_obj.getattr("embeddings").map_err(|e| {
                ProviderError::new(format!("Failed to get embeddings: {}", e), "together")
            })?;
...
fn convert_py_together_response(
    py_obj: &PyAny,
    model: &str,
) -> Result<ChatCompletion, ProviderError> {
    ...
    let choices: Vec<Choice> = if let Ok(list) = py_choices.downcast::<pyo3::types::PyList>() {
        ...
        for i in 0..list.len() {
            ...
            let message_obj = choice_obj.get_item("message").map_err(|e| {
                ProviderError::new(format!("Failed to get message: {}", e), "together")
            })?;
            ...
            let content: String = message_obj
                .get_item("content")
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get content: {}", e), "together")
                })?
                .extract()
                .unwrap_or_default();
            ...
            let finish_reason: String = choice_obj
                ...
                .map_err(|e| {
                    ProviderError::new(format!("Failed to get finish_reason: {}", e), "together")
                })?
                .extract()
...
fn convert_py_together_embedding_response(
    py_obj: &PyAny,
    model: &str,
) -> Result<EmbeddingsResponse, ProviderError> {
    let data = py_obj
