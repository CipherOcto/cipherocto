...
// Calls Together AI SDK via PyO3
...
impl TOGETHERProvider {
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.together.ai/".to_string(),
                env_api_key: "TOGETHER_API_KEY".to_string(),
                env_api_base: Some("TOGETHER_BASE_URL".to_string()),
                api_base: Some("https://api.together.ai/v1".to_string()),
    ...
    fn ensure_client(&self) -> Result<Py<PyAny>, ProviderError> {
        let mut client_guard = self
            .client
            .lock()
            .map_err(|e| ProviderError::new(format!("Lock error: {}", e), "together"))?;
        ...
        Python::with_gil(|py| {
            let openai = PyModule::import(py, "openai").map_err(|e| {
                ProviderError::new(format!("Failed to import openai: {}", e), "together")
            })?;

            let openai_class = openai.getattr("OpenAI").map_err(|e| {
                ProviderError::new(format!("Failed to get OpenAI: {}", e), "together")
            })?;

            let key = api_key
                .as_ref()
                .ok_or_else(|| ProviderError::new("No API key set", "together"))?;
            ...
            let client = openai_class
                .call((), Some(kwargs))
                .map_err(|e| ProviderError::new(format!("Failed to create client: {}", e), "together"))?;
...
impl LLMProvider for TOGETHERProvider {
    ...
    fn completion(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let chat = client_obj
                .getattr("chat")
                .map_err(|e| ProviderError::new(format!("Failed to get chat: {}", e), "together"))?;
            let completions = chat
                .getattr("completions")
                .map_err(|e| ProviderError::new(format!("Failed to get completions: {}", e), "together"))?;
            let create = completions
                .getattr("create")
                .map_err(|e| ProviderError::new(format!("Failed to get create: {}", e), "together"))?;
            ...
            create
                .call((), Some(kwargs))
                .map_err(|e| ProviderError::new(format!("SDK call failed: {}", e), "together"))
                .map(|obj| obj.into())
        })?;

        Python::with_gil(|py| convert_py_together_response(py_result.as_ref(py), model))
    }
    ...
    fn embedding(
        ...
        let py_result: Py<PyAny> = Python::with_gil(|py| {
            ...
            let embeddings = client_obj
                .getattr("embeddings")
                .map_err(|e| ProviderError::new(format!("Failed to get embeddings: {}", e), "together"))?;
        ...
        Python::with_gil(|py| convert_py_together_embedding_response(py_result.as_ref(py), model))
    }
...
fn convert_py_together_response(py_obj: &PyAny, model: &str) -> Result<ChatCompletion, ProviderError> {
    let id: String = py_obj
        .get_item("id")
        .map_err(|e| ProviderError::new(format!("Failed to get id: {}", e), "together"))?
        .extract()
        .unwrap_or_else(|_| format!("chatcmpl-{}", uuid::Uuid::new_v4()));

    let model_str: String = py_obj
        .get_item("model")
        .map_err(|e| ProviderError::new(format!("Failed to get model: {}", e), "together"))?
        .extract()
        .unwrap_or_else(|_| model.to_string());

    let py_choices = py_obj
        .get_item("choices")
        .map_err(|e| ProviderError::new(format!("Failed to get choices: {}", e), "together"))?;
    ...
    let choices: Vec<Choice> = if let Ok(list) = py_choices.downcast::<pyo3::types::PyList>() {
        ...
        for i in 0..list.len() {
            ...
            let message_obj = choice_obj
                .get_item("message")
                .map_err(|e| ProviderError::new(format!("Failed to get message: {}", e), "together"))?;
            let role: String = message_obj
                .get_item("role")
                .map_err(|e| ProviderError::new(format!("Failed to get role: {}", e), "together"))?
                .extract()
                .unwrap_or_else(|_| "assistant".to_string());
            let content: String = message_obj
                .get_item("content")
                .map_err(|e| ProviderError::new(format!("Failed to get content: {}", e), "together"))?
                .extract()
                .unwrap_or_default();

            let finish_reason: String = choice_obj
                .get_item("finish_reason")
                .map_err(|e| ProviderError::new(format!("Failed to get finish_reason: {}", e), "together"))?
                .extract()
        ...
        return Err(ProviderError::new("choices is not a list", "together"));
    };

    let usage_obj = py_obj
        .get_item("usage")
        .map_err(|e| ProviderError::new(format!("Failed to get usage: {}", e), "together"))?;

    let prompt_tokens: u32 = usage_obj
        .get_item("prompt_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get prompt_tokens: {}", e), "together"))?
        .extract()
        .unwrap_or(0);
    let completion_tokens: u32 = usage_obj
        .get_item("completion_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get completion_tokens: {}", e), "together"))?
        .extract()
        .unwrap_or(0);
    let total_tokens: u32 = usage_obj
        .get_item("total_tokens")
        .map_err(|e| ProviderError::new(format!("Failed to get total_tokens: {}", e), "together"))?
        .extract()
        .unwrap_or(0);
...
fn convert_py_together_embedding_response(py_obj: &PyAny, model: &str) -> Result<EmbeddingsResponse, ProviderError> {
    let data = py_obj
        .get_item("data")
        .map_err(|e| ProviderError::new(format!("Failed to get data: {}", e), "together"))?
        .downcast::<pyo3::types::PyList>()
        .map_err(|_| ProviderError::new("data is not a list", "together"))?;
    ...
    for i in 0..data.len() {
        ...
        let embedding_vec = item
            .get_item("embedding")
            .map_err(|e| ProviderError::new(format!("Failed to get embedding: {}", e), "together"))?
            .extract::<Vec<f32>>()
            .unwrap_or_default();
