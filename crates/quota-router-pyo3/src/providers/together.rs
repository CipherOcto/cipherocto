    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.together.com/".to_string(),
...
impl LLMProvider for TOGETHERProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "together") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("together package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("together: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
    ...
    fn embedding(
        ...
        Err(ProviderError::new(
            "together does not support embeddings",
            "together",
        ))
    }
