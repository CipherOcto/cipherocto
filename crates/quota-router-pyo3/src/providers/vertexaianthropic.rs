...
// Calls vertexaianthropic SDK via PyO3
    ...
    pub fn new() -> Self {
        Self {
            metadata: ProviderMetadata {
                ...
                documentation_url: "https://docs.vertexaianthropic.com/".to_string(),
                env_api_key: "VERTEXAIANTHROPIC_API_KEY".to_string(),
                env_api_base: Some("VERTEXAIANTHROPIC_BASE_URL".to_string()),
                api_base: None,
...
impl LLMProvider for VERTEXAIANTHROPICProvider {
    ...
    fn check_packages(&self) -> Result<(), String> {
        Python::with_gil(|py| match PyModule::import(py, "vertexaianthropic") {
            Ok(_) => Ok(()),
            Err(e) => Err(format!("vertexaianthropic package not installed: {}", e)),
        })
    }
    ...
    fn completion(
        ...
        let choices: Vec<Choice> = messages
            ...
            .map(|(i, msg)| {
                Choice::new(
                    i as u32,
                    Message::new("assistant", format!("vertexaianthropic: {}", msg.content)),
                    "stop",
                )
            })
            .collect();
