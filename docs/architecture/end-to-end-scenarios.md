...
| **CapabilityToken V2** | Bearer token signed by buyer. Model, max price, and request-shape constraints are carried as caveats, not as token fields. Outer envelope `CapabilityBundleV2`.                                                                                                                                                                                        | `crates/octo-cap-macaroon/src/bundle_v2.rs §CapabilityTokenV2`, RFC-0957                                                          |
...
| **NodeEnvelope**       | Unified mesh wire envelope. 8 fields (real struct at `crates/octo-protocol/src/envelope.rs`): `envelope_id`, `from_did`, `to_node_id`, `payload_kind`, `payload`, `authorization`, `nonce`, `expires_at_unix_ms`. `authorization: Vec<Authorization>` uses logical-AND composition across multiple authorizations per RFC-0871 §Adversary Analysis A6. | `architecture/octo-network-architecture.md §15 Key Data Types`, `crates/octo-protocol/src/envelope.rs`, RFC-0871 §Data Structures |
| **`envelope_id`**      | First field of `NodeEnvelope`. `BLAKE3-256` hash of canonical serialization of all other envelope fields; deterministic per envelope content. Used as the seen-set key (Scenario 11).                                                                                                                                                                  | `crates/octo-protocol/src/signing.rs` §`compute_envelope_id`                                                                      |
...
| **`PayloadKindId`**    | UUID discriminator inside `NodeEnvelope` identifying the inner payload type. `octo-protocol` defines ~22 entries: `IDENTITY_*`, `WALLET_*`, `QUOTA_*`, `REPUTATION_*`, `CAPABILITY_*`, `PAID_QUERY_*`.                                                                                                                                                 | mission `0870-b-envelope-adoption`, `crates/octo-protocol/src/payload_kind.rs`                                                    |
| **HopSignature**       | Per-hop Ed25519 signature over `BLAKE3-256(canonical_ser((chain_hash, hop_index, BLAKE3(inner_payload), envelope_id)))`. 4 fields (`hop_index`, `hop_did`, `signature`, `signer_pub`).                                                                                                                                                                 | `crates/octo-protocol/src/hop_signature.rs`                                                                                       |
...
| `Factory`         | `Factory(FactoryVet)` tuple — typed, not opaque. `FactoryVet` carries `target_vault_id: [u8; 32]`, `action_template: ActionTemplate`, `required_caller: Option<String>`, `pre_conditions: Vec<Constraint>`, `expiry_for_deploy_unix: u64`. |
...
| 6 Marketplace      | `marketplace_e2e` lib tests (24 tests pinning escrow/dispute/settlement invariants). GDP substrate coverage: `crates/octo-network/tests/gdp_discovery.rs` + `crates/octo-network/tests/gdp_deep.rs` (NOT the marketplace-e2e flow itself). | Wiremock-style Sybil resistance at marketplace layer                      |
...
| 2026-08-14 | Round 19 markdown (12 MED glossary cells trimmed: L46-50 envelope/chain_hash/payload_kind/HopSignature; L42 CapabilityBundleV2; L58 seen-set; L879 Factory; L960/963 test coverage map; 2 LOW RFC-0943 parens; 2 LOW change-log rows)                       | cc (review)     |
| 2026-08-14 | Round 19 technical (3 HIGH L48/L50/L482 HopSignature RFC-0871 §Data Structures mis-citation → `crates/octo-protocol/src/hop_signature.rs` + RFC-0970 §Data Structures; 1 MED L41 CapabilityToken V2 field-shape gloss → 6-field structure per bundle_v2.rs) | cc (review)     |
