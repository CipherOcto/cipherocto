...
| R1 ID | Description                                                                                                                                                                                                                                             |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C-1   | `clippy.toml` `[disallowed-methods]` table for `reqwest`/`hyper`/`ureq`/`isahc` constructor deny. Lint shell-script + key-swap denylist are the current defense; clippy is the durable closure.                                                         |
| C-4   | Real `IngressTransform::normalise(provider_kind, raw_response)` impl. Currently a 71-line stub; `eleven_step.rs::step9_cache_classify` re-implements parse inline.                                                                                      |
| M-1   | `sim.rs` 5 modes; plan + mission require 8 (per R19 fix).                                                                                                                                                                                               |
| M-3   | `tests/fixtures/exercise/eleven_step_goldens.json::step10_settlement_hash_blake3` is `blake3::hash(b"settlement-mock")`; the actual exercise derives settlement_hash from real `StoolapStore::settle`. Re-bind the golden to the real canonicalization. |
| M-4   | Step 9 hand-walks JSON. Coupled to C-4.                                                                                                                                                                                                                 |
| M-5   | `Egress::send` trait is paper-only (`TestEgress` is the only impl). Coupled to C-2.                                                                                                                                                                     |
| AC-10 | Cross-impl test passes only on non-zero digests; not byte-equivalent. Reconcile impl1 (SettlementEnvelope canonical) and impl2 (hand-rolled) field-ordering.                                                                                            |
| 15    | Workspace clippy fails (octo-wallet `#![warn(missing_debug_implementations)]` cascade to 12 errors). Not 0957-b-specific.                                                                                                                               |
