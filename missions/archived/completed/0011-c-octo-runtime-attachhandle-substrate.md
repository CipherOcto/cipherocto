...
- [ ] **AC-5** `agent run --detach --token-file <path>` mints + writes token to file (Layer C/D) per §Sub-step 3 + RFC-0011-c §F.2 (deferred to follow-on amendment per §Scope)
- [ ] **AC-6** `agent attach --token-file <path>` reads + binds (Layer C/D) per §Sub-step 4 + RFC-0011-c §F.2 (replaces attach dispatch stub) (deferred to follow-on amendment per §Scope)
- [ ] **AC-7** 9 NEW `OctoCliError` variants wired at `octo_cli::error` per RFC-0011-c §F.4 mirror + RFC-0011-c §9.3.6 substrate-faithful amendment (`AttachHandleExpired`, `AttachHandleBadSignature`, `AttachSessionMismatch`, `AttachSessionUnknown`, `InvalidSinceCursor`, `PersistenceError`, `RevocationError`, `TransportHandlerNotRegistered`, `InvalidSessionIdHex`); `InvalidSinceCursor` shares exit 53 slot with `AttachHandleExpired` per typed-discriminator preservation; `TransportHandlerNotRegistered` exits 59 (extension surface per [[cipherocto-design-principles]] §per-extension crates + registry); `InvalidSessionIdHex` exits 47 (CLI-parse failure on `octo agent revoke-attach --session-id`, CLI-boundary, distinct from substrate signature-verify `AttachHandleBadSignature` exit 54)
- [ ] **AC-8** `octo agent revoke-attach --session-id <HEX64>` primitive (Q-deferred 2) per RFC-0011-c §F.3 with in-memory revocation set; substrate takes `session_id: SessionId`, not a token (signature-verify is `attach_with_token`'s job, not revocation's)
...
| `OctoCliError::InvalidSessionIdHex { reason }`                            | CLI      | Layer C/D; CLI-parse failure on `octo agent revoke-attach --session-id` (exit 47). Distinct from `AttachHandleBadSignature` (exit 54, substrate signature-verify failure). CLI-boundary parse error per RFC-0011-c §9.3.6 substrate-faithful amendment                                                                                                                                                                                                                                                                                                                                                                                                |
...
**Scope:** Sub-steps 1+2 land in this mission cycle (RFC text + substrate code). Sub-step 5 also lands in this cycle (CLI primitive `octo agent revoke-attach --session-id`). Sub-steps 3 and 4 are deferred to follow-on amendment cycles — the substrate this mission ships (`mint_attach_handle`, `encode_token`, `attach_with_token`) is in place, but the CLI clap variants for `--token-file` on `agent run` and `agent attach` are not wired in this cycle (the `agent run --detach` and `agent attach` clap variants at `octo-cli/src/commands/agent.rs` do not yet expose `--token-file`). The mission's in-scope deliverables are: (a) RFC-0011-c §Follow-on §F.1-§F.5 text refresh, (b) substrate code (handle split + encoding + signing + persistence + 8 AttachError variants), (c) `OctoCliError` mirror envelope (9 NEW variants at `octo_cli::error` — 8 substrate-mirrored per §F.4 + `InvalidSessionIdHex` CLI-boundary per §9.3.6 substrate-faithful amendment) + CLI dispatch wiring for the one Layer C/D primitive that ships this cycle (`octo agent revoke-attach --session-id`). Sub-steps 3 and 4 prose preserved verbatim below as the substrate-faithful shape the follow-on CLI dispatch will bind against.
...
3. **CLI extension to `agent run --detach --token-file <path>`** (deferred to follow-on amendment) — `octo_cli::commands::agent` (Layer C/D; substrate reference RFC-0011-c §F.2). Add `--token-file <path>` clap arg to existing `AgentAction::Run` variant. On `--detach` dispatch, after `octo_runtime::spawn_agent(...)` returns, CLI boundary resolves caller DID → `IdentityKey` (per RFC-0011-c §F.5: `mint_attach_handle` takes `holder: &IdentityKey`), mints token via `octo_runtime::mint_attach_handle(holder: &IdentityKey, ...)`, serializes via `octo_runtime::encode_token`, writes to file. Token NOT included in `AgentRunOutput` payload (side-channel credential, not audit data).

4. **CLI dispatch replacement at `agent attach --token-file <path>`** (deferred to follow-on amendment) — replaces `Commands::Agent::Attach` dispatch stub. Read token bytes from `--token-file` → `octo_runtime::decode_token(...)` → `octo_runtime::attach_with_token(holder_pubkey, &token, since_unix)` async (3 args: `holder_pubkey: &[u8; 32]` resolved from caller DID at CLI boundary per §F.5). Populates `AgentAttachOutput { agent_id, runtime_handle, attached_at_unix, event_cursor }`. Includes the 8 `OctoCliError` variants (exits 53-59) wired at `octo_cli::error` per RFC-0011-c §F.4 mirror (merged sub-step per R1 finding S-M4 — CLI dispatch and error envelope are one cohesive surface; `TransportHandlerNotRegistered` exit 59 is the new variant added in this cycle for the handler-registry dispatch surface).

5. **CLI primitive `octo agent revoke-attach --session-id <HEX64>`** — new subcommand under `Commands::Agent::RevokeAttach` at `octo_cli::commands::agent`.
   - **5a. Clap args** — `--session-id <HEX64>` (required; parses 64-char hex string → `[u8; 32]` `SessionId`; CLI-boundary parse failure → `OctoCliError::InvalidSessionIdHex { reason }` exit 47 per RFC-0011-c §9.3.6 substrate-faithful amendment) and `--confirm` (global flag per RFC-0011 §Compatibility; suppresses `ConfirmationRequired` exit 2 in `Human`/`Ci` mode; ignored in `Dev` mode).
   - **5b. Substrate call** — calls `octo_runtime::revoke_attach_token(session_id)` per RFC-0011-c §F.3 `revoke_attach_token(session_id: SessionId)` signature. No token decode, no signature verify — revocation is keyed by `session_id`; signature-verify is `attach_with_token`'s job per §F.2 step (a), not revocation's.
   - **5c. Error envelope + success clause** — Returns the mode-gate envelope (exit 2, per §9.3.6 Errors row) OR `OctoCliError::RevocationError(String)` (exit 58; substrate failure) on failure; `exit 0` on success (Dev mode OR `--confirm` in Human/Ci mode) with `RevokeAttachOutput { session_id_hex, process_scoped: true }` — `process_scoped: true` because in-memory revocation set is lost on process restart (RFC-0011-c §F.3 + §Future Work).
...
| TV-AGT21 | `agent run --detach --token-file <path>`        | Fresh spawn                                                  | Token written to file; `AgentRunOutput` (exit 0) excludes token                                                         | DEFERRED to follow-on amendment; per §Sub-step 3 (Sub-step 3 itself deferred; CLI clap variant does not yet expose `--token-file`)                                                                                                               |
| TV-AGT22 | `octo agent revoke-attach --session-id <HEX64>` | Revoked session                                              | `RevokeAttachOutput { session_id_hex, process_scoped: true }` (exit 0) in Dev mode OR with `--confirm` in Human/Ci mode | NEW; per §Sub-step 5 + RFC-0011-c §F.3 (revocation keyed by session_id; CLI mode-gate envelope per RFC-0011 §Compatibility exits 2)                                                                                                              |
