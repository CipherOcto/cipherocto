...
- [ ] 4 test vectors per RFC-0011-w §Test Vectors (`tv_w_1` + `tv_w_1b` + `tv_w_2` + `tv_w_3`):
  - `tv_w_1`: variant construction + `exit_code()` returns 91 + `user_message()` renders the `#[error]` template with `key_id_hex` (8 hex chars via `hex::encode(key_id.to_be_bytes())`) + `known_keys_band` (Debug-rendered enum tag `None | Few | Many`) interpolated + `hint()` returns the prescribed operational remediation phrase per RFC-0011-c §F.5.1 paired-acceptance bridge (feature OFF build; Rust unit test; `tv_w_1` hint assertion added at R1.5.b per R1.B MEDIUM finding closure)
  - `tv_w_1b`: `KnownKeysBand::from_count(usize)` boundary coverage at 0 → `None` (verifier misconfiguration sentinel), 1 → `Few` (band lower edge), 8 → `Few` (off-by-one at `1..=8` upper edge), 9 → `Many` (off-by-one at `>8` lower edge), 16 → `Many` (interior band value); all 3 quantization bands plus the 8-vs-9 boundary sentinel exercised (feature ON build; Rust unit test; added at R1.5.b per R1.B HIGH finding closure)
  ...
  - `tv_w_3`: cross-RFC reference integrity — shell grep verifies all 4 G4 §Cross-RFC reference updates landed at their target locations in `rfcs/accepted/process/0011-h-oct-cli-network-subcommands.md` per RFC-0011-w §Test Vectors `tv_w_3` row (4 per-edit greps, one per G4 edit; NOT a Rust unit test, NOT cargo test target). The case-sensitive `slot 91.*RESERVED` regex is documented as NOT a useful tv_w_3 signal because the 4-edit list does not clear the regex hits at lines 62 + 545 (false positive on `slots 80 + 81 RESERVED` substring) + 801 (out-of-scope architectural pre-allocation statement) per R2.B H1 finding. Edit 2 grep uses whitespace-tolerant `\| 91 +\|.*NetworkKeyRotationUnknownId` because the parent RFC §Exit Codes row 91 cell uses 5-space column alignment (`| 91     |` per Markdown table convention; the unanchored single-space regex `^\| 91 \|` fails to match per R3.B HIGH finding). Edit 3 grep corrected per R3.A HIGH finding: `'Slot 91 activated per RFC-0011-w'` (matches actual parent RFC text at G21 row right column; the original `'Slot 91 activation per RFC-0011-w'` returned 0 hits because the parent RFC uses the participle "activated" not the noun "activation"). Edit 4 grep corrected per R3.A HIGH finding: `'Slot 91 is activated per RFC-0011-w'` (matches actual parent RFC text at [^rotation-error-slot-prealloc] footnote; the original `'Slot 91 .NetworkKeyRotationUnknownId. activated per RFC-0011-w'` required literal adjacency that the parent RFC does not have).
...
- [ ] `cargo test -p octo-cli --lib --features octo-attach-key-rotation` green (2 Rust vectors tv_w_2 + tv_w_1b + zero regressions; feature ON build)
