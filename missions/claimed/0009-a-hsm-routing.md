...
Claimed (2026-08-09). RFC-0009 amendment adds the HSM routing requirement; this mission implements the gap closure. Claimant: @cipherocto.
...
- [x] `crates/octo-wallet/src/identity.rs::IdentityKey` holds `signer: Arc<dyn HsmAdapter>` (CHANGED: was `SigningKey` directly)
- [x] `IdentityKey::sign(msg: &[u8]) -> Result<Signature, WalletError>` delegates to `self.signer.sign(msg)` and returns the wrapped `Signature` (CHANGED: was `self.0.sign(msg)` directly)
- [x] `IdentityKey::generate()` constructs `IdentityKey` with `signer: Arc::new(InMemorySigner::new(seed_bytes, public_key))` (default impl preserved for MVP)
- [x] `IdentityKey::from_seed(seed: [u8; 32]) -> Self` constructs with same default `InMemorySigner`
- [x] `HolderSignError` / `WalletError::Hsm(HsmError)` error variant exists for HSM failure propagation
- [x] Every signing call site in `octo-wallet` updates to use `Result` return type:
  - `crates/octo-wallet/src/identity.rs::sign` callers
  - `crates/octo-wallet/src/capability/macaroon.rs` (holder signature on mint) — via `MintError::Hsm(WalletError)`
  - `crates/octo-wallet/src/capability/discharge.rs` (discharge macaroon signatures)
  - `crates/octo-wallet/src/capability/zk_mint.rs` — no direct `IdentityKey::sign` call sites today; future mint paths will route through the adapter
  - `crates/octo-wallet/src/capability/redemption.rs` — no direct `IdentityKey::sign` call sites today
- [x] Test fixture: `InMemorySigner` produces byte-identical signatures to pre-refactor `ed25519_dalek::SigningKey::sign` for the same input (determinism parity check) — `in_memory_signer_byte_identical_to_raw_signing_key` test
- [x] Test fixture: `LedgerSigner` smoke test (`hsm::tests::ledger_signer_signs_smoke_test` + `both_signers_produce_same_signature_for_same_input`) continues to pass
- [x] `cargo test -p octo-wallet --lib` green (315/315 passed)
- [ ] `cargo test -p octo-wallet --test eleven_step_zk` — not re-run in this session; existing test suite unchanged
- [ ] `cargo test -p octo-wallet --test capability_zk_acceptance` — not re-run in this session; existing test suite unchanged
