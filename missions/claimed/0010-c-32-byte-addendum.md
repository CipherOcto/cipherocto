...
> **Claim-and-implement cascade (2026-08-24, user instruction "claim and implementaion IS on scope"):** Per Session 1 RFC-0010 substrate landing, the pre-existing borsh feature-gate mismatch was resolved via `crates/octo-ident/Cargo.toml` [[test]] entry `required-features = ["borsh"]` on `rich_did_document_tv` (preserves `octo-ident::DidDocument` free of unconditional borsh derives per design intent at `octo-identity-resolver-node/src/handlers/registration.rs:48`). AC-7 (`cargo clippy --all-targets -p octo-ident -- -D warnings`) now PASSES — clean both with and without `--features borsh`. Plus RFC-0010 v1.9.2 L111 inline amendment landed: doc-comment "Aliases RFC-0968 §3 `RecorderDid` storage form" → "Same byte layout as RFC-0968 §3 `RecorderDid` storage form" (removes misleading "Aliases" prefix that read as if `RawDid` were a Rust alias declaration; the byte layout is shared but the types are distinct). AC-1 through AC-8 ALL PASS. Status block still preserves OPEN per historical-mission-preservation — the substrate work was a 2026-08-19 inline retro-supersession pass, not a fresh claim-and-implement closure.
