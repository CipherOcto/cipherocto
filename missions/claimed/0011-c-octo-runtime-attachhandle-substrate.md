...
| `attach_with_token(holder_pubkey, token, since_unix)`                     | F.2      | Layer B async (defined at `octo_runtime` crate root in `lib.rs:103`); validates via 5-step chain (step (e) = transport-handler dispatch via `octo_runtime::handle::transport::HANDLE_TRANSPORT_REGISTRY`)                                                                                                                                                                                                                                                                   |
