...
**Scope:** Sub-steps 1+2 land in this mission cycle (RFC text + substrate code). Sub-steps 3-5 are SPECIFIED here (signature, invocation shape, error envelope mapping) but EXECUTED in a follow-on amendment mission that wires the Layer C/D CLI dispatch against the substrate this mission ships. The current cycle's in-scope deliverables are: (a) RFC-0011-c §Follow-on §F.1-§F.5 text refresh, (b) substrate code (handle split + encoding + signing + persistence + 8 AttachError variants), (c) `OctoCliError` mirror envelope (8 NEW variants at `octo_cli::error`) so the follow-on CLI wiring has substrate-faithful error surfaces to bind against. Sub-steps 3-5 prose preserved verbatim below for the follow-on mission to bind against.
