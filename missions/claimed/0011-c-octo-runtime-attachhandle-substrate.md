...
5. **CLI primitive `octo agent revoke-attach --session-id <HEX64>`** — new subcommand under `Commands::Agent::RevokeAttach` at `octo_cli::commands::agent`. Clap args: `--session-id <HEX64>` (required; parses 64-char hex string → `[u8; 32]` `SessionId`; CLI-boundary parse failure → `OctoCliError::InvalidSessionIdHex { reason }` exit 47 per RFC-0011-c §9.3.6 substrate-faithful amendment) and `--confirm` (optional; suppresses `ConfirmationRequired` exit 2 in `Human`/`Ci` mode; ignored in `Dev` mode per RFC-0011 §Compatibility). No token decode, no signature verify — revocation is keyed by `session_id` per RFC-0011-c §F.3 `revoke_attach_token(session_id: SessionId)` signature; signature-verify is `attach_with_token`'s job per §F.2 step (a), not revocation's. Calls `octo_runtime::revoke_attach_token(session_id)`. Returns `OctoCliError::AuditorDenied { command }` / `ConfirmationRequired { command }` (exit 2; mode-gate envelope) OR `OctoCliError::RevocationError(String)` (exit 58; substrate failure) on failure; `exit 0` on success (Dev mode OR `--confirm` in Human/Ci mode) with `RevokeAttachOutput { session_id_hex, process_scoped: true }` — `process_scoped: true` because in-memory revocation set is lost on process restart (RFC-0011-c §F.3 + §Future Work).
