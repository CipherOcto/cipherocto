...
- **§F.4 Errors** — see RFC-0011-c §Follow-on §F.4. `pub enum AttachError` in `octo_runtime::handle::error` with 7 variants (mirror → `OctoCliError` exits 53-58); `InvalidSinceCursor` (exit 53 shared slot with `AttachHandleExpired` per typed-discriminator preservation).
...
- [ ] **AC-7** 7 NEW `OctoCliError` variants wired at `octo_cli::error` per RFC-0011-c §F.4 mirror (see §Type Coverage); `InvalidSinceCursor` shares exit 53 slot with `AttachHandleExpired` per typed-discriminator preservation
