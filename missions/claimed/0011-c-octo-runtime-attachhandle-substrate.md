...
**Scope:** Sub-steps 1+2 land in this mission cycle (RFC text + substrate code). Sub-steps 3, 4, 5 also land in this cycle (CLI dispatch wiring against the substrate this mission ships) per the current substrate state at `octo-cli/src/commands/agent.rs`. The mission's in-scope deliverables are: (a) RFC-0011-c §Follow-on §F.1-§F.5 text refresh, (b) substrate code (handle split + encoding + signing + persistence + 8 AttachError variants), (c) `OctoCliError` mirror envelope (8 NEW variants at `octo_cli::error`) + CLI dispatch wiring for the three Layer C/D primitives (`agent run --token-file`, `agent attach --token-file`, `octo agent revoke-attach --session-id`). Sub-steps 3-5 prose preserved verbatim below as the substrate-faithful shape the CLI dispatch binds against.
