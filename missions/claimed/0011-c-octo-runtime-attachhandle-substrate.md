...
5. **CLI primitive `octo agent revoke-attach --session-id <HEX64>`** — new subcommand under `Commands::Agent::RevokeAttach` at `octo_cli::commands::agent`.
   - **5a. Clap args** — `--session-id <HEX64>` (required; parses 64-char hex string → `[u8; 32]` `SessionId`; CLI-boundary parse failure → `OctoCliError::InvalidSessionIdHex { reason }` exit 47 per RFC-0011-c §9.3.6 substrate-faithful amendment) and `--confirm` (optional; suppresses `ConfirmationRequired` exit 2 in `Human`/`Ci` mode; ignored in `Dev` mode per RFC-0011 §Compatibility).
