...
| v3.0    | 2026-08-22 | Phase 3.5 close-out per long-horizon plan v1.5 §Mission layout. AC verification per memory card `mission-0206-003-trait-moves-v30-status.md`: LANDED (push pending). HolderRegistry + Clock + HolderKind + HolderRecord + BearerCapsule + CapabilityTokenLike + CapabilityClass moved quota-router-storage → octo-cap-macaroon. StoolapDidRegistry + StoolapHolderRegistry impls moved → octo-ident-storage. 215 cap-macaroon + 29 ident-storage + 187 quota-storage + 71 wallet tests + clippy + fmt green. RFC-only YAML edits per R10.5 scope discipline (substrate code untouched). Pre-commit §cite validation: version pins + §section refs stripped per BLUEPRINT.md §RFC Reference Conventions (cross-references to v2.4 RFC body sections like §Adapter Crate List + §Trait Move Schedule + §Wiring Pattern + §Cargo.toml Cross-Cuts require v2.4 RFC body context that v3.0 draft renames; cites stripped to bare RFC numbers). Status transitions open→done. |
