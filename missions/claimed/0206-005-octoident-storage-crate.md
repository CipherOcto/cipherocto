...
description: Open 2026-08-20; RFC-0206 v2.0 §Adapter Crate List row 3 — create new adapter crate `crates/octo-ident-storage/` to hold StoolapDidRegistry impl at `src/did_registry.rs:139`. Required target for 0206-003-trait-moves.
metadata:
  node_type: mission
  type: project
...
Create new adapter crate `crates/octo-ident-storage/` per RFC-0206 v2.0 §Adapter Crate List row 3. This is the SOLE target crate for the StoolapDidRegistry impl move (owned by `0206-003-trait-moves`).

Covers:

- **New crate directory** `crates/octo-ident-storage/{Cargo.toml,src/lib.rs,src/did_registry.rs}`
- **Cargo.toml** declares `octo-storage = { path = "../octo-storage" }` (facade, per RFC §Cargo.toml Cross-Cuts — NOT direct substrate) + `octo-ident = { path = "../octo-ident" }` (trait declarer per RFC-0010 v1.3 storage extension) + workspace deps as needed
- **NO direct `stoolap` dep** (per RFC-0205 §Cargo.toml Pinning Layer A — substrate is sole fork consumer; verified by TV-0206-A9(a))
...
- RFC-0206 v2.0 §Adapter Crate List row 3 (StoolapDidRegistry impl target)
- RFC-0206 v2.0 §Trait Move Schedule
- RFC-0206 v2.0 TV-0206-A6, A9(a), A10, A11, A12
...
- DidRegistry trait declaration (lives in `octo-ident/` per RFC-0010 v1.3)
...
- RFC-0205 v2.0 (coupled pair; substrate Cargo.toml pin lands before this crate's deps resolve)
