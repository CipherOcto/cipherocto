...
- [x] `TrustRegistry`: map gateway_id → RelayScore (RFC §6). `gateway_id` is a canonical RFC-0968 recorder DID OR stable lineage identifier — NOT a raw `pubkey` (per RFC-0968 §28.4 amendments 21 + 22).
...
- [x] Diversity constraints: prefer diverse gateway connections; reuse RFC-0968 controller-cap + coalition-detection (amendment 50 (beyond A2 scope — future amendment round TBD), `MAX_COALITION_KM_PRODUCT = 100`) for cross-tree correlation
...
- Anti-Sybil: stake verification + diversity constraints + correlation detection. Correlation detection reuses RFC-0968 amendment 46 (deferred to RFC-0968-A2 — Sybil weighted-similarity classifier) weighted-similarity (`WEIGHTED_SIMILARITY_THRESHOLD = 0.60`) over `(subject, kind, layer)` aggregate weight vectors; PoRelay-specific cluster detection is layered ON TOP, not as a replacement.
