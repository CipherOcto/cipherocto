...
| v0.12   | 2026-08-12 | landed | Round-6 multi-reviewer code review (3 parallel reviewers: correctness/security, design/architecture, test/docs). AGGREGATE FINDINGS APPLIED (3 fixes; 2 MEDIUM dedupe + 1 MEDIUM, 1 LOW skip; R2 = 0): (1) **`reserved_slot_0006_not_allocated` source-line cross-ref (R3 MEDIUM)** — `known` array element type widened from `(&str, PayloadKindId)` to `(&str, PayloadKindId, u32)` carrying the source line of the `PayloadKindId` constant definition. Failure message now produces `"slot :0006 collision: {kind_name} (defined at payload_kind.rs:{def_line}) collides with reserved chain-response slot; colliding UUID bytes = {:?}; reserved UUID bytes = {:?}", kind.0, reserved.0)`. Tester see WHICH UUID literal collided + WHICH line defined the collider — debug time cut from "go grep the file" to "open line {def_line} and compare bytes". (2) **Failure message UUID bytes leak (R3 MEDIUM)** — failure message enriched with both colliding + reserved UUID byte arrays (`kind.0` + `reserved.0`). UUIDs are not secret here (they're RFC-allocated namespace identifiers, intentionally public per RFC-0870 §UUID Allocation). The leak is helpful, not hazardous — operators triaging a collision need to compare bytes; without this, the only signal was "collision detected". (3) **COVERAGE doc-note disambiguation (R3 MEDIUM)** — `COVERAGE` paragraph rewritten to lead with "21 entries total" + explicit "all listed here are defined in `payload_kind.rs`; if a constant lives in a different crate or in this crate but outside this file, this scan does not see it". Disambiguates IN-unit completeness (round-5) from cross-crate reach (round-4 SCOPE). **FALSE-POSITIVE SKIPPED (R1 LOW):** reviewer suggested adding metric-counter hookup for collision frequency. This test is build-time-only (runs `cargo test`); runtime collision counter lives in `octo-protocol::registry_collision_observability` per v0.9 review + ROADMAP. Adding a counter to a `#[test]` would couple runtime metrics to test infra — wrong direction. **R2 (design/architecture): 0 items** — all 10 categories returned "nothing new"; round-5 fix surface (21-entry completeness + SCOPE example accuracy) did not introduce new design-architecture issues. 67 octo-protocol lib tests (unchanged; same `reserved_slot_0006_not_allocated` test, expanded failure message + source-line cross-ref coverage; test count unchanged); 36 octo-identity-resolver-node lib tests + 7 cross_domain_chain + 9 cross_node_chain + 4 resolve_with_chain TV unchanged; cargo clippy -p octo-protocol -p octo-identity-resolver-node --all-targets -- -D warnings clean; cargo fmt --all clean. | |
