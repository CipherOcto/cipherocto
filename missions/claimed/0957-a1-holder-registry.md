...
**Cross-crate compat:** `cargo fmt --check` clean (verified 2026-08-07). `cargo clippy --workspace --all-targets --features full -- -D warnings` GREEN (2m 10s, post workspace-exclude commit `b99b1709`); package-scoped clippy on `octo-wallet` + `quota-router-storage` + `quota-router-core` clean. 12 holder_registry tests pass + 7 dispatch tests pass.
