...
| **M-3** | `tests/fixtures/exercise/eleven_step_goldens.json`                             | MAJOR    | Open. `step10_settlement_hash_blake3` is `blake3::hash(b"settlement-mock")`; actual `eleven_step.rs::eleven_step_exercise_green` derives settlement_hash from real `StoolapStore::settle`. Golden value unrelated to exercise output. **Deferral note:** AC-5 marked-as-done is technically true (fixture captured) but the captured value asserts nothing about real behavior — flag for re-binding. |
| **M-4** | `tests/eleven_step.rs:158-196`                                                 | MAJOR    | Open. Step 9 hand-walks JSON via `body_str.find("\"prompt_tokens\":")`. No serde. No `IngressTransform::parse` call. Coupled to C-4.                                                                                                                                                                                                                                                                  |
| **M-5** | `src/egress.rs:73-75` (`Egress::send` unused)                                  | MAJOR    | Open. Trait dead — only `TestEgress` impl. Production traffic flows through `proxy.rs` per C-2/C-3 path.                                                                                                                                                                                                                                                                                              |
| **15**  | `crates/octo-wallet/src/lib.rs:12` (`#![warn(missing_debug_implementations)]`) | MAJOR    | Open. Workspace clippy fails (`-D warnings`). Not 0957-b AC-6 falsifiable in isolation; flagged for the next clippy pass.                                                                                                                                                                                                                                                                             |
...
### Test surface added (R2)

- `crates/quota-router-core/src/egress/key_swap.rs` — 7 unit tests
- `crates/quota-router-core/tests/key_swap_boundary.rs` — 7 integration tests
- `.github/linters/no-provider-bound-cap.sh` — extended with structural-lint pass

### Test surface still open (R1 carryover)

- 5 sim modes → 8 (M-1)
- Real `IngressTransform::normalise` (C-4 + M-4)
- `clippy [disallowed-methods]` table (C-1 + C-3 HTTP-client-constructor side)
- Cross-impl byte-equivalent settlement_hash (AC-10) — current test passes but only asserts non-zero, not byte-equivalent
- Step-10 golden re-binding (M-3)
- Workspace clippy green (15)
