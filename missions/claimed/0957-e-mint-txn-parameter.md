...
Closed (Band A — 2026-08-06; audit-closure rolled up 2026-08-07). Claimed 2026-08-04 by @mmacedoeu; implementation landed (commit `e05f9639`): `CapabilityToken::mint` amended to 4-arg persistence-free per RFC-0957-A1 G3 (dropped `catalog: &dyn CapabilityCatalog` parameter; `holder_did: &str`, `initial_caveats: &[Caveat]`); `Macaroon::extend_chain` elevated to `pub(crate)` so mint can append initial caveats without catalog-based WrappedOnly chain guard (guard remains at `attenuate` / `verify_full` time). 10 call sites updated across `crates/octo-wallet/src/capability/{mod,wire}.rs` + `tests/{debug_redaction,redemption_subgraph,wire_v2_roundtrip}.rs`. **13/15 ACs GREEN** as of 2026-08-07 audit-closure roll-up (TV11 atomicity flipped GREEN via 0969-b1 closure). 2 remaining unchecked ACs are explicit deferrals with named owner + target per [[deferred-vs-unspecified]]: AC-3 (TV8 100K bench) → owner @cipherocto, target 2026-09-15 (criterion bench + 100K fixture); AC-5 (TV10 caller-side persistence) → owner @cipherocto, target 2026-08-28 (cross-crate integration test wiring).
...
- [ ] TV10: Caller-Side Persistence via TransactionExt — unit test: `mint(...)` produces token; explicit `txn.insert_holder_record(HolderRecord::from_capability(token, ...))` persists; subsequent `lookup(cap_root_hash)` returns the record. → **DEFERRED to follow-up mission per [[deferred-vs-unspecified]]**: `TransactionExt::insert_holder_record` method lives in `crates/quota-router-storage/src/transaction.rs` but `HolderRecord::from_capability` constructor wiring needs verification + the integration test spans both `octo-wallet` + `quota-router-storage` (cross-crate). **Deferral:** owner = @cipherocto, target = 2026-08-28 per [[deferred-vs-unspecified]] named-owner rule. Substrate has the underlying primitive (`Transaction::insert_holder_record` structural stub at `crates/quota-router-storage/src/transaction.rs:29` + `StoolapHolderRegistry::insert` concrete impl); missing piece is the cross-crate integration test wiring (octo-wallet side composes `mint` + transaction, quota-router-storage side persists + lookups). Will land as `0957-e-caller-side-persistence` follow-up.
- [x] TV11: `insert_dual` Atomicity — `txn.insert_dual(bearer_record, capability_record)` atomic. → **GREEN via Path B body rewrite** (2026-08-07): AC text references `0969-b-mint-dual-impl.md` (original phantom-laden scope). Mission was reconciled per [[no-phantom-mission-pointers]] into `0969-b1-insert-dual-impl.md` with pivoted scope to the canonical `Transaction::insert_dual` atomic body. Atomic impl + TV9-I1/I2/I3 tests landed in `0969-b1-insert-dual-impl` (Band A closed 2026-08-07; 12/12 ACs GREEN): `tv9_i1_insert_dual_happy_path` + `tv9_i2_insert_dual_rollback_on_capability_failure` + `tv9_i3_insert_dual_aborts_on_bearer_pk_collision`. `StoolapHolderRegistry::insert_dual` concrete atomic impl + `HolderRegistry::insert_dual` trait method (with default non-atomic fallback). AC body now cites the closing sub-mission.
...
**AC rollup:** 13/15 ACs GREEN (2026-08-07 audit-closure roll-up; TV11 flipped via 0969-b1 closure).
...
| AC-3: TV8 100K lookup bench                                                     | DEFERRED                                     | owner @cipherocto, target 2026-09-15; criterion bench + 100K fixture build                                         |
| AC-4: TV9 mint persistence-free                                                 | GREEN (compile-time guarantee via 4-arg sig) | mint has no catalog/txn params                                                                                     |
| AC-5: TV10 caller-side persistence                                              | DEFERRED                                     | owner @cipherocto, target 2026-08-28; cross-crate integration test wiring                                          |
| AC-6: TV11 insert_dual atomicity                                                | GREEN                                        | `missions/claimed/0969-b1-insert-dual-impl.md` (Band A closed 2026-08-07; 12/12 ACs GREEN; TV9-I1/I2/I3 tests)     |
