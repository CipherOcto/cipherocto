...
- [ ] **AC-3:** `crates/zk-vendor/stwo-sys/` (FFI cdylib, nightly toolchain, workspace-excluded) is the STWO home; cipherocto workspace loads `libstwo_sys.so` via `zk-vendor::loaded_library()` at runtime; MSRV pinned in `crates/zk-vendor/rust-toolchain.toml` (stable 1.75.0); `zk-verifier::verify_capability_zk` delegates via layering FFI > BLAKE3 stub — **2026-07-31 (Session 2 fix-up):** decoupled workspace pattern (no vendoring). STWO is built as `crates/zk-vendor/stwo-sys/` (separate cargo project, `crate-type = ["cdylib"]`, excluded from workspace via root `Cargo.toml` `exclude`) with its own `rust-toolchain.toml` pinning nightly-2025-06-23. The cipherocto workspace stays MSRV-stable (1.75.0). `zk-vendor::loaded_library()` libloading's into `libstwo_sys.so` at runtime; `vendor_state()` reports `Ffi` when loaded, `Stub` otherwise. `zk-verifier::verify_capability_zk` delegation layering: FFI > BLAKE3 stub (no vendored middle layer — initial S2 vendoring approach reverted 2026-07-31).
