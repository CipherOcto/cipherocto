...
- [x] **AC-3:** `crates/zk-vendor/stwo/` builds on stable rust (no `+nightly`); MSRV pinned in `crates/zk-vendor/rust-toolchain.toml`; `zk-verifier::verify_capability_zk` delegates to vendored STWO — **LANDED 2026-07-31 (Session 2)**: in-workspace vendored STWO crate at `crates/zk-vendor/stwo/` (lib name = `stwo`, package = `stwo-vendored`) ships a deterministic mock behind the upstream STWO API surface (`Prover`, `verify`, `Proof`, `PublicInputs`, `ProverError`, `VerifyError`); real `keep-stwo/stwo@cipherocto-stable` source drop deferred (PATCHES.md tracks 3 patch categories: curve25519 SIMD, `#![feature]` removal, dep pins). `zk-vendor::vendored_stwo() -> Option<stwo::Prover>` accessor + `VendoredPublicInputs/VendoredProof/vendored_verify/VendoredVerifyError` re-exports (feature-gated by `vendored-stwo`, default-on). `zk-verifier::verify_capability_zk` delegates with layering FFI > vendored > BLAKE3 stub. `git grep "+nightly" -- crates/zk-vendor/stwo/` empty; `git grep "#!\[feature" -- crates/zk-vendor/stwo/` empty; MSRV 1.75.0 pinned in `crates/zk-vendor/rust-toolchain.toml`. vendored commitment byte-identical to `zk_verifier::stub_commitment` (cross-impl shape test in `crates/zk-vendor/stwo/src/lib.rs::commitment_matches_zk_verifier_stub_shape`).
