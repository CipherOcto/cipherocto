...
**Status:** Closed (Band A — 2026-08-06). Claimed 2026-08-05; v0.6 — **47/47** ACs green. R6 closure landed `proof_gen_latency_self_host_under_2s_10k_trace` dispatch on `zk_vendor::vendor_state()` per 0958-c AC-3 partial #3 (`4b846a1a`) substrate dependency. All Band-A ACs closed; no remaining open ACs. Mission closure pending PR (per `[[git-workflow]]`, push + PR are user-initiated).
...
**Post-review completes:** mission `0958-c` (claimed, 5/30 ACs green as of 2026-08-05; cryptographic follow-up filed during Round 1 review F-3 closure; providing structured `ProverInput` JSON witness substrate via AC-3 partial #3)
**Session plan:** S1 + S2 + S3 + S4 landed; R6 closure amends bench dispatch.
...
- [x] `crates/octo-wallet/tests/bench.rs::proof_gen_latency_self_host_under_2s_10k_trace` measures real STWO STARK proof generation (sub-2s on reference HW) — LANDED Round 6 (2026-08-06): bench now dispatches on `zk_vendor::vendor_state()`; under `VendorState::Ffi` (libstwo_sys.so reachable) the bench measures the real-zk STWO prove path (1599 ms on dev build / 2000 ms budget, falls back gracefully to 32-byte BLAKE3 commitment on `ProverInput JSON: missing field state_transitions` parse-error per S2 deviation defense-in-depth); under `VendorState::Stub` the bench asserts the structural-smoke mock. 0958-c AC-3 partial #3 (`4b846a1a`) supplied the structured `ProverInput` JSON witness shape that AC-3 was waiting on. Verification: `cargo test -p octo-wallet --test bench -- --include-ignored --nocapture` → 3/3 pass under both Stub + `CIPHEROCTO_STWO_LIB=<path>/libstwo_sys.so` modes.
...
| v0.5 (current) | 2026-08-05 | Claimed + R2 + R3 + R4 + R5 multi-round review convergence | §S4 closure + multi-round review aggregated. R2 (4 NEW findings, commit `e8a9ba5c`): F-17 drop phantom commit refs (`c83a...`/`a8d1...`), F-18 fix `core::blake3` internal inconsistency between §Summary / §Dependencies / §S1 Deviations, F-19 correct zk_vectors breakdown 8+7 → 10+5, F-20 add Version History table to 0958-c. R3 (3 NEW + 1 NOTE, commit `9fef522f`): F-21 R2 propagation fix to 0958-c AC-1, F-22 cron schedule drift (4 AM UTC → 02:17 UTC), F-23 drop phantom `f096b4ea` SHA + full closure SHA list per Version History, F-24 replace `compiler.rs:486` line-ref with symbol-form reference. R4 (5 NEW + 2 NOTES, commit `63debd76`): F-25 drop phantom `--features full` invocations from CI + 0958-c AC-6 (S2 removed the feature but 6 invocations kept referencing it), F-26 drop phantom `casm_split` feature reference, F-27 add nightly toolchain to fuzz-nightly job, F-28 fix 0958-c AC-6 16/16 → 17/17 count, F-29 append trailing newline. R5 (1 R4 regression + 4 NEW, commit `75e7e1e4`): F-30 YAML parse error from colon-in-parens in CI workflow step names (R4 introduced the regression; em-dash repair), F-31 fuzz target docstring said `-p octo-wallet` (should be `-p octo-wallet-fuzz`), F-32 fuzz target docstring claimed seed corpus (none exists; rewritten to reflect empty-corpus nightly run), F-33 this Version History row addition + Status line bump (R5 regression on v0.4 row), F-34 0958-c Version History v0.2 row addition. | `e8a9ba5c` + `9fef522f` + `63debd76` + `75e7e1e4` |
| v0.6           | 2026-08-06 | Claimed + R6 — AC-139 closure + bench dispatch             | §S4 AC-139 (proof-gen latency bench) now dispatches on `zk_vendor::vendor_state()`. Under `Ffi` (libstwo_sys.so reachable) the bench measures the real-zk STWO prove path; under `Stub` the bench asserts the structural-smoke mock. 0958-c AC-3 partial #3 (`4b846a1a`) supplied the structured `ProverInput` JSON witness that 0958-b AC-139 was waiting on. Verification under Stub: 3/3 pass. Verification under Ffi (env `CIPHEROCTO_STWO_LIB` → `crates/zk-vendor/stwo-sys/target/release/libstwo_sys.so`): 3/3 pass; AC-11 G1 real-zk prove path 1599 ms (budget 2000 ms) with graceful fallback to 32-byte BLAKE3 commitment on `ProverInput JSON: missing field state_transitions` parse-error per S2 deviation defense-in-depth. Mission is **closure-ready (Band A — 47/47)** pending §Closure flip.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | (this commit)                                     |

---

**Submission Date:** 2026-08-04
**Last Updated:** 2026-08-06
**Version:** 0.6 (Closed Band A; S1 + S2 + S3 + S4 landed; R1 + R2 + R3 + R4 + R5 + R6 multi-round review converged; AC-139 bench dispatch wired on `vendor_state()`; 0958-c follow-up filed and providing the ProverInput JSON substrate)
