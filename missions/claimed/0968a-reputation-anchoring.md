...
- [ ] `reputation_anchors` is queryable by `did` (joined via `reputation_events`)
  — *ungrounded*: no query path grounded; the migration is a file on disk only.
- [ ] `reputation_anchors` stores only `EventId` values (not `AttestationId`)
  — *ungrounded*: schema-file-level only (no running store).
...
- [x] DID-rotation finality interaction (`MIN_FINALITY_BLOCKS = 12` threshold)
  — *ground, constant*: `crates/octo-reputation/src/constants.rs:42` `pub const MIN_FINALITY_BLOCKS: u64 = 12;` (replaces the deprecated `MIN_REPUTATION_ANCHOR_FINALITY_BLOCKS` alias at lines 45-49). The constant is declared and referenced in RFC-0955 §"Finality". The DID-rotation finality handler in the anchoring job is NOT wired — the job (`anchor_job.rs:119` `pub struct StubChainAnchorSubmitter;` + `plan_batches` at line 152) is a stub submitter, not a live reorg-aware finality check.
- [x] `AnchorTupleFanoutExceeded` (`MAX_ANCHORED_TUPLES_PER_CONTROLLER_PER_DAY = 100`)
  — *ground, constant + variant*: `crates/octo-reputation/src/constants.rs:66` `pub const MAX_ANCHORED_TUPLES_PER_CONTROLLER_PER_DAY: u64 = 100;`. The variant `ReputationError::AnchorTupleFanoutExceeded(u64) = 0x2A` at `crates/octo-reputation/src/error.rs:141` is declared. The anchor-time fanout check is grounded in `anchor_job.rs:160` `if exceeds_daily_fanout(existing_anchored_today, aggregates.len()) { ... }`. Verified end-to-end at the scheduler level; the **on-chain submission path** that emits the `0x2A` variant on overflow remains ungrounded.
...
- [ ] Test vectors at `crates/octo-reputation/tests/anchoring/canonical_blobs.rs::CANONICAL_ANCHOR_BLOB`
  — *ungrounded*: `find crates/octo-reputation/tests -name 'canonical_blobs.rs'` returns no file. The test path is aspirational.
