...
Closed (Band A — 2026-08-06). Claimed 2026-08-04 by @mmacedoeu; implementation landed (commit `2f078974`-prior): `crates/octo-wallet/src/capability/hop_envelope.rs` (387 lines) ships all 4 RFC-0970 §Data Structures types (`HopEnvelope`, `HopCapability`, `HopScope`, `InnerRequest`) with manual redacting Debug impls, all 4 §Algorithms (`wrap_for_hop`, `unwrap_at_destination`, `verify_chain_hash` FREE FUNCTION per R7-N3, `pure_forward` returning `InvalidScope` by design per Finding A22), 5-variant `HopError` enum, and `ForwardRequestPayload` extension (RFC-0970 §Phase 4 + RFC-0870 §Roles — landed in same file because it shares `InnerRequest`/`HopEnvelope`). 9/9 unit tests pass (`hop_scope_variants`, `hop_envelope_debug_redacts`, `wrap_then_unwrap_roundtrip`, `unwrap_audience_mismatch`, `unwrap_ttl_exceeded`, `verify_chain_hash_matches_last_envelope`, `verify_chain_hash_mismatch`, `forward_request_payload_new_has_no_hop_envelope`, `forward_request_payload_with_hop_envelope`). `HolderKind::HopCapability = 0x03` discriminator exists in `crates/quota-router-storage/src/holder_kind.rs`. 11/15 ACs green (4 types + 4 algorithms + Manual redacting Debug + TTL ms resolution + cross-crate compat). 4/15 ACs explicit deferrals with named owner per [[deferred-vs-unspecified]]: TV2 (three-hop chain) + TV3 (replay detection via HolderRegistry) + TV6 (RFC-0853 channel encryption) + TV7 (hop signature forgery with Ed25519 verification) + `HolderRecord::from_hop_capability` constructor + `DestinationNonceStore` stub + `node_epoch` plumbing + `audit_replay_log` append-only log → `0970-a1-holder-binding-and-crypto`.
...
- [ ] `InnerRequest` payload is encrypted (per Finding A16: compromised intermediate router MUST NOT read inner content). Use RFC-0853 channel encryption. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (cross-mission substrate: RFC-0853 channel encryption not yet bound to `InnerRequest.ciphertext` field; `InnerRequest` struct holds raw `Vec<u8>` ciphertext field but encryption binding deferred).

### HolderRegistry binding

- [ ] `crates/octo-wallet/src/capability/holder_registry.rs` (MODIFY) — `HolderRecord::from_hop_capability(hop_cap: HopCapability, mint_at_unix_ms: i64) -> Self` constructor. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (cross-mission co-author contract with `0957-c`; `HolderRecord` base struct exists but constructor binding for `HopCapability` not yet wired).
...
- [ ] `crates/quota-router-core/src/node/wrap.rs` (NEW) — `wrap_for_hop(inner: &InnerRequest, hop_key: &Ed25519Keypair, ttl_millis_unix: u64, node_epoch: u64) -> Result<HopEnvelope, HopError>`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (location drift: mission text specified `crates/quota-router-core/src/node/wrap.rs` but the algorithm landed in `crates/octo-wallet/src/capability/hop_envelope.rs`; signature drift: actual implementation uses `hop_key: &[u8; 32]` not `&Ed25519Keypair`, `wrapping_node_did: &str` + `next_hop_did: &str` not `(Did, Did)`, and lacks `node_epoch` parameter — all deviations documented inline).
- [ ] `unwrap_at_destination(envelope: &HopEnvelope, chain: &[HopEnvelope], expected_destination: Did, clock: &dyn Clock) -> Result<InnerRequest, HopError>` — chain verify + unwrap. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (signature drift: actual implementation uses `(envelope, expected_destination: &str, now_millis_unix: u64)` — no `chain` parameter, no `Clock` trait, no `Did` newtype; chain verify is a separate `verify_chain_hash` FREE FUNCTION per R7-N3).
...
- [ ] `crates/octo-wallet/src/capability/nonce_store_stub.rs` (NEW) — working stub for `DestinationNonceStore`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (cross-mission: full signature promotion to RFC-0009-B1 / RFC-0957-A2).
- [ ] Stub API: `pub struct DestinationNonceStoreStub; impl DestinationNonceStoreStub { pub fn new() -> Self; pub fn check_and_consume(&mut self, destination: Did, nonce: [u8;32], node_epoch: u64) -> Result<(), NonceError>; }`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]**.
...
- [ ] TV2: Three-Hop Chain — three sequential `wrap_for_hop` calls; destination unwraps full chain; `verify_chain_hash` matches. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (2-hop variant already covered by `verify_chain_hash_matches_last_envelope`; 3-hop variant not yet added).
- [ ] TV3: Replay Detection — duplicate `HopEnvelope.hop_envelope_id` returns `HopError::ReplayDetected`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (requires `DestinationNonceStore` stub + `HolderRegistry::lookup_active(hop_envelope_id)` integration).
...
- [ ] TV6: Intermediate Router Compromise — Inner Content Encrypted — compromised intermediate router reads `InnerRequest`; payload is encrypted (RFC-0853 channel encryption). → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (cross-mission: RFC-0853 channel encryption binding).
- [ ] TV7: Hop Signature Forgery — tampered `hop_signature` returns `HopError::HopSignatureInvalid`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (requires Ed25519 signature verification on `hop_cap.signature: [u8; 64]`; current implementation uses `blake3::hash(hop_key)` as signature derivation stub).
...
- [ ] TV10: Pure Forwarder — `pure_forward` produces a `HopEnvelope` with `HopScope::PureForwarder`; no `HolderKind::HopCapability` row inserted in `HolderRegistry`. → **DEFERRED to `0970-a1-holder-binding-and-crypto` per [[deferred-vs-unspecified]]** (current `pure_forward` returns `InvalidScope` by design per Finding A22; integration test verifying NO `HolderKind::HopCapability` row requires `HolderRegistry` harness).
...
| AC-3: `InnerRequest` payload encrypted                 | DEFERRED                  | `0970-a1-holder-binding-and-crypto` (RFC-0853 binding)                    |
| AC-4: `HolderRecord::from_hop_capability` constructor  | DEFERRED                  | `0970-a1-holder-binding-and-crypto`                                       |
| AC-5: `HolderKind::HopCapability = 0x03` discriminator | GREEN                     | `crates/quota-router-storage/src/holder_kind.rs`                          |
| AC-6: `wrap_for_hop` algorithm                         | DEFERRED                  | `0970-a1-holder-binding-and-crypto` (Ed25519 + node_epoch plumbing)       |
| AC-7: `unwrap_at_destination` algorithm                | DEFERRED                  | `0970-a1-holder-binding-and-crypto` (chain param + Clock trait)           |
...
| AC-10: `DestinationNonceStore` stub                    | DEFERRED                  | `0970-a1-holder-binding-and-crypto` (RFC-0009-B1 / RFC-0957-A2 promotion) |
| AC-11: Stub API surface                                | DEFERRED                  | `0970-a1-holder-binding-and-crypto`                                       |
