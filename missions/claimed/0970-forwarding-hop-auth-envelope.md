...
**Cross-crate compat:** `cargo fmt --check` clean (verified 2026-08-07). `cargo clippy --workspace --all-targets --features full -- -D warnings` GREEN (2m 10s, post workspace-exclude commit `b99b1709` resolving the legacy `tdlib-rs` feature-conflict blocker). Package-scoped clippy on touched crates clean.
