...
| AC-3 (On-chain proof submission to Stoolap) | **HARD-BLOCKED**              | No settlement carrier claimed. `missions/archived/0968-b-marketplace-integration.md` is COMPLETED (read-side reputation integration, 2026-07-30) but is the reputation/integration layer, NOT the settlement/octostream layer required by AC-3. The actual settlement carrier for proof-on-chain submission (RFC-0959 Ask Settlement Chain + RFC-0960 Grand Design Vaults, both Accepted) is not yet wired to the `crates/zk-verifier/` output path. RFC-0950 (Agent Mission Marketplace) was archived 2026-08-06 as superseded by the grand-design family. |
...
**Soft-block context:** "Mission: Stoolap Provider Integration" (`missions/archived/superseded/stoolap-provider-integration.md`, SUPERSEDED 2026-08-16 by the 0900-0999 economics family) was previously listed as the gate; its own open status + Multi-Provider Support block (now `missions/archived/superseded/multi-provider-support.md`, same superseded batch) no longer applies. This mission does NOT depend on those archived missions for AC-1, AC-2, AC-5 — those are pure verifier + UI work. Only AC-3 + AC-4 inherit the marketplace-carrier gap (still tracked separately under RFC-0902 multi-provider routing).
