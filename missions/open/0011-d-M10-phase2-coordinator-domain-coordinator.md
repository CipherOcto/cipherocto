...
description: Phase 2 (gated) per RFC-0011-d §Mission Decomposition M10 row: add `octo role select coordinator` + `domain-coordinator` clap subcommands + substrate entrypoints; gate on RFC-0855p-d (INDEX; chain: RFC-0855p-d1 + RFC-0855p-d2 + RFC-0855p-d3) + RFC-0855p-e reaching Accepted.
metadata:
...
- "Add `octo role select coordinator` + `domain-coordinator` clap subcommands + substrate entrypoints; gate on RFC-0855p-d (INDEX; chain: RFC-0855p-d1 + RFC-0855p-d2 + RFC-0855p-d3) + RFC-0855p-e reaching Accepted"
...
**HARD GATE**: RFC-0855p-d (INDEX; chain: RFC-0855p-d1 + RFC-0855p-d2 + RFC-0855p-d3) + RFC-0855p-e MUST reach Accepted status before this mission claims. Per [[deferred-vs-unspecified]] + BLUEPRINT §Mission Lifecycle.
...
- New error variant `RoleNotSelectable { reason: "RFC-0855p-d (INDEX; chain: RFC-0855p-d1 + RFC-0855p-d2 + RFC-0855p-d3) not Accepted" | "RFC-0855p-e not Accepted" | ... }` (already in M7 §variant list)
...
- `octo-coordinator` (Layer B; per RFC-0855p-d (INDEX; chain: RFC-0855p-d1 + RFC-0855p-d2 + RFC-0855p-d3)) — domain-coordinator substrate (M11)
