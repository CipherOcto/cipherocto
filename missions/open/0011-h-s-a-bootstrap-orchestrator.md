# 0011-h-s-a-bootstrap-orchestrator — Substrate additions for BootstrapOrchestrator + BootstrapConfig + BootstrapState

## Status

Open (2026-09-18) — Substrate-additions prerequisite per RFC-0011-h §Substrate-Additions Companion Missions row G1
...
Adds `BootstrapOrchestrator::start_bootstrap` + `::status()` + `BootstrapConfig::{from_toml, save_toml}`. Pre-requisite for `octo network mode show/set`, `octo network bootstrap`, `octo network status`.
...
- [ ] Substrate additions land in `crates/octo-network/src/mon/bootstrap.rs` per RFC-0011-h §Substrate-Additions row G1
