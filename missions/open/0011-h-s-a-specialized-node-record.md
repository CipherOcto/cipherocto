# 0011-h-s-a-specialized-node-record — Substrate additions for SpecializedNodeRecord::load + bind_to_did

## Status

Open (2026-09-18) — Substrate-additions prerequisite per RFC-0011-h §Substrate-Additions Companion Missions row G11

## RFC

RFC-0011-h §Substrate-Additions Companion Missions row G11

## Summary

Required by `octo network node show/bind` (deferred).

### Substrate additions target

```rust
// crates/octo-network/src/specialized/node_record.rs (NEW)
record
```
...
- [ ] Substrate additions land in `crates/octo-network/src/specialized/node_record.rs (NEW)` per RFC-0011-h §Substrate-Additions row G11
