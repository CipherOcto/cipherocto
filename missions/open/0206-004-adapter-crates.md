...
description: Open 2026-08-20 v2.0; RFC-0206 v2.0 §Adapter Crate List + §Cargo.toml Templates Layer B facade — 5 adapter crates (octo-vault-storage, octo-reputation-storage, octo-cap-macaroon-vault-storage, octo-matrix-session-store-storage, octo-policy-storage) + facade + 5 trait declarations (4 NEW + 1 MOVE: VaultLookup) + per-adapter fixtures. `octo-ident-storage` is the 6th adapter crate but owned by `0206-005-octoident-storage-crate`.
metadata:
  node_type: mission
  type: project
...
# Mission `0206-004-adapter-crates` v2.0 — OPEN 2026-08-20
...
Land RFC-0206 v2.0 §Adapter Crate List + §Cargo.toml Templates Layer B facade: 5 per-owner adapter crates + `crates/octo-storage/` facade + 5 trait declarations (4 NEW + 1 MOVE) + per-adapter test fixtures. Closes TV-0206-A6, TV-0206-A8 (adapter side), TV-0206-A9(a/b), TV-0206-A10, TV-0206-A11, TV-0206-A12, TV-0206-A14 gates.
...
**Out of mission scope**: `octo-ident-storage/` is the 6th adapter crate (RFC §Adapter Crate List row 3 — StoolapDidRegistry impl target) but is owned by `0206-005-octoident-storage-crate`, NOT this mission. This mission creates 5 adapters + facade + 5 trait declarations.
...
- Cross-TV: TV-0206-A3 (wildcard detector on substrate + facade) — substrate side owned by `0206-001`, facade side gated here
- **Per-trait register fns** (per RFC §Wiring Pattern, NOT generic `register<V: OwnerTrait>` helper): `register_vault_store`, `register_reputation_store`, `register_vault_lookup`, `register_session_store`, `register_policy_store` — each returns `Arc<dyn Trait>` for typed dispatch
...
- `Cargo.toml`: declares `octo-storage = { path = "../octo-storage" }` ONLY (per RFC §Cargo.toml Cross-Cuts "NOT direct substrate" rule); NO direct `stoolap` dep (TV-0206-A9(a))
- `Cargo.toml` escape hatch: if a specific adapter needs to import a substrate-only type (e.g. `TypedStatement` variant), document the exception in an inline `# WHY` comment referencing the RFC §Cargo.toml Cross-Cuts rule
- `src/lib.rs`: declares trait (NEW) + impl + per-trait register fn
- `tests/register_roundtrip.rs`: per-trait register fn + select + insert round-trip TV
...
- `crates/octo-storage/` facade exists with 4-item re-export set per RFC §Cargo.toml Templates Layer B
- TV-0206-A14 gate: `rg '\b\*\s*[,;}]' crates/octo-storage/src/lib.rs` output count equals 0
- TV-0206-A3 gate (facade side): `rg '\b\*\s*[,;}]' crates/octo-storage/src/lib.rs` output count equals 0 (substrate side owned by `0206-001`)
...
- New: `crates/octo-storage/Cargo.toml` (4-item re-export facade; revises current 8-item re-export set per RFC v2.0 surface)
- Edit: `crates/octo-storage/src/lib.rs` (replace 8-item `pub use` list with 4-item RFC v2.0 list: `Database`, `TypedStatement`, `AdapterAllowlist`, `register`; pre-edit 8 items: `apply_pending`, `open`, `open_in_memory`, `ApplyConfig`, `Migration`, `StaticMigration`, `StorageError`, `DEFAULT_TRACKER_TABLE`)
...
- RFC-0206 v2.0 §Three-Tier Architecture (Tier 2 = facade)
- RFC-0206 v2.0 §Cargo.toml Templates Layer B facade
- RFC-0206 v2.0 §Adapter Crate List
- RFC-0206 v2.0 §Cargo.toml Cross-Cuts (adapter deps via facade, not substrate)
- RFC-0206 v2.0 §Wiring Pattern (per-trait register fns)
- RFC-0206 v2.0 §Format Bypass Defense (adversarial fixtures)
- RFC-0206 v2.0 §Test Vectors TV-0206-A6, A8 (cross-ref), A9(a), A10, A11, A12, A14 (3 cross-ref)
...
- Mission `0206-001-substrate-newtype` (substrate `Database` type + TypedStatement enum + AdapterAllowlist)
- Mission `0206-002-layer-b-type-renames` (Layer B type renames in quota-router-storage + octo-vault)
...
- `0206-002-layer-b-type-renames` (29 Layer B type renames must land before adapter crates type-check)
