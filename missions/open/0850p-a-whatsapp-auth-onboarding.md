...
- [ ] `logging.rs`: redaction layer tests (8 cases) — including one that verifies `pn` is **NOT** in the redact keys (R3-H1: `assert!(!REDACT_KEYS.contains(&"pn"))`) and one that verifies the resolved `self_phone` log line in `Event::Connected` handler is **not** redacted (`assert!(formatted.contains("+1 555 123 4567"))`)
