...
- [ ] `cairo/src/lib.cairo` (function `main`) decomposed into 3 sub-functions: `verify_chain`, `verify_holder_sig`, `verify_inference_fold` (each targeting < 10 KB CASM as a soft budget; hard ceiling is the AC-4 hard constraints below per Round 2 fix F-36). Round 1 fix F-03: path syntax.
...
- **Upstream STWO composition API citation to be filled at claim time.** Round 2 fix F-40: the R1 fix F-09 added a placeholder citation requirement (`starkware-industries/stwo@<sha>` / `mmacedoeu/stwo@<sha>`); the concrete SHA is filled at mission-claim time when upstream is reachable. Owner: 0958-c claimant. Target: AC-4 claim time.
...
- **0958-a R4 Register items that 0958-b S1–S4 did not close.** Round 1 fix F-04: scope is L1–L3/L5 LOW items. Closure rationale goes in 0958-a as a new v0.5 row. Owner: 0958-c claimant (in coordination with 0958-a claimant @cipherocto per 0958-a line 6). Target: AC-5 closure date.

### AC-6 Deviations (clippy + tests + bench + fuzz)

- (Round 2 fix F-34: Reference HW profile deviation moved to §AC-4 Deviations.)
