...
- **Closed upstream inheritance — Corelib pin duplication.** Round 4 fix F-55: moved from §Risks row 5 (where it was masquerading as an active risk) to §AC-1 Deviations as a closed upstream-inheritance item. Closed by 0958-a R4 H5; 0958-c inherits the pin. Owner: N/A (closed). Target: N/A (closed).
- **Future corelib `core::blake3` migration** (Round 4 fix F-53): REMOVED from §AC-1 Deviations. The bullet contradicted its own "NOT in 0958-c scope" claim by being filed under a 0958-c Deviation. Corelib migration is tracked separately as a 0958-c post-v1 follow-up work item — when a future mission picks it up, that mission's §Acceptance Deviations will own the entry.

### AC-2 Deviations (Ed25519 holder signature verify)

(none; Round 4 fix F-58 removed the [mitigated] bullet per Round 2 fix F-41 — closed items belong in §Version History, not in §Acceptance Deviations. F-10's inline-default resolution is tracked in §Version History v0.3 row.)
...
- **STWO recursive composition fork plan.** Round 1 fix F-09 + Round 3 fix F-50 + Round 4 fix F-54: AC-4 baseline plan is to add a forked STWO commit (or upstream PR) that lands the composition API; if the upstream merge is rejected, AC-4 escalates to a corelib-pinned fork. Owner: 0958-c claimant. Target: AC-4 implementation start.
- **STWO upstream commit SHA citation.** Round 2 fix F-40 + Round 3 fix F-50 + Round 4 fix F-54: concrete SHA citation (`starkware-industries/stwo@<sha>` / `mmacedoeu/stwo@<sha>`) at mission-claim time when upstream is reachable. Owner: 0958-c claimant. Target: AC-4 claim time.
...
- **Reference HW profile not defined.** Round 2 fix F-34 + Round 4 fix F-56: moved from §AC-6 Deviations (where it was mis-located) to §AC-4 Deviations. AC-4 inherits RFC-0958 AC-11 gate (`<2s SelfHost 10K trace steps on reference HW`). Reference HW profile specification (CPU model + core count + RAM + STWO build flags + OS) goes in `docs/07-developers/zk-capability-circuit-guide.md` `§Performance targets` (the actual containing section carrying the inline `**Reference HW:**` line) before AC-4 closure. Owner: 0958-c claimant. Target: AC-4 closure date.

### AC-5 Deviations (honest-disclosure closure)

- **0958-a R4 Register items that 0958-b S1–S4 did not close.** Round 1 fix F-04 + Round 3 fix F-43 + Round 4 fix F-52: scope is L1–L3/L5 LOW items. Closure rationale goes in 0958-a as a new v0.5 row. Owner: 0958-c claimant (in coordination with 0958-a claimant @cipherocto per `missions/claimed/0958-a-zk-capability-circuit.md` §Status preamble blockquote — `Claim gate (2026-07-22): Mission claimable per BLUEPRINT Mission Lifecycle ... Claim filed 2026-07-22 by @cipherocto.`). Target: AC-5 closure date.

### AC-6 Deviations (clippy + tests + bench + fuzz)

- **[no open deviations] at this AC** (Round 4 fix F-59): explicit marker per §AC-2 [mitigated] convention. AC-4 §Reference HW profile moved here from §AC-6 per Round 2 fix F-34.
...
| v0.6 | 2026-08-05T23:59:55Z | Open | **Round 4 of 0958-c's own review (NOT 0958-b's review).** 9 NEW findings closed: 1 CRITICAL (F-51 R3 F-44 closure was FALSE — §Version History v0.5 row + §AC-4 first checkbox claimed 'design doc updated to reflect 0958-c's renamed + new sub-functions' but the design doc was NOT actually modified; R4 fix rewrote `docs/plans/2026-08-05-stage-2-verifier-split.md` end-to-end: renamed `verify_hmac_chain` → `verify_chain`, `verify_poseidon_fold` → `verify_inference_fold`, added `verify_holder_sig` (Ed25519 sub-circuit), updated Cost Estimate table (HMAC-BLAKE3 ~5 KB vs prior SHA-256 ~150 KB; Ed25519 ~5 KB new), added §History section documenting the 0958-b S2d HMAC-SHA-256 era vs 0958-c BLAKE3 + Ed25519 era, fixed `crates/zk-circuit/tests/casm_snapshot.rs` new-test name reference, fixed dev guide `§Performance targets` `**Reference HW:**` inline text — replaced phantom 'RFC-0958 §Implementation Reference' cross-reference with actual 'RFC-0958 §Performance Targets' + added STWO build flags + OS specs); 4 MAJOR (F-52 §AC-5 Deviations phantom '§Claim gate' (no such section in 0958-a) — replaced with actual `§Status preamble blockquote` reference, F-53 §AC-1 Deviations second bullet contradicted its own 'NOT in 0958-c scope' claim — removed, F-54 §AC-4 Deviations first bullet had ONE Owner + TWO Targets — split into 'STWO recursive composition fork plan' + 'STWO upstream commit SHA citation' bullets, F-55 §Risks row 5 closed deviation masquerading as active risk — moved to §AC-1 Deviations as closed upstream-inheritance item, retro-marked with `~~strikethrough~~` in §Risks table), F-56 §AC-4 Deviations last bullet references phantom '§Reference HW' + 'RFC-0958 §Implementation Reference' — corrected to actual `§Performance targets` section); 4 MINOR (F-57 §Status line 11 dangling 'see F-06 fix below' cross-reference — replaced with explicit '§Risks row 6' reference, F-58 §AC-2 Deviations [mitigated] bullet violates form statement — removed (closed items belong in §Version History), F-59 §AC-6 Deviations empty subsection — replaced with explicit `[no open deviations] at this AC` marker, F-17 sub-bullet — added 'Corelib pin duplication closed by 0958-a R4 H5' traceability to §AC-1 Deviations). | `ebae2af004f5d5a2b756406e340feec730c5e3b` |
