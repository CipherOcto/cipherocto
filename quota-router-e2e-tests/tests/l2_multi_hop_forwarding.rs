...
use quota_router::RouterNodeError;
...
async fn l2_t3_policy_cheapest() {
    ...
    ctx.policy_override = Some(quota_router::request::RoutingPolicy::Cheapest);
...
async fn l2_t4_policy_fastest() {
    ...
    let mut ctx = make_request("gpt-4o");
    ctx.policy_override = Some(quota_router::request::RoutingPolicy::Fastest);
    let result = cluster.nodes[0].route(&ctx, b"hello").await;
