...
pub enum Caveat {
    ...
    /// Verify-time check via `VaultLookup` trait (per review doc §20.6.1).
    #[serde(rename = "vault")]
    ...
    /// Verify-time check (RFC-0957 §Verify-Time Extension): parent chain MUST
...
/// **Wire form:** snake_case via `#[serde(rename_all = "snake_case")]` —
/// `NativeTokenTransfer` → `"native_token_transfer"`, etc. (Matches real
/// source and TV-0957-02 byte-exact pin.)
...
**Wire-form pinning:** Each new variant MUST have at least one byte-exact TV fixture in `crates/octo-cap-macaroon/tests/tv_0957_verify_time.rs` (TV-0957-01..09 cover the 9 variants; TV-0957-10 covers the `Raw` unknown-name rejection path; TV-0957-11..15 pin the verify-time steps; TV-0957-16..20 are regression tests; TV-0957-21 is a depth-3 multi-level WrappedOnly chain; TV-0957-22 is the MAX_WRAPPED_DEPTH=16 boundary per RFC-0965 §3.7 R7-F1). `PolicyReference.attenuation_witness` is serialized via `serde_bytes_arr64` (hex-string wire form); see TV-0957-09 for the byte-exact pin.

**Wire-format compatibility:** prior Caveat wire forms (v0.1..v2.0) deserialize unchanged; v2.1 tokens are ADDITIVE — no v2.0 consumer upgrade forced.

**Cross-reference:** RFC-0965 §3 (variant field types + attenuation rules); RFC-0965 §3.2 (`PermissionKind` enum); RFC-0965 §3.8 (FactoryVet struct + ActionTemplate); `crates/octo-cap-macaroon/src/caveat/mod.rs` (implementation).
...
Per the review doc §20.6.1 (4-step algorithm), `Macaroon::verify_for_vault_op` extends the structural `verify` path above with substrate lookups and chain walks that require runtime context not available to the HMAC chain re-derivation alone.

```rust
/// Vault-aware verify path. Distinct from `verify_full` (which is
/// structural only — no substrate adapter required). Implemented in
/// `crates/octo-cap-macaroon/src/macaroon.rs::verify_for_vault_op`.
///
/// 4-step algorithm per the review doc §20.6.1 (signature + wrapped-chain
/// integrity + per-vault lookup loop + optional attenuation
/// subsumption vs `expected_parent`):
///   step 1: `verify_signature(root_secret)` — HMAC chain
///           re-derivation against `root_secret`
///   step 2: `check_wrapped_chain(self, catalog)` — RFC-0965 §3.7
///           chain integrity (cycle, depth, missing parent)
///   step 3: walk self + `WrappedOnly` ancestors; collect every
///           `Caveat::Vault(vault_id)`; for each, lookup the vault
///           row and assert (a) exists, (b) `chain_id == op_chain_id`,
///           (c) state=Active. Empty collection yields
///           `VaultVerifyError::WrappedChainHasNoVault` per this
///           subsection (chainless-parent safe default).
///   step 4: attenuation subsumption vs `expected_parent` via
///           `set_subsumes_with_registry` (only if `Some`).
...
**`VaultLookup` trait injection:** `verify_for_vault_op` REQUIRES a `VaultLookup` adapter at config time. Production wiring of `OctoVaultLookup` (the substrate adapter bridging this trait to the `octo-vault` storage crate) is deferred to the S5.1 follow-on mission `0957-g1-octo-vault-lookup-glue` — out of scope for this v2.1 amendment; consumers of the v2.1 spec wire their own `VaultLookup` impl until S5.1 lands. The trait uses primitive types (`chain_id: [u8; 32]`, `is_active: bool`) — no `octo_vault::VaultState` enum import — to maintain Layer B → Layer A isolation per the §Architectural Principles (see `crates/octo-cap-macaroon/src/vault_lookup.rs` for the canonical type definitions).

**WrappedOnly invariant:** `WrappedChainHasNoVault` is enforced ONLY by `Macaroon::verify_for_vault_op` (the operational gate). `Macaroon::verify_full` (structural) validates signature + chain integrity + subsumption but does NOT perform the chainless-parent reject — that gate requires the substrate `VaultLookup` adapter, which the structural path intentionally does not depend on. Callers that need the chainless-parent invariant MUST use `verify_for_vault_op` (or pair `verify_full` with an explicit chain-walk + vault-row validation in their own glue).

**Cross-reference:** RFC-0965 §3 (9 new Caveat variants including Vault + WrappedOnly); RFC-0870 §NodeEnvelope Version Tag (wire-form `version_tag` byte participates in `envelope_id` derivation — replay-defense across the V1→V2 cutover); review doc §20.6.1 (algorithm source).
...
| 2.1        | 2026-08-17                                                                                                                                                                                                                                                                                                                                                                                          | **Accepted (amendment) — Verify-time invariant + Caveat DSL extension.** Added §Verify-Time Extension subsection documenting `Macaroon::verify_for_vault_op` (4-step algorithm per the review doc §20.6.1: signature verify → wrapped-chain integrity → per-vault lookup loop (exist + chain match + state=Active) → optional attenuation subsumption vs `expected_parent`) + `VaultLookup` trait injection (Layer B extension consumer, primitive types only — no `octo_vault::VaultState` enum import) + `WrappedOnly` parent-no-Vault-binding reject (only the operational gate; `verify_full` structural path is explicitly NOT required to enforce this — see §Verify-Time Extension). Added §Caveat DSL Extension subsection enumerating the 9 new Caveat variants: Vault + Permission + WrappedOnly + Factory map directly to RFC-0965 §3.x (Vault §3.1, Permission §3.2, AuditWindow §3.5, WrappedOnly §3.7, Factory §3.8); MaxUses maps to RFC-0965 §3.4; PolicyReference maps to RFC-0965 §3.10. **ValidRange + MaxPerTx are NEW in RFC-0957 v2.1** (ValidRange supersedes the RFC-0964 `Constraint::ValidRange`; MaxPerTx is distinct from RFC-0965 §3 amount-budget variants) — they have NO RFC-0965 §3.x number; the cross-ref here is RFC-0965 §3 conceptually (caveat-DSL surface), not §3.x specifically. + `PermissionKind` enum (5 variants per RFC-0965 §3.2, wire form snake_case via `#[serde(rename_all = "snake_case")]` matching source-of-truth `caveat/mod.rs`) + `FactoryVet` struct per RFC-0965 §3.8 — all implemented in `crates/octo-cap-macaroon/src/caveat/` per the v2.0 Per-Extension Crate Layout. Four pre-existing variants (`ValidAfter`, `RedemptionContext`, `Sharded`, `Payment`) remain unchanged by this amendment (plus additional v1 variants `AmountMax`, `PerAxisMax`, `Model`, `Provider`, `Before`, `Audience`, `RateLimit`, `InvocationHashBind`, `Jurisdiction`, `CacheStrategy`, `AskBinding`, `ThirdParty`, `Raw` that are also unmodified). Implementation mission: `missions/open/0957-c1-verify-time-amendment.md`; pre-req is mission `0957-g-verify-time-invariant` (LANDED 2026-08-17, commit `d007de54`). S5.1 substrate adapter `OctoVaultLookup` deferred to `0957-g1-octo-vault-lookup-glue` (Layer B glue crate pattern matches `TransportDeliveryCatalog`). Wire-form pins: 22 byte-exact TV fixtures in `crates/octo-cap-macaroon/tests/tv_0957_verify_time.rs` (TV-0957-01..10 Caveat DSL wire-form + Raw reject; TV-0957-11..15 verify-time step pins; TV-0957-16..20 regression including TV-0957-16/17 ancestor-scenario coverage after Round 2 re-scope; TV-0957-21 depth-3 multi-level WrappedOnly chain; TV-0957-22 MAX_WRAPPED_DEPTH=16 boundary per RFC-0965 §3.7 R7-F1). Attenuation monotonicity invariant preserved (new variants are typed-discriminator additions, no central enum edits). **Wire-format compatibility:** this amendment is ADDITIVE only — prior Caveat wire forms (v0.1..v2.0) remain valid; consumers MUST be able to deserialize v2.0 Caveats without modification. No v2.1 token forces an upgrade on a v2.0 consumer. **Risk acceptance:** landed outside §22 atomic-blocker bundle per user-chosen split-by-RFC decision (2026-08-17); see `docs/plans/2026-08-16-storage-layer-restructuring-execution-plan.md` §3 row 6. |
