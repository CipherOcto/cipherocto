...
pub enum Caveat {
    ...
    /// Audit window duration (RFC-0965 §3.4; 0 = instant).
    #[serde(rename = "audit_window")]
    AuditWindow { duration_secs: u64 },

    /// Max number of uses (RFC-0965 §3.5; 0 = unlimited).
...
**Wire-form pinning:** Each new variant MUST have at least one byte-exact TV fixture in `crates/octo-cap-macaroon/tests/tv_0957_verify_time.rs` (TV-0957-01..09 cover the 9 variants; TV-0957-10 covers the `Raw` unknown-name rejection path; TV-0957-11..15 pin the verify-time steps; TV-0957-16..20 are regression tests). `PolicyReference.attenuation_witness` is serialized via `serde_bytes_arr64` (hex-string wire form); see TV-0957-09 for the byte-exact pin.
...
///
/// 4-step algorithm per RFC-0957 §20.6.1 (signature + wrapped-chain
...
///           `VaultVerifyError::WrappedChainHasNoVault` per
///           §20.6.1 line 1328.
...
| 2.1        | 2026-08-17                                                                                                                                                                                                                                                                                                                                                                                          | **Accepted (amendment) — Verify-time invariant + Caveat DSL extension.** Added §Verify-Time Extension subsection documenting `Macaroon::verify_for_vault_op` (4-step algorithm per §20.6.1: signature verify → wrapped-chain integrity → per-vault lookup loop (exist + chain match + state=Active) → optional attenuation subsumption vs `expected_parent`) + `VaultLookup` trait injection (Layer B extension consumer, primitive types only — no `octo_vault::VaultState` enum import) + `WrappedOnly` parent-no-Vault-binding reject. Added §Caveat DSL Extension subsection enumerating the 9 new Caveat variants per RFC-0965 §3 (Vault, Permission, ValidRange, MaxPerTx, AuditWindow, MaxUses, WrappedOnly, Factory, PolicyReference) + `PermissionKind` enum (5 variants per RFC-0965 §3.2) + `FactoryVet` struct per RFC-0965 §3.8 — all implemented in `crates/octo-cap-macaroon/src/caveat/` per the v2.0 Per-Extension Crate Layout. Four pre-existing variants (`ValidAfter`, `RedemptionContext`, `Sharded`, `Payment`) remain unchanged by this amendment. Implementation mission: `missions/open/0957-c1-verify-time-amendment.md`; pre-req is mission `0957-g-verify-time-invariant` (LANDED 2026-08-17, commit `d007de54`). S5.1 substrate adapter `OctoVaultLookup` deferred to `0957-g1-octo-vault-lookup-glue` (Layer B glue crate pattern matches `TransportDeliveryCatalog`). Wire-form pins: 20 byte-exact TV fixtures in `crates/octo-cap-macaroon/tests/tv_0957_verify_time.rs` (10 Caveat DSL variant wire-form pins + 5 verify-time step pins + 5 regression tests). Attenuation monotonicity invariant preserved (new variants are typed-discriminator additions, no central enum edits). **Wire-format compatibility:** this amendment is ADDITIVE only — prior Caveat wire forms (v0.1..v2.0) remain valid; consumers MUST be able to deserialize v2.0 Caveats without modification. No v2.1 token forces an upgrade on a v2.0 consumer. **Risk acceptance:** landed outside §22 atomic-blocker bundle per user-chosen split-by-RFC decision (2026-08-17); see `docs/plans/2026-08-16-storage-layer-restructuring-execution-plan.md` §3 row 6. |
