...
#### Settlement-Time Vault Row Lookup (v2.0 wire form)
...
1. `cost_vault_id` present? Else reject with `SettlementError::CostVaultIdMissing` (the migration gate — pre-v2.0 rows are rejected by the v2.0 verifier until a follow-on migration populates them).
2. `vault_lookup.lookup_vault(cost_vault_id)` returns `Some(VaultRowSnapshot)`? Else reject with `SettlementError::CostVaultIdMissing` (no vault row at the given key — equivalent to "missing" for the v2.0 wire form).
3. `vault_snapshot.chain_id == envelope.chain_id`? Else reject with `SettlementError::ChainMismatch` (cross-chain settlement attempt).

**Substrate adapter reuse:** Step 2 reuses the `VaultLookup` trait from `octo_cap_macaroon::VaultLookup` (RFC-0957 §Verify-Time Extension + mission 0957-g LANDED). The settlement-time verifier takes `&dyn VaultLookup` (NOT a concrete `OctoVaultLookup`) — same trait that backs `Macaroon::verify_for_vault_op`. No shadow impl in `quota-router-storage` (the storage crate consumes the trait; the production adapter `OctoVaultLookup` lives in `octo-cap-macaroon-vault` per layer discipline).

**Why `&dyn VaultLookup`:** a concrete struct arg would force `quota-router-storage` to depend on `octo-cap-macaroon-vault` (Layer B substrate adapter), which is forbidden by the layer model (storage consumer crate depends on Layer B extensions, not on Layer B substrate adapters). The marketplace caller instantiates `OctoVaultLookup::new(substrate)` and passes it as `&dyn VaultLookup` to the verifier.

**Why pre-v2.0 rows are rejected (forward-only):** the v2.0 wire form appends Option-encoded bytes to the canonical preimage (see §Wire Format v2.0 below). Re-computing the v2.0 hash for a v1.0 envelope would require inferring the v2.0 fields from the v1.0 payload — there is no canonical derivation (the fields did not exist), so backfill is a no-op. The migration gate recognizes v1.0 rows as "legacy" and rejects them.

#### Cross-Chain Settlement Reject (v2.0 wire form)

When `envelope.chain_id != vault_row.chain_id`, the v2.0 verifier emits `SettlementError::ChainMismatch { vault_id, vault_chain_id, envelope_chain_id }`. The 3-tuple is the failure payload — both chain ids are reported so the auditor can trace which chains were involved.

**Audit query supported:** per review §20.7, "show all settlements against vault X" requires a UNIQUE INDEX on `cost_vault_id`. The v016 migration (`migrations/v016__settlement_chain_vault.sql`) adds `idx_se_cost_vault_id` + `idx_se_cost_vault_id_chain_id` composite index. Both are NULL-tolerant (Stoolap fork allows multiple NULLs in a UNIQUE column, so pre-v2.0 rows do NOT collide with each other).

**Attack class:** a malicious holder that holds a vault row on chain A attempting to settle with a `chain_id` pointer at chain B (where they hold no vault row but where the price is more favorable). The reject is enforced at the verifier layer — the underlying DB accepts the row ONLY when both fields are populated AND mutually consistent at the vault-row lookup.

#### Wire Format v2.0

The v2.0 `SettlementEnvelope` canonical preimage is the v1.0 preimage PLUS 2 optional-field suffixes:

```text
v1.0 preimage (model_wire || serde_json::to_vec(axes_consumed)
              || ask_id (32 bytes) || nonce (32 bytes)
...
**Canonical encoding of `Option<[u8; 32]>`:** 1-byte presence tag (0x00 = None, 0x01 = Some) followed by 32 bytes if `Some`. Total = 1 byte for `None`, 33 bytes for `Some`.

**Forward-compat:** v1.0 hashes are NOT replayed as v2.0 — the preimage change forces re-derivation. The stderr migration gate (`cost_vault_id IS NULL` → `CostVaultIdMissing`) prevents v1.0 rows from being verifiably replayed against v2.0 verifiers.

**Class A determinism:** the v2.0 preimage is byte-deterministic when:

- All 6 v1.0 inputs are identical (see §Settlement hash above)
- `cost_vault_id` and `chain_id` are encoded per the canonical Option tag rule
- `serde_json::to_vec(axes_consumed)` matches `Vec<(String, u64)>` JSON array form

**Reference implementation:** `crates/quota-router-storage/src/ask.rs::SettlementEnvelope::compute_settlement_hash` (mission 0959-c1 LANDED 2026-08-19). Cross-impl byte-equivalent assertion in `crates/quota-router-core/tests/eleven_step.rs::cross_impl_tv1_settlement_hash_matches` (impl1 = production; impl2 = hand-rolled reference; both MUST produce identical 32-byte digests).
...
stateDiagram-v2
  ...
  Recovering --> Active: rate stable < 0.85; window reset (R6 clarification: dual predicate per RFC-0959 §Lifecycle Requirements — rolling 100-call window avg strictly below 0.85 AND no individual measurement crossing 0.90)
...
| Approach                                                                                                                                                     | Pros                                                                                                                           | Cons                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Amend RFC-0909 (status quo RFC-0959)                                                                                                                    | Smaller surface; reuses RFC-0909 acceptance                                                                                    | FALSE PREMISE — RFC-0909 hashes SpendEvent via SHA-256, not BLAKE3 over (api_key_id, axis_consumed, invocation_hash); amendment would require rewriting RFC-0909's hash surface, breaking v69 verifiers; Option B (deep amend) requires RFC-0903 + RFC-0909 maintainer coordination |
...
| Bind `(cap_root_hash, ask_id, invocation_hash)` via separate signature envelope (RFC-0959)                                                              | All fields bound                                                                                                               | Replay defense absent without consumed-receipt index; cost not in hash; envelope signing schema inconsistent; forward-compat v69↔v70 contradictions                                                                                                                                 |
...
| 2.0        | 2026-08-19                                                                                                                                                                                                                                                                                                                                                                                          | **Wire-format amendment (mission 0959-c1, review §20.7).** Adds `cost_vault_id: Option<[u8; 32]>` + `chain_id: Option<[u8; 32]>` to `SettlementEnvelope`. The v2.0 canonical preimage appends a 1-byte Option presence tag (0x00 = None, 0x01 = Some) per new field before the 32-byte value. v2.0 is forward-only — v1.0 hashes are NOT replayed as v2.0 (the preimage change forces re-derivation). Settlement-time vault-row lookup reuses the `VaultLookup` trait from RFC-0957 (capability verify-time); the verifier takes `&dyn VaultLookup` (NOT a concrete `OctoVaultLookup`) — same trait that backs `Macaroon::verify_for_vault_op`. Cross-chain settlement reject enforces `envelope.chain_id == vault_row.chain_id`; `SettlementError::ChainMismatch` carries the 3-tuple `(vault_id, vault_chain_id, envelope_chain_id)`. v016 migration (`migrations/v016__settlement_chain_vault.sql`) adds `idx_se_cost_vault_id` UNIQUE INDEX + `idx_se_cost_vault_id_chain_id` composite index (NULL-tolerant for legacy v1.0 rows). 25 byte-exact TV at `crates/quota-router-storage/tests/tv_0959_settlement_wire.rs` TV-0959-01..25 (5 families: Dqa 16-byte BE round-trip, precision-loss-tested, cost_vault_id derivation cross-ref to `octo_vault::vault_id_unchecked`, cross-chain settlement reject, VaultLookup trait reuse).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
