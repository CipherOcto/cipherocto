...
**Accepted (v1.7, 2026-08-23).** [R16 fix-all cascade: YAML version 1.6 → 1.7; §0 v1.6 → v1.7; §5 v1.5 R16 promotion row DUPLICATE → v1.7; §0 version-to-round mapping re-aligned to match §5 VH (v1.1=R8 amendments, v1.2=R11 fix trail, v1.3=R12 R9 propagation gap, v1.4=R12 cross-RFC drift closure, v1.5=R12 fresh fix trail, v1.6=R15 fix trail, v1.7=R16 promotion + R3 substrate-pending annotations); §0 L17 "reconciled to v1.5" stale phrase → "reconciled to v1.7"; §0 L19 "v1.5 initial draft" stale phrase → "v1.0 initial draft" matching §5 VH v1.0 row; §0 L19 research doc "v2.0" stale pin → bare path per CLAUDE.md §RFC Reference Conventions; §2.1 trait declarations annotated RFC-defined substrate-pending; §2.1 SettlementEnvelope / WorkflowKind request-response types annotated RFC-defined substrate-pending; §2.3 FallthroughCondition / octo-workflow-composite crate annotated RFC-defined substrate-pending; §2.3 CapabilityKind → HolderKind (RFC-0957-A1) clarification; §2.4 + §2.5 policy_registry + policy_kind_authority tables annotated RFC-defined substrate-pending; §2.6 kind_uuid_registry annotated RFC-defined namespace strings; §3 Class C row annotated linter pending landing via 0206-006; §4 Cross-References: "RFC-0957 §Capability" → "RFC-0957 §CapabilityToken State Machine"; "RFC-0964 §0.1 outer-namespace tag 0x01" → "RFC-0964 §0 outer-namespace tag"; "RFC-0959 v2.1 §2 carry_proof attribution" dropped (RFC-0959 BurnEventRef has no carry_proof field); research doc v2.0 pin dropped per CLAUDE.md.]
...
pub trait InteropPolicy: Send + Sync {
    ...
    // Note: `SettlementEnvelope` is RFC-defined, substrate-pending landing via 0206-001 v3.0 + 0206-009
    // per RFC-0206 v3.0 §4 (verified: 0 source hits for `struct SettlementEnvelope` in crates/).
    // The `carry_proof: &[u8]` sub-field of `SettlementEnvelope` is the RFC-defined wire-form anchor
    // for the byte-vector layout per §2.1 R12 fix; ZK envelope marker at `carry_proof[16..20]` follows
...
| Class C AuthorityPolicy registration    | REJECTED at registration          | Substrate refuses `policy_registry` INSERT for Class C. **Note:** substrate-level linter `crates/octo-policy/src/class_lint.rs` cited by RFC-0008 §Implicit Assumptions is linter pending landing via mission 0206-006 — verified: 0 hits for `class_lint` in crates/. Until that linter lands, the §3 REJECTED row has no canonical enforcement implementation (RFC-defined pending substrate).                                                                                                                                                              |
