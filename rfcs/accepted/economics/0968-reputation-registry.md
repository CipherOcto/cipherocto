...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | --------------------------------------------------------------------------- | --------------------- |
| `Subject`  | `Did` (`did:octo:...`)                                                                                                                                      | Receives reputation signals     | Persistent                                                                  | RFC-0008, this RFC §6 |
...
- Floats: **NOT supported as native `f64` in canonical_ser.** `score_delta` is `octo_determin::Dfp` per RFC-0104, serialized in its canonical 24-byte `DfpEncoding::to_bytes()` form (`DfpEncoding::from_dfp(d).to_bytes()` in `determin/src/lib.rs`: 16-byte mantissa + 4-byte exponent + 4-byte class_sign, all big-endian). The encoded BLOB is bit-deterministic across compilers and platforms, so two replicas that produce the same event sequence produce byte-identical `score_delta` BLOBs and identical `event_id` digests.
...
CREATE TABLE reputation_events (
  ...
  -- (DfpEncoding::from_dfp(d).to_bytes() from determin): 16-byte
...
/// `dfp_div`, `dfp_abs`, `dfp_lt`); `Dfp` also exposes `abs` / `min` / `max`
/// / `clamp` / `is_nan` / `is_finite` / `is_valid` and `PartialOrd` / `Ord`
/// as inherent methods, so callers may use either the free-function or the
/// method form equivalently.
pub fn update_ewma(
    ...
    // v5.1: equivalent method form is `delta.abs()` + `d_abs < one`; the
    // RFC standardises on free-fn form for stability.
...
| ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `crates/oct-reputation/Cargo.toml`                                      | New workspace member; features: `default = []`, `stoolap = ["dep:quota-router-storage"]`, `mon = []`, `dc = []`, `marketplace = []`, `wallet = []`                                                                                                                                                                          |
| `crates/oct-reputation/src/{lib,core,event,recorder,reader,auditor}.rs` | New: kind-gated re-exports; core types and `Normalizer`; event ID derivation; recorder, reader, and auditor APIs                                                                                                                                                                                                            |
| `crates/oct-reputation/src/{attestor,rotation,suspension,retention}.rs` | New: attestation, DID rotation, governance suspension, and retention APIs                                                                                                                                                                                                                                                   |
| `crates/oct-reputation/src/{error,constants}.rs`                        | New: `ReputationError`, all `BLAKE3_REPUTATION_*_DOMAIN` constants, and `MAX_*` constants                                                                                                                                                                                                                                   |
| `crates/oct-reputation/src/storage/{mod,stoolap}.rs`                    | New: storage module and `StoolapReputationStore`; stoolap implementation gated by the `stoolap` feature                                                                                                                                                                                                                     |
| `crates/oct-reputation/src/kinds/{mod,mon,dc,marketplace,wallet}.rs`    | New: kind adapters; each adapter gated by its corresponding feature                                                                                                                                                                                                                                                         |
| `crates/oct-reputation/migrations/v003__reputation_events.sql`          | New: event log table                                                                                                                                                                                                                                                                                                        |
| `crates/oct-reputation/migrations/v004__reputation_aggregates.sql`      | New: aggregate table                                                                                                                                                                                                                                                                                                        |
| `crates/oct-reputation/migrations/v005__reputation_rotations.sql`       | New: rotation state table (Round 2 C3)                                                                                                                                                                                                                                                                                      |
| `crates/oct-reputation/migrations/v006__reputation_attestations.sql`    | New: Phase 1 attestation rows + event FK                                                                                                                                                                                                                                                                                    |
| `crates/oct-reputation/migrations/v007__aggregate_checkpoints.sql`      | New: pruned-prefix aggregate audit checkpoints (Round 6 M5: `last_event_unix_at_checkpoint`)                                                                                                                                                                                                                                |
| `crates/oct-reputation/migrations/v008__recorder_registration.sql`      | New: recorder registration rows + role bitfield (Round 6 C1 + C3)                                                                                                                                                                                                                                                           |
| `crates/quota-router-storage/src/migrations.rs`                         | Retain the `BUILTIN_MIGRATIONS` runner; load v003-v008 from `oct_reputation::migrations::{v003__reputation_events, v004__reputation_aggregates, v005__reputation_rotations, v006__reputation_attestations, v007__aggregate_checkpoints, v008__recorder_registration}`; quota-router-storage no longer hosts reputation code |
| `crates/octo-network/Cargo.toml`                                        | Add `oct-reputation = { path = "../oct-reputation", features = ["mon", "dc"] }`                                                                                                                                                                                                                                             |
| `crates/quota-router-core/Cargo.toml`                                   | Add `oct-reputation = { path = "../oct-reputation", features = ["marketplace"] }`                                                                                                                                                                                                                                           |
| `crates/octo-wallet/Cargo.toml`                                         | Add `oct-reputation = { path = "../oct-reputation", features = ["wallet"] }`                                                                                                                                                                                                                                                |
| `crates/octo-network/src/{mon,dc}/reputation.rs`                        | Shadow-write through the feature-gated MON/DC adapters (§7)                                                                                                                                                                                                                                                                 |
| `crates/quota-router-core/src/marketplace/scoring.rs`                   | Shadow-write Outcome + Latency through the marketplace adapter with layer=2                                                                                                                                                                                                                                                 |
...
| 3.4-r19 | 2026-07-25 | Round 19 follow-up fixes: (H1) Mission 0968a Summary now references the canonical RFC-0955 binding form — `reputation:blake3_digest` field carrying a 32-byte `BLAKE3(DfpEncoding::from_dfp(&reputation).to_bytes())` digest (was `reputation:u64`, which is 8 bytes and cannot carry the 24-byte Dfp encoding or any hash thereof); mission Summary is now consistent with the §3 Status block. (M1) Use case Round 10 M1 bullet now states the §13 table is monotonic `0x01..=0x28` with `Storage(_)` at 0x21 and `ScoreEncodingInvalid` at 0x28 (was `0x01..=0x27`, which dropped `ScoreEncodingInvalid` added in Round 16 L4). (M2) §25 Phase 5 reference cites RFC-0955 §`reputation` follow-up scope with the amendment-required caveat (was `§"reputation: u64" follow-up scope`, which referenced a non-existent anchor). (L1) Round 15 Notes (line 3130) cross-replica equality test description and Round 16 Notes L3 annotation (line 3149) now use the standardized form `DfpEncoding::from_dfp(&d).to_bytes()` (was `score_ewma.to_bytes()`, a method that does not exist on `Dfp`). No design changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 3.5-r20 | 2026-07-25 | Round 20 fixes: research no-on-chain-anchoring wording aligned with the Round 19 M2 RFC-0955 §`reputation` follow-up scope and amendment-required caveat; v3.4-r19 version-history references corrected to Round 15 Notes line 3130 and Round 16 Notes L3 line 3149. No design changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 3.6-r21 | 2026-07-25 | Round 21 fixes: off-by-one line numbers in the v3.4-r19 / v3.5-r20 version-history references (line 3129 → line 3130, line 3148 → line 3149); cross-file v3.5-r20 changelog lines propagated to research + use case + mission docs. No design changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 4.0     | 2026-07-25 | RFC-0968 promoted Draft → Accepted per BLUEPRINT.md RFC lifecycle (22 rounds of adversarial review completed; 130+ findings closed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 5.0     | 2026-07-25 | Layout alignment: reputation core moved from `crates/quota-router-storage/src/reputation/` to dedicated `crates/oct-reputation/` workspace member with kind-gated features (mon, dc, marketplace, wallet) and dedicated storage + kinds subdirs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 5.1     | 2026-07-25 | DFP operator surface closure: `octo-determin` lands `dfp_abs` / `dfp_neg` / `dfp_lt` / `dfp_le` / `dfp_gt` / `dfp_ge` / `dfp_eq` / `dfp_min` / `dfp_max` / `dfp_clamp` and `Dfp::abs` / `Dfp::min` / `Dfp::max` / `Dfp::clamp` / `Dfp::is_nan` / `Dfp::is_infinite` / `Dfp::is_finite` / `Dfp::is_zero` / `Dfp::is_valid` and `PartialOrd` + `Ord` impls on `Dfp`. §6 `update_ewma` doc updated to record both forms equivalent. §6 stale "Dfp has no abs/lt methods" claim retired. **Known precision gap:** `arithmetic::align_mantissa` lacks sticky-bit tracking on wide-exponent subtraction; RFC-0968 §23 EWMA test vectors reproduce as `1.0` for all four events. Reproduction baseline at `determin/tests/ewma_vectors.rs`. Precision fix tracked separately — once landing, swap baseline for the asserting variant.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- **L1 (LOW) — `Dfp.to_bytes()` API correctness:** the previous prose at lines 955 and 1069-1070 referenced `Dfp.to_bytes()` / `Dfp::to_bytes()`, which does not exist in `determin/src/lib.rs`. Replaced all occurrences with `DfpEncoding::from_dfp(d).to_bytes()`. This matches the canonical API: `Dfp` is a runtime type; the 24-byte wire form is produced by `DfpEncoding::from_dfp(...)` and read back by `DfpEncoding::from_bytes(...)`.
...
v3.2-r17 is a Round 16 conformance + Dfp-API audit round. It closes 10 in-file findings (C1-C10) against the canonical `octo_determin` API as it exists in `determin/src/lib.rs` + `determin/src/arithmetic.rs`. No design changes are introduced; every fix is a stale-reference or spec-tidying correction.
