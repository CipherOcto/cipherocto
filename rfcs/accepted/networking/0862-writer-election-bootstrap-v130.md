...
**Status:** Accepted (2026-08-18) — v2.0.12 amendment adds Lock-file hardening (mission 0862-c11) + Test vector coverage gap closure (mission 0862-c11-tv-coverage-gap) + Adjacent-substrate u128→Dqa (mission 0105-x); Status header synced with §Version History v2.0.12 row per M37 corpus-wide sync check (prior header read v1.4.0; VH row added 2026-08-18 with follow-on missions 0862-c11 + 0862-c11-tv-coverage-gap + 0105-x landing without corresponding Status header refresh).
...
> **Substrate:** RFC-0862 v2.0 + RFC-0855p-c (handover) + RFC-0863 (bootstrap)
...
| Reader Node            | (no identity; cached lease)                                                                        | Read-only; forwards writes                 | Stateful                       | RFC-0862 v2.0 §Roles             |
...
- New workspace member `octo-sync/` (root `Cargo.toml` `exclude = [...]`
  drops `"octo-sync"`; gated opt-in per RFC-0862 v2.0 §Acceptance Criteria).
...
- **`octo-coordinator-bft` Layer A crate** for Byzantine fault tolerance
  (per RFC-0862 v2.0 §Future Work). v1.4.0 ships crash-fault-tolerant
...
- **Snapshot + replay recovery plan** (per RFC-0862 v2.0 §Future Work).
...
- **Coordinator HA via key-share ceremony + M-of-N operator quorum**.
  Per RFC-0862 v2.0 §Future Work + AC#12, mission
...
22. **Layer-direction audit:** `crates/octo-ident` does NOT depend
    on `crates/octo-sync` directly (per RFC-0862 v2.0 R12 M19 + R13 M5
...
- F1-F7, F9-F10: see RFC-0862 v2.0 §Future Work
...
- RFC-0008 §RFC-0008 Execution Class Mapping (per R12 H3 — RFC-0104 has no
