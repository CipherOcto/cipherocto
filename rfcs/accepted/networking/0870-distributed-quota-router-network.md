...
**Construction contract:** `NodeEnvelope::build` REQUIRES `version_tag: u8` as an explicit positional argument. Unknown values (any `u8` outside `{0xA0, 0xA1}`) are rejected at construction time with `ProtocolError::UnsupportedVersion(u8)`. There is no default-fallback path — callers MUST pass `VERSION_TAG_V2` for new wire-form receipts and MUST NOT pass `VERSION_TAG_V1` (rejected at verify).
