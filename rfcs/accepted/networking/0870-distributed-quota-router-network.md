...
Per RFC-0870 §14.1, every `NodeEnvelope` carries an explicit `version_tag: u8` field selecting the wire-form variant. This subsection is **ADDITIVE** to the §NodeEnvelope Adoption table above — it does not replace the existing UUID mapping; it adds a wire-format discriminator that operates independently of `payload_kind`.
...
**Verify contract:** `NodeEnvelope::verify_version(&self) -> Result<(), ProtocolError>` returns `Ok(())` for V1 (`0xA0`) and V2 (`0xA1`) and `Err(ProtocolError::UnsupportedVersion(v))` for any other value. Verify-time policy post-cutover: V1 receipts are **deterministically rejected** (per RFC-0870 §14.1) — `verify_version` is the structural gate, and the operational rejection is enforced by the verify path's caller (e.g., `QuotaRouterHandler::on_receive` rejects with `ProtocolError::UnsupportedVersion(0xA0)`). Absent `version_tag` field at deserialization is also rejected (the field is mandatory in canonical encoding; missing byte = borsh decode error, not silent zero).
