...
| `Internal(String)`                                      | 64          | Substrate-internal `AttachError::Internal` additive variant per RFC-0011-c §Follow-on (Phase B substrate split); routed via the existing `From<AttachError>` wildcard arm in `crates/octo-cli/src/error.rs` (additive variant; the canonical substrate envelope); CLI wildcard arm collapses protocol-level errors from Layer D extension `Handler` impls (handshake failures, multiplex aggregation, connection failures, configuration errors) into exit 64 with sanitized `reason` payload |
...
- `pub struct KeySet` — registry of `(key_id -> pubkey)` lookups plus a grace-period acceptance window for in-flight tokens minted during a rotation event. State: `keys: BTreeMap<KeyId, [u8; 32]>` (active lookup table) + `grace_keys: BTreeMap<KeyId, [u8; 32]>` (rotated-out pubkeys still accepted during the grace window). The pubkey is retained in the grace map so `verify_attach_handle_payload_v2` can attempt a per-pubkey verify against the rotated-out pubkey matching the CLAIMED `key_id` (the D2.1 substrate-faithful grace semantics — grace = "single-pubkey verify scoped to claimed key_id", NOT "try-each-pubkey iteration"). Iterating every grace pubkey would let any holder of a grace key's private material forge tokens claiming arbitrary `key_id` values (forgery vulnerability caught at R1.5 review).
...
- `KeySet::grace_period() -> Vec<KeyId>` — diagnostic list of grace-period key ids in ascending order. NOT used by the verify path after the R1.5 grace-fix that scopes fallback to the claimed `key_id` (kept for diagnostics and KeySet population policy use; `#[allow(dead_code)]`).
...
- `pub fn verify_attach_handle_payload_v2(key_set: &KeySet, session_id, payload, mint_timestamp_unix, ttl_unix, key_id, sig) -> Result<(), AttachError>` — additive verify. Three-step lookup:
  ...
  2. Grace fallback: if `key_set.grace_key(key_id)` returns `Some(pubkey)` (the grace entry matching the CLAIMED `key_id` only — NOT iteration over every grace entry), verify against `pubkey`. On verify success, `Ok(())`. On verify failure, fall through to step 3. The grace fallback scope is bound by the `key_id` discriminator, NOT the grace-period size; rotating out a single key for the same `key_id` is the canonical rotation case (the standard case: same key_id, pubkey changed).
  3. Exhausted: return `AttachError::UnknownKeyId { key_id, known_keys }` carrying the diagnostic union of active + grace key ids.
- `verify_bytes` — private helper (non-cfg-gated, used by both v1 + v2 paths) accepting a precomputed message + pubkey bytes + signature. The v1 verify path passes the canonical bytes from `canonical_payload_bytes` (§F.1); the v2 verify path passes the canonical bytes from `canonical_payload_bytes_v2` (this section). Shared helper avoids re-canonicalizing per verify attempt.
...
Each Layer D extension crate returns `AttachedSession` (per §F.7.1) where `broadcast_rx` is wired by the Layer D crate itself — the unix handler (per §F.7.1) subscribes to its OWN process-local broadcast source for cross-process event delivery via its own protocol (the cross-process event bridge is a Layer D concern per [[cipherocto-design-principles]] §Layer direction, NOT a substrate concern and NOT a Stoolap-fork concern — the Stoolap fork is the revocation-ledger primitive only per [[stoolap-general-purpose-db]] HARD RED LINE). The substrate does not prescribe the wiring mechanism — that's a Layer D concern. The Phase C amendment **promotes the in-memory revocation set behind a trait-dispatched `RevocationStore`** so Layer D crates can swap in a Stoolap-backed store that survives process boundaries:
...
**Architecture decision (paired amendment):** cross-process revocation propagation is implemented via a **trait-dispatched `RevocationStore` substrate** backed by the Stoolap fork ledger. The trait lives in `octo-runtime` Layer B; the Stoolap-backed impl lives in the new `octo-runtime-revocation-store` crate (Layer D, per the per-extension crate pattern per [[cipherocto-design-principles]] §User extensibility). The "per-extension crate" category in §Layer model covers both transport adapters (the prior Phase B precedent) and persistence extensions sharing the trait-in-core / impl-in-extension / registry-in-core topology. **This amendment extends the canonical Layer D definition in [[cipherocto-design-principles]] §Layer model to include persistence adapters alongside transport adapters** (both share the trait-in-core / impl-in-extension / registry-in-core topology); the canonical layer model memory card is amended by this RFC.
...
- Two-slot singleton pattern (avoids the prior-phase reviewer deadlock on single-slot `OnceLock` override):
...
- `pub fn set_revocation_store(store: Arc<dyn RevocationStore>) -> Result<(), AttachError>` — writes to `ACTIVE_REVOCATION_STORE.set(store)`. On double-install, returns `Err(AttachError::Internal("revocation store already installed"))` (uses `AttachError::Internal(String)` per substrate `crates/octo-runtime/src/handle/error.rs` exit 64, NOT `AttachError::RevocationError(String)` per §F.4 slot 58 slot allocation — `RevocationError` is reserved for runtime revocation failures, not init failures). This is a NEW substrate discipline distinct from `HANDLE_TRANSPORT_REGISTRY` per §F.2 step (e) (which is `OnceLock::get_or_init` lazy-init + `Registry::register` last-write-wins); the difference is intentional because revocation-store init is a one-shot operator decision, not an idempotent extension registration.
- `pub fn install_revocation_store_default_with<F>(factory: F) -> Result<(), AttachError> where F: FnOnce() -> Result<Arc<dyn RevocationStore>, AttachError>` — factory-mediated init fn; routes through `set_revocation_store(store)`. The factory closure preserves the "try install, fall back to default on error" semantic at the API layer: a single call site catches the `Err` and emits a `WARN` log before the dispatcher falls back to `InMemoryRevocationStore` (per §F.7.5 failure-semantics block). A direct `set_revocation_store(install_default()?)` form would propagate errors at the CLI boundary and lose the fall-back. C → B → D wiring via factory closure (see [[cipherocto-design-principles]] §Layer direction diagram below). Layer B has NO compile-time dependency on Layer D; the factory type-erases the D impl via `Arc<dyn RevocationStore>`.
- `pub fn revoke_attach_token(session_id: SessionId) -> Result<(), AttachError>` — paired amendment rewrites the body to dispatch via `current_revocation_store().revoke_attach_token(session_id)`. Signature, return type, and error variants are unchanged from the existing surface; the change is internal (direct static access → trait-mediated `current_revocation_store()` lookup). Lazy-init of `DEFAULT_REVOCATION_STORE` on first call when `set_revocation_store` was never invoked.
- `pub fn is_token_revoked(session_id: &SessionId) -> bool` — paired amendment rewrites the body to dispatch via `current_revocation_store().is_token_revoked(session_id)`. Same signature preservation + same lazy-init pattern.
- `pub struct InMemoryRevocationStore { inner: RwLock<HashSet<SessionId>> }` — additive substrate type. Each `InMemoryRevocationStore` instance owns its own `RwLock`; the `Arc<dyn RevocationStore>` singleton (or the `Arc<InMemoryRevocationStore>` static default) carries the chosen instance for the lifetime of the process. The default impl's `is_token_revoked` catches a poisoned `RwLock` and returns `true` (fail-CLOSED preserves the explicit-operator-revocation guarantee); promoted behind the trait (no behavior change for default consumers per the existing §F.3 fork-fail-closed contract). `InMemoryRevocationStore::kind()` returns `"InMemoryRevocationStore"`.
...
The Cargo.toml `revocation-store-stoolap` feature is opt-in per the per-extension crate pattern; default CLI builds use the zero-dep `InMemoryRevocationStore` default. Cross-process revocation propagation is opt-in per operator decision.
...
1. Spawn-side: `octo agent run --detach --token-file /tmp/tok.bin` (writes the token file; spawns the runtime; spawn-side calls `install_revocation_store_default_with(octo_runtime_revocation_store::install_default)` on startup per the CLI main.rs wiring pattern → ledger open at `$CIPHEROCTO_DATA_DIR/revocation.stoolap`).
2. Attach-side: `octo agent attach --token-file /tmp/tok.bin` (reads the token file; binds via `UnixSocketHandler::bind` to the spawn-side process; attach-side ALSO calls the same `install_revocation_store_default_with` factory on startup → opens the SAME ledger at the SAME path).
3. Operator-side: `octo agent revoke-attach --session-id <HEX64>` (a third CLI process — distinct from spawn-side and attach-side — calls the same `install_revocation_store_default_with` factory on startup, then invokes `octo agent revoke-attach --session-id <HEX64>` which routes to `StoolapRevocationStore::revoke_attach_token` writing through the pre-check `SELECT 1 ... LIMIT 1` + `INSERT` pattern per §F.7.5 substrate additions above).
...
- `StoolapRevocationStore::is_token_revoked` returns `true` on ledger read failure (e.g., DB corruption, I/O error, schema drift) — fail-CLOSED preserves the explicit-operator-revocation guarantee; logs at ERROR via `tracing::error!` (with `kind()` value per §F.7.5 substrate additions above) before the fail-CLOSED return.
...
- `install_revocation_store_default_with` failure fallback: WARN log + retain default `InMemoryRevocationStore` + the dispatch continues with `ACTIVE_REVOCATION_STORE` empty (calls fall back to `DEFAULT_REVOCATION_STORE`). The CLI's main.rs wrapper applies the SAME WARN log per the inline wiring code block above (canonical wording: "revocation store install failed; falling back to InMemoryRevocationStore (process-local revocation, no cross-process propagation)").
