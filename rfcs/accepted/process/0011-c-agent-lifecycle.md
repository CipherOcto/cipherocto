...
- `pub async fn attach_with_token(holder_pubkey: &[u8; 32], token: &AttachHandle, since_unix: u64) -> Result<AttachedSession, AttachError>` (defined at the `octo_runtime` crate root in `crates/octo-runtime/src/lib.rs:103`) — validation chain: (a) signature verify via `verify_attach_handle_payload` (per §F.5), (b) revocation-set check via `is_token_revoked`, (c) `now_unix <= token.ttl_unix`, (d) `since_unix >= token.mint_timestamp_unix`, (e) session_id match against running session registry. **Step (e) is a substrate-boundary stub pending the follow-on registry-wiring amendment** (per lib.rs:152-177 docstring; panics in debug builds via `unimplemented!()`, returns `AttachError::UnknownSession` in release). Transport binding is dispatched via `token.transport` once the registry-wiring amendment lands; concrete transport bindings (InProcess direct handle, UnixSocket connect, Raw scheme dispatch) are Layer D adapter concerns that register into the substrate transport-handler trait. The leading `holder_pubkey` parameter mirrors the static-helper signature shape of `verify_attach_handle_payload` (Layer B pure helper, no `&self` binding).
