...
### Substrate layer-model note (RFC-0013)

The governance substrate name referenced in this RFC (`octo-governance`) resolves to a **two-crate canonical pattern** per `docs/research/2026-09-10-octo-audit-governance-settlement-modular-layer-research.md` Finding 4 (hybrid Layer A core + Layer B façade):

| RFC name (Layer B façade) | Substrate (Layer A frozen) | RFC defining split |
|---|---|---|
| `octo-governance` | `octo-governance-core` | RFC-0013 |

The façade re-exports ONLY from the substrate (no domain leakage). The substrate owns canonical `ProposalState` + `DecisionType` + `GovernanceModel` + `EmergencyAuthority` + `GovernancePolicy` + `GovernanceProposal` + pure tally helpers (`voting_weight`, `tally_quorum`) per RFC-0013 §Specification.

The IO functions declared in this RFC §Substrate `[ADD]` (`snapshot`, `attest`, `vote`) live in the domain crate `octo-network/mon/governance` (not in the substrate — substrate is IO-free per RFC-0013 §Why substrate is IO-free). The substrate owns canonical types + tally helpers; the domain owns IO. CLI dispatches to the domain; domain consumes the substrate for canonical types. No CLI Cargo dep changes; the façade name matches this RFC text exactly.
...
| 1.4     | 2026-09-10 | Accepted | Layer-model amendment: RFC-0013 substrate split documented. |
...
- RFC-0013 — Governance Substrate (`octo-governance-core` Layer A +
  `octo-governance` Layer B façade) per §Substrate layer-model note;
  canonical `ProposalState` + `DecisionType` + `GovernanceModel` +
  tally helpers
