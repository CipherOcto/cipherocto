...
**Substrate stub (`EmptyBridge`):** Per RFC-0011-h §Substrate-Faithfulness + Phase 12 R2.5 / Phase 13 R1.5 / Phase 14 R1.5 precedent, the substrate ships a public `EmptyBridge` unit-struct impl that handlers invoke unconditionally before any per-extension Layer D impl crate registers. `EmptyBridge::list()` returns an empty `Vec`; `EmptyBridge::propagate_to(_)` returns `BridgeError::Unreachable` for any input. Per-extension concrete impl crates (Layer D, OUT OF SCOPE) replace the stub via the per-extension crate pattern registration hook.
...
Phase 7 lands additively. No existing CLI subcommand changes. No NEW OctoCliError variants (REUSES slot 89). Existing substrate paths unchanged. Per-extension impl crates (Layer D) are unaffected by Phase 7 implementation (consumers of the `SlashBridge` trait). `EmptyBridge` stub (R1.5 fix) is shipped in `crates/octo-network/src/mon/slash_bridge.rs` and is invoked unconditionally by both `network_slash_bridge_list` + `network_slash_bridge_propagate` handlers until a per-extension Layer D impl crate registers at runtime.

## Test Vectors

7 test vectors total per RFC-0011-h §Test Vectors Phase 7 + Phase 5 precedent (tv_net5_* numbering):

**Coverage split per Phase 5 precedent:** Each vector splits its assertion across CLI-parse-level test (`crates/octo-cli/src/commands/network.rs` `tv_net7_*` fn) + substrate-trait-level test (`crates/octo-network/src/mon/slash_bridge.rs` `test_*` fn). The CLI tests cover clap parsing + handler dispatch to the trait boundary; the substrate tests cover the trait behavior (empty list, populated list, propagate success, propagate unreachable, error display, idempotency, `EmptyBridge` stub behavior). End-to-end CLI dispatch through a concrete per-extension impl crate is OUT OF SCOPE per RFC-0863 per-extension crate pattern + per-extension concrete impls are Layer D follow-on missions.

**Substrate test count (R1.5 fix):** 9 substrate tests in `crates/octo-network/src/mon/slash_bridge.rs` — `test_bridge_list_empty` + `test_bridge_list_populated` + `test_propagate_to_success` + `test_propagate_to_unreachable` + `test_bridge_error_display` + `test_propagate_to_twice_with_same_id_yields_identical_receipt` (R1.5 idempotency fix per `bugs` reviewer MAJOR [1]) + `test_empty_bridge_list_returns_empty` + `test_empty_bridge_propagate_returns_unreachable` + `test_empty_bridge_propagate_idempotent`.
...
| `tv_net7_2`  | `slash-bridge list`                                                                   | clap parses `--json` flag                                                                                                                                                                                                                                                                                 | populated `BridgedSlash` projection with hex encoding + total field (substrate `test_bridge_list_populated`)       |
| `tv_net7_3`  | `slash-bridge list`                                                                   | substrate `EmptyBridge` stub invoked unconditionally; `NetworkSlashBridgeListOutput` envelope has `total=0` + `slashes=Vec::new()` (R1.5 fix: substrate `EmptyBridge` shipped per Phase 12 R2.5 / Phase 13 R1.5 / Phase 14 R1.5 substrate-faithfulness precedent; envelope-level projection strengthened) | (covered by `test_empty_bridge_list_returns_empty`)                                                                |
...
4. **Add NEW `NetworkBridgeError` OctoCliError variant instead of REUSING slot 89.** Rejected per user decision; per the mid-rollout no-new-cli-errors precedent, Phase 7 REUSES slot 89 with distinct messages to avoid expanding the OctoCliError surface mid-rollout. A future post-rollout RFC may add a dedicated `NetworkBridgeError` variant after the 6-phase + Phase 7 rollout closes.
...
| `rfcs/accepted/process/0011-o-oct-cli-network-phase-7.md`                                 | (this RFC)                                                                                                                                 |
...
| v0.9.1  | 2026-09-21 | R1.5 fix sweep for retroactive multi-round DRY CLOSURE: substrate-faithfulness BLOCKER (registry gate removed per Phase 12 R2.5 / Phase 13 R1.5 / Phase 14 R1.5 precedent; public `EmptyBridge` stub shipped; handlers invoke substrate unconditionally); bugs MAJOR (idempotency contract enforced via `test_propagate_to_twice_with_same_id_yields_identical_receipt` + `test_empty_bridge_propagate_idempotent`); cite hygiene MAJOR 1-6 (`.gitignore line 46` → `[[docs-plans-scratchpad]]`; §Dependencies rewritten as bare bullets; phantom `[[no-new-cli-errors-during-rollout]]` cite dropped; `rfcs/draft/process/...` → `rfcs/accepted/process/...`; §Cross-references cross-ref TV count 6 → 7; "final amendment" stale claim dropped); cite hygiene MINOR (§Cross-references bold wrapper replaced with bare `[[RFC-XXXX]]` style; §Future Work stale promotions item dropped); test coverage MAJOR + MINOR (cross-ref TV count fixed; tv_net7_2 substrate fn name `test_bridge_list_populated` cited; tv_net7_3 strengthened to envelope-level projection using public `EmptyBridge`). |
| v0.9.2  | 2026-09-21 | Retroactive multi-round DRY CLOSURE gate GATE GREEN at R2 zero per Phase 12 RFC-0011-t precedent. R2 5 reviewers (bugs + test coverage + substrate-faithfulness + layer discipline + cite hygiene) returned ZERO findings. R1 aggregate 1 BLOCKER + 8 MAJOR + 7 MINOR + 1 NIT all closed in R1.5 fix sweep at `next c0676a30`; R2 verification confirmed no new issues introduced. 476/476 octo-cli + 1522/1522 octo-network tests pass; Layer A frozen preserved; per-extension crate pattern preserved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

## Cross-references

- [[RFC-0011-h]] — multi-phase rollout plan
- [[RFC-0855]] — `BridgedSlash` + `BridgeReceipt` + `BridgeError` substrate anchors
- [[RFC-0855p-b]] — `slash_envelope_id` 32-byte canonical encoding
- [[RFC-0863]] — `NetworkSender` per-extension crate pattern; `SendContext` substrate anchor
- [[RFC-0011-i]] — Phase 1 layer-C CLI dispatch pattern
- [[RFC-0011-j]] — Phase 2 slot 89 substrate-absent pattern
- [[RFC-0011-k]] — Phase 3 confirmation-flag pattern
- [[RFC-0011-l]] — Phase 4 `--dry-run` + `--confirm-acknowledge` pastejacking defense pattern
- [[RFC-0011-m]] — Phase 5 slot 89 REUSE pattern with substrate-absent companion gating
- [[RFC-0011-n]] — Phase 6 closure artifact pattern
- [[RFC-0011-o]] — Phase 7 G9 spec (this RFC)
