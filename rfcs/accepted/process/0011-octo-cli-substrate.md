...
`octo-wallet init` binary (wallet substrate amendment); replacements for
...
that amendment lands. Operators with scripts still calling `octo join`
or `octo status` should run `octo --help` for the current subcommand
list.
