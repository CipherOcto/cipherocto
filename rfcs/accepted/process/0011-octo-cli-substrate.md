...
| Version | Date       | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.0    | 2026-08-29 | RFC-0011 accepted. Five stub commands (`init`, `join`, `role`, `agent`, `status`) emit deprecation banner on stderr, exit 0. All five are `#[command(hide = true)]` variants in the `Commands` enum with live `print_*_deprecated` banner-emit functions in `commands/stub.rs`. Hidden from `--help`. `role` + `agent` promoted via RFC-0011-d Phase 1 + RFC-0011-c before v2.0 cut.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| v2.0    | 2026-09-17 | **Cut:** Three stub commands (`init`, `join`, `status`) removed entirely per §Compatibility timeline. `commands/stub.rs` + `tests/stub.rs` deleted; `commands::stub` module re-export removed from `commands/mod.rs`. `octo role` + `octo agent` migrated to first-class prior to this cut via RFC-0011-d Phase 1 (commit 63ffdf94, 2026-09-01) and RFC-0011-c agent-create (commit bc2c6cde, 2026-09-08).                                                                                                                                                                                                                                                                                                                                                                                                                  |
| v2.0+   | 2026-09-17 | **Post-cut state:** `OctoCliError::StaleStub` retained as a `#[non_exhaustive]` library-API soft sentinel with `replaced_by: &'static str` field for downstream library consumers that match on the variant; CLI operators never observe `StaleStub` post-cut (clap intercepts with `unrecognized subcommand`, exit 2). `StaleStub` Display surfaces `replaced_by` verbatim. **Replacement landing:** `octo-wallet init` (separate binary, replaces `octo init` — landed via wallet substrate amendment), `octo role select` (preserved as first-class, landed via RFC-0011-d Phase 1), `octo agent {create,run,list,destroy,attach}` (preserved as first-class, landed via RFC-0011-c). `octo network bootstrap` (replaces `octo join`) and `octo network status` (replaces `octo status`) DEFERRED to a future amendment. |
...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Args         | none                                                                                                                                                                                                                                                       |
| Flags        | `--confirm` (REQUIRED, see §Confirmation Flag Matrix), `--dry-run` (no `--grace-hours` flag — substrate hard-codes 24h grace internally; not operator-configurable; see note)                                                                              |
...
| Redaction    | `signature_proof` MUST be redacted (`RedactedHex` placeholder); `--confirm` per §Confirmation Flag Matrix                                                                                                                                                  |
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Args         | none                                                                                                                                                                                                                                                                                                                                                                                                  |
| Flags        | `--caveats <json>` (REQUIRED), `--holder <did>` (REQUIRED), `--root <cap_id>` (optional; defaults to wallet root), `--confirm` (REQUIRED, see §Confirmation Flag Matrix), `--confirm-acknowledge` (REQUIRED for complex-payload mutating commands per §Security Considerations), `--dry-run`                                                                                                          |
...
| Flags        | `--caveats <json>` (REQUIRED), `--confirm` (REQUIRED, see §Confirmation Flag Matrix), `--confirm-acknowledge` (REQUIRED for complex-payload mutating commands per §Security Considerations), `--dry-run`                                                                                                                                                                                                                                                                           |
...
The mode-appropriate confirmation flag is determined by the operator role (`--mode`)
and the command's payload complexity. The `require_confirm` gate at
`crates/octo-cli/src/commands/*` returns `ConfirmationRequired` (exit 2) unless
the required flag for the active role is present (and `--dry-run` is NOT set,
except in Auditor mode where denial precedes the dry-run bypass per R16 Lens-1 F2).

| Operator mode     | Identity show / list (read) | Identity rotate / revoke | Capability mint / attenuate           | Policy show / list (read) |
| ----------------- | --------------------------- | ------------------------ | ------------------------------------- | ------------------------- |
...
- `--confirm-acknowledge` is a global flag on `OperatorModeFlags` (R20 Lens-2 F7
  moved it from per-command to global). clap enforces `#[arg(requires = "confirm")]`
  so `--confirm-acknowledge` is rejected unless `--confirm` is also present.
- `--allow-write` is a global flag on `OperatorModeFlags`; Dev mode routes through
  the production substrate path (R20 Lens-4 F2) but still requires `--allow-write`
  for write admission so the developer does not accidentally write through `--dev`.
- `--dry-run` bypasses confirmation (preview only) for Human / Ci / Dev modes
  and is rejected for Auditor mode (read-only role must never see a side-effect
  preview — R16 Lens-1 F2).
- The two-step `--confirm` + `--confirm-acknowledge` flow for capability
  mint / attenuate is the pastejacking defense per §Security Considerations 1a.
...
### Stub command compatibility (historical)
...
Three stub commands (`octo init`, `octo join`, `octo status`) are removed
from the binary surface post-v2.0. Two (`octo role {…}`, `octo agent {…}`)
were superseded by first-class subcommands in their respective amendment
RFCs (RFC-0011-d Phase 1 + RFC-0011-c) and remain in the binary surface
today — the stub banner dispatch arms were the only thing removed for
those two. Replacement landing for `octo init` is the separate
`octo-wallet` binary (`octo-wallet init`, wallet substrate amendment);
note that `octo-wallet` is a different binary in the same workspace, not
an `octo` subcommand. Replacements for `octo join` (→ `octo network
bootstrap`) and `octo status` (→ `octo network status`) are DEFERRED to
a future amendment; calls to those post-v2.0 hit clap `unrecognized
subcommand` (exit 2) until that amendment lands. Operators with scripts
still calling `octo join` or `octo status` should run `octo --help` for
the current subcommand list. Operators with scripts still calling
`octo init` should consult `octo-wallet --help` (the replacement is a
separate binary in the same workspace, not an `octo` subcommand).
...
`#[non_exhaustive]` enum keeps it as a library-API soft sentinel for
downstream consumers of `OctoCliError` that match on the variant;
`#[non_exhaustive]` library surface must not lose variants per
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.0        | 2026-08-27                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Draft                | Initial draft (process — identity/capability/policy slice). Full RFC-0011 template per BLUEPRINT.md; spec cycle closed 2026-08-27 across R1–R6 (DRY) and 5-lens adversarial review closed 2026-08-28 across R7–R32 (R31+R32 zero-finding rounds per `rfc-0011-loop-dry-gate-closure`). |
| 2026-08-29 | **Promoted to Accepted.** Phase 1 substrate landed on `next` HEAD `e300f72e`; 4 of 5 RFC-0011 missions Completed (`bc228500` / `e16306b2` / `db857895` / `1bec9296`); M5 (`0011-deprecation-stub-removal`) stays Open, release-gated on v1.1. All 8 GitHub Actions CI workflows green on `e300f72e` (per `rfc-0011-ci-gate-green-2026-08-29`). Status header updated; file moved via `git mv` from `rfcs/draft/process/` to `rfcs/accepted/process/` per BLUEPRINT.md §RFC Acceptance Process. Review cycle satisfied: spec cycle DRY (R1–R6) + review loop DRY (R31+R32) + 5-lens adversarial review (correctness / security / layer-model / hygiene / spec-completeness) with documented closures. |
| 1.1        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: audit subcommands (per Status header amendment chain)                                                                                                                                                                                                                       |
| 1.2        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: reputation subcommands (per Status header amendment chain)                                                                                                                                                                                                                  |
| 1.3        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: agent lifecycle subcommands (per Status header amendment chain)                                                                                                                                                                                                             |
| 1.4        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: role provisioning subcommands (per Status header amendment chain)                                                                                                                                                                                                           |
| 1.5        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: vault operations subcommands (per Status header amendment chain)                                                                                                                                                                                                            |
| 1.6        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: mesh operations subcommands (per Status header amendment chain)                                                                                                                                                                                                             |
| 1.7        | TBD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Accepted (amendment) | Amendment: governance subcommands (per Status header amendment chain)                                                                                                                                                                                                                  |
...
- 2026-08-29 — Promoted Draft → Accepted per BLUEPRINT.md §RFC Acceptance Process (file moved via `git mv`; Status header updated; VH row added documenting landing commit stack and CI gate closure; Authorship Note placeholder disclosure removed per BLUEPRINT §RFC Process).
- 2026-08-27 — Initial draft (this version)
