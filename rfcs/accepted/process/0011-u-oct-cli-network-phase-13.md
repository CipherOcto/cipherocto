...
- **Per-extension concrete impl crates** (Layer D): OUT OF SCOPE; substrate types in Layer B expose the inspector + forwarder surface for future follow-on Layer D adapter missions
...
- 2026-09-21 — R3.5 fix sweep: replaced "substrate traits" with "substrate types" in §Roles and Authorities to match §Design Goal 3 fix (Phase 11 R3 cite hygiene consistency lesson; 3 reviewers independently flagged the same line 64 inconsistency in R3).
- 2026-09-21 — Accepted (Draft → Accepted promotion after 4-round multi-round DRY gate: R1 fail → R1.5 fix → R2 zero → R3 zero → R3.5 fix → R4 zero = GATE GREEN; promoted at `next 253ee1b5`).
