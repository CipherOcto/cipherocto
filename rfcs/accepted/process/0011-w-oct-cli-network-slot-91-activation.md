...
8. **G8 — Test vector coverage.** 3 minimum test vectors:
   - `tv_w_1` — variant construction + `exit_code()` returns 91 + `user_message()` renders the `#[error]` template with `key_id_hex` + `known_keys_band` interpolated per RFC-0011-h §Redaction Layer (Rust unit test; `cargo test -p octo-cli --lib`)
...
**Edit 1 — §Error Handling summary paragraph** (parent RFC §Error Handling summary paragraph at line 545; this paragraph is the §Error Handling section's prose summary that mentions "All 10 defined ... + slot 91 pre-allocated per G21" — the table itself has no row 91, the §Exit Codes table at line 570 is the canonical row-91 location; per the substrate-faithfulness audit at parent RFC §Key Files to Modify line 919 the §Error Handling table ends at row 90):
...
The §Error Handling table itself has no row 91 — only the §Exit Codes table at line 570 contains the row 91 entry, which is updated per Edit 2 below. The summary paragraph at line 545 is the canonical §Error Handling reference for the slot 91 pre-allocation → activation transition.
...
| -------- | ----------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tv_w_1` | Variant existence             | OFF                             | `OctoCliError::NetworkKeyRotationUnknownId { key_id_hex: String, known_keys_band: KnownKeysBand }` constructs successfully; `exit_code()` returns `91`; `user_message()` renders the `#[error]` template with `key_id_hex` + `known_keys_band` interpolated per RFC-0011-h §Redaction Layer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- [ ] Task 5: Update parent RFC-0011-h §Error Handling summary paragraph (line 545), §Exit Codes row 91, §Substrate-Additions G21 row right column, [^rotation-error-slot-prealloc] footnote (4 edits per §Detailed Design §Cross-RFC reference updates)
- [ ] Task 6: Create paired mission YAML `0011-h-s-a-network-key-rotation-unknown-id.md` in `missions/open/` (Open status)
- [ ] Task 7: Add 3 test vectors per G8 (`tv_w_1` + `tv_w_2` + `tv_w_3`)
...
- [ ] Task 10: `cargo test -p octo-cli --lib --features octo-attach-key-rotation` green (1 Rust vector `tv_w_2` + zero regressions; feature ON build)
...
| `crates/octo-cli/Cargo.toml`                                                                                | ADD `octo-attach-key-rotation` feature propagation clause: `octo-attach-key-rotation = ["octo-runtime/octo-attach-key-rotation"]` (NO `dep:` prefix because `octo-runtime` is already declared as a required dep at `crates/octo-cli/Cargo.toml:96`; per §Detailed Design §Feature flag wiring + [[substrate-faithfulness-verification]] R3.A HIGH-1 finding) |
| `rfcs/accepted/process/0011-h-oct-cli-network-subcommands.md`                                               | EDIT §Error Handling summary paragraph (line 545); EDIT §Exit Codes row 91; EDIT §Substrate-Additions G21 row right column; EDIT [^rotation-error-slot-prealloc] footnote                                                                                                                                                                                     |
| `missions/open/0011-h-s-a-network-key-rotation-unknown-id.md`                                               | NEW paired companion mission YAML (Open status)                                                                                                                                                                                                                                                                                                               |
| `crates/octo-cli/src/error.rs` (tests)                                                                      | ADD 3 unit tests per §Test Vectors                                                                                                                                                                                                                                                                                                                            |
