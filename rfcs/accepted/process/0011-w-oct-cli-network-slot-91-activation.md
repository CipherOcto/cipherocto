...
Accepted (2026-09-22, updated 2026-09-22 v1.1 closeout) — RFC-0011-w activates the slot 91 pre-allocation from RFC-0011-h §Error Handling row 91 + §Exit Codes row 91 + §Substrate-Additions G21 row + [^rotation-error-slot-prealloc] footnote. ONE NEW `OctoCliError` variant (slot 91 mints) + ONE NEW `From<octo_runtime::AttachError>` translation arm (feature-gated per substrate `octo-attach-key-rotation` Cargo feature) + parent RFC-0011-h cross-reference updates. No new CLI subcommand. No substrate change. Paired with new amendment-chain companion mission YAML `0011-h-s-a-network-key-rotation-unknown-id` per [[no-phantom-mission-pointers]]. DRY CLOSURE gate GREEN at R4 (0/0/0/0 findings across 4 rounds) before implementation slice. Implementation slice landed via 4-commit pattern per Phase 5 RFC-0011-m 5-commit precedent: paired-YAML Claimed transition + Cargo.toml feature propagation at `next ef2251a4`, Layer C substrate additions at `next c230dc70`, parent RFC 4 cross-RFC reference updates + paired-YAML Completed transition at `next 31907573`, RFC promotion Draft → Accepted at `next 825043f0`. Post-LANDED retroactive multi-round DRY CLOSURE achieved GATE GREEN at R4 across 5 reviewer axes (R4.A + R4.B + R4.C + R4.D + R4.E all CLEAN) per Phase 14 RFC-0011-v R1 fail R1.5 fix R2 fail R2.5 fix R3 fail R3.5 fix R4 zero precedent. R1.5 fix sweeps at `next 8115c7f3` (cite hygiene) + `next fb76e1f6` (test coverage broadening). R3.5 fix sweep at `next bd9e9e5c` (tv_w_3 regex Option A corrections + docstring collapse + test vector roster reconciliation across 7 RFC/YAML/memory-card locations). v1.1 closeout added `## Compatibility` + `## Implicit Assumptions Audit` sections per docs/BLUEPRINT.md §Specification §Cross-RFC Consistency Checklist (Compatibility + Implicit Assumptions Audit both REQUIRED for all RFCs).
...
Per docs/BLUEPRINT.md §Specification §Compatibility (REQUIRED for all RFCs touching operator-facing surfaces). Compatibility analysis is organized across three axes:

**Backward compatibility.** The amendment is purely additive on the existing `#[non_exhaustive] OctoCliError` enum. Existing variants (slots 79, 82-90) are unchanged. Existing `From<octo_runtime::AttachError>` arms are unchanged. The translation arm for `UnknownKeyId` is added without disturbing existing variant arms. Existing CLI behavior (per `octo-cli` v0.1.0 with feature OFF) is preserved bit-for-bit; `cargo test -p octo-cli --lib` returns 477 passed (unchanged from the prior baseline of 476 plus the pre-existing tv_w_1). The variant mints unconditionally but is unreachable in default builds because the translation arm is feature-gated, so the existing CLI exit-code surface for non-key-rotation paths is identical to the pre-amendment state.

**Forward compatibility.** Slot 91 was pre-allocated per RFC-0011-h [^rotation-error-slot-prealloc] footnote (rewritten to past-tense at Edit 4 of this amendment per `next 31907573`) + §Substrate-Additions G21 row right column (Edit 3 of this amendment per `next 31907573`). Pre-allocation preserves the future-amendment band 79-99 contract per RFC-0011-h §Substrate-Additions G3 row (parent RFC §Substrate-Additions Companion Missions G3 row, line 62): 11 defined variants + 2 RESERVED slots (80, 81) + 8 RESERVED slots (92-99) = 21 slot positions total (band width unchanged per pre-allocation contract). Slots 92-99 remain RESERVED for future amendments beyond `-w`; this amendment does not consume any of them. Wire format: no impact (operator-facing render only); post-PQC `KeyId` discriminator migration is deferred per RFC-0011-c §F.5.1 paired-acceptance bridge migration notes.

**Substrate compatibility.** `crates/octo-runtime` is unchanged across the 4-commit LANDED slice (`git diff --stat crates/octo-runtime` returns zero hits between `next ef2251a4~1..next 825043f0`). The CLI translation arm at `crates/octo-cli/src/error.rs` §`From<octo_runtime::AttachError>` mirrors substrate `AttachError::UnknownKeyId` shape (`key_id: KeyId`, `known_keys: Vec<KeyId>`), feature gating (`#[cfg(feature = "octo-attach-key-rotation")]`), and field types one-for-one per RFC-0011-c §F.5.1 paired-acceptance bridge. The Cargo.toml feature propagation clause `octo-attach-key-rotation = ["octo-runtime/octo-attach-key-rotation"]` (no `dep:` prefix because `octo-runtime` is declared as a required dep at `crates/octo-cli/Cargo.toml`, not `optional = true`) activates the substrate feature on the existing required dep per [[substrate-faithfulness-verification]] R3.A HIGH-1 finding.

## Implicit Assumptions Audit

Per docs/BLUEPRINT.md §Specification §Implicit Assumptions Audit (REQUIRED for all RFCs). The "Nothing should be implied" rule applies: every assumption relied upon but not enforced by types, runtime validation, or test coverage MUST be listed with blast radius + mitigation.

| Assumption                                                                                                                                                                                                                     | Where Relied Upon                                                                                       | Blast Radius if False                                                                                                                                                                                                                                                                     | Mitigation / Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Substrate `AttachError::UnknownKeyId` signature is stable (fields `key_id: KeyId`, `known_keys: Vec<KeyId>`)                                                                                                                   | §Detailed Design §Translation arm; §Test Vectors tv_w_2; Appendix A substrate-faithfulness verification | CLI translation arm compiles against a substrate variant with mismatched field signature → compile error → full CLI build fails → downstream consumers (per-extension crates) blocked from this slot                                                                                      | Substrate-faithfulness audit at RFC-0011-w Draft time (Appendix A) verified by direct read of `crates/octo-runtime/src/handle/error.rs`. Substrate variant is RFC-frozen per RFC-0011-c §F.5.1 paired-acceptance bridge. Re-verified at R4 (substrate-faithfulness axis CLEAN).                                                                                                                                                                                                                                |
| `KeyId = u32` typed version discriminator (NOT a 32-byte opaque type)                                                                                                                                                          | §Detailed Design §Redaction Layer; §Test Vectors tv_w_2; Appendix A                                     | `hex::encode(key_id.to_be_bytes())` would produce 64 hex chars instead of 8 if `KeyId` were a 32-byte type → operator envelope overflow + slot-arithmetic interpretation drift                                                                                                            | Verified at RFC-0011-w Draft time at `crates/octo-runtime/src/handle/key_id.rs` per `grep -nE 'pub type KeyId                                                                                                                                                                                                                                                                                                                                                                                                  | pub struct KeyId'`. `u32`covers 4B key generations; hex-encoded as 8 chars via`key_id.to_be_bytes()`. Re-verified at R4 (tv_w_2 asserts `key_id_hex: "deadbeef"`from`key_id: 0xdeadbeef`). |
| Parent RFC-0011-h §Error Handling + §Exit Codes + §Substrate-Additions G21 row + [^rotation-error-slot-prealloc] footnote text remains stable for the 4-edit list (Edit 1-4 per §Detailed Design §Cross-RFC reference updates) | §Detailed Design §Cross-RFC reference updates; §Test Vectors tv_w_3 shell grep; G4 design goal          | If a future parent RFC amendment modifies §Error Handling summary arithmetic, §Exit Codes row 91 cell, G21 row right column, or [^rotation-error-slot-prealloc] footnote wording → `tv_w_3` greps may fail and need re-baselining; cross-RFC reference integrity assertion cannot be made | tv_w_3 explicitly documents the 4 per-edit greps per §Test Vectors table `tv_w_3` row with the corrected regexes per R3.A HIGH Option A fix (commits `8115c7f3` R1.5.a cite hygiene + `bd9e9e5c` R3.5 cite hygiene). Future parent RFC amendments that change these targets MUST update RFC-0011-w §Test Vectors tv_w_3 row + Task 10b shell grep to re-baseline. ACCEPTED RISK: per the multi-amendment chain pattern, this is the responsibility of the next amendment RFC that touches the parent RFC text. |
| `#[non_exhaustive]` discipline on `OctoCliError` preserved                                                                                                                                                                     | §Detailed Design §Variant definition; §Appendix C slot arithmetic summary                               | If `#[non_exhaustive]` is dropped, downstream consumers (CLI tools, scripts, library users) must be updated to handle the new variant → breaking change for additive-only contract → slot arithmetic drift                                                                                | `crates/octo-cli/src/error.rs` line 13 carries `#[non_exhaustive]` attribute on `OctoCliError` enum (verified at R4 layer-discipline axis). No future RFC may remove the `#[non_exhaustive]` attribute without an explicit Layer B breaking-change amendment per [[cipherocto-design-principles]] §Extension over enumeration + [[feedback_clippy_zero_warnings]] discipline.                                                                                                                                  |
| §Redaction Layer compliance: no raw `key_id` (32-byte) or `known_keys` (Vec) leakage in operator envelope                                                                                                                      | §Detailed Design §Variant definition; §Adversary Analysis Q1 + Q5 + Q7; §Security Considerations        | If the variant leaks raw substrate types into operator envelope → PII or verifier state exposure → compliance posture violation per RFC-0011-h §Redaction Layer parent RFC                                                                                                                | Operator envelope uses `key_id_hex: String` (8 hex chars per `KeyId = u32`) + `known_keys_band: KnownKeysBand` (3-band categorical redaction: None / Few / Many) — both redactions are type-enforced (no raw substrate types in variant fields). §Adversary Analysis Q1 + Q5 + Q7 document the threat model. Re-verified at R4 (substrate-faithfulness axis CLEAN; tv_w_1 + tv_w_1b + tv_w_2 all assert redacted output).                                                                                      |
| Variant sits unused in default builds until F1 future RFC provides CLI dispatch surface                                                                                                                                        | §RFC-0008 Execution Class Mapping; §Future Work F1                                                      | Until F1 lands, the `NetworkKeyRotationUnknownId` variant exists in the enum but is unreachable in normal CLI invocation → no observable behavior change in default builds → user-facing surface remains the existing exit-code envelope                                                  | ACCEPTED RISK. Documented in §Future Work F1: `octo network rebind-*` dispatch arms today return `Option<RebindCommit>` / `Option<RebindAbort>` not `Result<_, AttachError>`. Adding a CLI dispatch surface requires a substrate change to route through `AttachError`. This is intentionally out of scope for RFC-0011-w per [[cipherocto-design-principles]] §Separation of concerns (variant minting + dispatch surface are separate concerns). Deadline for F1 closure: future RFC, no specific date.      |
| Layer A frozen contract preserved across the amendment slice                                                                                                                                                                   | §Appendix B Layer A frozen check; §Adversary Analysis preamble                                          | If Layer A crates are modified → semantic-compatibility break across the protocol → cross-RFC dependency graph requires re-validation                                                                                                                                                     | Verified at R4 (layer-discipline axis CLEAN): zero diff to `crates/octo-governance-core` + `crates/octo-audit-core` + `crates/octo-settlement-core` (real Layer A crates in scope per Phase 14 R2.5 lesson; no phantom `octo-vault-core` or `octo-wallet-core` cites). Pre-existing layer-A preservation is enforced by RFC-0011-h §Stable Abstractions Principle + [[cipherocto-design-principles]] §Stable Abstractions Principle.                                                                           |
| Cargo feature propagation works without `dep:` prefix (octo-runtime is a required dep, not optional)                                                                                                                           | §Detailed Design §Feature flag wiring; §Cargo.toml edit at `next ef2251a4`                              | If a future CLI refactor moves `octo-runtime` from required to `optional = true` dep → feature propagation silently no-ops → translation arm becomes unreachable in all build configs                                                                                                     | `dep:` prefix syntax is conditionally required per Cargo docs when the dep is `optional = true`. Current state: `octo-runtime` declared as required dep at `crates/octo-cli/Cargo.toml`. If a future refactor flips this, the propagation clause MUST be updated to use `dep:octo-runtime/octo-attach-key-rotation` per [[substrate-faithfulness-verification]] R3.A HIGH-1 finding. Future CI lint: not in scope for this amendment. ACCEPTED RISK: tracked in future refactor work.                          |
...
| 1.1     | 2026-09-22 | Post-LANDED closeout. Multi-round DRY CLOSURE gate achieved GATE GREEN at R4 (0/0/0/0 findings across 4 rounds: R1 fail R1.5 fix R2 fail R2.5 fix R3 fail R3.5 fix R4 zero). 7-commit LANDED slice + fix sweeps verified at R4 (R4.A bugs correctness CLEAN; R4.B test coverage CLEAN; R4.C substrate-faithfulness CLEAN; R4.D layer discipline CLEAN; R4.E cite hygiene CLEAN). Added `## Compatibility` section (backward / forward / substrate axes; per docs/BLUEPRINT.md §Specification §Compatibility REQUIRED). Added `## Implicit Assumptions Audit` section (7 assumptions: substrate signature stability + `KeyId = u32` typed discriminator + parent RFC-0011-h cross-reference target stability + `#[non_exhaustive]` discipline + §Redaction Layer compliance + F1 future RFC dispatch surface ACCEPTED RISK + Layer A frozen contract + Cargo feature propagation `dep:` prefix conditional; per docs/BLUEPRINT.md §Specification §Implicit Assumptions Audit REQUIRED). tv_w_1b NEW quantization boundary coverage test added at R1.5.b per R1.B HIGH finding closure; 4 test vectors total (tv_w_1 + tv_w_1b + tv_w_2 + tv_w_3). `cargo test -p octo-cli --lib`: 477 passed (OFF); 479 passed (ON with `--features octo-attach-key-rotation`); clippy clean x2; fmt clean; cargo build --workspace clean. |
