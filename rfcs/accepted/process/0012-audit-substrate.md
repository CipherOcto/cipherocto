...
| 1.3     | 2026-09-13 | Mission 0012-audit-verify-chain-tests LANDED — 14 `verify_chain_vectors.rs` tests (10 RFC-0012 §Test Vectors + 4 boundary variants) with RFC-0012 §Test Vectors ID alias comments; `StoolapAuditSink` DOMAIN adapter (Layer B façade) at `crates/octo-audit/src/storage/stoolap.rs` wired to substrate `AppendOnlyAuditSink`; AC-1..AC-8 ticked. |
