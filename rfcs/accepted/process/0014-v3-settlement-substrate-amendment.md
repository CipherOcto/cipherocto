...
This section enumerates the substrate amendments + acceptance rollouts that built up the settlement substrate posture documented by RFC-0014-v3. Each row carries the amendment round, the substrate commit(s) that landed the change, and the acceptance rollout mission that verified the substrate-faithful posture survived downstream consumers.

| Round         | Substrate commits                                                                                                                                       | Scope                                                                                                                                                                                                                                                                                                                                                    | Acceptance mission                               |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| RFC-0014-v2   | (initial substrate extension pattern; canonical 10-pattern scrubber pattern list per §FW6)                                                              | Substrate-frozen settlement extension pattern; canonical scrubber pattern list (single source of truth for §S5.1)                                                                                                                                                                                                                                        | —                                                |
| RFC-0014-v3   | `f33410ce` + `934242ce` (per-façade scrubber + DOMAIN adapter migrations + `SettlementHashOpaque` Layer A newtype + `SinkSpecific` payload cap posture) | Per-façade 10-pattern scrubber at `octo-settlement` (defect 1b closure); DOMAIN adapter migration contract (Pattern 6 registry); shadow 8-variant `SettlementError::SettlementHashMismatch` + `AskNotFound` + `AlreadyConsumed` migrate `[u8; 32]` → `SettlementHashOpaque` (defect 2 oracle); `SinkSpecific` payload cap-at-scrubber posture (defect 3) | `0014-v3-settlement-substrate-amendment-rollout` |
| RFC-0014-v3.1 | **DEFERRED — lands at acceptance** (paired with `0012-v3.1`)                                                                                            | §FW2 substrate-level `[u8; 32]` Debug redaction; §FW2a paired-acceptance `SettlementError::{AskNotFound, AlreadyConsumed}` Display format-string vector migration; §FW3 substrate-level `SinkSpecific` cap; §FW4 clippy lint activation                                                                                                                  | —                                                |
