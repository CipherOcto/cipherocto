...
4. **Audit append + rollback contract** — every successful transition appends an `AuditEventKind::AgentTransition` row to `AppendOnlyAuditSink` per RFC-0012. On fail-closed audit-append failure the in-memory state is rolled back per §6.1 (5). The wallet substrate does NOT have special-case handling for `AlreadyExists` — if the audit-core propagates `AlreadyExists` as `Err`, the wallet's fail-closed rollback contract applies (blanket `.map_err` per `agent.rs` §transition_agent audit append branch).
...
- **(1) TOCTOU mitigation** — `std::sync::Mutex::lock()` on GLOBAL `AGENT_REGISTRY: OnceLock<Mutex<BTreeMap<Uuid, AgentRecord>>>` (canonical substrate pattern per §AGENT_REGISTRY). Mutex poison → `WalletError::Config(String)` (canonical poison-mapping per §Config; same as the read-path call sites in `agent` module). The `cli_fns::register_agent` call site maps mutex poison to `WalletError::OsRng` instead (different substrate-canonical mapping per `cli_fns` module; the wallet read+write path under `agent` uses the `Config` variant uniformly). The lock is acquired AFTER reason validation (§6.1 (6)); held across lookup + caller-attestation + state-machine work + audit append + rollback (or success) and released after the audit append completes (or rolls back). `current_state` is re-read INSIDE the lock. The single-writer invariant is enforced via the GLOBAL lock — concurrent callers serialize at lock acquisition. The per-agent rejection path is the state-machine guard (§6.1 (3) below), not a per-agent `try_lock`.
...
- **(5) Audit append + rollback contract** — every successful non-idempotent transition appends an `AuditEventKind::AgentTransition { agent_id, from, to, reason }` row to the canonical `AppendOnlyAuditSink` per RFC-0012 (parent `AuditEvent::at_millis_unix` carries the timestamp — variant payload does NOT carry `at_unix`). Substrate-faithful symmetry: parent struct is the canonical timestamp carrier; variants carry transition-specific payload only. Rollback contract: any `Err` from `append_agent_transition_event` propagates via blanket `.map_err` per `agent.rs` §transition_agent audit append branch → `WalletError::AuditUnavailable(String)` + in-memory state rolled back to `from`. On audit append failure:
  - The substrate maps the error to `WalletError::AuditUnavailable(String)` (canonical String payload per `WalletError::AuditUnavailable` mapping). String payload is `format!("{e:?}")` Debug-formatted `AuditError` per `agent.rs` §transition_agent audit append branch when `octo-audit-internal` feature is enabled; static placeholder when the feature is disabled. Wallet surface contract is identical in both branches.
...
- **(6) Reason length + control-char filter reuse** — `transition_agent` calls `validate_reason(reason)` (RFC-0015 KEEP primitive per §6.2.5 validate_reason) upstream; the primitive's `WalletError::{ReasonContainsControlChars(String), ReasonTooLong(usize)}` errors propagate through unchanged. Empty string allowed; ≤256 **bytes** enforced (longer → `WalletError::ReasonTooLong(usize)` with the byte length as payload; canonical unit per `validate_reason` substrate impl `reason.len()` and RFC-0015 §6.2.5 `REASON_MAX_BYTES`); control characters (`U+0000`-`U+001F`, `U+007F`) are REJECTED upfront before any state-machine work. Non-UTF-8 input rejected at CLI (substrate trust). C1 range gap (carry-over from RFC-0015 §6.2.5): see §Adversary Analysis Reason-field XSS row for the canonical residual gap explanation + `TV-WLT-AGT-13c` reference.
- **(7) Single-callsite per agent** — GLOBAL `AGENT_REGISTRY` std Mutex serializes concurrent `transition_agent` calls (canonical substrate pattern per §6.1 (1)). The `AlreadyInTransition(Uuid)` variant is declared in `crates/octo-wallet/src/error.rs` §AlreadyInTransition but NOT constructed by `transition_agent` at RFC-0015-a acceptance (dead surface — concurrent callers serialize via the GLOBAL std Mutex). The variant is reserved for a future paired-acceptance substrate amendment per §Discipline at first call site (CLAUDE.md §Architectural Principles); the `#[non_exhaustive]` declaration is preserved for documentation + CLI-mirror alignment only.

### §6.2 `TransitionReceipt` projection

**Layer designation:** Layer B projection defined in `crates/octo-wallet/src/agent.rs` (NOT introduced into `octo-settlement-core` Layer A or any other Layer A frozen crate). Additive to the existing `octo-wallet` (Layer B) public surface per §RFC-0015 R40 substrate-first ordering.
...
pub struct TransitionReceipt {
    ...
    /// transition (best-effort `SystemTime::now()` wall-clock
    /// per substrate at `crates/octo-wallet/src/agent.rs`
    /// `transition_agent`).
...
**Substrate-faithful note:** the 5 fields mirror the substrate struct verbatim (per §TransitionReceipt struct): `agent_id` + `previous_state` + `current_state` + `transitioned_at_unix` + `audit_log_entry`. The CLI surfaces this via `OutputEnvelope<TransitionOutput>` with the canonical RFC-0010 DID form. The `TransitionReceipt` projection is additive to `crates/octo-wallet/src/agent.rs` (Layer B façade; no parallel abstraction to `AuditEvent`). The `audit_log_entry: [u8; 32]` field IS the canonical chain-hash (raw `[u8; 32]` bytes per substrate at `crates/octo-wallet/src/agent.rs` §TransitionReceipt; CLI converts to Hex32 wire form at the output boundary via `OutputEnvelope<TransitionOutput>` — the conversion is display-only, NOT a substrate-level encoding).
...
**Self-loop rule (uniform across all three states per §6.1 (4)):** `transition_agent(uuid, target, _)` with `target == current_state` is an idempotent same-state no-op — returns `Ok(TransitionReceipt { ..., audit_log_entry: [0u8; 32] })` WITHOUT audit append. Substrate-canonical at `crates/octo-wallet/src/agent.rs` (the `if from == target { return ... }` early-return runs BEFORE the `matches!` arm). The rule applies uniformly to `Registered`, `Running`, AND `Terminated` — the table's "Allowed transitions TO" column documents ONLY the valid cross-state edges.
...
| `AgentState::Terminated` | `TERMINATED` (terminal)                     | `state = "terminated"` | (terminal; no cross-state edges — self-loop is the only valid call per the rule above) |
...
The `AuditEventKind::AgentTransition` variant lives in `octo-audit-core` Layer A (frozen). The feature `octo-audit-internal` is declared in `octo-audit-core/Cargo.toml` (Layer A itself); `octo-audit/Cargo.toml` propagates it. The variant is gated `#[cfg(feature = "octo-audit-internal")]` in the Layer A core; visible only when the feature is enabled at the core. The cfg-gate is the Layer A frozen preservation mechanism: the variant exists in source but is invisible to default builds, preserving the frozen-contract invariant (no additive variants visible to consumers without paired amendment acceptance).

The bridge resolves when RFC-0012 acceptance lands: the cfg-gate is removed, the variant becomes permanent. Until then, `transition_agent` audit append depends on the feature being enabled at compile-time; this is the substrate-faithful paired-acceptance contract per §6.5 (Layer A Paired-Acceptance Bridge).
...
7. **`caller_did` provenance from active `IdentityHandle` (HSM-bound)** — same discipline as RFC-0015 §Implicit Assumptions Audit row 5. Substrate does NOT authenticate `caller_did`; only enforces `caller_did == agent.holder_did` (per §6.1 (2)). CLI / programmatic caller MUST source `caller_did` from active `IdentityHandle` (HSM-bound per RFC-0009 §Identity Struct + §HsmAdapter Integration) at Layer B façade boundary. Per-process trust boundary at the façade.
...
4. **Audit append rollback contract** (paired with RFC-0012 `AuditEventKind::AgentTransition`) — covers `octo_audit::AuditError` variants per §6.1 (5):
   ...
   - `AlreadyExists` is the audit-sink-internal idempotent-retry signal at the `octo-audit-core` layer per the canonical `AuditError::AlreadyExists` variant. The wallet substrate does NOT have special-case handling for `AlreadyExists` — if the audit-core propagates `AlreadyExists` as `Err`, the wallet's fail-closed rollback contract applies (`WalletError::AuditUnavailable(String)`; state mutation rolled back).
...
Substrate-level test vectors (`crates/octo-wallet/src/agent.rs` test module). RFC-0015 KEEP vectors are unaffected. RFC-0015-a KEEP vectors land at RFC-0015-a acceptance (paired with RFC-0012 acceptance).

**Numbering scheme:** see RFC-0016 §Test Vectors for canonical scheme + sub-letter convention. RFC-0015-a owns TV-WLT-AGT-3-11c (write path); RFC-0015 owns TV-WLT-AGT-1/2/12-23 (read path; sub-letter 13a-13h within `validate_reason`).
...
| TV-WLT-AGT-10  | `transition_agent(caller_did, uuid, _, _)`                                  | Concurrent call against same `agent_id`                                                                                                                | Second caller serializes at GLOBAL `AGENT_REGISTRY` std Mutex lock acquisition; first call completes (audit append succeeds → state transition applied) OR fails (audit append fails → state rolled back); second caller observes the post-rollback or post-success state. Per §6.1 (7), the `AlreadyInTransition(uuid)` substrate variant is RESERVED for a future paired-acceptance amendment and is NOT constructed at RFC-0015-a acceptance; the canonical concurrent-call substrate behavior is serialized lock acquisition + state-machine guard. | Concurrent-call serialized via GLOBAL `AGENT_REGISTRY` std Mutex per §6.1 (1) + §6.1 (7); `AlreadyInTransition(uuid)` is a declared-but-dead surface at RFC-0015-a acceptance                                              |
...
| TV-WLT-AGT-11b | `transition_agent(caller_did, uuid, _, _)` with audit sink gap              | `append_agent_transition_event` returns `AuditError::SequenceGap { event_id: 43, prev: 42 }`                                                           | `Err(WalletError::AuditUnavailable(<Debug-formatted AuditError string>))` + in-memory state rolled back (per §6.1 (5) rollback contract)                                                                                                                                                                                                                                                                                                                                                                                                                | Rollback contract (SequenceGap fail-closed trigger per §6.1 (5))                                                                                                                                                           |
| TV-WLT-AGT-11c | `transition_agent(caller_did, uuid, _, _)` with audit sink idempotent retry | `append_agent_transition_event` returns `AuditError::AlreadyExists(42)` (propagated as Err at audit-core layer per substrate-canonical 3-variant form) | `Err(WalletError::AuditUnavailable(<Debug-formatted AuditError string>))` + in-memory state rolled back per §6.1 (5). The wallet substrate's blanket `.map_err` propagates any Err from `append_agent_transition_event`; no special-case for `AlreadyExists` (the substrate does NOT normalize AlreadyExists to Ok at RFC-0015-a acceptance).                                                                                                                                                                                                           | Fail-closed rollback contract (AlreadyExists propagated as Err) — the audit-core layer's canonical 3-variant form per RFC-0012 always propagates AlreadyExists as Err; the wallet's fail-closed rollback contract applies. |
...
- **Phase 2.5 (RFC-0015-a acceptance, paired with RFC-0012 acceptance)** — `transition_agent` write function lands on `octo-wallet` Layer B; 3 NEW ADDITIVE `WalletError` variants (`AlreadyInTransition(Uuid)`, `InvalidStateTransition { from, to }`, `AuditUnavailable(String)`) land on `octo-wallet/src/error.rs`; `AuditEventKind::AgentTransition { agent_id, from, to, reason }` variant lands in `octo-audit-core` (Layer A frozen; parent `AuditEvent::at_millis_unix` carries the timestamp). `octo-wallet`'s `octo-audit` Layer B façade dep is PRE-EXISTING per substrate-first ordering; no NEW Cargo.toml dep lands at RFC-0015-a acceptance (substrate uses `std::sync::Mutex` already in §AgentRecord struct).
...
- `crates/octo-wallet/Cargo.toml` — no NEW dep at RFC-0015-a acceptance. The pre-existing `octo-audit = { path = "../octo-audit" }` Layer B façade dep carries the canonical `append_agent_transition_event` re-export path (substrate uses `std::sync::Mutex` already in §AgentRecord struct per `use std::sync::{Mutex, OnceLock};`).
...
// Canonical contract lives in §6.1 transition_agent spec (substrate-faithful
// narrative + rustdoc). The pseudocode body for `transition_agent` follows.
...
pub fn transition_agent(
    ...
    // 5. In-flight transition guard: NOT constructed at RFC-0015-a acceptance.
    //    The `WalletError::AlreadyInTransition(uuid)` variant is declared but
    //    dead-surface at RFC-0015-a acceptance per §6.1 (7); concurrent callers
    //    serialize at the GLOBAL std Mutex lock acquisition (§6.1 (1)).
    ...
    //    via `append_agent_transition_event(&payload, transitioned_at_unix)`
    //    (substrate-canonical per `agent.rs` §transition_agent audit append branch,
    //    re-exported from `octo_audit::audit_write`).
    // 10. If append fails (any `AuditError` variant — uniform `.map_err`
    //     blanket conversion per `agent.rs` §transition_agent audit append
    //     branch) — ROLLBACK in-memory state to `from`; return
    //     `WalletError::AuditUnavailable(format!("{e:?}"))` (fail-closed).
    //     No special-case for `AuditError::AlreadyExists`; the substrate's
    //     blanket `.map_err` propagates the Err, triggering rollback.
