...
- Maintainer: @cipherocto (CipherOcto maintainers; coordinate promotion to Accepted via 7-day review + 2 maintainer approvals per BLUEPRINT.md §RFC Acceptance Process; v1.0 author + v1.1/v1.2/v1.3 amendment lead)
- Maintainer: @mmacedoeu (CipherOcto maintainer; v1.0 approval + v1.1 + v1.2 + v1.3 amendment sign-off)
...
- **Hardware wallet + MPC integration** — Phase H (RFC-0853 F2) and Phase I (RFC-0853 F3); ZK signing key custody inherits from RFC-0009 §Vault — Provider-Key Handling.
...
/// provider vault slot (RFC-0009 §Vault — Provider-Key Handling) that the capability is bound to. The
...
pub struct PublicInputs {
    ...
    /// **v1.4:** provider vault slot ID (RFC-0009 §Vault — Provider-Key Handling). Stable identifier
...
// substrate consumes this crate). Cross-ref RFC-0009 §Vault — Provider-Key Handling substrate for the wrapping
...
| Verified            | Consumed            | Settlement complete (RFC-0959 v2.0 independent settlement chain)                                                                                                                                | Yes                               | Emit `TokenConsumed { root_id, settlement_receipt_hash }`                                                                                                        | n/a                                                                           |
...
3. **mlock + zeroize** on prover process memory (inherited from RFC-0009 §Vault — Provider-Key Handling).
...
- **Hardware Wallet / MPC signer** — Phase H / I (RFC-0853 F2/F3); ZK signing key custody inherits from RFC-0009 §Vault — Provider-Key Handling.
...
**Verdict:** MITIGATED via STWO verify (which attests the circuit's Poseidon assertion). Live debugger attack inherited from RFC-0009 §Security Considerations.
...
| RFC-0909 (Economics: Deterministic Quota Accounting)   | **Accepted (v69, folder `final/`)** | None — coexistence only. Symmetry with RFC-0959 v2.0; no direct dependency.                            |
...
| IA-8 | Capability root secret is zeroized after proof gen                                | §Algorithms `mint_with_zk` (L271-301) step 4 CASM hash check (zeroize location: §Key Handling Rules L495-499); §Key Handling Rules (L495-499, includes header L494)                                                 | Retention → verifier compromise                        | Inherit from RFC-0957 §IA-7 + RFC-0009 §Vault — Provider-Key Handling                                             |
...
| IA-11 | **v1.4:** `provider_slot_id` is a stable, globally-unique identifier for the vault slot that the capability is bound to; proofer and verifier MUST agree on the slot ID for the proof to verify. Sourced from RFC-0009 §Vault — Provider-Key Handling substrate. | §Data Structures `PublicInputs.provider_slot_id` (v1.4 field); §Algorithms `verify_capability_zk` (slot comparison); §Algorithms `mint_with_zk` (slot sourcing from vault) | Cross-slot replay: attacker captures a proof bound to slot A, replays against token claiming slot B → authorization grant against wrong slot | Verifier compares `proof.public_inputs.provider_slot_id` against token envelope's `provider_slot_id` (RFC-0957 §Wire Format v3 slot segment); mismatch returns `ZkVerifyError::PublicInputMismatch`. Mint API rejects empty / unparseable slot IDs. Vault substrate (RFC-0009 §Vault — Provider-Key Handling) provides slot ID provenance. |
...
| 10  | **Live debugger on prover process** (extracts `cap_root_secret` mid-proof)          | High   | Inherited from RFC-0009 §Security Considerations; mlock + zeroize; out-of-scope at MVP                                              |
...
| 12  | **Replay of in-flight proof against same verifier** (no nonce-based dedup)          | Low    | `PublicInputMismatch` rejects + downstream settlement receipt uniqueness (RFC-0959 v2.0); no nonce-based dedup       |
...
### Compatibility Across Versions (v1.3 amendment)

- v1.0 (2026-07-21 Accepted): baseline; no ClockSkewExceeded variant; verifier signature is 3-arg.
...
- **F4:** Signed CASM (build pipeline signature verification at verify time). **v1.3 promotion path:** F4 is **REQUIRED** for full A2 mitigation per §Adversary A2 v1.1 R1 H5 fix; current verdict is PARTIALLY MITIGATED. Implementation path: (1) stoolap fork `cairo/build.sh` emits `{CASM bytes, build-pipeline Ed25519 sig over CASM}`; (2) `ProofBundle` adds `casm_build_sig: Ed25519Signature` field; (3) `verify_capability_zk` requires `casm_build_sig` to verify against pinned pipeline pubkey before `casm_hash` check. Until F4 lands, A2 residual risk = supply-chain compromise (build pipeline trust).
