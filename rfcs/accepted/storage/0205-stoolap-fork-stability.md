...
Accepted (v2.0) — promoted 2026-08-20 by user instruction per /goal "if you need a redesign 2.0 go for it" + explicit promote-accepted instruction.
...
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Condition 1** — Sibling RFC frozen at Accepted (coupled pair)                  | ✓ RFC-0206 v2.0 promoted Accepted in same commit batch (this RFC + sibling land together per BLUEPRINT.md §Dependency Validation Rules → 2-Cycle Atomic Promotion, rule 5)                                                                                         |
| **Condition 2** — Phase 0-1 complete (cipherocto-stewards-meta + Phase 1.1-1.11) | ✗ NOT YET LANDED — cipherocto-stewards-meta repo not bootstrapped; Phase 1.1-1.11 implementation items (firmware-allowlist.toml, test-harness-allowlist.toml, frozen-source-allowlist.toml, external-root-config.toml, runbook, audit directories) not yet created |
...
- Bootstrap `cipherocto-stewards-meta` repo (Phase 0.1)
- Land 11 Phase 1 deliverables (allowlist tomles + runbook + audit dirs)
- Dispatch R10 reviewers (≥2) against committed v2.0 RFC bodies
- Land `crates/octo-storage-core` newtype refactor + 29 Layer B TYPE renames (RFC-0206 v2.0 §Implementation Phases Phase 1.2-1.9)
