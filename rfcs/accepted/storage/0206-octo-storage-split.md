...
| **Condition 2** — Phase 1.1-1.9 complete (substrate newtype + 29 TYPE renames + 5 adapter crates) | ✗ NOT YET LANDED — `crates/octo-storage-core` newtype not implemented; Layer B TYPE renames not applied; 5 adapter crates (`octo-vault-storage`, `octo-reputation-storage`, `octo-cap-macaroon-vault-storage`, `octo-matrix-session-store-storage`, `octo-policy-storage`) not created |
