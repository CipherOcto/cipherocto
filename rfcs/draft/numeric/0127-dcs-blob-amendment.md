...
Draft (v6, adversarial review round 12 -- third comprehensive review fixes)
...
- **Extra data after last field**: If bytes remain after all schema fields are deserialized, the dispatcher returns error (trailing garbage). Conformant data has no trailing bytes.
- **Field ID mismatch**: The wire format uses u32_be field IDs. If the serialized field ID does not match the schema's expected field ID at that position, the dispatcher returns error. Field ordering is schema-declaration order, not wire-order.
