...
| Operator runs `bind-envelope rebind-commit` on wrong target                                 | HIGH — irreversible state change if substrate commits  | see §Security Considerations pastejacking-defense bullet (canonical cite for `--dry-run` + double-confirm flags). Audit-log chain is OUT OF SCOPE for this amendment (no §Audit Log subsection in this RFC; substrate-owned logging per RFC-0011-g §Audit Reporting). |
...
| `tv-network-dry-run-deny-1`                 | `bind-envelope rebind-abort`                                                                                                                                      | `--dry-run` explicit + operator denied at prompt → exit 88 (`NetworkDryRunDenied`) (operator overrode dry-run preview by declining at the confirmation prompt)                                  |
