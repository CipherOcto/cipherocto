...
**† Substrate umbrella:** All 3 RebindEnvelope builders (Prepare, Commit, Abort) use the `RebindEnvelope` umbrella enum at `crates/octo-network/src/mon/bind_envelope.rs`. The 3 substrate-function cells above cite the per-variant builder only. **Prepare + Abort** Sub-action cells use `--confirm-acknowledge` alone per RFC-0011 §Confirmation Flag Matrix non-pastejacking-risk row; Commit Sub-action cell additionally requires `--confirm SECOND` flag per pastejacking-defense risk model.
...
RFC-0011-c/d/e/f/g all below slot 79; slots 79 + 82-90 free at Draft time. Per substrate-faithfulness audit 2026-09-18, slots 80 + 81 revert to candidate status if either substrate surface lands a fallible path in a future amendment (rationale in footnote [†] below).
...
| ------ | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| 0      | `Ok`                          | Success                                                                                                                     |
| 2      | `ValueValidation`             | clap default (closed-enum / numeric-range rejection; covers L271 --hops u16 + L526 --depth clamp + L608 abort-2 BogusValue) |
...
**† Operator note (per L5 simplification):** All test vectors in this section that use `--envelope-id <hex>` have the flag mapping to output struct field `domain_id` per §Subcommand Taxonomy rebind row.
...
- [ ] Mission `0011-h-network-peers-identity` (peers + identity)
  ...
  - [ ] Test vectors tv-network-peers-list-1/2 + tv-network-peers-get-1/2 (incl. tv-network-redact-did-1 redaction cross-cut per L579) + tv-network-identity-show-1/2
