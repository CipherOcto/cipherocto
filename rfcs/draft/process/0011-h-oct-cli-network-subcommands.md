...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --- | -------- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bootstrap`                      | DEFERRED — substrate-additions prerequisite (BootstrapOrchestrator missing; impl in `octo_transport::bootstrap`, surfaced via `octo-network` facade) | no  | no  | no       | Bootstrap Authority | see G1 companion; migration: reroute via `octo network peers list` + `octo network trust-graph render` for equivalent visibility                                                                    |
...
pub struct NetworkSlashStatsOutput {
  ...
  pub per_did: Vec<DidSlashCount>,     // DEFERRED — substrate lacks per-DID iterator (no `iter_dids()` accessor verified; surface returns empty vec until G6 `0011-h-s-a-slash-store` companion lands the iterator); aggregate-only surface today
}
...
| `tv-network-bind-envelope-rebind-prepare-3` | `octo network bind-envelope rebind-prepare --envelope-id <redacted>` in CI mode (`OCTO_CLI_CI=1` set, `--confirm-acknowledge`, `--no-dry-run`)                    | exit 90 (`NetworkCIRebindDenied`); substrate `RebindCoordinator::mark_committed` not reached because CLI gate fires first per §CI Mode Rationale (operator note: `--envelope-id <hex>` corresponds to output struct field `domain_id` per §Subcommand Taxonomy rebind row; cross-ref: envelope-id → domain_id footer above) |
...
| ---- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| G1   | `0011-h-s-a-bootstrap-orchestrator`                             | `BootstrapOrchestrator` struct + `start_bootstrap(BootstrapConfig) → BootstrapOutcome` + `status() → BootstrapState` + `BootstrapConfig::{from_toml, save_toml}` + `BootstrapConfig::bootstrap_mode` accessor (impl in `crates/octo-transport/src/bootstrap.rs`; surfaced via `octo-network` Layer B facade)                                                                                                                                                                                                                                                                                | `octo network bootstrap` (deferred); `octo network status` (deferred)                                                                          |
| G2   | (subsumed by G1)                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —                                                                                                                                              |
...
[^g1-blocked]: **Bootstrap substrate surface BLOCKED per §Substrate-Additions G1** — substrate currently lacks `BootstrapOrchestrator` struct + `BootstrapOrchestrator::start_bootstrap` + `BootstrapOrchestrator::status` + `BootstrapConfig::{from_toml, save_toml}` (canonical impl in `crates/octo-transport/src/bootstrap.rs`, surfaced via `octo-network` Layer B facade). Both `bootstrap` and `status` Sub-action cells inherit this BLOCKED state; CLI dispatches hit clap `UnrecognizedSubcommand` exit 2 pre-landing. Companion G1 closes this gate (per Pattern C appendix).
...
6. ** lesson encoded.** Original R1 draft produced ~30 CRIT substrate-faithfulness findings. Per [[substrate-faithfulness-verification]], substrate must be verified by grep BEFORE RFC is drafted, not after. Current rewrite grounded in actual `octo-network` cargo grep output (verified).
