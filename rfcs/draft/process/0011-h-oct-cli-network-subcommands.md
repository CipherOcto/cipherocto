...
| `mode show`                      | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::from_toml` missing in `mon/bootstrap.rs` per substrate-audit 2026-09-18)                                            | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; read-only once loader lands                                                                                                               |
| `mode set`                       | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::save_toml` missing per substrate-audit 2026-09-18)                                                                  | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until `save_toml` lands                                     |
| `authority show`                 | yes                                                                                                                                                                                            | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                                                                                                            |
| `authority rotate`               | BLOCKED (pending G8 `0011-h-s-a-seed-list-authority-rotate`; `rotate_post_fork` missing in substrate per substrate-audit 2026-09-18)                                                           | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; operators manually edit `OCTO_HOME/octotransport/bootstrap.toml` and restart until rotation primitive lands                               |
...
| `slash list` / `slash show`      | BLOCKED (pending G6 `0011-h-s-a-slash-store`; `SlashReputationStoreCompat::list` + `peer_reputation(did)` missing in substrate per substrate-audit 2026-09-18)                                 | no  | no  | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; operators use `slash stats` for aggregate view + `slash excluded <did>` for per-DID exclusion check until `list`+`peer_reputation` land   |
| `trust-graph render`             | yes                                                                                                                                                                                            | yes | yes | yes      | Operator            | read-only                                                                                                                                                                                                                                                            |
| `coordinator show`               | BLOCKED (pending G12b `0011-h-s-a-coordinator-record-loader`; `CoordinatorRecord::load` static method missing per substrate-audit 2026-09-18)                                                  | yes | yes | redacted | Coordinator         | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; coordinator DID redacted in Audit; once loader lands                                                                                      |
| `coordinator admin`              | BLOCKED (pending G12 `0011-h-s-a-coordinator-admin-trait`; coordinator admin substrate missing per substrate-audit 2026-09-18)                                                                 | no  | no  | no       | Coordinator         | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; companion mission `0011-h-s-a-coordinator-admin-trait` delivers the substrate surface                                                     |
| `governance tally`               | BLOCKED (pending G3b; per §Substrate-Additions)                                                                                                                                                | yes | yes | yes      | Governance Voter    | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; read-only once helper lands                                                                                                               |
...
| `bind-envelope show`             | BLOCKED (pending G22 `0011-h-s-a-bind-envelope-lookup`; `BindEnvelope::load` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18)                      | yes | yes | redacted | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; payload bytes never surfaced                                                                                                                |
| `bind-envelope rebind-prepare`   | BLOCKED (pending G21; per §Substrate-Additions)                                                                                                                                                | no  | yes | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; `--dry-run` default + `--confirm-acknowledge` required; reversible; payload-builder only                                                  |
| `bind-envelope rebind-commit`    | BLOCKED (pending G21; per §Substrate-Additions)                                                                                                                                                | no  | yes | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89; `--dry-run` default + `--confirm-acknowledge` + `--confirm` SECOND flag (pastejacking defense per §Subcommand Taxonomy row); irreversible |
...
| `discovery advertisement show`   | BLOCKED (pending G23 `0011-h-s-a-discovery-advertisement-cache`; `MissionAdvertisementCache::get` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18) | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                                                                                   |
| `discovery invitation show`      | BLOCKED (pending G24 `0011-h-s-a-discovery-invitation-cache`; `MissionInvitationCache::get` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18)       | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                                                                                   |
...
| `bind-envelope show`           | (read; BLOCKED — pending G22 `0011-h-s-a-bind-envelope-lookup` closure; `BindEnvelope::load(domain_id)` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18)                   | Substrate lookup surface absent; companion mission `0011-h-s-a-bind-envelope-lookup` per §Substrate-Additions G22 row closes this gate. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | RFC-0871 §Data Structures                                                                                         | `0011-h-network-bind-envelope` §bind-envelope-show   | 4     | Operator            |
| `bind-envelope rebind-prepare` | (write; --dry-run default; --confirm-acknowledge required)[^g21-blocked]                                                                                                                                               | `RebindEnvelope::Prepare(RebindPrepare)` builder --confirm double-gate NOT required (prepare only reserves resources and is fully reversible via substrate accept-rollback per RFC-0871 §Algorithms); --confirm-acknowledge per non-pastejacking-risk row (see caption footer). Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | RFC-0871                                                                                                          | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     | Operator            |
| `bind-envelope rebind-commit`  | (write; --dry-run default; --confirm-acknowledge; --confirm SECOND flag required (pastejacking per §Security Considerations))[^g21-blocked]                                                                            | `RebindEnvelope::Commit(RebindCommit)` builder. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | RFC-0871                                                                                                          | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     | Operator            |
...
| `discovery advertisement show` | (read; BLOCKED — pending G23 `0011-h-s-a-discovery-advertisement-cache` closure; `MissionAdvertisementCache::get` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18)         | Substrate lookup surface absent; companion mission `0011-h-s-a-discovery-advertisement-cache` per §Substrate-Additions G23 row closes this gate. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-advertisement  | 5     | Operator            |
| `discovery invitation show`    | (read; BLOCKED — pending G24 `0011-h-s-a-discovery-invitation-cache` closure; `MissionInvitationCache::get` lookup method missing in substrate per R2 substrate-faithfulness re-verification 2026-09-18)               | Substrate lookup surface absent; companion mission `0011-h-s-a-discovery-invitation-cache` per §Substrate-Additions G24 row closes this gate. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-invitation     | 5     | Operator            |
| `mode set`                     | (write; BLOCKED — pending G1 `0011-h-s-a-bootstrap-orchestrator` closure; `BootstrapConfig::save_toml` missing in `crates/octo-network/src/mon/bootstrap.rs` per R2 substrate-faithfulness re-verification 2026-09-18) | `BootstrapConfig::save_toml(path)` writer. Companion mission `0011-h-s-a-bootstrap-orchestrator` per §Substrate-Additions G1 row closes this gate. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | RFC-0851p-a §3 Mode A                                                                                             | `0011-h-network-mode` §mode-set                      | 2     | Bootstrap Authority |
| `authority rotate`             | (write; BLOCKED — pending G8 `0011-h-s-a-seed-list-authority-rotate` closure; `SeedListAuthority::rotate_post_fork` missing in substrate)                                                                              | `SeedListAuthority::rotate_post_fork(new_authority, governance_quorum_proof) → Result<SeedListAuthority, SeedAuthorityError>` builder. Companion mission `0011-h-s-a-seed-list-authority-rotate` per §Substrate-Additions G8 row closes this gate. Pre-landing → exit 89 (`NetworkSubstrateUnavailable`) per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | RFC-0851p-a §1 BootstrapNode Registry                                                                             | `0011-h-network-authority` §authority-rotate         | 2     | Bootstrap Authority |
...
[^g1-blocked]: **Bootstrap substrate surface BLOCKED per §Substrate-Additions G1** — substrate currently lacks `BootstrapOrchestrator` struct + `BootstrapOrchestrator::start_bootstrap` + `BootstrapOrchestrator::status` + `BootstrapConfig::{from_toml, save_toml}` in `crates/octo-network/src/mon/bootstrap.rs`. Both `bootstrap` and `status` Sub-action cells inherit this BLOCKED state; CLI dispatches hit clap `UnrecognizedSubcommand` exit 2 pre-landing. Companion mission `0011-h-s-a-bootstrap-orchestrator` closes this gate (per Pattern C appendix).
...
- [ ] Mission `0011-h-network-bind-envelope`
  ...
  - [ ] `bind-envelope rebind-prepare` (write; --dry-run default; BLOCKED — see §Subcommand Taxonomy + §Substrate-Additions G21 `0011-h-s-a-attached-handle-key-rotation`); `RebindPrepare` builder; `NetworkRebindPrepareOutput`
...
- [ ] Drift-closure mission `0011-h-0851p-a-seed-health-check-archive` — archive `0851p-a-seed-health-check` (frontmatter `status: Claimed` + §Status prose "LANDED 2026-08-13 (drift-closure)" mismatch per [[memory-is-never-status-ground-truth]])
