...
**† DEFERRED rows:** All DEFERRED rows have paired companion missions in §Substrate-Additions; migration fallback documented in the paired mission YAML. Operators calling pre-landing hit clap `ValueValidation` (exit 2) per §Exit Codes.
...
| `mode set`                       | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::save_toml` missing per substrate-audit 2026-09-18)                                  | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until `save_toml` lands                                   |
| `authority show`                 | yes                                                                                                                                                            | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                        |
| `authority rotate`               | BLOCKED (pending G8 `0011-h-s-a-seed-list-authority-rotate`; `rotate_post_fork` missing in substrate per substrate-audit 2026-09-18)                           | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; operators manually edit `OCTO_HOME/octotransport/bootstrap.toml` and restart until rotation primitive lands                             |
...
| `coordinator admin`              | BLOCKED (pending G12 `0011-h-s-a-coordinator-admin-trait`; coordinator admin substrate missing per substrate-audit 2026-09-18)                                 | no  | no  | no       | Coordinator         | clap arm registered, gated on substrate; companion mission `0011-h-s-a-coordinator-admin-trait` delivers the substrate surface                                                   |
...
| `authority rotate`             | (write; BLOCKED — pending G8 `0011-h-s-a-seed-list-authority-rotate` closure; `SeedListAuthority::rotate_post_fork` missing in substrate)                                                                              | `SeedListAuthority::rotate_post_fork(new_authority, governance_quorum_proof) → Result<SeedListAuthority, SeedAuthorityError>` builder. Companion mission `0011-h-s-a-seed-list-authority-rotate` per §Substrate-Additions G8 row closes this gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | RFC-0851p-a §1 BootstrapNode Registry                                                                             | `0011-h-network-authority` §authority-rotate         | 2     | Bootstrap Authority |
...
RFC-0011-c/d/e/f/g all below slot 79; slots 79 + 82-90 free at Draft time. Per substrate-faithfulness audit 2026-09-18, slots 80 + 81 revert to candidate status if either substrate surface lands a fallible path in a future amendment (rationale in footnote [†] which appears immediately below this paragraph).
...
Narrative threat model. Operator-facing decision matrix in §Adversarial Review below. §Adversarial Review table references these bullets as canonical cite; table is narrative.
