...
- **Redaction placeholder form:** RFC-0011 §Redaction layer uses `[REDACTED:<kind>]` placeholders. `peer DID` → `[REDACTED:did]`, `coordinator_id` → `[REDACTED:coordinator_id]`, `multiaddr` → `[REDACTED:multiaddr]`. Audit mode applies same placeholders.
...
- **`bind-envelope rebind-prepare` / `rebind-commit` / `rebind-abort` chain integrity.** Three-part chain (prepare → commit | abort) is governed by substrate nonce + monotonic timestamp gate per RFC-0871 §Data Structures. CLI builders emit substrate-derived freshness — operators CANNOT inject arbitrary timestamp/nonce values. Double-commit is substrate-rejected via idempotency check. **Double-confirmation gate:** `rebind-commit` (irreversible state mutation) requires BOTH `--dry-run` default OFF + explicit `--confirm-acknowledge` flag + `--no-dry-run` flag. `rebind-abort` is reversible (drops pending prepare payload, no state change to bound envelope); single-confirmation via `--confirm-acknowledge` sufficient. Signing-identity key compromise is mitigated by HSM wallet requirement per RFC-0015-a §6.5 paired-acceptance bridge + `0011-h-s-a-attached-handle-key-rotation` companion substrate enabling key rotation invalidating stale signatures.
...
| G2   | (subsumed by G1)                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —                                                                                                                                                                                                              |
| G3b  | `0011-h-s-a-voting-tally-canonical-bytes`                       | Substrate-additions companion for `governance tally` (substrate-faithful row in §Subcommand Taxonomy): add free fn `governance_proposal_canonical_bytes(&GovernanceProposal) -> [u8; 32]` (blake3 hash surface) in `octo-governance` Layer B facade (re-exporting core types from `octo-governance-core` Layer A frozen). MUST NOT add method to `GovernanceProposal` (Layer A frozen contract preserved). Current substrate has no such fn — verified via cargo grep returning empty (2026-09-18). `canonical_bytes_hash` field in `NetworkGovernanceTallyOutput` requires this substrate helper. Without it, `governance tally` cannot surface the canonical-bytes hash substrate-faithfully.                                                                                                                                                                                                                                                                                                                                                                            | `octo network governance tally` (this amendment) — helper must close BEFORE CLI mission opens                                                                                                                  |
...
| 0.5     | 2026-09-18 | R10.5: dry_run field rename + matrix bootstrap/status rows + bind-chain integrity + F5 hedge trim. |
