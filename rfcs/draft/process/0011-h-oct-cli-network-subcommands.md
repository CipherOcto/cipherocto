...
3. **Mutating operations unsafe.** Substrate-faithful slice has 3 mutating subcommands (the `bind-envelope rebind-{prepare,commit,abort}` payload builders); every one must mirror §Confirmation Flag Matrix; every mutating subcommand defaults to `--dry-run`. (`slash mark` was originally counted but is read-only inquiry against `SlashReputationStoreCompat` aggregate; CLI never mints slashes — minting is Class B per RFC-0008 §Execution Class Mapping and out of scope for this amendment.)
...
| `slash mark`                     | CLI cannot mint SlashEnvelopes (substrate-only mint path)                                                                                                                                                                                                                  | no  | no  | no       | n/a                 | out of scope per RFC-0008 §Execution Class Mapping                                                                                                                                                  |
| `slash list` / `slash show`      | BLOCKED pending G6 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                                  | no  | no  | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; operators use `slash stats` for aggregate view + `slash excluded <did>` for per-DID exclusion check until `list`+`peer_reputation` land   |
...
| `coordinator admin`              | BLOCKED pending G12 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                                 | no  | no  | no       | Coordinator         | clap arm registered, gated on substrate[^clap-arm-gated]; companion mission `0011-h-s-a-coordinator-admin-trait` delivers the substrate surface                                                     |
...
| `governance vote`                | OUT                                                                                                                                                                                                                                                                        | no  | no  | no       | n/a                 | lives in RFC-0011-g Phase 2 (`octo governance vote` per RFC-0011-g §2.4); not exposed via `octo network` since substrate is the same `VotingTally` instance owned by governance tier                |
...
**† Operator note (per R5 simplification):** All test vectors in this section that use `--envelope-id <hex>` have the flag mapping to output struct field `domain_id` per §Subcommand Taxonomy rebind row.
...
- [ ] File 2 follow-on missions when work enters Phase 1 of its own RFC/mission cycle (placeholder paths in `missions/open/` to be filed at CLAIMED-time per [[no-phantom-mission-pointers]]; the prometheus-metric-export + operator-guide companion work is not yet scheduled under any RFC and is therefore not cite-locked in this amendment)
