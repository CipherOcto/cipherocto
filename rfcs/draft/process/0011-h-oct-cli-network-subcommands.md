...
- RFC-0855p-b — Coordinator Lifecycle per [^sep-policy-rationale]
...
| --------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Operator (CLI caller) | OCTO_HOME path + capability caveats                                           | read (15 active + 2 deferred Operator role); write (`bind-envelope rebind-{prepare,commit,abort}`) | RFC-0011 §Roles and Authorities + RFC-0011-d §Roles and Authorities        |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --- | -------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bootstrap`                      | DEFERRED — substrate-additions prerequisite (`BootstrapOrchestrator::start_bootstrap` + `BootstrapConfig::{from_toml, save_toml}` missing; `BootstrapMode` enum + `SeedListAuthority` + `verify_authority` live in `crates/octo-network/src/mon/bootstrap.rs`) | no  | no  | no       | Bootstrap Authority | see G1 companion; migration: reroute via `octo network peers list` + `octo network trust-graph render` for equivalent visibility                                                                     |
...
| `slash list` / `slash show`      | BLOCKED pending G6 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                      | no  | no  | no       | Operator            | gated on substrate; use `slash stats` + `slash excluded` until `list`+`peer_reputation` land per [^slash-degraded-view]                                                                              |
...
| `coordinator admin`              | BLOCKED pending G12 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                     | no  | no  | no       | Coordinator         | gated on substrate; no operator workaround today; companion mission `0011-h-s-a-coordinator-admin-trait`[^coordinator-admin-substrate] delivers the substrate surface                                |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------- | -------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **Read-only** (`peers`, `identity`, `mode show`, `authority show`, `slash stats`, `slash excluded`, `trust-graph`, `coordinator show`, `governance tally`, `governance rotation status`, `bind-envelope show`, `discovery advertisement show`, `discovery invitation show`) | n/a                     | n/a                       | no                               | no                                                    | no                                                                                         |
| **`bind-envelope rebind-prepare`** (BLOCKED G21)                                                                                                                                                                                                                            | write; reversible       | yes (always emit preview) | yes (must pass to lift dry-run)  | no                                                    | no (non-pastejacking)                                                                      |
| **`bind-envelope rebind-commit`** (BLOCKED G21)                                                                                                                                                                                                                             | write; **irreversible** | yes (always emit preview) | yes (must pass to lift dry-run)  | **YES** (must pass alongside `--confirm-acknowledge`) | **yes** (pastejacking defense — operator must type `--confirm` after reading preview)      |
...
| **`mode set`** (BLOCKED G1)                                                                                                                                                                                                                                                 | write; **irreversible** | yes (always emit preview) | yes (must pass to lift dry-run)  | **YES** (pastejacking defense)                        | **yes** (pastejacking defense — bootstrap lifecycle state change is high-blast-radius)     |
| **`authority rotate`** (BLOCKED G8)                                                                                                                                                                                                                                         | write; **irreversible** | yes (always emit preview) | yes (must pass to lift dry-run)  | **YES** (pastejacking defense)                        | **yes** (pastejacking defense — rotate_post_fork requires quorum proof; high-blast-radius) |
| **`coordinator admin`** (BLOCKED G12)                                                                                                                                                                                                                                       | write; **irreversible** | yes                       | yes                              | **YES**                                               | **yes** (pastejacking defense — coordinator lifecycle state changes are high-blast-radius) |
...
| `slash stats`                  | (read)                                                                                                                                          | `SlashReputationStoreCompat::{did_count, total_slashes, global_slash_count}`[^slash-stats-cite]. `per_did` accessor DEFERRED to G6 `0011-h-s-a-slash-store` companion mission per §Substrate-Additions                                                                                                                                                                                                     | RFC-0855p-b                                                                                                       | `0011-h-network-slash-stats` §slash-stats                                         | 2     | Operator            |
...
| G2   | (subsumed by G1)                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                                                              |
| G3b  | `0011-h-s-a-voting-tally-canonical-bytes`                       | Adds free fn `governance_proposal_canonical_bytes(&GovernanceProposal) -> [u8; 32]` in `octo-governance` Layer B facade[^g3b-helper-detail]                                                                                                                                                                                                                                                                                                                               | `octo network governance tally` (this amendment) — helper must close BEFORE CLI mission opens                                                  |
...
[^slash-stats-cite]: Aggregate reader surface. `did_count` + `total_slashes` + `global_slash_count` live on `SlashReputationStoreCompat`. Cross-platform witness aggregation lives in `crates/octo-network/src/mon/slash_aggregation.rs` per mission 0850p-c-cross-platform-witness — SEPARATE substrate concern per [[cipherocto-design-principles]] §Separation of concerns (out of CLI scope; substrate-internal aggregation only). Slash offense code allocation cross-references RFC-0855p-b §B. Slash Offense Codes (registry owner; aggregation pathway is the mission's, code registry is the RFC's).

[^rebind-abort-cite]: Reversible rebind-abort per RFC-0871 §Algorithms: substrate accept-rollback makes abort a no-op double-call safe; `--confirm-acknowledge` alone (no `--confirm` double-gate) per non-pastejacking-risk row. Per non-pastejacking-risk `rebind-prepare` row alignment.
...
[^sep-policy-rationale]: Coordinator Lifecycle substrate (`mon/slash_aggregation.rs`, `slash_aggregation.rs` cross-platform witness vote aggregation) is owned by mission 0850p-c-cross-platform-witness SEPARATE from RFC-0011-h per [[cipherocto-design-principles]] §Separation of concerns. Slash offense code allocation cross-references RFC-0855p-b §B. Slash Offense Codes; aggregation pathway lives in `mon/slash_aggregation.rs` and is NOT touched by this amendment.

[^slash-degraded-view]: Pre-landing operator workaround for BLOCKED `slash list`/`slash show`. Equivalent visibility: `slash stats` (aggregate view: distinct_did_count + total_slashes + global_slash_count) and `slash excluded <did>` (per-DID exclusion check). Aggregate reader surface lives on `SlashReputationStoreCompat` per [^slash-stats-cite]. Companion mission G6 (`0011-h-s-a-slash-store`) closes the gap.

[^governance-tally-signature]: `VotingTally::into_canonical(self, proposal_id, issuer, decision, state, voting_opens_at_millis, voting_closes_at_millis, total_eligible_weight) -> GovernanceProposal` per substrate signature at `crates/octo-network/src/mon/governance.rs`. Receiver is `self` (consuming; not `&mut self`); 7 separate args (NOT a single `GovernanceProposal`); return is `GovernanceProposal` (NOT `Vec<u8>`). CLI MUST call before any subsequent field access would be a use-after-move substrate violation. The 7 args are sourced from substrate `GovernanceProposal` state. `canonical_bytes_hash` field BLOCKED per §Substrate-Additions G3b until the canonical-bytes hashing helper `governance_proposal_canonical_bytes(&GovernanceProposal) -> [u8; 32]` lands in substrate.

[^g3b-helper-detail]: G3b helper-detail. `GovernanceProposal` itself is Layer A frozen (`crates/octo-governance-core/src/proposal.rs`); the helper is a free fn that does NOT mutate the type, preserving the Layer A frozen crypto contract via the underlying `blake3_hash` reference. `mon/governance.rs` Layer B imports the type and provides `VotingTally::into_canonical` that converts tally state to a `GovernanceProposal`. Verified absent.

[^coordinator-admin-substrate]: Companion mission `0011-h-s-a-coordinator-admin-trait` (G12 row in §Substrate-Additions) delivers coordinator admin substrate surface (rotate, suspend, reactivate) + `CoordinatorRecord::load(coordinator_id)`. CLI `octo network coordinator admin` subcommand dispatches through this substrate once the companion lands. Governing RFC for these admin actions lands in a follow-up amendment per BLUEPRINT ordering (substrate-first).
...
[^ci-deny-default-list]: The 5 CI-DENY-default write subcommands: `bind-envelope rebind-{prepare,commit,abort}` + `mode set` + `authority rotate`. CI agents cannot author ANY of these state changes (forward OR rollback). See §CI Mode Rationale for the full per-subcommand rationale table.
...
| ID  | Title                                            | Scope                                                                                                                                                                                |
| --- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| F1  | Per-extension crates for network adapters        | Layer D adapter extensions; CLI MAY gain `--adapter <name>` flags per [[cipherocto-design-principles]] §User extensibility                                                           |
| F2  | Network streaming tail mode                      | `octo network envelope inspect --follow` per RFC-0011-a streaming pattern                                                                                                            |
| F3  | Marketplace / pricing integration                | Quota router marketplace pricing (RFC-0900 series)                                                                                                                                   |
| F4  | CLI automation hooks                             | RFC-0917 Python SDK + HTTP proxy as programmatic surface; CLI is operator escape hatch                                                                                               |
| F5  | Multi-network topology                           | Operator selects single network identity per invocation (single active network per `$OCTO_HOME/octotransport/bootstrap.toml` `network_id` field); multi-network aggregation deferred |
| F6  | Network simulation / replay                      | Substrate-side replay surface for testing                                                                                                                                            |
| F7  | Federation across CipherOcto instances           | Per RFC-0863 + RFC-0871                                                                                                                                                              |
| F8  | Per-RFC `§Network CLI Surface` amendments        | RFC-0851, RFC-0851p-b, RFC-0855, RFC-0855p-c, RFC-0855p-b, RFC-0861, RFC-0862, RFC-0871, RFC-0008 each gain `§Network CLI Surface` section; each its own DRY cycle                   |
| F9  | Draft→Accepted promotions for 10 networking RFCs | RFC-0850p-d, RFC-0850p-e, RFC-0850p-f, RFC-0852, RFC-0854, RFC-0856, RFC-0857, RFC-0858, RFC-0859, RFC-0860 — each requires its own DRY cycle before promotion                       |
...
- RFC-0862p-a — Writer Election Bootstrap (v1.4.0 patch)
