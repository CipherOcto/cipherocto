...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --- | --- | -------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `bootstrap`                      | DEFERRED — substrate-additions prerequisite (BootstrapOrchestrator missing)                                                            | no  | no  | no       | Bootstrap Authority | see G1 companion; migration: reroute via `octo network peers list` + `octo network trust-graph render` for equivalent visibility                                                                                                     |
...
| `mode show`                      | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::from_toml` missing in `mon/bootstrap.rs`)                   | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; read-only once loader lands                                                                                                               |
| `mode set`                       | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::save_toml` missing)                                         | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until `save_toml` lands                                     |
| `authority show`                 | yes                                                                                                                                    | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                                                                            |
| `authority rotate`               | BLOCKED (pending G8 `0011-h-s-a-seed-list-authority-rotate`; `rotate_post_fork` missing in substrate)                                  | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until rotation primitive lands                              |
...
| `slash list` / `slash show`      | BLOCKED (pending G6 `0011-h-s-a-slash-store`; `SlashReputationStoreCompat::list` + `peer_reputation(did)` missing in substrate)        | no  | no  | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; operators use `slash stats` for aggregate view + `slash excluded <did>` for per-DID exclusion check until `list`+`peer_reputation` land   |
| `trust-graph render`             | yes                                                                                                                                    | yes | yes | yes      | Operator            | read-only                                                                                                                                                                                                                            |
| `coordinator show`               | BLOCKED (pending G12b `0011-h-s-a-coordinator-record-loader`; `CoordinatorRecord::load` static method missing)                         | yes | yes | redacted | Coordinator         | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; coordinator DID redacted in Audit; once loader lands                                                                                      |
| `coordinator admin`              | BLOCKED (pending G12 `0011-h-s-a-coordinator-admin-trait`; coordinator admin substrate missing)                                        | no  | no  | no       | Coordinator         | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; companion mission `0011-h-s-a-coordinator-admin-trait` delivers the substrate surface                                                     |
| `governance tally`               | BLOCKED (pending G3b; per §Substrate-Additions)                                                                                        | yes | yes | yes      | Governance Voter    | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; read-only once helper lands                                                                                                               |
...
| `bind-envelope rebind-prepare`   | BLOCKED (pending G21; per §Substrate-Additions)                                                                                        | no  | yes | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; `--dry-run` default + `--confirm-acknowledge` required; reversible; payload-builder only                                                  |
| `bind-envelope rebind-commit`    | BLOCKED (pending G21; per §Substrate-Additions)                                                                                        | no  | yes | no       | Operator            | clap arm registered, gated on substrate; pre-landing → exit 89 per §Error Handling row 89; `--dry-run` default + `--confirm-acknowledge` + `--confirm` SECOND flag (pastejacking defense per §Subcommand Taxonomy row); irreversible |
...
| `bind-envelope show`           | (read; BLOCKED — pending G22 `0011-h-s-a-bind-envelope-lookup` closure; `BindEnvelope::load(domain_id)` lookup method missing in substrate )                                | Substrate lookup surface absent; companion G22 closes this gate. Pre-landing → exit 89 per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | RFC-0871 §Data Structures                                                                                         | `0011-h-network-bind-envelope` §bind-envelope-show   | 4     | Operator            |
| `bind-envelope rebind-prepare` | (write; --dry-run default; --confirm-acknowledge required)[^g21-blocked]                                                                                                    | `RebindEnvelope::Prepare(RebindPrepare)` builder --confirm double-gate NOT required (prepare only reserves resources and is fully reversible via substrate accept-rollback per RFC-0871 §Algorithms); --confirm-acknowledge per non-pastejacking-risk row (see caption footer). Pre-landing → exit 89 per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | RFC-0871                                                                                                          | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     | Operator            |
| `bind-envelope rebind-commit`  | (write; --dry-run default; --confirm-acknowledge; --confirm SECOND flag required (pastejacking per §Security Considerations))[^g21-blocked]                                 | `RebindEnvelope::Commit(RebindCommit)` builder. Pre-landing → exit 89 per §Error Handling row 89                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | RFC-0871                                                                                                          | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     | Operator            |
...
The 3 CI-DENY subcommands reuse the existing CLI confirm-gate pattern. No new substrate required; pure CLI gate per RFC-0011 §Confirmation Flag Matrix.
...
- [ ] Drift-closure mission `0011-h-0851p-a-seed-health-check-archive` — archive `0851p-a-seed-health-check`
...
6. **R1 dry-review lesson encoded.** Original R1 draft produced ~30 CRIT substrate-faithfulness findings. Per [[substrate-faithfulness-verification]], substrate must be verified by grep BEFORE RFC is drafted, not after. Current rewrite grounded in actual `octo-network` cargo grep output (verified).
...
Substrate-faithful amendments that add `NetworkAction` variants MUST re-verify substrate citations via cargo grep before drafting, per [[substrate-faithfulness-verification]]. Closure artifacts for this RFC (including the R1 dry-review + substrate-faithfulness audit log) live in §Implementation Phases Phase 6 + `docs/audits/network-cli-gap-recon.md`.
