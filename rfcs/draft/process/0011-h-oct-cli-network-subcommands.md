...
| G6  | `0011-h-s-a-slash-store`                                        | Extend existing Layer B façade `SlashReputationStoreCompat` in `reputation/slash_store.rs` (wraps `quota_router_storage::slash_store` Layer D); add `list(reason_filter: Option<u16>) → Vec<SlashEnvelopeSummary>` + `show(slash_id: [u8;32]) → Option<SlashEnvelope>`. No new parallel façade: extends `mon/slash_aggregation.rs` reuse, not new CRUD surface, per [[cipherocto-design-principles]] §No parallel abstractions + §Separation of concerns | `octo network slash list/show` (deferred)                             |
