...
| `trust-graph render`           | (read; --depth 1-100; --format ascii or dot)                                    | `TrustGraph::render(GraphFormat::{Ascii, Dot})`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | RFC-0851p-a §F.4          | `0011-h-network-trust-graph`                         | 1     |
...
| `bind-envelope show`           | (read)                                                                          | `BindEnvelope::{canonical_bytes, is_participant}`; CLI computes blake3 hash via `blake3::hash(&envelope.canonical_bytes())` (external `blake3` crate — Layer A frozen primitive, no in-house wrapper) and surfaces hex only; raw `Vec<u8>` payload bytes NEVER echoed (no-payload-bytes-leak invariant per §Security Considerations)                                                                                                                                                                                                                               | RFC-0871 §Data Structures | `0011-h-network-bind-envelope` §bind-envelope-show   | 4     |
...
| `discovery advertisement show` | (read)                                                                          | `MissionAdvertisement::{advertisement_hash, is_encrypted, is_ttl_exceeded(current_hops: u16)}` — `current_hops` is a METHOD ARGUMENT only (the struct has NO `current_hops` field per substrate `crates/octo-network/src/mon/discovery.rs:73`). `--hops` accepts u16 range 0-65535 (clap rejects >65535 with exit 2 `ClapParse` per `is_ttl_exceeded` method signature); default `--hops 0`; `--hops` arg is the sole input to `is_ttl_exceeded` evaluation                                                                                                        | RFC-0851 §Discovery       | `0011-h-network-discovery` §discovery-advertisement  | 5     |
...
| G6b  | `0011-h-s-a-slash-store-loader`                                 | Substrate-additions companion for `slash stats` + `slash excluded` (substrate-faithful rows in §Subcommand Taxonomy): add `query_attestations → SlashReputationStoreCompat` loader wiring so CLI can hydrate the in-memory `RwLock<HashMap<RecorderDid, u32>>` from persisted `ReputationStore::query_attestations` results without reaching private Layer B items. `SlashReputationStoreCompat::{default, record_slash}` already public on the façade; this mission adds the persistence→façade load path described in the substrate `slash_store.rs` §module docstring as S3+ future work. Falls under `octo_network::reputation::*` public façade; CLI never reaches private items.                                                                                                                           | `octo network slash stats` + `slash excluded` (this amendment) — loader must close BEFORE CLI mission opens |
...
| ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.1     | 2026-09-18 | Initial Draft. 17 subcommands, 5 families. 10 error slots 79-88. 18 companion missions.                                                                                                                                                                                                                                                                   |
