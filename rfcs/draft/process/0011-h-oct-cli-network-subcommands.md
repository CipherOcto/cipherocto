...
Narrative threat model. Operator-facing decision matrix in §Adversarial Review below. Canonical cite for pastejacking defense: §Security Considerations pastejacking-defense bullet (L507). Canonical cite for canonical_bytes_hash no-payload-bytes-leak invariant: §Security Considerations envelope-payload invariant bullet (L506). Rebind nonce/timestamp gate + depth clamp + key-compromise mitigation: §Security Considerations witness-integrity bullet (L510). Adversarial Review table references these bullets rather than re-narrating.
...
- **Redaction placeholder form.** RFC-0011 §Redaction layer `[REDACTED:<kind>]` placeholders cover `[REDACTED:did]` (L337, L522, L576), `[REDACTED:coordinator_id]` (L521, L577), `[REDACTED:gateway_id]` (L432, L578). `file_path` + `domain_id` are scrubber-tracked per RFC-0011 §Rehydration boundary — they surface scrubbed by default, never with `[REDACTED:<kind>]` form. Audit mode applies same placeholders.
...
| --------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rebind-prepare`      | DENY default | CI gate fires regardless of `--confirm-acknowledge` presence; the only CI bypass is `--allow-ci-rebind` (DEBUG-ONLY escape hatch per §Security Considerations); exit 90 per §Error Handling.   |
| `rebind-commit`       | DENY default | Irreversible state mutation; CI cannot satisfy double-confirm gate (`--confirm-acknowledge` + `--confirm`). exit 90 per §Error Handling.                                                       |
...
| `peers/identity/etc.` | ALLOW in CI  | All read paths; per §Execution Class Mapping (read-side determinism).                                                                                                                          |
