...
> **Amendment chain:** Subordinate amendment to RFC-0011. Closes the gap DEFERRED from 0011-deprecation-stub-removal v2.0 cut (2026-09-17): `octo network bootstrap` + `octo network status` deferred to a future amendment — this is that amendment, scoped to substrate-faithful slice of the full network CLI gap. Substrate-faithful scope bounded by what `octo-network` actually exposes today; substrate-missing slices become explicit companion missions (§Substrate-Additions Companion Missions).
...
`NetworkAction` enum (15 variants, `#[non_exhaustive]` per F-14):
...
pub struct NetworkPeersListOutput {
    pub peers: Vec<GatewayCacheSummary>,     // gateway_id, gateway_class, inserted_epoch
...
pub struct NetworkSlashStatsOutput {
    ...
    pub per_did: Vec<DidSlashCount>,          // (redacted_did, count) — DID redacted in Audit
}
...
**Cross-amendment collision note:** Sibling amendments (`RFC-0011-c` agent, `RFC-0011-d` role, `RFC-0011-e` vault, `RFC-0011-f` mesh, `RFC-0011-g` governance) consume slots from the same parent band. At RFC promotion time, this amendment MUST verify against sibling amendments' §Exit Code tables that slots 79-89 are free. If collision found, shift to next available block in the 79-99 band.
...
| 65    | `StaleStub`              | RFC-0011 (post-v2.0 cut; retained for library-API soft sentinel) |
...
| `octo-coordinator-types` runtime is loaded      | `coordinator show`                              | Coordinator lifecycle state unavailable                  | Substrate lazy-loads via `octo_runtime::coordinator_load()`; if not loaded → exit 84                           |
| `reputation` crate is loaded                    | `slash stats`, `slash excluded`                 | Stats unavailable                                        | Substrate lazy-loads via `octo_reputation::slash_store_load()`; if not loaded → exit 89 (reserved)             |
...
**Cross-binary:** Operators using Python SDK / HTTP proxy (when RFC-0917 Accepted) will get same substrate-faithful slice via REST/gRPC. CLI is operator escape hatch. Full schema parity is OUT OF SCOPE for this RFC; cross-binary parity owned by RFC-0917.
...
| `tv-network-bind-envelope-show-1`   | `bind-envelope show`           | 3-participant envelope; `body_redacted = true`                                            |
...
**Coverage:** 5 GAPs (G3, G4, G5, G15-partial, G18-partial) are substrate-faithful TODAY via `peers`, `identity`, `trust-graph`; 5 GAPs are local config reads (G7, G8-show, G15-stats, G6-stats, G18-tally) covered by `mode show`, `authority show`, `slash stats`, `governance tally`; 10 GAPs require substrate-additions.
...
RFC-0011-h §Subcommand Taxonomy (mode show)

## Summary

CLI binding for `BootstrapMode` substrate enum read. `mode set` is DEFERRED per §Substrate-Additions Companion Missions until `0011-h-s-a-bootstrap-orchestrator` mission closes.

## Acceptance Criteria

- [ ] `octo network mode show` reads `$OCTO_HOME/octotransport/bootstrap.toml` + parses to `BootstrapMode`
- [ ] Returns `NetworkModeShowOutput` with `mode` + `source_path`
- [ ] `OctoCliError::NetworkConfigParseFailed` variant ADDED with slot 82 + Display impl + test
- [ ] Test vectors TV-NETWORK-MODE-SHOW-1/2 in `crates/octo-cli/tests/network.rs`
- [ ] Layer direction verified (no CLI → substrate reverse deps)
- [ ] Cargo clippy --workspace --all-targets -- -D warnings clean
- [ ] Cargo test -p octo-cli --lib green
- [ ] Prettier-clean mission YAML

### Type Coverage

| RFC-0011-h Type                          | Implemented By |
| ---------------------------------------- | -------------- |
| `Commands::Network`                      | This mission   |
| `NetworkAction::Mode`                    | This mission   |
| `NetworkModeShowOutput`                  | This mission   |
| `NetworkModeSubAction::Show`             | This mission   |
| `OctoCliError::NetworkConfigParseFailed` | This mission   |
