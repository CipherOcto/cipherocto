...
| `discovery advertisement show` | (read; --hops u16 0-65535 default 0)                                                                                                                                                                                   | `MissionAdvertisement::{advertisement_hash, is_encrypted, is_ttl_exceeded(current_hops: u16)}` — `current_hops` is a METHOD ARGUMENT only (the struct has NO `current_hops` field per substrate `MissionAdvertisement` definition). `--hops` accepts u16 range 0-65535 (clap rejects >65535 with exit 2 (`ValueValidation`; clap rejects) per `is_ttl_exceeded` method signature); default `--hops 0`; `--hops` arg is the sole input to `is_ttl_exceeded` evaluation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-advertisement  | 5     | Operator            |
...
pub struct NetworkRebindCommitOutput {
    ...
    pub prepared_evidence_hex: String,        // hex-encoded substrate `RebindCommit::prepared_evidence` (substrate field is `Vec<u8>` populated by `RebindCoordinator::commit_envelope` at `crates/octo-network/src/mon/rebind.rs:170-190`; current producer writes length-prefixed u32-LE sorted platform name strings with NO hash applied and signatures deferred to a follow-up mission per substrate comment `rebind.rs:174-176` — variable-length, not a 32-byte digest; field-doc contract at `bind_envelope.rs:162-165` documents sha256(sorted pairs) but current producer does not implement that contract)
    pub dry_run: bool,                        // true when --dry-run (no substrate mutation)
}
...
| ------ | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0      | `Ok`                          | Success                                                                                                                                                                                                                    |
| 2      | `ValueValidation`             | clap default (closed-enum / numeric-range rejection; covers `--hops u16` discovery-advertisement-show row + `--depth clamp` trust-graph-render row + `abort-2 BogusValue` rebind-abort BogusValue row in §Test Vectors)    |
...
| 86     | `NetworkInvalidDid`           | DID codec rejection per RFC-0010 (DID redacted in variant name)                                                                                                                                                            |
