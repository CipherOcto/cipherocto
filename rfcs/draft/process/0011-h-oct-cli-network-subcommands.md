...
| Governance Voter      | `VotingTally` (substrate-canonical (the VotingTally struct definition is in `crates/octo-network/src/mon/governance.rs`)`) | gate `governance tally` read                                         | substrate-canonical (no governing RFC); spec surface deferred to substrate |
...
| `governance tally` reads stale tally                                                        | LOW — substrate owns freshness                         | Tally substrate owned by `VotingTally` (substrate-canonical (the VotingTally struct definition is in `crates/octo-network/src/mon/governance.rs`)`); CLI surfaces current state only                        |
