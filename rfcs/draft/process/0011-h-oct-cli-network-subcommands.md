...
| 80   | <sup>[†]</sup> (RESERVED)     | No substrate failure path; see footnote [†]                                              |
| 81   | <sup>[†]</sup> (RESERVED)     | No substrate failure path; see footnote [†]                                              |
...
Narrative threat model. Operator-facing decision matrix in §Adversarial Review below. Canonical cite for canonical_bytes_hash no-payload-bytes-leak invariant: §Security Considerations "Envelope payload bytes never echoed" bullet. Canonical cite for pastejacking defense: §Security Considerations "Mutating subcommands gated + pastejacking defense." bullet. Rebind nonce/timestamp gate + depth clamp + key-compromise mitigation: §Security Considerations "Witness integrity + freshness + input gates." bullet. §Adversarial Review table references these bullets rather than re-narrating.
...
> **† Exit code for all 3 DENY-default rows above:** exit 90 (`NetworkCIRebindDenied`) per §Error Handling row 90. The CLI gate fires BEFORE substrate dispatch regardless of `--confirm-acknowledge` presence.
...
- [ ] Mission `0011-h-network-peers-identity` (peers + identity)
  ...
  - [ ] Test vectors tv-network-peers-list-1/2 + tv-network-peers-get-1/2 (incl. tv-network-did-1) + tv-network-identity-show-1/2
