...
| `mode show`                      | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::from_toml` missing in `mon/bootstrap.rs`)                                                                                                                                                       | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; read-only once loader lands                                                                                                               |
| `mode set`                       | BLOCKED (pending G1 `0011-h-s-a-bootstrap-orchestrator`; `BootstrapConfig::save_toml` missing)                                                                                                                                                                             | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until `save_toml` lands                                     |
| `authority show`                 | yes                                                                                                                                                                                                                                                                        | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                                           |
| `authority rotate`               | BLOCKED (pending G8 `0011-h-s-a-seed-list-authority-rotate`; `rotate_post_fork` missing in substrate)                                                                                                                                                                      | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; operators manually edit `$OCTO_HOME/octotransport/bootstrap.toml` and restart until rotation primitive lands                              |
...
| `slash list` / `slash show`      | BLOCKED (pending G6 `0011-h-s-a-slash-store`; `SlashReputationStoreCompat::list` + `peer_reputation(did)` missing in substrate)                                                                                                                                            | no  | no  | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; operators use `slash stats` for aggregate view + `slash excluded <did>` for per-DID exclusion check until `list`+`peer_reputation` land   |
| `trust-graph render`             | yes                                                                                                                                                                                                                                                                        | yes | yes | yes      | Operator            | read-only                                                                                                                                                                                           |
| `coordinator show`               | BLOCKED (pending G12b `0011-h-s-a-coordinator-record-loader`; `CoordinatorRecord::load` static method missing)                                                                                                                                                             | yes | yes | redacted | Coordinator         | clap arm registered, gated on substrate[^clap-arm-gated]; coordinator DID redacted in Audit; once loader lands                                                                                      |
| `coordinator admin`              | BLOCKED (pending G12 `0011-h-s-a-coordinator-admin-trait`; coordinator admin substrate missing)                                                                                                                                                                            | no  | no  | no       | Coordinator         | clap arm registered, gated on substrate[^clap-arm-gated]; companion mission `0011-h-s-a-coordinator-admin-trait` delivers the substrate surface                                                     |
| `governance tally`               | BLOCKED (pending G3b; per §Substrate-Additions)                                                                                                                                                                                                                            | yes | yes | yes      | Governance Voter    | clap arm registered, gated on substrate[^clap-arm-gated]; read-only once helper lands                                                                                                               |
...
| `bind-envelope rebind-prepare`   | BLOCKED (pending G21; per §Substrate-Additions)                                                                                                                                                                                                                            | no  | yes | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; `--dry-run` default + `--confirm-acknowledge` required; reversible; payload-builder only                                                  |
| `bind-envelope rebind-commit`    | BLOCKED (pending G21; per §Substrate-Additions)                                                                                                                                                                                                                            | no  | yes | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; `--dry-run` default + `--confirm-acknowledge` + `--confirm` SECOND flag (pastejacking defense per §Subcommand Taxonomy row); irreversible |
