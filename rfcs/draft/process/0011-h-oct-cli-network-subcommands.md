...
| `slash stats`                  | (read)                                                                                                                     | `SlashReputationStoreCompat::did_count()` (count of distinct `RecorderDid` entries) + `total_slashes()` (sum across all DIDs) + `global_slash_count(did: &RecorderDid) -> u32` (per-DID lookup); CLI aggregates `distinct_did_count` + `total_slashes` over the in-memory `HashMap<RecorderDid, u32>` iteration since substrate exposes individual methods, not a struct aggregate                                                                                                                                                                                 | RFC-0855p-b               | `0011-h-network-slash-stats` §slash-stats            | 2     |
...
pub struct NetworkGovernanceTallyOutput {
    ...
    pub canonical_bytes_hash: String,         // blake3 hex of `GovernanceProposal::canonical_bytes() -> [u8; 32]` (BLOCKED — pending G3b `0011-h-s-a-voting-tally-canonical-bytes` closure per §Substrate-Additions; absent in substrate TODAY, verified 2026-09-18 via cargo grep returning empty)
}
...
> Note: All `OctoCliError` variants listed are **RFC-proposed Draft-stage additive** under `#[non_exhaustive]` per RFC-0011 §Error Envelope; landing requires paired substrate-additions companion mission closure (§Substrate-Additions) plus the 5-len DRY CLOSURE gate per [[cipherocto-design-principles]] §Extension over enumeration. Slot allocation is preliminary pending final acceptance. |
...
| 86   | `NetworkInvalidDid`                      | —                  | —                 | —                  | —                 | —                       |
| 87   | `NetworkConfirmRequired`                 | —                  | —                 | —                  | —                 | —                       |
| 88   | `NetworkDryRunDenied`                    | —                  | —                 | —                  | —                 | —                       |
| 89   | `NetworkSubstrateUnavailable` (RESERVED) | —                  | —                 | —                  | —                 | —                       |

Sibling amendment slot consumption: RFC-0011-c uses slots 41-44 (Agent), 49 (AttachFailed), 51 (SubstrateNotReady), 53-59 (AttachHandle substrate, 7 slots); RFC-0011-d uses slots 31 (RoleNotFound), 32 (StakeInsufficient), 33 (RoleNotSelectable), 35 (SignerMismatch); RFC-0011-e uses slots 23 (VaultNotOwned), 24 (InsufficientBalance), 25 (RoleNotProvisioned), 26 (ChainIdMismatch / InvalidChainId); RFC-0011-f uses slots 27 (MeshIdentityUnknown), 28 (InvalidTtlHops / InvalidEndpointScheme shared), 29 (MeshCapabilityInsufficient), 30 (EnvelopeAuthorizationFailed / RpcTimeout shared), 31 (MeshMethodUnknown) with 32-34 reserved per RFC-0011-f §Exit Codes; RFC-0011-g uses slots 35 (SignerMismatch — note slot 35 shared with RFC-0011-d, resolved by exit-code precedence in §Status), 2 (ClapParse), 51 (GovernanceSubstrateError — note slot 51 shared with RFC-0011-c SubstrateNotReady). All sibling slots fall below slot 79. Slots 79-89 free at Draft time per sibling amendment §Exit Code tables. No collision. Slot 89 reserved as `NetworkSubstrateUnavailable` for follow-on amendments (substrate-additions companion missions may need additional slots).
...
- **Envelope payload bytes never echoed.** Per RFC-0011 §Redaction layer + RFC-0871 §Security Considerations. `bind-envelope show` returns metadata + `canonical_bytes_hash` (blake3 hex) only; raw bytes from `BindEnvelope::canonical_bytes()` NEVER surface in `OutputEnvelope` or logs — only the blake3 hex digest surfaces. Test vectors assert no leak. CLI computes hash via `octo_cap_macaroon::blake3_hash(&envelope.canonical_bytes())` (substrate wrapper per `crates/octo-cli/Cargo.toml:134-139` R17 Lens-4 F1 parallel-abstraction ban — no direct `blake3` crate dep) per §Subcommand Taxonomy.
...
- **`bind-envelope rebind-prepare` / `rebind-commit` / `rebind-abort` chain integrity.** Three-part chain (prepare → commit | abort) is governed by substrate nonce + monotonic timestamp gate per RFC-0871 §Data Structures. CLI builders emit substrate-derived freshness — operators CANNOT inject arbitrary timestamp/nonce values. Double-commit is substrate-rejected via idempotency check. Signing-identity key compromise is mitigated by HSM wallet requirement per RFC-0015-a §6.5 paired-acceptance bridge + `0011-h-s-a-attached-handle-key-rotation` companion substrate enabling key rotation invalidating stale signatures.
...
| `tv-network-bind-envelope-prep-3`   | `bind-envelope rebind-prepare` | Happy-path: `--confirm-acknowledge` + `--no-dry-run` + valid signer → success + `RebindPrepare` canonical bytes + signature surfaced |
...
| G2   | (subsumed by G1)                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —                                                                                                                                                                                                              |
| G3b  | `0011-h-s-a-voting-tally-canonical-bytes`                       | Substrate-additions companion for `governance tally` (substrate-faithful row in §Subcommand Taxonomy): add `GovernanceProposal::canonical_bytes() -> [u8; 32]` (blake3 hash surface) to `octo-governance` Layer B facade (re-exporting core types from `octo-governance-core` Layer A frozen). Current substrate has NO `GovernanceProposal::canonical_bytes` method — verified via cargo grep on `GovernanceProposal::canonical_bytes` returning empty (2026-09-18). `canonical_bytes_hash` field in `NetworkGovernanceTallyOutput` requires this substrate helper. Without it, `governance tally` cannot surface the canonical-bytes hash substrate-faithfully.                                                                                                                                                                                                                                                                                                                                                                                                          | `octo network governance tally` (this amendment) — helper must close BEFORE CLI mission opens                                                                                                                  |
...
| G6b  | `0011-h-s-a-slash-store-loader`                                 | Substrate-additions companion for `slash stats` + `slash excluded` (substrate-faithful rows in §Subcommand Taxonomy): add `query_attestations → SlashReputationStoreCompat` loader wiring so CLI can hydrate the in-memory `RwLock<HashMap<RecorderDid, u32>>` from persisted `ReputationStore::query_attestations` results without reaching private Layer B items. Read methods (`is_excluded(did)`, `did_count()`, `total_slashes()`, `global_slash_count(did)`) are substrate-faithful TODAY and do NOT depend on this loader — those substrate items are wired directly via the public façade (see §Subcommand Taxonomy L212-L213). G6b adds ONLY the persistence→façade hydration path described in the substrate `slash_store.rs` §module docstring as S3+ future work (surviving process restart). Without G6b, in-memory state survives only the current process lifetime; with G6b, attestation-derived counts can be re-loaded from persisted queries across restarts. Falls under `octo_network::reputation::*` public façade; CLI never reaches private items. | `octo network slash stats` + `slash excluded` (this amendment) — loader must close BEFORE CLI mission opens (only for cross-restart persistence; same-process substrate-faithful read methods work without it) |
...
- [ ] Mission `0011-h-network-bind-envelope`
  ...
  - [ ] Test vectors tv-network-bind-envelope-show-1/2 + tv-network-bind-envelope-prep-1/2 + tv-network-bind-envelope-prep-3 + tv-network-bind-envelope-commit-1/2 + tv-network-bind-envelope-abort-1/2
...
| ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.1     | 2026-09-18 | Initial Draft. 17 subcommands, 5 families. 10 error slots 79-88. 19 companion missions.                                                                                                                                                                                                                                                                         |
| 0.2     | 2026-09-18 | R5.5 fix sweep: fabricated fn refs → companion missions; wire-format notes; Pattern C fenced; VH rows compress.                                                                                                                                                                                                                                                 |
| 0.3     | 2026-09-18 | R6.5 fix sweep: `DomainCoordinatorRecord` (fictional) → canonical `CoordinatorRecord`; slot allocations table corrected; `--hops` exit-86 dropped (clap rejects OOR); `NetworkSubstrateUnavailable` slot 89 reserved; Exit Codes expanded; Performance Targets + 2 rows; Adversarial Review + 3 rebind-* threats; `slash mark` mutating count reconciled.       |
| 0.4     | 2026-09-18 | R9.5 fix sweep: `blake3` cite → `octo_cap_macaroon::blake3_hash` substrate wrapper; `GovernanceProposal::canonical_bytes` relocated Layer A→B; GAP count 20→21; companion missions 18→19; §Security Considerations +3 bullets; §Output Envelope rebind-commit/abort structs; §Adversarial Review nonce/timestamp forgery; Layer B→C `octo_mesh` cite corrected. |
