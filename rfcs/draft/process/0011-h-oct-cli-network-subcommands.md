...
| `governance tally`             | (read; BLOCKED — pending G3b `0011-h-s-a-voting-tally-canonical-bytes` closure)                                                                                                                                                                                                 | `VotingTally::{total_for, total_against, into_canonical}`. CLI MUST read total_for/total_against BEFORE moving self into into_canonical (`into_canonical(&mut self, proposal: GovernanceProposal) -> GovernanceProposal` per substrate signature at `crates/octo-network/src/mon/governance.rs` — reading fields after the call would be a use-after-move substrate violation; the return type is `GovernanceProposal`, NOT `Vec<u8>` or any byte slice). The `GovernanceProposal` requires 7-arg proposal context (proposal_id, issuer, decision, state, voting_opens_at_millis, voting_closes_at_millis, total_eligible_weight); CLI call site must supply context from substrate. `canonical_bytes_hash` field BLOCKED per §Substrate-Additions G3b: `into_canonical` returns `GovernanceProposal` but the `canonical_bytes_hash` field requires the G3b helper `governance_proposal_canonical_bytes(&GovernanceProposal) -> [u8; 32]` (gap not yet landed in substrate; CLI surfaces `[REDACTED:canonical_bytes_hash]` until G3b lands) | substrate-canonical (VotingTally at `the VotingTally struct definition in `crates/octo-network/src/mon/governance.rs``; no governing RFC) | `0011-h-network-governance` §governance-tally                 | 3     | Governance Voter    |
...
- [ ] Mission `0011-h-network-bind-envelope`
  ...
  - [ ] 3 `OctoCliError` variants (`NetworkConfirmRequired`, `NetworkDryRunDenied`, `NetworkCIRebindDenied`); slots 80, 81, 89 RESERVED per substrate-faithfulness audit 2026-09-18
