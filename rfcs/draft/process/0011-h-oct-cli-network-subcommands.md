...
| `mode set`                       | BLOCKED pending G1 closure[^substrate-path]; pre-landing exit 89[^exit89]; CI mode exit 90 (NetworkCIRebindDenied)                                                                                                                                             | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; no operator workaround today; bootstrap lifecycle pauses until G1 lands                                                                    |
| `authority show`                 | yes                                                                                                                                                                                                                                                            | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                                            |
| `authority rotate`               | BLOCKED pending G8 closure[^substrate-path]; pre-landing exit 89[^exit89]; CI mode exit 90 (NetworkCIRebindDenied)                                                                                                                                             | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; no operator workaround today; bootstrap lifecycle pauses until G8 lands (rotate_post_fork requires quorum proof; no manual fallback)       |
| `slash excluded` / `slash stats` | yes                                                                                                                                                                                                                                                            | yes | yes | redacted | Operator            | DID redacted in Audit                                                                                                                                                                                |
| `slash mark`                     | out of scope per RFC-0008 §Execution Class Mapping                                                                                                                                                                                                             | no  | no  | no       | n/a                 | out of scope per RFC-0008 §Execution Class Mapping (slashing is Class B per RFC-0008; substrate-only mint path; CLI cannot mint `SlashEnvelope`s; clap arm NOT registered → exit 2 on invocation)    |
...
| `bind-envelope rebind-prepare`   | BLOCKED pending G21 closure[^substrate-path]; pre-landing exit 89[^exit89]; CI mode exit 90 (NetworkCIRebindDenied)                                                                                                                                            | no  | yes | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; `--dry-run` default + `--confirm-acknowledge` required; reversible; payload-builder only                                                   |
| `bind-envelope rebind-commit`    | BLOCKED pending G21 closure[^substrate-path]; pre-landing exit 89[^exit89]; CI mode exit 90 (NetworkCIRebindDenied)                                                                                                                                            | no  | yes | no       | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; `--dry-run` default + `--confirm-acknowledge` + `--confirm` SECOND flag (pastejacking defense per §Subcommand Taxonomy row); irreversible  |
...
| `discovery advertisement show`   | BLOCKED (pending G23 `0011-h-s-a-discovery-advertisement-cache`; cache type + lookup method both absent in substrate; companion adds both )                                                                                                                    | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                  |
| `discovery invitation show`      | BLOCKED (pending G24 `0011-h-s-a-discovery-invitation-cache`; cache type + lookup method both absent in substrate; companion adds both )                                                                                                                       | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                  |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------- | -------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **Read-only** (`peers`, `identity`, `mode show`, `authority show`, `slash stats`, `slash excluded`, `trust-graph`, `coordinator show`, `governance rotation status`, `bind-envelope show`, `discovery advertisement show`, `discovery invitation show`) | n/a                     | n/a                       | no                               | no                                                    | no                                                                                         |
...
| `discovery advertisement show` | (read; BLOCKED — pending G23 `0011-h-s-a-discovery-advertisement-cache` closure; cache type + lookup method both absent in substrate; companion adds both ) | Substrate lookup surface absent; companion G23 adds the full cache type. Pre-landing exit 89[^exit89]                                                                                                                                                                                                                                                                                                      | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-advertisement  | 5     | Operator            |
| `discovery invitation show`    | (read; BLOCKED — pending G24 `0011-h-s-a-discovery-invitation-cache` closure; cache type + lookup method both absent in substrate; companion adds both )    | Substrate lookup surface absent; companion G24 adds the full cache type. Pre-landing exit 89[^exit89]                                                                                                                                                                                                                                                                                                      | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-invitation     | 5     | Operator            |
...
| `slash mark`                   | (out of scope per RFC-0008 §Execution Class Mapping)                                                                                                        | Substrate-only mint path per RFC-0008 §Execution Class Mapping (slashing is Class B); CLI cannot mint `SlashEnvelope`s; clap arm NOT registered → exit 2 on invocation                                                                                                                                                                                                                                     | RFC-0008 §Execution Class Mapping                                                                                 | (out of scope per RFC-0008 §Execution Class Mapping) | n/a   | n/a                 |
...
pub struct NetworkRebindCommitOutput {
  ...
  pub prepared_evidence_hex: String,    // hex-encoded substrate `RebindCommit::prepared_evidence` (Vec<u8>; variable-length, NOT a 32-byte digest); see §Substrate-Additions G21 (RebindCoordinator::commit_envelope) for sha256 producer-contract gap
  pub dry_run: bool,            // true when --dry-run (no substrate mutation)
}
...
| 90     | `NetworkCIRebindDenied`       | CI mode detected on rebind-* write path OR `mode set` OR `authority rotate` write path (see §CI Mode Rationale for detection mechanism + the 5 CI-DENY-default rows that fire this exit)                                                             |
