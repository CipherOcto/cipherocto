...
| `status`                         | DEFERRED — substrate-additions prerequisite (BootstrapState accessor missing)                                     | no  | no  | no       | Bootstrap Authority | see G1 companion; migration: same fallback as `bootstrap`                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `mode show`                      | BLOCKED pending G1 closure[^substrate-path]; pre-landing exit 89[^exit89]                                         | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; read-only once loader lands                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `coordinator show`               | BLOCKED pending G12b closure[^substrate-path]; pre-landing exit 89[^exit89]                                       | yes | yes | redacted | Coordinator         | clap arm registered, gated on substrate[^clap-arm-gated]; coordinator DID redacted in Audit; once loader lands                                                                                                                                                                                                                                                                                                                                                                  |
...
| `governance tally`               | BLOCKED pending G3b closure[^substrate-path]; pre-landing exit 89[^exit89]                                        | yes | yes | yes      | Governance Voter    | clap arm registered, gated on substrate[^clap-arm-gated]; read-only once helper lands                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `bind-envelope show`             | BLOCKED (pending G22 `0011-h-s-a-bind-envelope-lookup`; `BindEnvelope::load` lookup method missing in substrate ) | yes | yes | redacted | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; payload bytes never surfaced                                                                                                                                                                                                                                                                                                          |
...
| `bind-envelope rebind-abort`     | yes (write; reversible)                                                                                           | no  | yes | no       | Operator            | `--dry-run` default + `--confirm-acknowledge` required; reversible; CI mode exit 90 (NetworkCIRebindDenied)                                                                                                                                                                                                                                                                                                                                                                     |
| `discovery advertisement show`   | BLOCKED (pending G23 `0011-h-s-a-discovery-advertisement-cache`)[^cache-missing-substrate]                        | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                                                                                                                                                                                                                                                                             |
| `discovery invitation show`      | BLOCKED (pending G24 `0011-h-s-a-discovery-invitation-cache`)[^cache-missing-substrate]                           | yes | yes | yes      | Operator            | clap arm registered, gated on substrate[^clap-arm-gated]; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                                                                                                                                                                                                                                                                             |
...
- **P2P gossip peers** — peer-to-peer propagation substrate exists (`mon/gossip.rs`), but peer-to-peer gossip NOT operator-facing. CLI exposes `gossip stats` DEFERRED until `Gossip::stats()` substrate lands. **Fallback:** use `octo network peers list` (gateway-cache surface per RFC-0851) for equivalent peer observability.
- **Routing substrate operators** — `routing send` (onion relay, RFC-0863) + `router status` (quota marketplace, RFC-0870) DEFERRED until `NetworkSender` + `QuotaRouterNode` substrate land. Both are substrate-facing, not operator-facing CLI surfaces. **Fallback:** use `octo mesh peer list` per RFC-0011-f (peer-table surface; pre-existing operator escape hatch) for equivalent peer-table visibility.
- **Election voting operators** — `election show` + `cast_vote` DEFERRED until `WriterElection` substrate lands (RFC-0862p-a substrate not yet implemented; G18 `0011-h-s-a-writer-election` companion mission). **Fallback:** use `octo governance snapshot --proposal-state <state> --json` per RFC-0011-g §Subcommand Taxonomy for proposal-level state visibility (the proposal-side substrate is implemented; the election-side substrate is not).
...
| `slash stats`                  | (read)                                                                                                                                                          | Aggregate reader surface: `SlashReputationStoreCompat::{did_count, total_slashes, global_slash_count}`. `per_did` accessor DEFERRED to G6 `0011-h-s-a-slash-store` companion mission per §Substrate-Additions. Cross-platform witness aggregation lives in `crates/octo-network/src/mon/slash_aggregation.rs` per mission 0850p-c-cross-platform-witness — SEPARATE substrate concern per [[cipherocto-design-principles]] §Separation of concerns (out of CLI scope; substrate-internal aggregation only). Slash offense code allocation cross-references RFC-0855p-b §B. Slash Offense Codes (registry owner; aggregation pathway is the mission's, code registry is the RFC's). | RFC-0855p-b                                                                                                       | `0011-h-network-slash-stats` §slash-stats                                         | 2     | Operator            |
...
| CI mode detected on any of the 5 CI-DENY-default write subcommands: `bind-envelope rebind-{prepare,commit,abort}` + `mode set` + `authority rotate`. CI mode gate is evaluated BEFORE the `--confirm-acknowledge` check, so in CI mode the operator always sees exit 90 regardless of `--confirm-acknowledge` presence AND regardless of subcommand reversibility. The variant name `NetworkCIRebindDenied` carries the rebind-* prefix as a historical lock (the first CI-DENY-default family added to the surface); the exit code covers all 5 CI-DENY-default subcommands per §CI Mode Rationale | 90                 | `CIDenyDefault`              | `NetworkCIRebindDenied` (NEW; covers all 5 CI-DENY-default rows) | n/a                     |
...
[^governance-tally-signature]: `VotingTally::into_canonical(self, proposal_id, issuer, decision, state, voting_opens_at_millis, voting_closes_at_millis, total_eligible_weight) -> GovernanceProposal` per substrate signature at `crates/octo-network/src/mon/governance.rs`. Receiver is `self` (consuming; not `&mut self`); 7 separate args (NOT a single `GovernanceProposal`); return is `GovernanceProposal` (NOT `Vec<u8>`). CLI MUST call before any subsequent field access would be a use-after-move substrate violation. The 7 args are sourced from substrate `GovernanceProposal` state via the `GovernanceProposalRegistry` accessor in the `octo-governance` Layer B facade (`crates/octo-governance/src/proposal_registry.rs`); the proposal ledger lookup + snapshot loader are the substrate-side sources for the 7-arg payload (proposal_id from `GovernanceProposal::proposal_id`, issuer from `GovernanceProposal::issuer_did`, decision from current `VoteDecision`, state from `GovernanceProposal::state`, voting_opens_at_millis/closes_at_millis/total_eligible_weight from `GovernanceProposal` public fields). `canonical_bytes_hash` field BLOCKED per §Substrate-Additions G3b until the canonical-bytes hashing helper `governance_proposal_canonical_bytes(&GovernanceProposal) -> [u8; 32]` lands in substrate.
...
- [ ] Drift-closure mission for `0851p-a-seed-health-check` (YAML to be created when this Phase 6 work enters its own DRY cycle; stub pointer only — phantom slug is intentional pending CLAIMED-time)
- [ ] File 2 follow-on companion missions (paths filed at CLAIMED-time)
...
| --- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| F1  | Per-extension crates for network adapters        | Layer D adapter extensions; CLI MAY gain `--adapter <name>` flags per [[cipherocto-design-principles]] §User extensibility                                                           | Planned |
| F2  | Network streaming tail mode                      | `octo network envelope inspect --follow` per RFC-0011-a streaming pattern                                                                                                            | Planned |
| F3  | Marketplace / pricing integration                | Quota router marketplace pricing (RFC-0900 series)                                                                                                                                   | Planned |
| F4  | CLI automation hooks                             | RFC-0917 Python SDK + HTTP proxy as programmatic surface; CLI is operator escape hatch                                                                                               | Planned |
| F5  | Multi-network topology                           | Operator selects single network identity per invocation (single active network per `$OCTO_HOME/octotransport/bootstrap.toml` `network_id` field); multi-network aggregation deferred | Planned |
| F6  | Network simulation / replay                      | Substrate-side replay surface for testing                                                                                                                                            | Planned |
| F7  | Federation across CipherOcto instances           | Per RFC-0863 + RFC-0871                                                                                                                                                              | Planned |
| F8  | Per-RFC `§Network CLI Surface` amendments        | RFC-0851, RFC-0851p-b, RFC-0855, RFC-0855p-c, RFC-0855p-b, RFC-0861, RFC-0862, RFC-0871, RFC-0008 each gain `§Network CLI Surface` section; each its own DRY cycle                   | Planned |
| F9  | Draft→Accepted promotions for 10 networking RFCs | RFC-0850p-d, RFC-0850p-e, RFC-0850p-f, RFC-0852, RFC-0854, RFC-0856, RFC-0857, RFC-0858, RFC-0859, RFC-0860 — each requires its own DRY cycle before promotion                       | Planned |
