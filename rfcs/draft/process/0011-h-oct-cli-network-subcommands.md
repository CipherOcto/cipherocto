...
| Governance Voter      | `VotingTally` (substrate-canonical at `the VotingTally struct definition in `crates/octo-network/src/mon/governance.rs``) | gate `governance tally` read                                         | substrate-canonical (no governing RFC); spec surface deferred to substrate |
...
| `bind-envelope rebind-commit`  | (write; --dry-run default; --confirm-acknowledge; --confirm SECOND flag for irreversible state mutation per ## Security Considerations                                                                         |

> **CI override flag (`--allow-ci-rebind`)** — DEBUG-ONLY escape hatch for CI mode rebind-* write paths. Hidden from `--help` output per RFC-0011 §Test Vectors (CI override is not a stable operator surface). Reserved for emergency operator override only; surfaces in changelog as experimental and removed before v1.0. Operators MUST NOT script against this flag; substrate-level double-confirm gate is the canonical pastejacking defense.

pastejacking-defense bullet; --no-dry-run to apply; irreversible; BLOCKED — pending G21 `0011-h-s-a-attached-handle-key-rotation` closure) | `RebindEnvelope::Commit(RebindCommit)` builder (umbrella enum at `crates/octo-network/src/mon/bind_envelope.rs`). Key-id surface BLOCKED per §Substrate-Additions G21 (substrate currently lacks `AttachedHandleKeyRotation` trait + `current_key_id()` method; D2.1 only landed primitives per RFC-0011-c §F.5.1) | RFC-0871 | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4 | Operator |
...
pub struct NetworkPeersListOutput {
    pub peers: Vec<GatewayCacheProjection>,  // CLI-side read-only DTO; substrate-faithful fields derived from GatewayCacheEntry per `crates/octo-network/src/gdp/cache.rs:11`
...
/// NOT a substrate surface; substrate has `GatewayCacheEntry` per `crates/octo-network/src/gdp/cache.rs:11`.
...
| `--confirm-acknowledge` missing on write (CLI gate; fires before substrate dispatch per RFC-0011 §Confirmation Flag Matrix)                                                                                                                                                                                                                                                                                                                                                       | 87                 | `NetworkConfirmRequired` (NEW)           | n/a                     |
...
- **Witness integrity + freshness + input gates.** Substrate nonce + monotonic timestamp gate per RFC-0871 §Data Structures governs rebind ordering (CLI builders emit substrate-derived freshness, operators CANNOT inject arbitrary values). Double-commit rejected via substrate idempotency per RFC-0871 §Algorithms (idempotency check on rebind coordinator `mark_committed`). Verify substrate confirms at `crates/octo-network/src/mon/rebind.rs` `RebindCoordinator::mark_committed` + `RebindCoordinator::abort` per audit 2026-09-18. Substrate does NOT expose an `accept` method; the canonical commit path is `mark_committed(&mut self)` and rollback path is `abort(&mut self, reason: RebindAbortReason)`. Tally freshness is substrate-owned via `VotingTally` (substrate-canonical at `the VotingTally struct definition in `crates/octo-network/src/mon/governance.rs``; CLI never caches across invocations). RFC-0011-g governs the CLI attestation + vote calls, not the underlying `VotingTally` type surface. `--depth` clamp 1-100 on `trust-graph render` (substrate OOM defense; `--depth 0` → exit 85; `--depth 1000000` → clamp 100 with warning).
...
| `governance tally` reads stale tally                                                        | LOW — substrate owns freshness                         | Tally substrate owned by `VotingTally` (substrate-canonical at `the VotingTally struct definition in `crates/octo-network/src/mon/governance.rs``); CLI surfaces current state only                         |
...
| `tv-network-dry-run-deny-1`                 | `bind-envelope rebind-abort`   | `--dry-run` explicit + operator denied at prompt → exit 88 (`NetworkDryRunDenied`)                                                                                                              | > **tv-network-bind-envelope-rebind-abort-1** (NEW) — `octo network bind-envelope rebind-abort --envelope-id <redacted> --reason <redacted>` in CI mode (`OCTO_CLI_CI=1` set) → exit 90 `NetworkCIRebindDenied`. Substrate `RebindCoordinator::abort` is NEVER reached because the CLI gate fires before substrate dispatch per §CI Mode Rationale. |
