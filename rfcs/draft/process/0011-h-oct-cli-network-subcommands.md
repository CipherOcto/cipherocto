...
| `discovery advertisement show` | (read)                                            | `MissionAdvertisement::{advertisement_hash, is_encrypted, is_ttl_exceeded(current_hops: u16)}`. `--hops` accepts u16 range 0-65535 (clap rejects >65535 with exit 2 `ClapParse` per substrate type); default `--hops 0`; when envelope header carries `current_hops` field, substrate surfaces that value in output regardless of `--hops`; `--hops` arg only affects manual override for `is_ttl_exceeded` evaluation                                                                                                                                             | RFC-0851 §Discovery       | `0011-h-network-discovery` §discovery-advertisement  | 5     |
