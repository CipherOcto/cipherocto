...
- RFC-0010 — Canonical DID Codec (peer DID wire form at every substrate boundary; RFC-0010 §2 ledger_chain_registry Table is the substrate-side chain-id authority)
...
| Peer DID codec is RFC-0010 canonical              | All peer-DID-sourcing subcommands               | Legacy DID form could bypass codec gate                  | `octo_ident::CanonicalCodec::parse(s, allow_legacy=false)` at every substrate boundary (RFC-0011-f G2 pattern; codec spec lives in RFC-0010 §Data Structures (canonical OctoID identifier codec; chain-id registration authority lives in RFC-0010 §2 ledger_chain_registry Table))                                                     |
...
| --------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rebind-prepare`      | DENY default | CI gate fires regardless of `--confirm-acknowledge` presence; the only CI bypass is `--allow-ci-rebind` (DEBUG-ONLY escape hatch per §Security Considerations). |
| `rebind-commit`       | DENY default | Irreversible state mutation; CI cannot satisfy double-confirm gate (`--confirm-acknowledge` + `--confirm`).                                                     |
| `rebind-abort`        | DENY default | Roll-back state operation; substrate MAY interpret double-abort as no-op per RFC-0871 §Algorithms idempotency, but CI pastejacking risk remains.                |
...
> **† Exit code for `bind-envelope rebind-prepare`, `bind-envelope rebind-commit`, `bind-envelope rebind-abort` CI-DENY-default rows:** exit 90 (`NetworkCIRebindDenied`) per §Error Handling row 90. The CLI gate fires BEFORE substrate dispatch regardless of `--confirm-acknowledge` presence.
...
[^slash-stats-cite]: Aggregate reader surface. `did_count` + `total_slashes` + `global_slash_count` live on `SlashReputationStoreCompat`. Cross-platform witness aggregation lives in `crates/octo-network/src/mon/slash_aggregation.rs` per RFC-0850p-c — SEPARATE substrate concern per [[cipherocto-design-principles]] §Separation of concerns (out of CLI scope; substrate-internal aggregation only).
...
- **F5 — Multi-network topology.** Operator selects single network identity per CLI invocation (single active network per `$OCTO_HOME/octotransport/bootstrap.toml` `network_id` field; multi-network aggregation is a future amendment).
...
| 0.5     |      | R10.5: dry_run rename + matrix rows + bind-chain integrity + F5 hedge trim. |
