...
- RFC-0010 — Canonical DID Codec (peer DID wire form at every substrate boundary; RFC-0010-v17 §Data Structures ledger_chain_registry Table is the substrate-side chain-id authority)
...
| `mode set`                       | BLOCKED pending G1 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                      | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; no operator workaround today; bootstrap lifecycle pauses until G1 lands                                                                    |
| `authority show`                 | yes                                                                                                                                                                                                                                                            | yes | yes | yes      | Bootstrap Authority | read-only                                                                                                                                                                                            |
| `authority rotate`               | BLOCKED pending G8 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                      | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; no operator workaround; bootstrap lifecycle pauses until G8 lands (rotate_post_fork requires quorum proof; no manual fallback)             |
| `slash excluded` / `slash stats` | yes                                                                                                                                                                                                                                                            | yes | yes | redacted | Operator            | DID redacted in Audit                                                                                                                                                                                |
| `slash mark`                     | out of scope per RFC-0008 §Execution Class Mapping                                                                                                                                                                                                             | no  | no  | no       | n/a                 | out of scope per RFC-0008 §Execution Class Mapping                                                                                                                                                   |
...
| `bind-envelope show`             | BLOCKED (pending G22 `0011-h-s-a-bind-envelope-lookup`; `BindEnvelope::load` lookup method missing in substrate )                                                                                                                                              | yes | yes | redacted | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; payload bytes never surfaced                                                |
...
| `discovery advertisement show`   | BLOCKED (pending G23 `0011-h-s-a-discovery-advertisement-cache`; cache type + lookup method both absent in substrate; companion adds both )                                                                                                                    | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                   |
| `discovery invitation show`      | BLOCKED (pending G24 `0011-h-s-a-discovery-invitation-cache`; cache type + lookup method both absent in substrate; companion adds both )                                                                                                                       | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                   |
...
CI mode (operator role = `CI` per §Per-Subcommand Capability Caveat Matrix) bypasses `--confirm-acknowledge` ONLY if the underlying subcommand is reversible. All 3 `bind-envelope rebind-*` subcommands plus `mode set` plus `authority rotate` return exit 90 (`NetworkCIRebindDenied`) per §Error Handling row 90 — CI agents cannot author bind-envelope state changes (reversible or not) nor bootstrap lifecycle changes. This is a defense-in-depth measure, not a substitute for human review.
...
| `slash mark`                   | (out of scope — CLI cannot mint SlashEnvelopes)                                                                                                             | Substrate-only mint path; CLI cannot produce SlashEnvelopes. Operators calling pre-landing hit clap `UnrecognizedSubcommand` exit 2. Future amendment would need to extend RFC-0008 §Execution Class Mapping                                                                                                                                                                                               | RFC-0008 §Execution Class Mapping                                                                                 | (out of scope per RFC-0008 §Execution Class Mapping) | n/a   | n/a                 |
...
pub struct NetworkRebindCommitOutput {
  ...
  pub prepared_evidence_hex: String,    // hex-encoded substrate `RebindCommit::prepared_evidence` (Vec<u8>; variable-length, NOT a 32-byte digest); see §Substrate-Additions G22 for sha256 producer-contract gap
  pub dry_run: bool,            // true when --dry-run (no substrate mutation)
}
...
| <sup>[†]</sup> 80                                                                                                                                                                                             | (RESERVED)         | n/a                                      | n/a                     |
| <sup>[†]</sup> 81                                                                                                                                                                                             | (RESERVED)         | n/a                                      | n/a                     |
...
| DID codec rejection (RFC-0010-v17)                                                                                                                                                                            | 86                 | `NetworkInvalidDid` (NEW)                | DID redacted            |
| `--confirm-acknowledge` missing on write, OR for `rebind-commit` only `--confirm` missing alongside `--confirm-acknowledge` present (CLI gate; fires before substrate dispatch per §Confirmation Flag Matrix) | 87                 | `NetworkConfirmRequired` (NEW)           | n/a                     |
...
| CI mode detected on rebind-* write path; CI mode gate is evaluated BEFORE the `--confirm-acknowledge` check, so in CI mode the operator always sees exit 90 regardless of `--confirm-acknowledge` presence    | 90                 | `NetworkCIRebindDenied` (NEW)            | n/a                     |
...
| Peer DID codec is RFC-0010 canonical              | All peer-DID-sourcing subcommands               | Legacy DID form could bypass codec gate                  | `octo_ident::CanonicalCodec::parse(s, allow_legacy=false)` at every substrate boundary (RFC-0011-f G2 pattern; codec spec lives in RFC-0010-v17 §Data Structures (canonical OctoID identifier codec; chain-id registration authority lives in RFC-0010-v17 §Data Structures ledger_chain_registry Table))                               |
...
- **Witness integrity + freshness + input gates.** Substrate nonce + monotonic timestamp gate per RFC-0871 §Data Structures governs rebind ordering (CLI builders emit substrate-derived freshness, operators CANNOT inject arbitrary values). Verify substrate confirms at `crates/octo-network/src/mon/rebind.rs` `RebindCoordinator::commit_envelope(&mut self) -> ()` (infallible transition) + `RebindCoordinator::abort(&mut self, reason: RebindAbortReason) -> CoordinatorState` (infallible state-return ). Substrate does NOT expose an `accept` method nor a Result-returning variant; both substrate methods are infallible state-transition primitives. Double-commit rejected via substrate idempotency check in `commit_envelope` (infallible: returns `()` even on duplicate; CLI abort preview stays accurate). Rollback path returns the NEW `CoordinatorState` after the abort transition; CLI MAY inspect the returned state to confirm rollback completed. Tally freshness is substrate-owned via `VotingTally` (the struct definition is in `crates/octo-network/src/mon/governance.rs`); CLI never caches across invocations. RFC-0011-g governs the CLI attestation + vote calls, not the underlying `VotingTally` type surface. The `--depth` clamp 1-100 on `trust-graph render` is CLI-side depth clamp protecting substrate from OOM (substrate `TrustGraph::render(&self, GraphFormat) -> String` has zero depth awareness per substrate-faithfulness audit); `--depth 0` → exit 85; `--depth 1000000` → clamp 100 with warning.
...
| `bind-envelope rebind-abort`   | DENY default | Roll-back state operation; substrate MAY interpret double-abort as no-op per RFC-0871 §Algorithms idempotency, but CI pastejacking risk remains.                |
...
[^slash-stats-cite]: Aggregate reader surface. `did_count` + `total_slashes` + `global_slash_count` live on `SlashReputationStoreCompat`. Cross-platform witness aggregation lives in `crates/octo-network/src/mon/slash_aggregation.rs` per RFC-0855p-b — SEPARATE substrate concern per [[cipherocto-design-principles]] §Separation of concerns (out of CLI scope; substrate-internal aggregation only).
...
[^sep-policy-rationale]: Coordinator Lifecycle substrate (`mon/slash_aggregation.rs`, `slash_aggregation.rs` cross-platform witness vote aggregation) is owned by RFC-0855p-b SEPARATE from RFC-0011-h per [[cipherocto-design-principles]] §Separation of concerns. Slash reason code allocation cross-references RFC-0855p-b §B. Slash Offense Codes; aggregation pathway lives in `mon/slash_aggregation.rs` and is NOT touched by this amendment.

[^slash-mark-classb]: `slash mark` is read-only inquiry against `SlashReputationStoreCompat` aggregate — CLI never mints SlashEnvelopes. Minting is Class B per RFC-0008 §Execution Class Mapping and out of scope for this amendment. RFC-0855p-b owns mint path; this amendment only surfaces the inquiry half.
...
| --- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ |
| F1  | Per-extension crates for network adapters        | Layer D adapter extensions; CLI MAY gain `--adapter <name>` flags per [[cipherocto-design-principles]] §User extensibility                                                           | Out of scope |
| F2  | Network streaming tail mode                      | `octo network envelope inspect --follow` per RFC-0011-a streaming pattern                                                                                                            | Out of scope |
| F3  | Marketplace / pricing integration                | Quota router marketplace pricing (RFC-0900 series)                                                                                                                                   | Out of scope |
| F4  | CLI automation hooks                             | RFC-0917 Python SDK + HTTP proxy as programmatic surface; CLI is operator escape hatch                                                                                               | Out of scope |
| F5  | Multi-network topology                           | Operator selects single network identity per invocation (single active network per `$OCTO_HOME/octotransport/bootstrap.toml` `network_id` field); multi-network aggregation deferred | Out of scope |
...
| F8  | Per-RFC `§Network CLI Surface` amendments        | RFC-0851, RFC-0851p-b, RFC-0855, RFC-0855p-c, RFC-0855p-b, RFC-0861, RFC-0862, RFC-0871, RFC-0008 each gain `§Network CLI Surface` section; each its own DRY cycle                   | Out of scope |
| F9  | Draft→Accepted promotions for 10 networking RFCs | RFC-0850p-d, RFC-0850p-e, RFC-0850p-f, RFC-0852, RFC-0854, RFC-0856, RFC-0857, RFC-0858, RFC-0859, RFC-0860 — each requires its own DRY cycle before promotion                       | Out of scope |
