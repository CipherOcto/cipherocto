...
| Peer DID codec is RFC-0010 canonical              | All peer-DID-sourcing subcommands               | Legacy DID form could bypass codec gate                  | `octo_ident::CanonicalCodec::parse(s, allow_legacy=false)` at every substrate boundary (RFC-0011-f G2 pattern; codec spec lives in RFC-0010 §2 ledger_chain_registry Table)                                                                                                                                                             |
...
| `tv-network-bind-envelope-rebind-abort-2`   | `bind-envelope rebind-abort`                                                                                                                                      | `--reason timeout` + `--confirm-acknowledge` + `--no-dry-run` + valid signer → success (mirrors `tv-network-bind-envelope-rebind-commit-4` flag set)                                                                                                                       |
...
| `tv-network-bind-envelope-rebind-abort-5`   | `bind-envelope rebind-abort`                                                                                                                                      | `--dry-run` explicit + operator denied at prompt → exit 88 (`NetworkDryRunDenied`) (operator overrode dry-run preview by declining at the confirmation prompt)                                                                                                             |
...
- [ ] Mission `0011-h-network-bind-envelope`
  ...
  - [ ] Test vectors tv-network-bind-envelope-show-1/2 + tv-network-bind-envelope-rebind-prepare-1/2/3/4 + tv-network-bind-envelope-rebind-commit-1/2/3/4/5 + tv-network-bind-envelope-rebind-abort-1/2/3/4 + tv-network-dry-run-deny-1 (per §Test Vectors)
