...
| `bind-envelope rebind-commit`  | (write; --dry-run default; --confirm-acknowledge; --no-dry-run to commit; irreversible; BLOCKED — pending G21 `0011-h-s-a-attached-handle-key-rotation` closure) | `RebindCommit` struct builder + key-id surface via `AttachedHandleKeyRotation::current_key_id()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | RFC-0871                  | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     |
...
pub struct NetworkBindEnvelopeShowOutput {
    ...
    pub canonical_bytes_hash: String,         // blake3 hex of canonical_bytes()
}
...
pub struct NetworkRebindPrepareOutput {
    pub domain_id: String,                    // bind envelope domain identifier (canonical hex form)
    pub new_bind: String,                     // proposed replacement `BindEnvelope` (canonical hex form)
...
pub struct NetworkRebindCommitOutput {
    pub domain_id: String,                    // bind envelope domain identifier (canonical hex form)
    pub new_bind: String,                     // committed replacement `BindEnvelope` (canonical hex form)
...
**Cross-amendment slot consumption (no collision):** RFC-0011-c/d/e/f/g all below slot 79; slots 79-89 free at Draft time.
...
- **Envelope payload bytes never echoed (canonical invariant).** `bind-envelope show` returns metadata + `canonical_bytes_hash` (blake3 hex) only; raw `Vec<u8>` from `BindEnvelope::canonical_bytes()` NEVER surface in `OutputEnvelope` or logs. Test vectors (`tv-network-bind-envelope-show-1`) assert no payload byte leak across envelope payload bytes. Invariant name: `no-payload-bytes-leak`.
...
- **Redaction placeholder form:** RFC-0011 §Redaction layer uses `[REDACTED:<kind>]` placeholders. Forms: `[REDACTED:did]`, `[REDACTED:coordinator_id]`, `[REDACTED:gateway_id]`, `[REDACTED:file_path]`. Audit mode applies same placeholders.
...
- **`bind-envelope rebind-*` chain integrity.** Three-part chain (prepare → commit | abort) governed by substrate nonce + monotonic timestamp gate per RFC-0871 §Data Structures. CLI builders emit substrate-derived freshness; operators CANNOT inject arbitrary timestamp/nonce values. Double-commit substrate-rejected via idempotency check per RFC-0871 §Commit lifecycle; CLI cannot bypass via rapid chained invocations.
- **`bind-envelope rebind-*` confirmation gate.** All 3 rebind ops (prepare, commit, abort) require `--dry-run` default + explicit `--confirm-acknowledge` + `--no-dry-run` flag. Reversibility differs: `rebind-commit` is irreversible state mutation → TWO confirm flags (`--confirm` + `--confirm-acknowledge`) per RFC-0011 §Confirmation Flag Matrix human column for high-blast-radius writes (pastejacking defense). `rebind-abort` is reversible (drops pending prepare payload, no committed-state change) → single `--confirm-acknowledge` flag. `rebind-prepare` is payload-builder only (no substrate state mutation) → single `--confirm-acknowledge` flag.
