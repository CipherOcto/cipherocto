...
pub struct NetworkRebindCommitOutput {
    ...
    pub prepared_evidence_hex: String,        // hex-encoded substrate `RebindCommit::prepared_evidence` (substrate field is `Vec<u8>` holding a 32-byte SHA-256 digest of sorted platform/signature WITNESS pairs per `crates/octo-network/src/mon/bind_envelope.rs:162-165`; substrate encodes the digest, raw WITNESS pairs never surface)
    pub dry_run: bool,                        // true when --dry-run (no substrate mutation)
}
...
| Operator runs `bind-envelope rebind-commit` on wrong target                                 | HIGH — irreversible state change if substrate commits  | see §Security Considerations pastejacking-defense bullet (canonical cite for `--dry-run` + double-confirm + audit-log chain)                                                                                |
...
- [ ] Mission `0011-h-network-bind-envelope`
  ...
  - [ ] Test vectors tv-network-bind-envelope-show-1/2 + tv-network-bind-envelope-rebind-prepare-1/2/3/4 + tv-network-bind-envelope-rebind-commit-1/2/3/4/5 + tv-network-bind-envelope-rebind-abort-1/2/3/4 + tv-network-dry-run-deny-1 (commit-3 covers missing `--confirm` SECOND flag exit 87 pastejacking defense; commit-4 covers triple-flag valid signer success)
