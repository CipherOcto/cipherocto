...
| `slash stats`                  | (read)                                                                                                                                                                                                                                                     | `SlashReputationStoreCompat::{did_count, total_slashes, global_slash_count}` (see G6b for aggregation rationale; cross-platform witness aggregation lives in `mon/slash_aggregation.rs` per RFC-0850p-c, SEPARATE substrate concern per [[cipherocto-design-principles]] §Separation of concerns)                                                                                                                                                                                                                                                                  | RFC-0855p-b               | `0011-h-network-slash-stats` §slash-stats            | 2     |
...
| `bind-envelope show`           | (read)                                                                                                                                                                                                                                                     | `BindEnvelope::{canonical_bytes, is_participant}`. Single canonical cite for blake3 + no-leak invariant: §Security Considerations.                                                                                                                                                                                                                                                                                                                                                                                                                                 | RFC-0871 §Data Structures | `0011-h-network-bind-envelope` §bind-envelope-show   | 4     |
| `bind-envelope rebind-prepare` | (write; --dry-run default; --confirm-acknowledge; --no-dry-run to commit; BLOCKED — pending G21 `0011-h-s-a-attached-handle-key-rotation` closure)                                                                                                         | `RebindPrepare` struct builder + key-id surface via `AttachedHandleKeyRotation::current_key_id()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | RFC-0871                  | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     |
...
| `bind-envelope rebind-abort`   | (write; --dry-run default; --confirm-acknowledge; --no-dry-run to commit)                                                                                                                                                                                  | `RebindAbort { domain_id, reason, dissenters, signature }` builder                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | RFC-0871                  | `0011-h-network-bind-envelope` §bind-envelope-rebind | 4     |
...
| `bootstrap`                    | DEFERRED — substrate-additions prerequisite (`BootstrapOrchestrator` missing)                                                                                                                                                                              | `BootstrapOrchestrator::start_bootstrap(BootstrapConfig)` + `status()` (per G1 row). Operators calling pre-landing hit clap `UnrecognizedSubcommand` exit 2; reroute via `0011-h-s-a-bootstrap-orchestrator` companion mission                                                                                                                                                                                                                                                                                                                                     | RFC-0851p-a §F.2          | `0011-h-network-bootstrap` (deferred)                | 6     |
| `status`                       | DEFERRED — substrate-additions prerequisite (`BootstrapState` missing)                                                                                                                                                                                     | `BootstrapOrchestrator::status() → BootstrapState` (per G1 row). Operators calling pre-landing hit clap `UnrecognizedSubcommand` exit 2; fall back to `octo network peers list` + `octo network trust-graph render` for equivalent visibility                                                                                                                                                                                                                                                                                                                      | RFC-0851p-a §F.2          | `0011-h-network-status` (deferred)                   | 6     |
...
pub struct DidSlashCount {
    ...
    pub slash_count: u32,                     // substrate SlashReputationStoreCompat::global_slash_count(did) via iter_dids() iterator (cargo grep on SlashReputationStoreCompat trait surface 2026-09-18 returned iter_dids accessor; if substrate helper absent, CLI surface DEFERRED — see G6b companion mission)
}
...
**Coverage:** 3 GAPs (G3, G4, G5) are substrate-faithful TODAY via `peers`, `identity`, `trust-graph`; 4 local config reads (G8-show, G6-stats via G6b-loader, G3b-canonical-bytes, G12b-coordinator-record) covered by §Subcommand Taxonomy rows; 14 GAPs require substrate-additions. GAP table contains 24 rows (G1-G21 + 3 helper G-rows: G3b, G6b, G12b). Substrate-additions companion mission table contains 19 active entries across 18 GAP rows (G16 contributes 2 missions for 1 GAP).
