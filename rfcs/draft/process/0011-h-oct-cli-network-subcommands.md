...
3. **Mutating operations unsafe.** Substrate-faithful slice has 3 mutating subcommands (the `bind-envelope rebind-{prepare,commit,abort}` payload builders); every one must mirror §Confirmation Flag Matrix; every mutating subcommand defaults to `--dry-run`. (`slash mark` is read-only inquiry per [^slash-mark-classb].)
...
| `mode set`                       | BLOCKED pending G1 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                      | no  | no  | no       | Bootstrap Authority | clap arm registered, gated on substrate[^clap-arm-gated]; no operator workaround; bootstrap lifecycle pauses until G8 lands                                                                          |
...
| `coordinator admin`              | BLOCKED pending G12 closure[^substrate-path]; pre-landing exit 89[^exit89]                                                                                                                                                                                     | no  | no  | no       | Coordinator         | gated on substrate; companion mission `0011-h-s-a-coordinator-admin-trait`[^coordinator-admin-substrate] delivers the substrate surface                                                              |
...
| `discovery advertisement show`   | BLOCKED (pending G23 `0011-h-s-a-discovery-advertisement-cache`; `MissionAdvertisementCache::get` lookup method missing in substrate )                                                                                                                         | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                   |
| `discovery invitation show`      | BLOCKED (pending G24 `0011-h-s-a-discovery-invitation-cache`; `MissionInvitationCache::get` lookup method missing in substrate )                                                                                                                               | yes | yes | yes      | Operator            | clap arm registered, gated on substrate; pre-landing to exit 89 (NetworkSubstrateUnavailable) per Error Handling row 89; read-only                                                                   |
...
| **Bootstrap lifecycle** (`bootstrap`, `status`, `mode set`, `authority rotate`) (BLOCKED G1 / G8)                                                                                                                                                       | write; **irreversible** for `mode set` + `authority rotate` | yes                       | yes                              | **YES** (pastejacking defense for irreversible state writes) | **yes** for irreversible; no for `bootstrap` itself                                        |
...
4. **For pastejacking-risk writes only:** operator re-invokes again with `--confirm` as a SECOND flag. This breaks the clipboard-pastejacking attack: a pastejacked flag string would land `--confirm-acknowledge` but not `--confirm`, so the CLI gate still holds.

CI mode (operator role = `CI` per §Per-Subcommand Capability Caveat Matrix) bypasses `--confirm-acknowledge` ONLY if the underlying subcommand is reversible. Irreversible writes (`rebind-commit`, `mode set`, `authority rotate`) return exit 90 (`NetworkCIRebindDenied`) per §Error Handling row 90 — CI agents cannot author irreversible state changes. This is a defense-in-depth measure, not a substitute for human review.
...
| `discovery advertisement show` | (read; BLOCKED — pending G23 `0011-h-s-a-discovery-advertisement-cache` closure; `MissionAdvertisementCache::get` lookup method missing in substrate ) | Substrate lookup surface absent; companion G23 closes this gate. Pre-landing → exit 89[^exit89]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-advertisement  | 5     | Operator            |
| `discovery invitation show`    | (read; BLOCKED — pending G24 `0011-h-s-a-discovery-invitation-cache` closure; `MissionInvitationCache::get` lookup method missing in substrate )       | Substrate lookup surface absent; companion G24 closes this gate. Pre-landing → exit 89[^exit89]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | RFC-0851 §8 Discovery Lifecycle                                                                                   | `0011-h-network-discovery` §discovery-invitation     | 5     | Operator            |
