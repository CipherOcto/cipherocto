...
| Flags        | `--caveats <json>` (REQUIRED), `--dry-run`                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Max uses     | `{ "type": "max_uses", "value": { "count": <u32> } }`              | `count > 0`; `count == 0` means unlimited per substrate (RFC-0965 caveat catalog MaxUses entry)                                                                 |
...
| Single use   | `{ "type": "max_uses", "value": { "count": 1 } }`                  | nested `count` form per substrate `Caveat::MaxUses { count: 1 }` (no dedicated `SingleUse` variant)                                                             |
...
| Caveat array length      | ≤ 16 caveats | Catalog exhausts at 8 types; 16 is a sanity upper bound       |
...
- Capability subcommands: `capability {list,mint,attenuate}`
