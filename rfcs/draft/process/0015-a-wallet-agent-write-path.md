...
- **(5) Audit append + rollback contract** — every successful non-idempotent transition appends an `AuditEventKind::AgentTransition { agent_id, from, to, reason }` row to the canonical `AppendOnlyAuditSink` per RFC-0012 (parent `AuditEvent::at_millis_unix` carries the timestamp — variant payload does NOT carry `at_unix`). Substrate-faithful symmetry: parent struct is the canonical timestamp carrier; variants carry transition-specific payload only. On audit append failure (any `AuditError` variant returned from `append_agent_transition_event`):
...
| Audit-append failure → state inconsistency | Operator / substrate bug   | Audit sink transient failure (`SinkSpecific` / `SequenceGap` / `AlreadyExists`) | State advances without audit log entry | ROLLBACK contract per §6.1 (5): any `Err` from `append_audit_event` (uniform `.map_err` blanket conversion per `agent.rs` §transition_agent audit append branch) → in-memory state rolled back; `WalletError::AuditUnavailable(String)` propagated (fail-closed). The wallet substrate does NOT have special-case handling for `AlreadyExists` — blanket `.map_err` propagates any Err. | Persisted rollback window; concurrent reader may observe stale state during rollback; substrate-faithful best-effort rollback                                                                                                                                                                                |
...
| TV-WLT-AGT-11c | `transition_agent(caller_did, uuid, _, _)` with audit sink idempotent retry | `append_audit_event` returns `AuditError::AlreadyExists(42)` (propagated as Err at audit-core layer per substrate-canonical 3-variant form) | `Err(WalletError::AuditUnavailable(<Debug-formatted AuditError string>))` + in-memory state rolled back per §6.1 (5). The wallet substrate's blanket `.map_err` propagates any Err from `append_audit_event`; no special-case for `AlreadyExists` (the substrate does NOT normalize AlreadyExists to Ok at RFC-0015-a acceptance).                                                                                                                                                                                                                      | Fail-closed rollback contract (AlreadyExists propagated as Err) — the audit-core layer's canonical 3-variant form per RFC-0012 always propagates AlreadyExists as Err; the wallet's fail-closed rollback contract applies. |
...
- `crates/octo-wallet/Cargo.toml` — no NEW dep at RFC-0015-a acceptance. The pre-existing `octo-audit = { path = "../octo-audit" }` Layer B façade dep carries the canonical `append_audit_event` re-export path (substrate uses `std::sync::Mutex` already in §AgentRecord struct per `use std::sync::{Mutex, OnceLock};`).
