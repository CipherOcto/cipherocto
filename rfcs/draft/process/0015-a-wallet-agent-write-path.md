...
- **(1) TOCTOU mitigation** — `parking_lot::Mutex::try_lock()` per `agent_id` (canonical substrate pattern per `crates/octo-wallet/src/error.rs` §AlreadyInTransition docstring). The substrate raises `WalletError::AlreadyInTransition(Uuid)` when the per-agent lock is held (transient, retry-safe). On success the lock is held across reason filter + state-machine work + audit append and released on completion (or rollback). `current_state` is re-read INSIDE the lock. **Rationale for per-agent try_lock:** the canonical substrate pattern uses `parking_lot::Mutex::try_lock()`; concurrent calls against the same `agent_id` serialize at lock acquisition without blocking the global registry. The GLOBAL `AGENT_REGISTRY: OnceLock<Mutex<BTreeMap<Uuid, AgentRecord>>>` provides the `HashMap`-level guard for insert/remove; per-agent `parking_lot::Mutex` provides the per-agent contention guard for state-machine work.
...
- **(5) Audit append + rollback contract** — every successful non-idempotent transition appends an `AuditEventKind::AgentTransition { agent_id, from, to, reason }` row to the canonical `AppendOnlyAuditSink` per RFC-0012 (parent `AuditEvent::at_millis_unix` carries the timestamp — variant payload does NOT carry `at_unix`). Substrate-faithful symmetry: parent struct is the canonical timestamp carrier; variants carry transition-specific payload only. On audit append failure (`AuditError` variant returned from `append_audit_event`):
  - The substrate maps the error to `WalletError::AuditUnavailable(String)` (canonical String payload). The String carries `format!("{e:?}")` of the substrate `AuditError` (Debug is derived unconditionally per `crates/octo-audit-core/src/error.rs`); no cfg-gating on the wallet surface — the envelope payload shape is stable across build configurations.
...
- **(7) Single-callsite per agent** — per-agent `parking_lot::Mutex::try_lock()` serializes concurrent `transition_agent` calls against the same `agent_id` per §6.1 (1). Concurrent calls against an already-locked agent observe `WalletError::AlreadyInTransition(Uuid)` per `crates/octo-wallet/src/error.rs` §AlreadyInTransition docstring (transient, retry-safe; CLI surfaces as substrate reason per `OctoCliError::AlreadyInTransition(uuid)` at `crates/octo-cli/src/error.rs` §AlreadyInTransition, exit 43). The lock is held across the FIRST caller's reason filter + state-machine work + audit append per §6.1 (1) + §6.1 (5); the per-agent lock releases on completion (or rollback).
...
**Substrate-faithful symmetry note:** the substrate keeps `WalletError::AlreadyInTransition(Uuid)` and `WalletError::InvalidStateTransition { from, to }` as SEPARATE variants (per §AlreadyInTransition vs §InvalidStateTransition). The CLI mirrors with SEPARATE variants (per §AlreadyInTransition vs §transition_agent audit append branch-390). Both share exit 43 per §exit_codes write-path slot. The substrate-faithful principle: CLI variant payload mirrors substrate variant payload (no parallel abstractions). The `from` + `to` fields serialize as `String` at the CLI boundary (per `Display for AgentState` impl at §Display for AgentState: lowercase "registered" / "running" / "terminated") — the CLI never pattern-matches on the substrate `AgentState` enum directly.
