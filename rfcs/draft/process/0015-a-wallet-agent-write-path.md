...
- **(7) Single-callsite per agent** — GLOBAL `AGENT_REGISTRY` std Mutex serializes concurrent `transition_agent` calls (canonical substrate pattern per §6.1 (1)). Dead-variant contract for `AlreadyInTransition(Uuid)`: declared, not constructed at RFC-0015-a acceptance per §Discipline at first call site (CLAUDE.md §Architectural Principles). All other sections cite this contract; the variant surface is reserved for a future paired-acceptance substrate amendment and the `#[non_exhaustive]` declaration is preserved for documentation + CLI-mirror alignment only.
