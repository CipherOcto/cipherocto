...
| Audit-append failure → state inconsistency | Operator / substrate bug   | Audit sink transient failure (`SinkSpecific` / `SequenceGap` / `AlreadyExists` if propagated as Err) | State advances without audit log entry | ROLLBACK contract per §6.1 (5): any `Err` from `append_audit_event` → in-memory state rolled back; `WalletError::AuditUnavailable(String)` propagated (fail-closed). The wallet substrate does NOT have special-case handling for `AlreadyExists` — the audit-core layer may either propagate it as `Err` (rollback applies) or normalize it transparently. | Persisted rollback window; concurrent reader may observe stale state during rollback; substrate-faithful best-effort rollback                                                                                                                                                                                |
