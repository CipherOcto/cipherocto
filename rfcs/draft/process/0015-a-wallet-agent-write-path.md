...
- **(1) TOCTOU mitigation** — `std::sync::Mutex::lock()` on GLOBAL `AGENT_REGISTRY: OnceLock<Mutex<BTreeMap<Uuid, AgentRecord>>>` (canonical substrate pattern per §AGENT_REGISTRY). Mutex poison → `WalletError::Config(String)` (canonical poison-mapping per §Config; same as the read-path call sites `list_owned_agents` + `lookup_agent` + `transition_agent` in `agent` module). The `cli_fns::register_agent` call site maps mutex poison to `WalletError::OsRng` instead (different substrate-canonical mapping per `cli_fns` module; the wallet read+write path under `agent` uses the `Config` variant uniformly). The lock is held across reason filter + state-machine work + audit append and released after the audit append completes (or rolls back). `current_state` is re-read INSIDE the lock. **Rationale for std Mutex:** the canonical substrate pattern uses std `Mutex`; mutex poison is mapped to `WalletError::Config` for the `agent` module. The single-writer invariant is enforced via the GLOBAL lock — concurrent callers serialize at lock acquisition. The per-agent rejection path is the state-machine guard (§6.1 (3) below), not a per-agent `try_lock`.
...
- **(7) Single-callsite per agent** — the GLOBAL `AGENT_REGISTRY` std Mutex serializes concurrent `transition_agent` calls (canonical substrate pattern per §6.1 (1)). Per-agent rejection is the state-machine guard (§6.1 (3)) + the `AlreadyInTransition(Uuid)` substrate variant. The `AlreadyInTransition(Uuid)` variant is declared in `crates/octo-wallet/src/error.rs` §AlreadyInTransition; the substrate does NOT currently construct it (no code path in `transition_agent` raises it — the variant is reserved for a future paired-acceptance substrate amendment; until then, concurrent callers serialize via the GLOBAL std Mutex and the variant surface is dead code at RFC-0015-a acceptance). Per §Discipline at first call site (CLAUDE.md §Architectural Principles), the dead variant should ideally be deferred until paired with the substrate amendment that constructs it; the additive `#[non_exhaustive]` declaration is preserved at RFC-0015-a acceptance for documentation + CLI-mirror alignment only. The CLI surfaces `AlreadyInTransition` per `OctoCliError::AlreadyInTransition(uuid)` at `crates/octo-cli/src/error.rs` §AlreadyInTransition, exit 43.
...
**Substrate-faithful divergence note:** the `octo-wallet::AgentState` enum (3-variant: `Registered`, `Running`, `Terminated`) is a projection of `octo-runtime::AgentState` (7-variant: `Registered`, `Active`, `Busy`, `Suspended`, `Rejected`, `Terminated`, `Retired`). The wallet façade collapses `Active`/`Busy`/`Suspended` to `Running` and drops `Rejected`/`Retired` from the wallet surface. The full 7-variant form lives in `crates/octo-runtime/src/handle.rs` (out of RFC-0015 scope). An audit-log entry carrying `Terminated` cannot be reconstructed back to `Suspended`/`Retired` from the wallet state alone; the runtime layer is the source of truth for the full state space. A future RFC-0002 companion amendment may split the wallet enum back to match the runtime surface; until then, the 3-var wallet projection is canonical.
...
The `AuditEventKind::AgentTransition` variant lives in `octo-audit-core` Layer A (frozen). Per RFC-0012 paired-amendment, the variant is gated `#[cfg(feature = "octo-audit-internal")]` — visible only when the wallet substrate feature is enabled. The cfg-gate is the Layer A frozen preservation mechanism: the variant exists in source but is invisible to default builds, preserving the frozen-contract invariant (no additive variants visible to consumers without paired amendment acceptance).
