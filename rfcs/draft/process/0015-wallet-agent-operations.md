...
- `crates/octo-wallet/src/error.rs` — append `WalletError::AgentNotFound(Uuid)` + `WalletError::ForbiddenHolderMismatch` + `WalletError::ReasonContainsControlChars(char)` (3 KEEP variants per §6.3, with `ReasonContainsControlChars` promoted from DEFERRED to KEEP at R21.5 H-2 fix per §6.2.5 + §6.8 H-2 amendment; the variant payload is `char`, declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern, NOT a standalone newtype struct). The DEFERRED variants `AlreadyInTransition(Uuid)` + `InvalidStateTransition { from, to }` + `ReasonTooLong(usize)` + `AuditUnavailable` are NOT added at R4.5 — they land with the RFC-0012-v2 + RFC-0015 re-implementation.
