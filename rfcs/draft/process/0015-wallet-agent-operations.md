...
// R21 finding M-3 fix: `Cursor` and `AgentFilter` struct definitions
// anchored explicitly (prior v1.0 referenced cursor semantics in 3 places
// without an anchoring struct def). `Cursor(pub String)` is an opaque
// forward-compat token; `AgentFilter.cursor` is `Option<Cursor>`, reserved
// for Phase 2 multi-page iteration per §Implementation Phases.
...
pub struct AgentFilter {
    ...
    pub cursor: Option<Cursor>,  // forward-compat opaque token; reserved for Phase 2 per §Implementation Phases
}
```

- **Filter semantics** — server-side `holder_did == filter.holder_did.unwrap_or(caller_did.clone())`; substrate enforces `filter.holder_did.is_none() || filter.holder_did.as_ref().map(Did::as_str) == Some(caller_did.as_str())` per RFC-0009 §Identity (mismatch → `WalletError::ForbiddenHolderMismatch`; substrate-faithful caller-attestation per §6.2.1 `caller_did: &Did` parameter pattern); `state == filter.state` (exact match, no wildcards); `limit` clamp; `cursor` reserved (ignored on Phase 1 reads). Note: `Did` has no `Deref<Target=str>` impl per `crates/octo-wallet/src/identity_record.rs` — `as_deref()` is unavailable; the canonical access pattern is `as_ref().map(Did::as_str)`.
...
4. **`register_agent` precedes any read** — substrate assumes the agent_id returned by `register_agent` exists in the in-memory registry before any `list_owned_agents` call returns a non-empty `Vec`; CLI precondition check. **R21 finding M-4 substrate-faithful cross-reference:** `register_agent` is the EXISTING `octo_wallet::cli_fns::register_agent(manifest: &AgentManifest, capability_root: &CapabilityId, active_did: &Did) -> Result<AgentManifest, WalletError>` per `crates/octo-wallet/src/cli_fns.rs` §`register_agent` (hard-checked 2026-09-11). RFC-0015 R4.5 KEEP does NOT add a new `register_agent` function — the existing CLI FFI function is the canonical substrate contract. The CLI mission `0011-c-agent-create-subcommand.md` Sub-step 3 calls this existing function (per §6.5 CLI integration contract); the `0011-c-agent-{list,run,destroy,attach}-subcommand` missions consume the read surface or transition surface produced by `register_agent`. Substrate-faithful principle: no parallel `register_agent` abstractions (per [[cipherocto-design-principles]] §No parallel abstractions); RFC-0015 reuses the canonical CLI FFI function.
...
- **Phase 2 (RFC-0011-c read-only missions)** — CLI missions `0011-c-agent-{create,list,attach}-subcommand` consume the read surface; mutation traces per RFC-0011-c §Test Vectors. **Cursor support (R21 M-3 forward-compat note):** the `cursor: Option<Cursor>` field on `AgentFilter` (per §6.2.1 anchored struct defs) is RESERVED for Phase 2 multi-page iteration; the substrate accepts and returns the cursor as opaque, but the CLI does not surface cursor-based iteration at Phase 2 — the cursor is forward-compat for >1024-agent registries (Future Work §`list_owned_agents` cursor support). The substrate-faithful pattern: the field exists at R4.5 acceptance (no substrate amendment required; pure struct field addition); Phase 2 wires the CLI round-trip, Phase 2.5+ may consume it.
...
pub fn list_owned_agents(
    ...
    //    `filter.holder_did.is_none() || filter.holder_did.as_ref()
    //    .map(Did::as_str) == Some(caller_did.as_str())`; mismatch →
    //    ForbiddenHolderMismatch. Note: `Did` has no `Deref<Target=str>`
    //    impl per `crates/octo-wallet/src/identity_record.rs` —
    //    `as_deref()` is unavailable; the canonical access pattern is
    //    `as_ref().map(Did::as_str)`.
