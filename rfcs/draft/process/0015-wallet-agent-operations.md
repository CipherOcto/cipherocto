...
| `ReasonTooLong(usize)` (NEW, **DEFERRED**)                  | `transition_agent` (write-only; reason > 256 chars; DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows — substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock)                                                                                                                                                                                           | `InvalidFilter` (16)          | parent reserved             |
...
- **`crates/octo-wallet/Cargo.toml`** — add `parking_lot = { version = "0.12", features = ["deadlock_detection"] }` (Layer B years-stable; canonical Rust mutex ecosystem) per §6.2.2 (1) + Phase 2.5. The dep is required by the DEFERRED `transition_agent` write-path Phase 2.5 lock-mode contract (`parking_lot::Mutex::try_lock()`); the dep lands at RFC-0015 R4.5 acceptance so the unblocked Phase 2.5 implementation does not require a follow-on amendment.
...
| `WalletError::ReasonTooLong(len)`                  | `OctoCliError::InvalidFilter(reason)`     | 16       | parent reserved | DEFERRED NEW ADDITIVE (substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock per §6.8 rows)                                      |
