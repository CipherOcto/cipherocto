...
2. **`pub fn validate_reason(reason: &str) -> Result<(), WalletError>`** — NEW ADDITIVE function per RFC-0015 §6.2.5; the `WalletError::ReasonContainsControlChars(char)` enum variant is also NEW ADDITIVE per R21.5 H-2 amendment. String-level control-char filter primitive; no substrate amendment required at the primitive level (the variant payload `char` is the only substrate addition).
...
- **KEEP** — `WalletError::ReasonContainsControlChars(char)` (NEW ADDITIVE per R21.5 H-2 fix — promoted from DEFERRED to KEEP alongside `validate_reason` primitive; the variant payload is `char`, declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern; RFC-0015 amendment to `octo-wallet/src/error.rs` adds this variant (NOT a standalone newtype struct). The variant is consumed by `validate_reason` AND by DEFERRED `transition_agent` write path per §6.2.2 (6)).
...
/// - `Err(WalletError::ReasonContainsControlChars(char))` if `reason`
///   contains any control character in `U+0000`-`U+001F` or `U+007F`.
///   The variant carries the offending character as a payload
///   (substrate-faithful diagnostic; the operator sees which control
///   character triggered the rejection). NOTE (R21 finding H-2 fix):
///   the variant is KEEP at RFC-0015 R4.5 acceptance alongside
///   `validate_reason` (per §6.2.5 H-2 amendment + §6.8 row `ReasonContainsControlChars` removal +
///   §Implementation Phases Phase 2.5 amendment); the primitive's
///   return type resolves to the KEEP variant at R4.5 acceptance. The
///   variant is ALSO consumed by the DEFERRED `transition_agent` write
///   path at Phase 2.5 (paired semantics — the write path's control-char
///   filter per §6.2.2 (6) calls `validate_reason` upstream and the
///   variant propagates through unchanged).
...
- **Multi-char return semantics (R22 finding M-25 first-class restatement):** when `reason` contains multiple control characters (e.g., a string with embedded `\x00\x01\x02`), the primitive returns the FIRST occurrence (deterministic per input per §6.6 determinism requirement); the `char` payload in `WalletError::ReasonContainsControlChars(char)` is the FIRST control character encountered, not the last or any arbitrary one. The order is enforced by single-pass scan from byte index 0 forward; the primitive does NOT aggregate multiple control chars into a list (the variant is single-char tuple payload per §6.3; aggregation would require a separate variant or container type).
...
- **Substrate-faithful note:** the primitive lands at R4.5 KEEP because it requires NO substrate amendment (string-level filter on UTF-8 input; pure function). The error variant it returns (`WalletError::ReasonContainsControlChars(char)`) is KEEP at RFC-0015 R4.5 acceptance per R21 finding H-2 fix (the variant payload is `char`, declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct). The primitive is exercisable today as `validate_reason(reason: &str) -> Result<(), WalletError>` once the variant lands (paired with the KEEP primitive at R4.5 acceptance); the DEFERRED `transition_agent` write path at Phase 2.5 consumes the SAME variant (paired semantics — the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The KEEP variant covers both the primitive's standalone test coverage (per TV-WLT-AGT-13..13f) and the DEFERRED write path's paired consumption at Phase 2.5; no re-promotion or re-implementation needed across phases.
...
**R21 finding H-2 amendment — `WalletError::ReasonContainsControlChars(char)` PROMOTED to KEEP at R21.5:** the variant was DEFERRED in §6.8 row `ReasonContainsControlChars` of prior versions; R21.5 promotes it to KEEP at RFC-0015 R4.5 acceptance alongside the `validate_reason` primitive per §6.2.5 (the primitive's return type resolves to the KEEP variant at R4.5 acceptance). The variant payload is `char`, declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct. The substrate-faithful symmetry mirrors `WalletError::AgentNotFound(Uuid)` per §6.2.3. The variant is consumed by the KEEP `validate_reason` primitive at RFC-0015 R4.5 acceptance AND will be consumed by the DEFERRED `transition_agent` write path at Phase 2.5 (paired semantics; the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The variant is therefore KEEP at R4.5 and stays KEEP through Phase 2.5 (no re-promotion needed).
...
| TV-WLT-AGT-13b | `validate_reason("\x1b[31mred")` (unit test)                                                                                  | ANSI escape sequence reason                                                                | `Err(WalletError::ReasonContainsControlChars)` (KEEP at R21.5 H-2 fix per §6.2.5; variant payload `char` is the offending `U+001B` ESC per §6.2.5 multi-char return semantics)                                            | Verifies filter rejects `U+001B` ESC                                                                                                            |
...
| TV-WLT-AGT-13d | `validate_reason("\x00null")` (unit test)                                                                                     | NUL byte reason                                                                            | `Err(WalletError::ReasonContainsControlChars)` (KEEP at R21.5 H-2 fix per §6.2.5; variant payload `char` is the offending `U+0000` NUL per §6.2.5 multi-char return semantics)                                            | Verifies filter rejects `U+0000` NUL                                                                                                            |
| TV-WLT-AGT-13e | `validate_reason("\r\n[ADMIN] approved")` (unit test)                                                                         | CRLF-injection reason                                                                      | `Err(WalletError::ReasonContainsControlChars)` (KEEP at R21.5 H-2 fix per §6.2.5; variant payload `char` is the FIRST occurrence per §6.2.5 multi-char return semantics — `\r` is encountered before `\n`)                | Verifies filter rejects `U+000A` LF / `U+000D` CR                                                                                               |
...
- `crates/octo-wallet/src/error.rs` — append `WalletError::AgentNotFound(Uuid)` + `WalletError::ForbiddenHolderMismatch` + `WalletError::ReasonContainsControlChars(char)` + `WalletError::ReasonTooLong(usize)` (4 KEEP variants per §6.3 — `ReasonContainsControlChars` promoted from DEFERRED to KEEP at R21.5 H-2 fix per §6.2.5 + §6.8 H-2 amendment; `ReasonTooLong` promoted from DEFERRED to KEEP at R25 M-3 fix per §6.2.5 length cap; both payloads are inline thiserror tuple-payload enum-variant patterns, NOT standalone newtype structs). The DEFERRED variants `AlreadyInTransition(Uuid)` + `InvalidStateTransition { from, to }` + `AuditUnavailable` are NOT added at R4.5 — they land with the RFC-0012-v2 + RFC-0015 re-implementation.
...
| `WalletError::ReasonContainsControlChars(char)`    | `OctoCliError::InvalidFilter(reason)`     | 16       | parent reserved | **KEEP** (R21.5 H-2 fix; variant payload `char` declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern, NOT a standalone newtype struct)                                                                                                      |
