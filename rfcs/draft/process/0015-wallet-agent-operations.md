...
1. **`pub fn list_owned_agents(caller_did: &Did, filter: &AgentFilter) -> Result<Vec<AgentSummary>, WalletError>`** — NEW ADDITIVE function per RFC-0015 §6.2.1; the `AgentFilter`/`AgentSummary`/`AgentState` TYPES are substrate-faithful to `crates/octo-wallet/src/agent.rs` but the function itself is RFC-0015 amendment to the substrate. Server-side filter on `holder_did`, `state`, `limit`, `cursor`; returns `Vec<AgentSummary>` per-call (cursor is forward-compat opaque token).
2. **`pub fn validate_reason(reason: &str) -> Result<(), WalletError>`** — NEW ADDITIVE function per RFC-0015 §6.2.5; the `WalletError::ReasonContainsControlChars(String)` enum variant is also NEW ADDITIVE per R21.5 H-2 amendment (R27.5 C-S1 fix — payload type is `String` carrying hex-escaped code-point notation, NEVER the raw byte; prevents attacker-byte echo back to the terminal). String-level control-char filter primitive; no substrate amendment required at the primitive level (the variant payload `String` is the only substrate addition).
3. **`WalletError::AgentNotFound(Uuid)`** — NEW ADDITIVE enum variant per RFC-0015 §6.3 + §6.8 (the RFC-0015 amendment to `octo-wallet/src/error.rs` adds this variant); mirrors the existing `WalletError::AgentAlreadyExists(Uuid)` shape.
4. **`WalletError::ForbiddenHolderMismatch`** — NEW ADDITIVE enum variant per RFC-0015 §6.3 + §6.8 (the RFC-0015 amendment to `octo-wallet/src/error.rs` adds this variant); HIGH sec fix per §6.2.1 (multi-DID enumeration prevention).
...
- **KEEP** — `list_owned_agents(caller_did: &Did, filter: &AgentFilter) -> Result<Vec<AgentSummary>, WalletError>` (NEW ADDITIVE per RFC-0015 §6.2.1; the `AgentFilter`/`AgentSummary`/`AgentState` types are substrate-faithful, the function is RFC-0015 amendment).
- **KEEP** — `validate_reason(reason: &str) -> Result<(), WalletError>` (NEW ADDITIVE primitive string-level filter per RFC-0015 §6.2.5; the variant it returns is also NEW ADDITIVE per R21.5 H-2).
- **KEEP** — `WalletError::AgentNotFound(Uuid)` (NEW ADDITIVE error variant per RFC-0015 §6.3 + §6.8; mirrors existing `WalletError::AgentAlreadyExists(Uuid)` shape; the RFC-0015 amendment to `octo-wallet/src/error.rs` adds this variant).
- **KEEP** — `WalletError::ForbiddenHolderMismatch` (NEW ADDITIVE error variant per RFC-0015 §6.3 + §6.8; HIGH sec fix per §6.2.1; multi-DID enumeration prevention).
- **KEEP** — `WalletError::ReasonContainsControlChars(String)` (NEW ADDITIVE per R21.5 H-2 fix — promoted from DEFERRED to KEEP alongside `validate_reason` primitive; the variant payload is `String`, declared inline as `WalletError::ReasonContainsControlChars(String)` — thiserror tuple-payload enum-variant pattern; RFC-0015 amendment to `octo-wallet/src/error.rs` adds this variant (NOT a standalone newtype struct). **R27.5 C-S1 security fix:** the `String` payload carries hex-escaped code-point notation (e.g., `<U+001B>` for ESC), NOT the raw byte — prevents attacker-byte echo via `Display` impl (terminal-render hijack vector). The variant is consumed by `validate_reason` AND by DEFERRED `transition_agent` write path per §6.2.2 (6)).
- **DEFER** — `transition_agent(...)` write function (RFC-0015 §6.2.2 DEFERRED — depends on RFC-0012-v2 amendment that adds `AuditEventKind::AgentTransition` to Layer A frozen `octo-audit-core`; this variant does not exist in the substrate as of 2026-09-11). The `transition_agent` write function itself is a DEFERRED NEW ADDITIVE per RFC-0015 §6.2.2 (substrate amendment to `octo-wallet/src/agent.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 acceptance paired unblock).
- **DEFER** — `WalletError::AuditUnavailable` (RFC-0015 §6.3 DEFERRED — paired with `transition_agent` write path; DEFERRED NEW ADDITIVE per RFC-0015 §6.8 row `AuditUnavailable` — substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance).
- **DEFER** — `WalletError::AlreadyInTransition(Uuid)` (DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows; write-only; see §6.8 DEFERRED SURFACE).
- **DEFER** — `WalletError::InvalidStateTransition { from, to }` (DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows; write-only; see §6.8 DEFERRED SURFACE).
- **DEFER** — `WalletError::ReasonTooLong(usize)` (DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows; write-only; see §6.8 DEFERRED SURFACE).
...
- **RFC-0012-v2** — `AuditEventKind::AgentTransition { agent_id, from, to, reason }` variant addition to `octo-audit-core` (Layer A frozen; parent `AuditEvent::at_millis_unix` carries the timestamp per R21 M-1 fix — variant payload does NOT duplicate the parent timestamp). **(DRAFT — required substrate amendment for the `transition_agent` write path DEFERRED in §6.2.2 / §6.3 / §6.8 DEFERRED SURFACE.)** Without this amendment, `transition_agent` cannot persist state-machine transitions to the canonical `AppendOnlyAuditSink` per RFC-0012.
...
Layer direction: CLI (Layer C/D) → `octo-wallet` (Layer B) → `octo-audit` (Layer B façade) → `octo-audit-core` (Layer A frozen) per RFC-0016 §6.4 re-export pattern. No reverse deps. No business logic in CLI; substrate owns invariants. No direct B→A edges.

### §6.2 Public surface additions (`crates/octo-wallet/src/agent.rs`)

Five additive items = 3 functions + 2 error variants, layered atop the existing `AgentManifest` / `AgentState` / `AgentSummary` / `AgentFilter` / `CapabilityId` types already present in the same file:
...
3. **`lookup_agent`** — point-lookup function (KEEP per H-6 amendment; substrate-faithful per §6.2.5 lookup_agent)
4. **`WalletError::AgentNotFound(Uuid)`** — error variant (KEEP per R5.5)
5. **`WalletError::ForbiddenHolderMismatch`** — error variant (KEEP per R5.5; NEW per R2)
...
// R23 finding H-1 fix: `AgentFilter` struct definitions anchored explicitly
// to substrate-faithful types (prior R22.5 anchored `holder_did: Option<Did>`
// + `cursor: Option<Cursor>` newtype, but the substrate defines BOTH as
// `Option<String>` per `crates/octo-wallet/src/agent.rs` §`AgentFilter`,
// hard-checked 2026-09-11). The substrate-faithful form drops the
// `Cursor(pub String)` newtype (was speculative at R22.5) and uses the
// canonical `Option<String>` shape for both `holder_did` and `cursor`
// (opaque forward-compat token reserved for Phase 2 multi-page iteration
// per §Implementation Phases).
pub struct AgentFilter {
    pub holder_did: Option<String>, // substrate-faithful per `crates/octo-wallet/src/agent.rs` §`AgentFilter` (hard-checked 2026-09-11; canonical wire form is RFC-0010 string, NOT the typed `Did` newtype — string equality at the substrate boundary is the canonical caller-attestation pattern per §6.2.1)
    pub state: Option<AgentState>,
    pub limit: Option<usize>,    // None = substrate default 1024; hard ceiling 1024; values > 1024 clamp with no error
    pub cursor: Option<String>,  // forward-compat opaque token; reserved for Phase 2 per §Implementation Phases
}
```

- **Filter semantics** — server-side `holder_did == filter.holder_did.unwrap_or(caller_did.as_str().to_owned())`; substrate enforces `filter.holder_did.is_none() || filter.holder_did.as_deref() == Some(caller_did.as_str())` per RFC-0009 §Identity (mismatch → `WalletError::ForbiddenHolderMismatch`; substrate-faithful caller-attestation per §6.2.1 `caller_did: &Did` parameter pattern). **R23 finding H-1 fix:** the comparison is `&str == &str` because `AgentFilter.holder_did` is `Option<String>` (canonical RFC-0010 wire form) — the substrate-faithful canonical access pattern is `as_deref()` on the `Option<String>`, NOT `as_ref().map(Did::as_str)`. The prior R22.5 text (anchoring `Option<Did>`) is superseded by the R23 substrate-faithful correction. `state == filter.state` (exact match, no wildcards); `limit` clamp; `cursor` reserved (ignored on Phase 1 reads). **Canonicalization note (M-11 fix):** the substrate does NOT canonicalize per RFC-0010 — bytes-equality on canonical form is substrate-faithful, but the substrate does not parse the `&str` to the typed `Did` newtype at the substrate boundary (the wire form is `Option<String>` per `AgentFilter.holder_did`). The CLI mission MUST pre-canonicalize via `Did::from_str` (RFC-0010 §Chain-id Derivation) before calling `list_owned_agents`; two semantically-equal DIDs that differ in canonical-form bytes would fail the `&str == &str` comparison at the substrate boundary. This is the substrate-faithful cost of accepting the canonical `Option<String>` wire form (no substrate-side `Did::from_str` re-canonicalization per RFC-0010 §Chain-id Derivation).
- **Return semantics** — empty `Vec` when zero matches (NOT an error per RFC-0011-c §9.3.3 TV-AGT6); summaries sorted by `registered_at_unix DESC`, with secondary sort by `agent_id` (canonical UUID v5; namespace+name based, deterministic per substrate implementation) ASC as deterministic tiebreaker for entries sharing the same `registered_at_unix` (substrate-faithful ordering per §6.6 determinism requirement — per `crates/octo-wallet/src/cli_fns.rs` §`register_agent`, `agent_id` is UUIDv5 derived from `(manifest_digest, active_did)` in the CipherOcto-agent namespace, NOT UUIDv7).
...
- **(1) TOCTOU mitigation** (forward-looking, R23 finding H-6 fix — explicit `try_lock` mode) — the substrate acquires a per-`(holder_did, agent_id)` `parking_lot::Mutex::try_lock()` AFTER looking up holder_did by `uuid`; on contention the second caller observes `WalletError::AlreadyInTransition(Uuid)` immediately (paired with §6.2.2 (7) single-callsite-per-agent); the first caller's lock is held across reason filter + state-machine work + audit append and released after the audit append completes (or rolls back). `current_state` is re-read INSIDE the lock. The lookup-by-uuid reveals `holder_did`; the lock is then keyed on `(holder_did, agent_id)`. The order matters: substrate first resolves the agent to learn its holder_did, THEN takes the per-`(holder_did, agent_id)` `try_lock`; this avoids a parallel lookup-then-relookup pattern that would race on agent reassignment. The `transition_agent` write path (DEFERRED per §6.8 row 1) uses `parking_lot::Mutex::try_lock()` per RFC-0015 §Implementation Phases Phase 2.5 — `parking_lot` is added as a Cargo.toml dependency at the RFC-0015 re-implementation mission YAML §Key Files (paired-DEFER per M-3); the dep does NOT land at RFC-0015 R4.5 acceptance (Layer B years-stable; canonical Rust mutex ecosystem; rationale: `parking_lot::Mutex` provides `try_lock()` without poisoning — Rust std `Mutex::try_lock()` poisons on holder-panic; parking_lot returns lock error without poisoning the lock state, allowing graceful Phase 2.5 deferred-write-path fallback to canonical substrate `WalletError::AlreadyInTransition` rather than poisoned-lock panics). The §6.2.2 (1) Phase 2.5 lock-mode contract depends on this dep. Mark the lock-mode as PLANNED NOT EXISTING today. The lock mode is `parking_lot::Mutex::try_lock()` (non-blocking; immediate `AlreadyInTransition` return on contention) — NOT `lock()` (blocking) which would risk substrate-internal deadlock under load and would not surface a transient-retry error per RFC-0011-c §Error Handling. The lock is released after the audit append completes (or rolls back).
...
- **(5) Audit append + rollback contract** (forward-looking) — every successful transition appends an `AuditEventKind::AgentTransition { agent_id, from, to, reason }` row to the canonical `AppendOnlyAuditSink` per RFC-0012 (forward-looking; write path lands post-0012-v2 acceptance per §6.8 DEFERRED SURFACE). **R21 finding M-1 fix:** the variant payload does NOT carry `at_unix` — the parent `AuditEvent` struct carries `at_millis_unix` per RFC-0012, and a variant-level `at_unix` field would duplicate the parent timestamp (substrate-faithful symmetry: the parent struct is the canonical timestamp carrier; variants carry transition-specific payload only). The substrate-faithful principle: no redundant timestamp fields at the variant payload level when the parent struct already carries a canonical timestamp. The substrate-faithful rollback contract is enumerated below — see RFC-0016 §6.4 (canonical `octo_audit::error::AuditError` re-export shadows the substrate `octo_audit_core::AuditError`; façade-type name canonical in `octo_audit` scope; substrate reachable via fully-qualified `octo_audit_core::AuditError` path per R21 H-1 fix); `SinkSpecific`/`AlreadyExists`/`SequenceGap` live in `octo_audit_core::AuditError` per `crates/octo-audit-core/src/error.rs`. On ANY `octo_audit::error::AuditError` variant returned from `append_audit_event` (post-0012-v2 acceptance per §6.8 DEFERRED SURFACE):
...
- **(7) Single-callsite per agent** (forward-looking, R23 finding H-6 fix — paired with §6.2.2 (1) explicit lock-mode specification) — concurrent `transition_agent` against the same `(holder_did, agent_id)` returns `WalletError::AlreadyInTransition` (transient, retry-safe; CLI surfaces as substrate reason). The `WalletError::AlreadyInTransition(Uuid)` variant is NEW ADDITIVE (lands with the `transition_agent` function per §6.8 row `AlreadyInTransition`; substrate amendment to `octo-wallet/src/error.rs`); the `try_lock` semantics are a Phase 2.5 deferred-write-path contract per §Implementation Phases Phase 2.5 (PLANNED NOT EXISTING today; depends on `parking_lot` Cargo.toml dep + `transition_agent` re-implementation). The Phase 2.5 substrate-faithful implementation uses `parking_lot::Mutex::try_lock()` (non-blocking; per §6.2.2 (1)) — the second caller's `try_lock` returns immediately with `WouldBlock`, which the substrate maps to `WalletError::AlreadyInTransition(uuid)`. Single-callsite-per-agent invariant is the Phase 2.5 lock-mode contract; the lock is held across the FIRST caller's reason filter + state-machine work + audit append per §6.2.2 (1).
...
///   (substrate-faithful diagnostic; the operator sees which control
///   character triggered the rejection). **R27.5 C-S1 security fix:**
///   the `String` carries hex-escaped code-point notation
///   (e.g., `<U+001B>` for ESC, `<U+0000>` for NUL), NOT the raw byte —
///   the `Display` impl MUST NOT echo the raw byte back to the terminal
///   (attacker-byte echo is a terminal-render hijack vector; redactor-clean
///   hex-escaped form prevents log-injection / pager-hijack attacks).
///   NOTE (R21 finding H-2 fix): the variant is KEEP at RFC-0015 R4.5
///   acceptance alongside `validate_reason` (per §6.2.5 H-2 amendment +
///   §6.8 row `ReasonContainsControlChars` removal + §Implementation
///   Phases Phase 2.5 amendment); the primitive's return type resolves
///   to the KEEP variant at R4.5 acceptance. The variant is ALSO
///   consumed by the DEFERRED `transition_agent` write path at Phase 2.5
///   (paired semantics — the write path's control-char filter per
///   §6.2.2 (6) calls `validate_reason` upstream and the variant
...
- **Length semantics:** the primitive enforces a hard 256-char cap at R4.5 KEEP (per R25 finding M-3 promote — the cap was previously the responsibility of `transition_agent` (DEFERRED per §6.2.2 (6)); promoting to the primitive closes the gap with no new substrate amendment beyond what RFC-0015 already plans); `reason.len() > 256` is rejected upfront with `Err(WalletError::ReasonTooLong(usize))`. **Future-cap clock source (M-15 fix):** the future-cap clock source is `std::time::SystemTime::now()` (RFC-0014 §Data Structures `timestamp_unix` field) — the canonical substrate clock for receipt-store writes; NTP skew is bounded by the 86400-second window. **R25 finding M-3 — `WalletError::ReasonTooLong(usize)` PROMOTED to R4.5 KEEP alongside `validate_reason`:** the variant was DEFERRED in §6.8 row `ReasonTooLong`; R25 promotes it to KEEP at RFC-0015 R4.5 acceptance alongside the `validate_reason` primitive (per the M-3 amendment to §6.3 + §6.8 + §Implementation Phases Phase 2.5). The variant payload is `usize` (carrying the offending byte length), declared inline as `WalletError::ReasonTooLong(usize)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct. The variant is consumed by the KEEP `validate_reason` primitive at RFC-0015 R4.5 acceptance AND will be consumed by the DEFERRED `transition_agent` write path at Phase 2.5 (paired semantics). CLI parser caps length at parse time per RFC-0011-c §Security considerations as upstream defense-in-depth.
- **Multi-char return semantics (R22 finding M-25 first-class restatement; R27.5 C-S1 fix):** when `reason` contains multiple control characters (e.g., a string with embedded `\x00\x01\x02`), the primitive returns the FIRST occurrence (deterministic per input per §6.6 determinism requirement); the `String` payload in `WalletError::ReasonContainsControlChars(String)` is the FIRST control character encountered rendered as hex-escaped code-point notation (e.g., `<U+0000>` for the first `\x00`), not the last or any arbitrary one. The order is enforced by single-pass scan from byte index 0 forward; the primitive does NOT aggregate multiple control chars into a list (the variant is single-`String` tuple payload per §6.3; aggregation would require a separate variant or container type).
...
- **Substrate-faithful note:** the primitive lands at R4.5 KEEP because it requires NO substrate amendment (string-level filter on UTF-8 input; pure function). The error variant it returns (`WalletError::ReasonContainsControlChars(String)`) is KEEP at RFC-0015 R4.5 acceptance per R21 finding H-2 fix (the variant payload is `String`, declared inline as `WalletError::ReasonContainsControlChars(String)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct; **R27.5 C-S1 fix:** payload `String` carries hex-escaped code-point notation per the §`validate_reason` return-type annotation, NOT the raw byte, to prevent terminal-render hijack via attacker-byte echo). The primitive is exercisable today as `validate_reason(reason: &str) -> Result<(), WalletError>` once the variant lands (paired with the KEEP primitive at R4.5 acceptance); the DEFERRED `transition_agent` write path at Phase 2.5 consumes the SAME variant (paired semantics — the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The KEEP variant covers both the primitive's standalone test coverage (per TV-WLT-AGT-13..13f) and the DEFERRED write path's paired consumption at Phase 2.5; no re-promotion or re-implementation needed across phases.
...
#### §6.2.5 lookup_agent — KEEP (H-6 amendment — substrate-faithful point lookup)

**Status:** KEEP at RFC-0015 R4.5 acceptance — substrate-faithful point lookup; companion to §6.2.3 `WalletError::AgentNotFound(Uuid)`. The function is exposed at the substrate boundary so the KEEP variant `WalletError::AgentNotFound(Uuid)` has a KEEP source (per H-6 amendment closing the symmetry-only dead-code note in §6.2.3).

```rust
/// Point lookup of an agent manifest by canonical UUID.
///
/// Substrate-faithful to `crates/octo-wallet/src/agent.rs` `AgentManifest`
/// struct. Returns the canonical `AgentManifest` on hit, `WalletError::AgentNotFound(uuid)`
/// on miss (per §6.2.3 NEW ADDITIVE variant).
///
...
- **Where raised:** the function is invoked by `0011-c-agent-show-subcommand` (CLI consumer; RFC-0011-c §9.3.x) for point-lookup before transition operations. KEEP at RFC-0015 R4.5 acceptance — no substrate amendment required beyond the `WalletError::AgentNotFound(Uuid)` NEW ADDITIVE per §6.2.3.
- **Error semantics:** `WalletError::AgentNotFound(uuid)` on miss (NEW ADDITIVE per §6.2.3 — KEEP at R4.5); `WalletError::ForbiddenHolderMismatch` on caller/holder mismatch per §6.2.4.
- **Substrate-faithful symmetry note (H-6 fix):** at R4.5 KEEP, `WalletError::AgentNotFound(Uuid)` now has a KEEP source (`lookup_agent` per §6.2.5) AND the DEFERRED `transition_agent` Phase 2.5 write path per §6.2.2 (3) state-machine authority + TV-WLT-AGT-6 existence check. The symmetry-only dead-code note from R20.5 finding M-12 in §6.2.3 is closed at H-6.
...
| `ReasonContainsControlChars(String)` (NEW, **KEEP** at R21.5; **R27.5 C-S1 payload fix**: `String` payload carries hex-escaped code-point notation per `<U+XXXX>` form, NEVER the raw byte — prevents attacker-byte echo back to terminal via `Display`) | `validate_reason` (per §6.2.5; control-char filter primitive at R4.5 KEEP); ALSO consumed by DEFERRED `transition_agent` write path per §6.2.2 (6). The variant payload is `String`, declared inline as `WalletError::ReasonContainsControlChars(String)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct.                                                            | `InvalidFilter` (16)          | parent reserved             |
| `AlreadyInTransition(Uuid)` (NEW, **DEFERRED**)                                                                                                                                                                                                          | `transition_agent` (write-only; concurrent call; DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows — substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock)                                                                                                                                                                                                                                                             | `AlreadyInTransition` (43)    | §9.8 slot 43                |
| `InvalidStateTransition { from, to }` (NEW, **DEFERRED**)                                                                                                                                                                                                | `transition_agent` (write-only; illegal transition; DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows — substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock)                                                                                                                                                                                                                                                          | `AlreadyInTransition` (43)    | §9.8 slot 43                |
| `ReasonTooLong(usize)` (NEW, **KEEP** per R25 M-3 promote)                                                                                                                                                                                               | `validate_reason` (KEEP per §6.2.5 length cap; consumed by `transition_agent` DEFERRED Phase 2.5 write path per §6.2.2 (6)). The variant payload is `usize` (offending byte length), declared inline as `WalletError::ReasonTooLong(usize)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3. CLI parser caps length at parse time per RFC-0011-c §Security considerations as upstream defense-in-depth. | `InvalidFilter` (16)          | parent reserved             |
| `AuditUnavailable` (NEW, **DEFERRED**)                                                                                                                                                                                                                   | `transition_agent` (write-only; `AppendOnlyAuditSink` unreachable per RFC-0012; DEFERRED NEW ADDITIVE per RFC-0015 §6.8 rows — substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock)                                                                                                                                                                                                                              | `AuditSubstrateNotReady` (52) | RFC-0011-c §9.8 slot 52     |
...
**R25 finding — `InvalidStateTransition` CLI info-loss:** the substrate `{ from, to }` payload is DROPPED at the CLI boundary per the R21 M-6 variant unification; the CLI may surface the cause via the `OctoCliError::AlreadyInTransition(uuid)` Display string (includes `from: <state>` + `to: <state>` concatenated per the `AgentState` `Display` impl).
...
> **Substrate-faithful drift (replaces RFC-0002 §Agent State Machine spec diagram until RFC-0002-v2 lands; R23 finding H-7 fix — drift claim reduced):** the working substate pair `ACTIVE ↔ BUSY` is collapsed into the single observable `Running` variant. Operators querying agent state see `Running` for both currently-idle (was ACTIVE) and currently-executing (was BUSY) agents. The audit row records the canonical `AgentState` transition (Running, etc.) without working/non-working substate distinction; consumers requiring the substate split must consult the registry snapshot at the audit-event timestamp. The prior v1.0 text claimed the `AuditEventKind::AgentTransition` log row carried the prior `BUSY/ACTIVE` distinction internally — this is DROPPED per R23 H-7 because the §6.2.2 (3) payload + §6.8 row 1 variant payload carries no substate field (only `agent_id`, `from`, `to`, `reason` per the RFC-0012-v2 amendment). The drift is now: substrate collapses for both the registry AND the audit log; the working/non-working split is unrecoverable from the audit log alone (consumers must pair audit log with registry snapshot at the audit-event timestamp).
...
| `0011-c-agent-destroy-subcommand.md` | `transition_agent(caller_did, uuid, Terminated, reason)` then audit append — **DEFERRED** per §6.8           | Sub-step 3 (**DEFERRED** — Sub-step 3 substrate amendment lands with RFC-0012-v2 (requires `AuditEventKind::AgentTransition` variant in `octo-audit-core` Layer A frozen) per M-10 amendment) |
...
- **Read determinism** — `list_owned_agents` returns the same result for the same input across runs (substrate is single-writer; the registry is in-memory + persisted atomically per write); sorting is deterministic (`registered_at_unix DESC`, with secondary sort by `agent_id` canonical UUID v5 ASC as deterministic tiebreaker per §6.2.1 — UUIDv5 is namespace+name based per `crates/octo-wallet/src/cli_fns.rs` §`register_agent`; NOT UUIDv7).
...
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `transition_agent(caller_did, uuid, target, reason)` write | RFC-0012-v2: `AuditEventKind::AgentTransition { agent_id, from, to, reason }` in `octo-audit-core` (parent `AuditEvent::at_millis_unix` carries the timestamp per §6.2.2 (5) R21 M-1 fix — variant payload does NOT carry `at_unix`) | `AuditEventKind` has 3 variants only (`Insert`, `Revoke`, `Sync`); NO `AgentTransition` variant. `crates/octo-audit-core/src/event.rs` §`AuditEventKind` enum. | RFC-0012-v2 ACCEPTED + `transition_agent` re-implemented per §6.2.2 (with `caller_did` parameter; holder_did resolved by uuid-lookup; rollback per §6.2.2 (covers SinkSpecific fail-closed + SequenceGap fail-closed + AlreadyExists idempotent-success)) |
...
**R21 finding H-2 amendment — `WalletError::ReasonContainsControlChars(String)` PROMOTED to KEEP at R21.5:** the variant was DEFERRED in §6.8 row `ReasonContainsControlChars` of prior versions; R21.5 promotes it to KEEP at RFC-0015 R4.5 acceptance alongside the `validate_reason` primitive per §6.2.5 (the primitive's return type resolves to the KEEP variant at R4.5 acceptance). The variant payload is `String`, declared inline as `WalletError::ReasonContainsControlChars(String)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct. **R27.5 C-S1 security fix:** the `String` payload carries hex-escaped code-point notation per `<U+XXXX>` form (e.g., `<U+001B>` for ESC), NEVER the raw byte — `Display` impl MUST NOT echo attacker bytes back to the terminal (pager-hijack mitigation). The substrate-faithful symmetry mirrors `WalletError::AgentNotFound(Uuid)` per §6.2.3. The variant is consumed by the KEEP `validate_reason` primitive at RFC-0015 R4.5 acceptance AND will be consumed by the DEFERRED `transition_agent` write path at Phase 2.5 (paired semantics; the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The variant is therefore KEEP at R4.5 and stays KEEP through Phase 2.5 (no re-promotion needed).
...
4. **`register_agent` precedes any read** — substrate assumes the agent_id returned by `register_agent` exists in the in-memory registry before any `list_owned_agents` call returns a non-empty `Vec`; CLI precondition check. **R23 finding H-2 substrate-faithful cross-reference:** `register_agent` is the EXISTING `octo_wallet::cli_fns::register_agent(manifest: &AgentManifest, capability_root: &CapabilityId, active_did: &Did) -> Result<uuid::Uuid, WalletError>` per `crates/octo-wallet/src/cli_fns.rs` §`register_agent` (hard-checked 2026-09-11) — returns the assigned `agent_id` Uuid for subsequent list/destroy calls. RFC-0015 R4.5 KEEP does NOT add a new `register_agent` function — the existing CLI FFI function is the canonical substrate contract. The CLI mission `0011-c-agent-create-subcommand.md` Sub-step 3 calls this existing function (per §6.5 CLI integration contract); the `0011-c-agent-{list,run,destroy,attach}-subcommand` missions consume the read surface or transition surface produced by `register_agent`. Substrate-faithful principle: no parallel `register_agent` abstractions (per [[cipherocto-design-principles]] §No parallel abstractions); RFC-0015 reuses the canonical CLI FFI function. The prior R22.5 text claimed `Result<AgentManifest, WalletError>` which is a substrate-faithful drift (the substrate returns `Uuid`, not `AgentManifest`); R23 H-2 corrects to `Result<uuid::Uuid, WalletError>` per the hard-checked substrate.
5. **Audit substrate is available** — DEFERRED per §6.8 (paired with `transition_agent` write path).
6. **Operator config dir writable** — agent registry persists to `$OCTO_HOME/wallet/agents`; substrate raises `WalletError::Config(String)` on registry-corruption / unwritable paths (per §6.2.1 error semantics + `crates/octo-wallet/src/error.rs` §`WalletError::Config` variant, hard-checked 2026-09-11). CLI surfaces `OctoCliError::NoOctoHome` upstream (exit 27 per `crates/octo-cli/src/error.rs` §`NoOctoHome`) when the substrate path is fully unreachable; the substrate-faithful principle: CLI maps substrate errors to CLI exit codes at the façade boundary per RFC-0011-a §7.4 Substrate `[ADD]` error-envelope pattern. **R21 finding M-5 substrate-faithful note:** the prior v1.0 "no substrate `WalletError` variant enumerated" gap is resolved by clarifying that `WalletError::Config(String)` is the canonical substrate variant covering operator-config-dir-unavailable cases (registry corruption, unwritable path, missing parent dir). The substrate-faithful pattern: substrate carries the canonical error type; CLI surfaces a CLI-specific exit code at the façade boundary. RFC-0015 R4.5 KEEP does NOT add a new `WalletError::OperatorConfigDirUnavailable(PathBuf)` variant — the existing `WalletError::Config(String)` covers the case (substrate-faithful symmetry per [[cipherocto-design-principles]] §Stable Abstractions Principle).
...
3. **Reason field is operator-controlled** — DEFERRED per §6.8 for the write path. When write path unblocks: CLI parser caps at 256 chars + UTF-8 (exit 16 on parse failure per `OctoCliError::InvalidFilter`); substrate enforces control-char filter (`U+0000`-`U+001F`, `U+007F`) at the substrate boundary per §6.2.2 (Reason length + control-char filter) (MEDIUM sec fix). Defense-in-depth: CLI parses length/UTF-8 at parse time; substrate rejects control chars at write time so any future caller (CLI, wallet-on-host daemon, programmatic API) inherits the same control-char filter. Substrate does not interpret the string as code; no shell metachar escape surface. ASCII printable + non-control Unicode (U+0020+) is allowed; control chars U+0000-U+001F and U+007F are rejected per §6.2.2 (Reason length + control-char filter). **C1 range accepted residual (R22 finding M-2 first-class call-out — promoted from TV-WLT-AGT-13f footnote):** the filter scope is `U+0000`-`U+001F` + `U+007F`; the C1 range (`U+0080`-`U+009F`, including `U+0085` NEL, `U+009B` 8-bit CSI, `U+009D` 8-bit OSC) passes the filter (per TV-WLT-AGT-13f). Modern UTF-8 terminals render the C1 range safely; some legacy / non-UTF-8 terminals may interpret them as control sequences. The gap is the accepted residual risk pending RFC-0015-v2 substrate amendment that may widen the filter. Substrate-faithful principle: substrate today does not have a `validate_reason_c1_strict` primitive; the accepted gap is the substrate-faithful cost of accepting before the C1 filter ships.
...
Substrate-level test vectors (`crates/octo-wallet/src/agent.rs` test module). Write-path vectors (TV-WLT-AGT-3 through TV-WLT-AGT-11) are **DEFERRED** per §6.8 — they exercise `transition_agent` which requires RFC-0012-v2 acceptance. TV-WLT-AGT-1, 2, 12, 13, 13b, 13c, 13d, 13e, 13f KEEP at R5.5; 3-11 DEFERRED per §6.8; the `validate_reason` primitive tests (TV-WLT-AGT-13 series) are KEEP at RFC-0015 R4.5 acceptance per §6.2.5 — the primitive requires no substrate amendment and lands independently of the DEFERRED `transition_agent` write path (the primitive has standalone test coverage at RFC-0015 R4.5 KEEP acceptance, paired with the KEEP `WalletError::ReasonContainsControlChars(String)` variant carrying the hex-escaped code-point notation `<U+XXXX>` per R21.5 H-2 + R27.5 C-S1 multi-char return semantics).
...
| TV-WLT-AGT-12  | `list_owned_agents(caller_did, &filter { holder_did: Some("did:octo:other-...") })`                                           | Caller DID ≠ filter `holder_did` (RFC-0010 string form per R23 H-1 substrate-faithful fix) | `Err(WalletError::ForbiddenHolderMismatch)`                                                                                                                                                                                                                                                    | HIGH sec fix (caller-attestation per §6.2.1)                                                                                                    |
...
| TV-WLT-AGT-13b | `validate_reason("\x1b[31mred")` (unit test)                                                                                  | ANSI escape sequence reason                                                                | `Err(WalletError::ReasonContainsControlChars("<U+001B>".to_string()))` (KEEP at R21.5 H-2 fix + **R27.5 C-S1 fix**: variant payload `String` carries hex-escaped code-point notation; `Display` impl MUST NOT echo raw byte)                                                                   | Verifies filter rejects `U+001B` ESC (R27.5 C-S1 expected output uses hex-escaped `<U+XXXX>` notation, NOT raw byte)                            |
...
| TV-WLT-AGT-13d | `validate_reason("\x00null")` (unit test)                                                                                     | NUL byte reason                                                                            | `Err(WalletError::ReasonContainsControlChars("<U+0000>".to_string()))` (KEEP at R21.5 H-2 fix + **R27.5 C-S1 collateral**: variant payload `String` carries hex-escaped code-point notation for the offending `U+0000` NUL per §6.2.5 multi-char return semantics)                             | Verifies filter rejects `U+0000` NUL                                                                                                            |
| TV-WLT-AGT-13e | `validate_reason("\r\n[ADMIN] approved")` (unit test)                                                                         | CRLF-injection reason                                                                      | `Err(WalletError::ReasonContainsControlChars("<U+000D>".to_string()))` (KEEP at R21.5 H-2 fix + **R27.5 C-S1 collateral**: variant payload `String` carries hex-escaped code-point notation for the FIRST occurrence per §6.2.5 multi-char return semantics — `\r` is encountered before `\n`) | Verifies filter rejects `U+000A` LF / `U+000D` CR                                                                                               |
...
| TV-WLT-AGT-13g | `validate_reason("")` (unit test)                                                                                             | Empty reason string                                                                        | `Ok(())` per §6.2.5 docstring + §6.2.2 (6) "empty string allowed"                                                                                                                                                                                                                              | Verifies filter accepts empty reason per §6.2.5 docstring                                                                                       |
| TV-WLT-AGT-13h | `validate_reason("a".repeat(257))` (unit test)                                                                                | Reason > 256 chars (boundary exceedance per §6.2.5 length cap)                             | `Err(WalletError::ReasonTooLong(257))` (KEEP at R25 M-3 promote per §6.3 row + §6.2.5 length cap; payload `usize` is the offending byte length per §6.2.5 + §6.3 R25 M-3 amendment)                                                                                                            | Verifies 256-char length cap enforcement; closes R26 sec MEDIUM #2 (256-char cap boundary)                                                      |
| TV-WLT-AGT-14  | `list_owned_agents(caller_did, &AgentFilter { holder_did: None, limit: Some(0) })`                                            | Empty registry + limit=0                                                                   | `Ok(Vec::new())` (limit=0 not rejected at list path per §6.2.5 validation rules which only reject `Some(0)` for `AuditFilter`; `AgentFilter.limit` is clamp-only per §6.2.1 limit semantics)                                                                                                   | limit=0 edge case (substrate applies limit clamp; returns empty Vec)                                                                            |
| TV-WLT-AGT-15  | `list_owned_agents(caller_did, &AgentFilter { holder_did: None, cursor: Some("invalid") })`                                   | Malformed cursor (forward-compat opaque token per §6.2.1)                                  | `Err(WalletError::InvalidFilter("cursor decode failed".into()))` (R26 H-5 amendment; substrate applies cursor decode at read boundary; malformed cursor is rejected upfront per §6.2.1 filter semantics)                                                                                       | Cursor malformed edge case (cursor reserved for Phase 2; substrate rejects malformed input)                                                     |
...
- **Phase 2 (RFC-0011-c read-only missions)** — CLI missions `0011-c-agent-{create,list,attach}-subcommand` consume the read surface; mutation traces per RFC-0011-c §Test Vectors. **Cursor support (R23 H-1 forward-compat note):** the `cursor: Option<String>` field on `AgentFilter` (per §6.2.1 anchored struct defs) is RESERVED for Phase 2 multi-page iteration; the substrate accepts and returns the cursor as opaque, but the CLI does not surface cursor-based iteration at Phase 2 — the cursor is forward-compat for >1024-agent registries (Future Work §`list_owned_agents` cursor support). The substrate-faithful pattern: the field exists at R4.5 acceptance (no substrate amendment required; pure struct field addition per `crates/octo-wallet/src/agent.rs` §`AgentFilter`, hard-checked 2026-09-11); Phase 2 wires the CLI round-trip, Phase 2.5+ may consume it. The R22.5 speculative `Cursor(pub String)` newtype is removed per R23 H-1 (no substrate amendment required; the substrate uses the canonical `Option<String>` form).
- **Phase 2.5 (RFC-0012-v2 acceptance)** — `AuditEventKind::AgentTransition { agent_id, from, to, reason }` variant lands in `octo-audit-core` (Layer A frozen; parent `AuditEvent::at_millis_unix` carries the timestamp per R21 M-1 fix). This unblocks §6.8 DEFERRED SURFACE: 6 DEFER items total = 5 RFC-0015 §6.8 items + row 6 audit append (paired with RFC-0016-v2 acceptance). The 5 §6.8 items are `transition_agent` + `WalletError::AlreadyInTransition` + `InvalidStateTransition` + `ReasonTooLong` + `AuditUnavailable` land at this phase; row 6 is the audit append of the `AgentTransition` event row (paired with RFC-0016-v2 `append_audit_event` write path). **R21 finding H-2 amendment:** `WalletError::ReasonContainsControlChars(String)` (R27.5 C-S1 fix: payload `String` carries hex-escaped code-point notation) is NOT in the Phase 2.5 unblock list — the variant is KEEP at RFC-0015 R4.5 acceptance (per H-2 fix); the Phase 2.5 paired consumption via `transition_agent` is a no-op re-implementation (the variant already exists).
...
- `crates/octo-wallet/Cargo.toml` — **NO new deps at R4.5 KEEP** per M-3 paired-DEFER fix; the `parking_lot = { version = "0.12", features = ["deadlock_detection"] }` dep (Layer B years-stable; canonical Rust mutex ecosystem) is paired with the DEFERRED `transition_agent` write-path Phase 2.5 lock-mode contract (`parking_lot::Mutex::try_lock()` per §6.2.2 (1)) and lands with the RFC-0015 re-implementation mission YAML §Key Files, NOT at RFC-0015 R4.5 acceptance. Rationale: `parking_lot::Mutex` provides `try_lock()` without poisoning (Rust std `Mutex::try_lock()` poisons on holder-panic; parking_lot returns lock error without poisoning the lock state, allowing graceful Phase 2.5 deferred-write-path fallback to canonical substrate `WalletError::AlreadyInTransition` rather than poisoned-lock panics).
...
- `crates/octo-wallet/src/error.rs` — append `WalletError::AgentNotFound(Uuid)` + `WalletError::ForbiddenHolderMismatch` + `WalletError::ReasonContainsControlChars(String)` + `WalletError::ReasonTooLong(usize)` (4 KEEP variants per §6.3 — `ReasonContainsControlChars` promoted from DEFERRED to KEEP at R21.5 H-2 fix per §6.2.5 + §6.8 H-2 amendment; **R27.5 C-S1 fix:** payload `String` carries hex-escaped code-point notation, NOT raw byte, to prevent attacker-byte echo via `Display` impl; `ReasonTooLong` promoted from DEFERRED to KEEP at R25 M-3 fix per §6.2.5 length cap; both payloads are inline thiserror tuple-payload enum-variant patterns, NOT standalone newtype structs). The DEFERRED variants `AlreadyInTransition(Uuid)` + `InvalidStateTransition { from, to }` + `AuditUnavailable` are NOT added at R4.5 — they land with the RFC-0012-v2 + RFC-0015 re-implementation.
...
| Crate              | Layer                       | Substrate anchor (§symbol ref)                                                                        | Role at RFC-0015 R4.5 KEEP                                                                                                  |
| ------------------ | --------------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `octo-wallet-core` | Layer A frozen (RFC-0009)   | `IdentityHandle` struct per `crates/octo-wallet-core/src/identity.rs` §`IdentityHandle`               | Canonical identity substrate; HSM-bound `caller_did` provenance source for §6.2.1 + §6.2.2 caller-attestation pattern       |
| `octo-wallet`      | Layer B façade (RFC-0011-c) | `crates/octo-wallet/src/agent.rs` §`AgentManifest` + `crates/octo-wallet/src/error.rs` §`WalletError` | Façade; KEEP additive items (`list_owned_agents` + `validate_reason` + `lookup_agent` + 2 error variants) land here at R4.5 |
| `octo-audit-core`  | Layer A frozen (RFC-0012)   | `AuditEventKind` enum (3 variants)                                                                    | Canonical substrate for paired-DEFER `transition_agent` audit append (lands with RFC-0012-v2 per §6.8 row 6)                |

Layer direction: `octo-wallet` (Layer B) → `octo-wallet-core` (Layer A frozen) for HSM-bound identity; `octo-wallet` (Layer B) → `octo-audit-core` (Layer A frozen) for the paired-DEFER audit append. No direct B→A edges in RFC-0015 R4.5 KEEP (the audit-append direction lands with the RFC-0012-v2 + RFC-0015 re-implementation per §6.8). Layer B → Layer A is the canonical façade-to-substrate hop per CLAUDE.md §Architectural Principles.
...
pub fn list_owned_agents(
    ...
    //    `filter.holder_did.is_none() || filter.holder_did.as_deref() == Some(caller_did.as_str())`; mismatch →
    //    ForbiddenHolderMismatch. Per R23 H-1 fix: `AgentFilter.holder_did` is `Option<String>` (canonical RFC-0010 wire form); comparison is `&str == &str` via `as_deref()`. The substrate does not parse to the typed `Did` newtype at the substrate boundary.
...
pub fn transition_agent(
    ...
    //    (parent `AuditEvent::at_millis_unix` carries the timestamp per R21 M-1 fix).
...
| `WalletError::ReasonContainsControlChars(String)`  | `OctoCliError::InvalidFilter(reason)`     | 16       | parent reserved | **KEEP** (R21.5 H-2 fix + **R27.5 C-S1 payload fix**: variant payload `String` declared inline as `WalletError::ReasonContainsControlChars(String)` — thiserror tuple-payload enum-variant pattern, NOT a standalone newtype struct; payload carries hex-escaped code-point notation, NEVER raw byte, to prevent attacker-byte echo via `Display` impl) |
| `WalletError::AlreadyInTransition(uuid)`           | `OctoCliError::AlreadyInTransition(uuid)` | 43       | §9.8 slot 43    | DEFERRED NEW ADDITIVE (R21.5 M-6 fix; substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock per §6.8 rows)                                                                                                                                                                           |
| `WalletError::InvalidStateTransition { from, to }` | `OctoCliError::AlreadyInTransition(uuid)` | 43       | §9.8 slot 43    | DEFERRED NEW ADDITIVE (R21.5 M-6 fix; substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock per §6.8 rows)                                                                                                                                                                           |
| `WalletError::ReasonTooLong(len)`                  | `OctoCliError::InvalidFilter(reason)`     | 16       | parent reserved | **KEEP** (R25 M-3 promote; payload `usize` declared inline as `WalletError::ReasonTooLong(usize)` — thiserror tuple-payload enum-variant pattern, NOT a standalone newtype struct; consumed by KEEP `validate_reason` primitive at R4.5 acceptance AND DEFERRED `transition_agent` write path Phase 2.5)                                                |
| `WalletError::AuditUnavailable`                    | `OctoCliError::AuditSubstrateNotReady`    | 52       | RFC-0011-c §9.8 | DEFERRED NEW ADDITIVE (substrate amendment to `octo-wallet/src/error.rs` landing with the RFC-0015 R4.5 acceptance + RFC-0012-v2 paired unblock per §6.8 rows)                                                                                                                                                                                          |

**R21 finding M-6 amendment:** the CLI variant for `AlreadyInTransition` + `InvalidStateTransition` is renamed from `OctoCliError::InvalidStateTransition { from, to }` to `OctoCliError::AlreadyInTransition(Uuid)` (mirrors substrate payload shape; eliminates asymmetric payload shape between substrate `Uuid` and CLI `{ from, to }` structs). The substrate-faithful principle: CLI variant payload mirrors substrate variant payload (no parallel abstractions). The slot 43 reservation per RFC-0011-c §9.8 is preserved; the variant name changes only.
