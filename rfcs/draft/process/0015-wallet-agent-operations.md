...
- **KEEP** — `WalletError::ReasonContainsControlChars(char)` (R21.5 H-2 fix — promoted from DEFERRED to KEEP alongside `validate_reason` primitive; substrate-faithful diagnostic payload carrying the offending control character; consumed by `validate_reason` AND by DEFERRED `transition_agent` write path per §6.2.2 (6)).
...
///   `validate_reason` (per §6.3 L250 amendment + §6.8 row 5 removal +
...
- **Substrate-faithful note:** the primitive lands at R4.5 KEEP because it requires NO substrate amendment (string-level filter on UTF-8 input; pure function). The error variant it returns (`WalletError::ReasonContainsControlChars(char)`) is KEEP at RFC-0015 R4.5 acceptance per R21 finding H-2 fix (the variant is a simple additive `pub struct` carrying the offending `char` payload, mirroring the substrate-faithful symmetry pattern per [[cipherocto-design-principles]] §Composition over inheritance). The primitive is exercisable today as `validate_reason(reason: &str) -> Result<(), WalletError>` once the variant lands (paired with the KEEP primitive at R4.5 acceptance); the DEFERRED `transition_agent` write path at Phase 2.5 consumes the SAME variant (paired semantics — the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The KEEP variant covers both the primitive's standalone test coverage (per TV-WLT-AGT-13..13f) and the DEFERRED write path's paired consumption at Phase 2.5; no re-promotion or re-implementation needed across phases.
...
| `ReasonContainsControlChars(char)` (NEW, **KEEP** at R21.5) | `validate_reason` (per §6.2.5; control-char filter primitive at R4.5 KEEP); ALSO consumed by DEFERRED `transition_agent` write path per §6.2.2 (6) | `InvalidFilter` (16)          | parent reserved             |
...
**R21 finding H-2 amendment — `WalletError::ReasonContainsControlChars(char)` PROMOTED to KEEP at R21.5:** the variant was DEFERRED in §6.8 row 5 of prior versions; R21.5 promotes it to KEEP at RFC-0015 R4.5 acceptance alongside the `validate_reason` primitive per §6.2.5 (the primitive's return type resolves to the KEEP variant at R4.5 acceptance). The variant signature is `pub struct ReasonContainsControlChars(pub char)` (substrate-faithful diagnostic payload carrying the offending character); the substrate-faithful symmetry mirrors `WalletError::AgentNotFound(Uuid)` per §6.2.3. The variant is consumed by the KEEP `validate_reason` primitive at RFC-0015 R4.5 acceptance AND will be consumed by the DEFERRED `transition_agent` write path at Phase 2.5 (paired semantics; the write path's control-char filter per §6.2.2 (6) calls `validate_reason` upstream). The variant is therefore KEEP at R4.5 and stays KEEP through Phase 2.5 (no re-promotion needed).
...
| `WalletError::ReasonContainsControlChars(char)`    | `OctoCliError::InvalidFilter(reason)`     | 16       | parent reserved | **KEEP** (R21.5 H-2 fix) |
