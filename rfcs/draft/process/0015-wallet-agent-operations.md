...
- **RFC-0015-a** — `octo-wallet` Agent Write-Path Amendment (sibling; write-path surface per §6.8)
- **AuditEventKind `AgentTransition` extension** (future substrate amendment paired with RFC-0015-a acceptance per §6.8)
...
| 3   | `pub fn validate_reason(reason: &str) -> Result<(), WalletError>`                        | `crates/octo-wallet/src/agent.rs` §validate_reason                             | `533b07a4`    |
...
Seven write-path items DEFERRED to RFC-0015-a. Fully spec-ed per [[deferred-vs-unspecified]] so the contract is locked at RFC-0015 acceptance time even though acceptance is paused:
...
| 10  | `WalletError::AlreadyInTransition(Uuid)`                                                                                        | `crates/octo-wallet/src/error.rs` §AlreadyInTransition                                                 | RFC-0015-a §6.3         |
| 11  | `WalletError::InvalidStateTransition { from, to }`                                                                              | `crates/octo-wallet/src/error.rs` §InvalidStateTransition                                              | RFC-0015-a §6.3         |
| 12  | `WalletError::AuditUnavailable(String)`                                                                                         | `crates/octo-wallet/src/error.rs` §AuditUnavailable                                                    | RFC-0015-a §6.3         |
...
pub struct AgentFilter {
    ...
    pub cursor: Option<String>,  // pagination cursor; None = start from beginning; substrate-canonical opaque cursor format
}
```
...
Writes DEFERRED to RFC-0015-a per §6.8.
...
2. **Reason string is UTF-8, ≤256 bytes, no control chars** — substrate enforces byte-length cap + control-char filter (`U+0000`-`U+001F`, `U+007F`) at the substrate boundary per §6.2.5 (validate_reason; substrate measures bytes via `len()` per `crates/octo-wallet/src/agent.rs` §validate_reason length cap). CLI parser caps length at parse time (exit 16 per `OctoCliError::InvalidFilter`) as upstream defense-in-depth per RFC-0011-c §Security considerations.
...
5. **`caller_did` provenance from active identity handle (HSM-bound)** — substrate does NOT authenticate `caller_did`; only enforces `caller_did == agent.holder_did` (per §6.2.1). Fabricated `caller_did` (compromised CLI) passes substrate check trivially. CLI / programmatic caller MUST source `caller_did` from active identity handle (HSM-bound per `octo-wallet/src/identity.rs` §IdentityKey + `octo-wallet/src/identity_record.rs` §IdentityRecord + RFC-0009 §Identity Struct + §HsmAdapter Integration) at Layer B façade boundary. Pattern: `caller_did: &Did` borrowed from HSM-bound identity record retrieved from process session state. Substrate does not look up HSM. Per-process trust boundary at façade. (Same discipline for RFC-0015-a §Implicit Assumptions Audit row 7.)

## Security Considerations

1. **Reason field is operator-controlled** — `validate_reason` enforces byte-length cap (≤256 bytes, per `len()` not codepoint-count) + control-char filter (`U+0000`-`U+001F`, `U+007F`) at the substrate boundary per §6.2.5 (validate_reason) (MEDIUM sec fix). Defense-in-depth: CLI parses length/UTF-8 at parse time; substrate rejects control chars at the primitive boundary so any future caller inherits the same filter. Substrate does not interpret the string as code; no shell metachar escape surface. ASCII printable + non-control Unicode (`U+0020+`) is allowed; control chars `U+0000`-`U+001F` and `U+007F` are rejected. C1 gap per §6.2.5.
...
| UUID echo in `AgentNotFound`   | Compromised CLI | Echo unknown UUID in error payload        | Confirm UUID existence in target DID's registry | Per §Security Considerations row 2: timing differences accepted at KEEP per RFC-0008 §Adversary Analysis                 | Accepted low-severity leak (UUID is operator-supplied; echo confirms DID/UUID pairing exists, not existence; mitigation is per-process trust boundary) |
...
| TV-WLT-AGT-13c | `validate_reason("\u{0085}NEL")` (unit test)                                                                                    | C1 control character (NEL, `U+0085`) reason                              | `Ok(())`                                                                                                                                                                                                                                    | C1 range (`U+0080`-`U+009F`) passes the filter — documented residual gap per §Security Considerations row 1; modern UTF-8 terminals render C1 safely, but some legacy / non-UTF-8 terminals may interpret them as control sequences |
...
| TV-WLT-AGT-20  | `register_agent` deterministic UUIDv5 check: same `(manifest_digest, active_did)` inputs → same UUID across N consecutive calls | Determinism across consecutive register calls                            | All N calls return the same `Uuid` (UUIDv5 derived from `(manifest_digest, active_did)` in the CipherOcto-agent namespace per §6.2.1 substrate-faithful ordering note; NOT UUIDv7)                                                          | Substrate-faithful UUIDv5 determinism (namespace+name based; substrate-faithful determinism per §6.6 determinism requirement)                                                                                                       |
...
- `crates/octo-wallet/Cargo.toml` — **NO new deps at KEEP.** The write-path substrate uses `std::sync::Mutex` (already in `crates/octo-wallet/src/agent.rs` §AGENT_REGISTRY) per RFC-0015-a §6.1 (1).
- `crates/octo-wallet/src/agent.rs` — append `list_owned_agents` + `lookup_agent` + `validate_reason` (existing types; ~80 LoC incl. tests). `transition_agent` is DEFERRED (see §6.8).
...
| `octo-audit-core` | Layer A frozen (RFC-0012)                        | §AuditEventKind enum (3 variants: `Insert`, `Revoke`, `Sync` + `#[non_exhaustive]` per Extension-over-enumeration pattern, per RFC-0012 §AuditEventKind) | Canonical substrate for paired-DEFER `transition_agent` audit append — DEFERRED to Phase 2.5 (lands with future substrate amendment + RFC-0015-a); `octo-wallet` reaches `octo-audit-core` via canonical B→B→A hop (`octo-wallet` → `octo-audit` Layer B façade → `octo-audit-core` Layer A frozen) — pre-existing per substrate-first ordering |

Layer direction (KEEP): `octo-wallet` (Layer B) → `octo-ident` (Layer B, identity substrate per RFC-0010 + RFC-0009) for DID codec + identity substrate hops; canonical façade-to-substrate hops only per CLAUDE.md §Architectural Principles. The `octo-wallet` → `octo-audit` B→B hop is pre-existing on `next` HEAD per substrate-first ordering; the `octo-audit` → `octo-audit-core` B→A hop is the canonical façade hop for `verify_chain` re-export + `AuditError` re-export (RFC-0012 Layer A frozen 3-variant form).

No changes to Layer A crates (`octo-audit-core`, etc.) at RFC-0015 KEEP acceptance; no CLI binary changes; no envelope / redactor / exit-code table changes.
...
`transition_agent` is DEFERRED — see §6.8 (full signature + paired variants + lock-mode contract land with RFC-0015-a paired with the Layer A amendment that adds the `AgentTransition` extension to RFC-0012 (Extension-over-enumeration pattern)).
...
| `WalletError::ForbiddenHolderMismatch`             | `OctoCliError::ForbiddenHolderMismatch`             | 17       | parent reserved | KEEP (substrate-canonical mapping per §exit_codes row 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `WalletError::AuditUnavailable(String)`            | `OctoCliError::AuditSubstrateNotReady`              | 52       | RFC-0011-c §9.8 | DEFERRED NEW ADDITIVE (landed with RFC-0015-a per RFC-0015-a §6.3 + §Appendix B)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

### Appendix C. Mermaid diagram — CLI → substrate → registry flow (KEEP read-only — see §6.8)
...
sequenceDiagram
    ...
    CLI-->>Op: OutputEnvelope<AgentShowOutput> exit 0 or 42 or 17
```
