...
- **KEEP** — `WalletError::ReasonContainsControlChars(char)` (R21.5 H-2 fix — promoted from DEFERRED to KEEP alongside `validate_reason` primitive; the variant payload is `char`, declared inline as `WalletError::ReasonContainsControlChars(char)` — thiserror tuple-payload enum-variant pattern; substrate-faithful symmetric to `WalletError::AgentNotFound(Uuid)` per §6.2.3 — NOT a standalone newtype struct. The variant is consumed by `validate_reason` AND by DEFERRED `transition_agent` write path per §6.2.2 (6)).
...
- **(1) TOCTOU mitigation** (forward-looking, R23 finding H-6 fix — explicit `try_lock` mode) — the substrate acquires a per-`(holder_did, agent_id)` `parking_lot::Mutex::try_lock()` AFTER looking up holder_did by `uuid`; on contention the second caller observes `WalletError::AlreadyInTransition(Uuid)` immediately (paired with §6.2.2 (7) single-callsite-per-agent); the first caller's lock is held across reason filter + state-machine work + audit append and released after the audit append completes (or rolls back). `current_state` is re-read INSIDE the lock. The lookup-by-uuid reveals `holder_did`; the lock is then keyed on `(holder_did, agent_id)`. The order matters: substrate first resolves the agent to learn its holder_did, THEN takes the per-`(holder_did, agent_id)` `try_lock`; this avoids a parallel lookup-then-relookup pattern that would race on agent reassignment. The substrate-faithful primitive is `parking_lot::Mutex::try_lock()` (non-blocking; immediate `AlreadyInTransition` return on contention) — NOT `lock()` (blocking) which would risk substrate-internal deadlock under load and would not surface a transient-retry error per RFC-0011-c §Error Handling. The lock is released after the audit append completes (or rolls back).
...
- **(7) Single-callsite per agent** (forward-looking, R23 finding H-6 fix — paired with §6.2.2 (1) explicit lock-mode specification) — concurrent `transition_agent` against the same `(holder_did, agent_id)` returns `WalletError::AlreadyInTransition` (transient, retry-safe; CLI surfaces as substrate reason). The substrate-faithful implementation uses `parking_lot::Mutex::try_lock()` (non-blocking; per §6.2.2 (1)) — the second caller's `try_lock` returns immediately with `WouldBlock`, which the substrate maps to `WalletError::AlreadyInTransition(uuid)`. Single-callsite-per-agent invariant is the substrate-faithful lock mode; the lock is held across the FIRST caller's reason filter + state-machine work + audit append per §6.2.2 (1).
...
| `AlreadyInTransition(Uuid)` (NEW, **DEFERRED**)             | `transition_agent` (write-only; concurrent call)                                                                                                                                                                                                                                                                                                                                                                       | `AlreadyInTransition` (43)    | §9.8 slot 43                |
| `InvalidStateTransition { from, to }` (NEW, **DEFERRED**)   | `transition_agent` (write-only; illegal transition)                                                                                                                                                                                                                                                                                                                                                                    | `AlreadyInTransition` (43)    | §9.8 slot 43                |
...
| `WalletError::AlreadyInTransition(uuid)`           | `OctoCliError::AlreadyInTransition(uuid)` | 43       | §9.8 slot 43    | DEFERRED (R21.5 M-6 fix)                                                                                                                                                                            |
| `WalletError::InvalidStateTransition { from, to }` | `OctoCliError::AlreadyInTransition(uuid)` | 43       | §9.8 slot 43    | DEFERRED (R21.5 M-6 fix)                                                                                                                                                                            |
