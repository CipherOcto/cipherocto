...
> **Substrate-faithful note:** rows 2-5 carry the 4 RFC-0016 additive substrate variants in substrate enum declaration order per Trail v1.2 row 12.
...
6. **`#[non_exhaustive]` extension surface** — see Trail v1.5 row 20.
...
| ------------------------------------ | --------------------------------- | ------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------- |
| Append-rollback via sink bypass      | Compromised CLI                   | Forged AuditEvent row                                               | Fabricate transition log                    | see §Security                              | Substrate bug = total compromise (low)                                    |
| Read-during-write race               | Concurrent reads                  | Read inconsistency                                                  | Data inconsistency                          | see §Security                              | Transient (retry-safe)                                                    |
| Cross-tenant receipt read            | Co-tenant on shared receipt store | Read another tenant's receipts                                      | Reconnaissance / enumeration                | see §Security                              | In-process co-tenant reads not blocked (per-process trust boundary)       |
| CLI-shape error variant leakage      | Compromised CLI                   | Surface substrate error payload containing secret                   | Leak private key / path / mnemonic          | see §Security                              | Scrubber pattern miss = leakage window (low, scrubber covers 18 patterns) |
