...
| Downgrade to bare `ReceiptId` lookup | Compromised CLI                   | Bypass `ReceiptId` newtype via `From<[u8; 32]>` for unmapped digest | Denial of service via reverse-mapping panic | see §6.4 (substrate substrate contract per RFC-0014)                                                 | Substrate bug = panic surface (low, RFC-0014 substrate contract enforces) |
