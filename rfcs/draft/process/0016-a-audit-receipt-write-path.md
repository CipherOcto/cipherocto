...
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `append_audit_event`                                | see §6.2                                                                                                                          | RFC-0012 (AuditEventKind extensions)                |
...
// (cfg-gated per §Implicit Assumptions item 7).
...
// Type alias per CLAUDE.md §Extension over enumeration (Layer B aliases Layer A)
...
> **Substrate-faithful note:** rows 2-5 carry the 4 RFC-0016 additive substrate variants in substrate enum declaration order per Trail v1.2 row 12. Row 1 collapse-group substrate enum declaration order is `SequenceGap` (1) + `AlreadyExists` (2) + `SinkSpecific` (3) + `ChainHashMismatch` (8) — non-contiguous in substrate enum (positions 1, 2, 3, 8) per `octo-audit-core/src/error.rs` declaration order; the 4 RFC-0016 additive variants occupy positions 4-7 between them.
...
| 6           | adapter-type-name substring registry — applies via `scrub_adapter_error_with(s, &adapter_types)` from caller-supplied `&[&str]` registry (e.g., `&["StoolapAuditSink"]` per `ADAPTER_TYPES` const in `crates/octo-audit/src/storage/stoolap.rs`); adapter types are NOT a closed set per CLAUDE.md §Extension over enumeration (extension surface, explicit registry parameter) | substring replace (no regex) → `<redacted-adapter>`                                        | `ADAPTER_TYPES` const in `crates/octo-audit/src/storage/stoolap.rs` + `octo_audit::scrub::scrub_adapter_error_with` |
...
**Acceptance criterion:** acceptance of RFC-0012 without canonical-bytes-on-write at the Layer B façade is a regression on the append-rollback attack surface (invariant ownership per §Key Files).
...
6. **`#[non_exhaustive]` extension surface** — see Trail v1.5 row 20 + Trail v1.6 row 28 (substrate-faithful 4 Layer A frozen + 1 Layer B + 1 Layer C enumeration).
7. **`AuditEventKind::AgentTransition` cfg-gate** — see Trail v1.5 row 21.
...
2. **CLI exit-code additions** — see §6.7 (slot allocation per RFC-0011 §Exit Codes)
3. **Façade type-alias additions** — adds `StatusRef` per §6.6
4. **Substrate-faithful `list_receipts` signature** — see §6.1 row + §6.5
...
See §Pairing invariant for pairing rationale + crate-layer anchors + 5-step acceptance order.

- **Phase 0 (RFC-0016 acceptance at R2 KEEP)** — read surface lands; this amendment DEFERRED
- **Phase 1 (RFC-0012 acceptance)** — `AuditEventKind` extensions + single-writer lock on `AppendOnlyAuditSink::append` (substrate trait + canonical-bytes-on-write per §6.10)
...
- **Phase 4 (RFC-0016 acceptance — paired with Phase 1 + 2 + 3)** — write-path + projection + ACL + scrubber surface per §6.1
...
- `crates/octo-audit-core/src/chain.rs` — RFC-0012: see §6.10 (`canonical_bytes` + `compute_chain_hash` free functions, NOT trait methods)
...
- `crates/octo-audit-core/src/sink.rs` — RFC-0012: see §6.11 (`AppendOnlyAuditSink::append` single-writer type-level via `&mut self`; canonical-bytes-on-write enforced at Layer B façade per §6.10, NOT at this trait)
...
| `octo-wallet`          | Layer B façade (RFC-0015) | `WalletError` (see §Implicit Assumptions item 6)                                                       | Per-`WalletError` variant substrate attestation + `transition_agent` rollback contract  |
...
- **paired with substrate amendment** — see §Pairing invariant
...
This section documents per-amendment substrate-faithful sweeps that reconcile RFC text to paired implementation substrate reality. Each amendment entry cites the DRY loop round + reviewer that flagged the drift + the substrate ground-truth identifier.
...
| 4   | §Implicit Assumptions: add `#[non_exhaustive]` extension-surface item                                                                                           | see CLAUDE.md §Extension over enumeration                                                                                                                                                                             | §Implicit Assumptions item 6 documents the `#[non_exhaustive]` set (per Trail v1.5 row 20 canonical six-type set); §6.7 substrate column notes the catch-all arm requirement for the per-variant `From<AuditError>` conversions |
...
| #   | Amendment                                                                | Substrate ground truth                                                                                                                                                                                                                                                                                      | Acceptance criterion                                                                                                                                          |
| --- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 20  | §Implicit Assumptions item 6 `#[non_exhaustive]` set extended to 6 enums | Layer A frozen per RFC-0012 + RFC-0014-v2: `AuditError` (8-var) + `AuditEventKind` + `AuditChainError` + `ReceiptStatus` (4-var). Layer B façade per RFC-0015: `WalletError`. Layer C consumer-side per RFC-0011: `OctoCliError`. All 6 carry `#[non_exhaustive]` per CLAUDE.md §Extension over enumeration | §Implicit Assumptions item 6 documents the six-enum `#[non_exhaustive]` set with per-enum substrate anchors (without file:line per [[no-line-refs-anywhere]]) |
...
| 22  | §IA #6: `#[non_exhaustive]` extension surface + cfg-gate acknowledgment  | §IA #6 cross-refs Trail v1.5 row 20; §IA #7 cross-refs Trail v1.5 row 21 (single source per R30.5b si simplification)                                                                                                                                                                                       | §IA #6 cites row 20; §IA #7 cites row 21 (per-item scope)                                                                                                     |
...
| 25  | §6.10 + §6.11 + §Implementation Phases Phase 1 + §Key Files row + Appendix B Mermaid: phantom `RFC-0012 §S5` (`AuditEvent` struct unchanged) → `§S2` (`AppendOnlyAuditSink::append` invariants canonical) | RFC-0012-v2 §S2 declares `AppendOnlyAuditSink::append` invariants                                                                                                 | All 5 sites (§6.10 + §6.11 + Phase 1 + Key Files row + Appendix B Mermaid) cite RFC-0012-v2 §S2 (enumerated per single source principle)  |
...
| 28  | Trail v1.5 row 20: ambiguous 6-enum arithmetic → explicit Layer-by-Layer classification (4 Layer A + 1 Layer B + 1 Layer C)                                                                               | 4 Layer A frozen: `AuditError` + `AuditEventKind` + `AuditChainError` + `ReceiptStatus`; 1 Layer B façade: `WalletError`; 1 Layer C consumer-side: `OctoCliError` | Trail v1.5 row 20 enumerates all 6 enums by name with correct Layer classification                                                        |
