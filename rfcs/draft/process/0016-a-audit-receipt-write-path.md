...
> **v1.1 Substrate-Faithful Sweep (2026-09-14):** §6.9 scrubber count 13 → 18 (substrate canonical count per `octo_audit::scrub` module docstring); Pattern 12 char range "64 chars" → "16-64 hex chars" (R6.5 regex relaxation); §6.7 mapping table adds `AuditError::ChainHashMismatch { event_id }` row (R2.5 collapse); §Implicit Assumptions adds `#[non_exhaustive]` extension-surface item (Layer A frozen contract on `AuditError` + `AuditChainError` + `AuditEventKind`). Driven by paired implementation R1-R11 DRY loop substrate ground-truth verifications; no design-intent changes — text catches up to substrate reality. See §Substrate-Faithful Amendment Trail for per-amendment acceptance criteria.
...
> **Substrate-faithful note (R2.5 collapse):** `AuditError::ChainHashMismatch { event_id }` is the collapsed form per the R2.5 review (struct variant with `event_id: u64`; per-variant `event_id` carries the failing chain-link row identifier). Pre-R2.5 the substrate carried two variants (`ChainHashMismatch { expected: [u8; 32], got: [u8; 32] }` carrying raw digest bytes plus `ChainLinkBroken { event_id: u64 }`); the collapse removed the raw digest leak surface per RFC-0012-v3 §Adversary Analysis. The paired RFC-0011-a `#[error(transparent)] From<AuditError>` conversion at `octo-cli/src/error.rs` maps the collapsed `ChainHashMismatch { event_id }` into `OctoCliError::Internal(reason)` (CLI exit 64) per the canonical `[ADD]` envelope pattern — the `event_id` surfaces as the `<reason>` payload for operator diagnostics.
...
The substrate-side scrubber applies the canonical 18-pattern list before constructing `SinkSpecific(String)` per §Compatibility #4. The 18 patterns split into 9 closed-form + 1 extension-surface + 8 substrate-extended forms per substrate `octo_audit::scrub` (canonical module docstring):

1. Ed25519 private key hex (64 chars) — substrate Pattern 1 (hex ≥32 char lookaround-anchored)
2. Capability-secret base64 (padded + unpadded) — substrate Pattern 14
3. BLS12-381 Fr scalar hex (64 chars = 32 bytes per RFC wire form) — substrate Pattern 1 (subset)
4. secp256k1 64-char hex — substrate Pattern 1 (subset)
5. BIP39 mnemonic (12/15/18/21/24-word) — substrate Pattern 15
6. JWT three-segment form (`header.payload.signature`) — substrate Pattern 16
7. WIF base58 (51 chars) — substrate Pattern 17
8. Filesystem absolute path (POSIX `/...`, Windows `C:\...`, macOS `/Users/...`) → `<OCTO_HOME>/...` canonical placeholder — substrate Pattern 2
9. PGP private key block (`-----BEGIN PGP PRIVATE KEY BLOCK-----`) — substrate Pattern 11
10. OpenSSH private key (`-----BEGIN OPENSSH PRIVATE KEY-----`) — substrate Pattern 12
11. PEM block (`-----BEGIN ... PRIVATE KEY-----`) — substrate Pattern 13
12. X.509 cert serial `0x`-prefixed hex (16-64 hex chars per R6.5 regex relaxation; RFC-5280 §5.1.2.3 permits 1-20 octets = 2-40 hex after `0x`, observed certs use 8-16 bytes = 16-32 hex; Pattern 1 hex ≥32 catch is insufficient because `0x` prefix boundary is not word-boundary-aligned) — substrate Pattern 18
13. `<REDACTED>` marker idempotency (preserve verbatim, do NOT double-scrub per R21 L-2) — substrate inline guard (no RE_* constant)
14. Table-name refs (PostgreSQL + Stoolap/SQLite forms: `relation "x" does not exist`, `no such table: x`) — substrate Pattern 3
15. SQLSTATE prefixes (5-char SQL standard error codes) — substrate Pattern 4
16. io error chains (`os error N`) — substrate Pattern 5
17. URL credentials (`scheme://user:pass@host`, IPv6-literal-aware balanced-bracket) — substrate Pattern 5b
18. ANSI-CSI escape sequences (`\x1b\[...m`) — substrate Pattern 5c

**Substrate-canonical addition (Pattern 6):** adapter-type-name substring registry — applies via `scrub_adapter_error_with(s, ADAPTER_TYPES)`. Adapter types are NOT a closed set per CLAUDE.md §Extension over enumeration; Pattern 6 is extension surface applied via explicit registry parameter. Substrate module docstring L45 codifies the 18-pattern split as `Patterns 1-5e (9 regexes) + Pattern 6 (substring) + Patterns 11-18 (8 regexes) = 18 patterns`. This RFC §6.9 enumeration is the user-facing naming; substrate pattern numbers (1, 2, 3, 4, 5, 5b, 5c, 5d, 5e, 6, 11-18) are the canonical substrate identifiers per `octo-audit::scrub` module docstring.
...
6. **`#[non_exhaustive]` extension surface** — `octo_audit_core::AuditError` + `octo_audit_core::AuditChainError` + `octo_audit_core::event::AuditEventKind` carry `#[non_exhaustive]` per CLAUDE.md §Extension over enumeration pattern (Layer A frozen contract; additive variants land without central enum edit per RFC-0012-v2 §Extension Surface). The CLI-shape envelope conversion in §6.7 MUST use `#[non_exhaustive]`-aware matching (catch-all `_ => map_to_internal(reason)` arm) to avoid breakage when substrate adds new variants at acceptance-time or at future paired-acceptance amendments.
...
- `crates/octo-audit/src/scrubber.rs` — NEW; canonical 18-pattern substrate-side scrubber
...
| --- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | §6.9 scrubber count 13 → 18                                                                     | `octo_audit::scrub` module docstring: "Patterns 1-5e (9 regexes) + Pattern 6 (substring) + Patterns 11-18 (8 regexes) = 18 patterns"                                                                 | §6.9 enumerates all 18 patterns with substrate pattern numbers (1, 2, 3, 4, 5, 5b, 5c, 5d, 5e, 6, 11-18) alongside RFC user-facing naming (1-18) |
| 2   | §6.9 Pattern 12 char range: 64 → 16-64 hex                                                      | `octo_audit::scrub` Pattern 18 regex `0x[A-Fa-f0-9]{16,64}` per R6.5 reconciliation                                                                                                                  | §6.9 Pattern 12 user-facing entry documents 16-64 hex range with R6.5 rationale                                                                  |
| 3   | §6.7 mapping table: add `AuditError::ChainHashMismatch { event_id }` row                        | `octo_audit_core::error::AuditError::ChainHashMismatch { event_id }` collapsed variant per R2.5                                                                                                      | §6.7 table includes the row mapping substrate → CLI-shape `OctoCliError::Internal(reason)` (CLI exit 64) with R2.5 rationale note                |
| 4   | §Implicit Assumptions: add `#[non_exhaustive]` extension-surface item                           | `octo_audit_core::error::AuditError` (L15) + `octo_audit_core::error::AuditChainError` (L105) + `octo_audit_core::event::AuditEventKind` (L39) carry `#[non_exhaustive]` per Layer A frozen contract | §Implicit Assumptions item 6 documents `#[non_exhaustive]` + the `catch-all _ => map_to_internal(reason)` matching requirement                   |
...
**Amendment acceptance test:** every amendment lands at substrate-faithful parity with paired implementation `crates/octo-audit/src/scrub.rs` (18 patterns) + `crates/octo-audit-core/src/error.rs` (`ChainHashMismatch` + `#[non_exhaustive]`) + `crates/octo-audit-core/src/event.rs` (`#[non_exhaustive]` on `AuditEventKind`). Pre-R1 implementation substrate code review (R1-R11 5-len DRY loop, R6.5 + R9.5 + R11.5 fix rounds) confirmed the canonical 18-pattern count + `ChainHashMismatch` collapse + `#[non_exhaustive]` substrate state.

## Version History

- v1.1 (2026-09-14) Substrate-Faithful Sweep: §6.9 scrubber count 13 → 18 + Pattern 12 char range 64 → 16-64 hex + §6.7 mapping table `ChainHashMismatch` row + §Implicit Assumptions `#[non_exhaustive]` extension-surface item. Driven by paired implementation R1-R11 DRY loop; no design-intent changes.
