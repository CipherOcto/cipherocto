...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | -------- | ---------------------------- |
| `SequenceGap { event_id, prev }` / `AlreadyExists(u64)` / `SinkSpecific(String)` / `ChainHashMismatch { event_id }` (4 substrate variants; row 1 collapse → `Internal`; row 1 includes 3 pre-existing variants + `ChainHashMismatch` collapsed at R2.5; CLI conversion uses `sanitize_substrate_error(&cap_substrate_payload(&format!(...)))` per-variant body per `octo-cli/src/error.rs` per-variant arm block) | `OctoCliError::Internal(reason)`         | 64       | parent reserved              |
...
| 6           | adapter-type-name substring registry — applies via `scrub_adapter_error_with(s, ADAPTER_TYPES)` from `octo_audit::scrub::ADAPTER_TYPES`; adapter types are NOT a closed set per CLAUDE.md §Extension over enumeration (extension surface, explicit registry parameter) | substring replace (no regex) → `<redacted-adapter>`                                        | `octo_audit::scrub::scrub_adapter_error_with` |
...
6. **`#[non_exhaustive]` extension surface** — see Trail v1.5 row 20 for canonical five-enum `#[non_exhaustive]` set with substrate-ground-truth citations. Per-variant `From<AuditError>` conversions in §6.7 MUST use a catch-all arm (`_ => Self::Internal(sanitize_substrate_error(&format!("audit substrate error: {e}")))` per `octo-cli/src/error.rs` catch-all arm) to avoid breakage when substrate adds new variants at acceptance-time or at future paired-acceptance amendments. `AuditEventKind::AgentTransition` is cfg-gated under `#[cfg(feature = "octo-audit-internal")]` per `octo-audit-core/src/event.rs` (see §6.2 + §6.10 + Trail v1.5 row 21); the catch-all arm must not assume the variant is visible at the call site.
...
| ---------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `octo-audit-core`      | Layer A frozen (RFC-0012) | `AuditEventKind` (extended) + `AppendOnlyAuditSink` (`&mut self` single-writer enforcement, Rust borrow-checker type-level; computes chain_hash via `compute_chain_hash` free function) + `canonical_bytes` + `compute_chain_hash` free functions | Canonical substrate write surface (encoding owned by Layer A)                           |
