...
CLI surfaces the `u64` as a decimal integer via `Display` impl (canonical wire form for `octo audit show <receipt-id>`). The 32-byte hex form referenced by RFC-0011-a §7.4 is mapped at the CLI boundary via `From<[u8; 32]> -> ReceiptId` only after the substrate accepts RFC-0014-v2 amendment to add the digest-to-id mapping function; until then, CLI surfaces `u64` decimal form directly.
...
| Façade variant (R20.5 finding L-2 rename) | CLI variant                              | CLI exit | RFC-0011-a slot            | Redactor contract                                                                                                                            |
| ----------------------------------------- | ---------------------------------------- | -------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `AuditError::ReceiptNotFound(decimal)`    | `OctoCliError::ReceiptNotFound(decimal)` | 17       | RFC-0011-a §Error Handling | `decimal` is canonical decimal `u64` form of the substrate `Receipt::receipt_id` (per §6.2.2 + §6.2.6); redactor-clean (no secret material). |
...
| Façade variant (R20.5 finding L-2 rename) | CLI variant                              | CLI exit | RFC-0011-a slot            | Status   |
| ----------------------------------------- | ---------------------------------------- | -------- | -------------------------- | -------- |
| `AuditError::ReceiptNotFound(decimal)`    | `OctoCliError::ReceiptNotFound(decimal)` | 17       | RFC-0011-a §Error Handling | KEEP     |
