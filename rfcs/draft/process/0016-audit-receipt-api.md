...
- `AuditEventKind::AgentTransition` extension on `octo-audit-core` (Layer A frozen) → RFC-0012-v2
- `Receipt` field extensions (`model`, `cost_dqa`, `capability_root`, `subject_did`) on `octo-settlement-core` (Layer A frozen) → RFC-0014-v2
- Canonical `ReceiptStatus { Ok, Partial, Reject }` enum on `octo-settlement-core` → RFC-0014-v2
...
pub fn list_receipts(filter: &AuditFilter) -> Result<Vec<octo_settlement::Receipt>, octo_audit_core::AuditError> {
    ...
    // 4. Sort by canonical Receipt::timestamp_unix DESC (substrate-faithful per RFC-0014 §Data Structures)
...
- UNCONDITIONAL — no `subject_did` field (DEFERRED to RFC-0016-a per RFC-0014-v2 substrate amendment)
- UNCONDITIONAL — no `status` field (DEFERRED to RFC-0016-a per RFC-0014-v2 substrate amendment)
...
- **Read determinism** — `list_receipts` returns the same results for the same filter across runs (sort key is canonical `Receipt::timestamp_unix` per RFC-0014 §Data Structures; secondary sort `Receipt::receipt_id` ASC as deterministic tiebreaker)
...
| ------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------- |
| `append_audit_event`                                   | RFC-0012-v2 (Layer A `AuditEventKind` extensions)    | RFC-0016-v2 + RFC-0012-v2 accepted                            |
| `ReceiptId(pub u64)` newtype                           | RFC-0014-v2 (digest-to-id reverse-mapping)           | RFC-0014-v2 accepted + `receipt_id_for_digest` function added |
| `ChainHash(pub [u8; 32])` newtype                      | RFC-0012-v2 (`append_audit_event` return type)       | RFC-0012-v2 accepted + write path implemented                 |
| `ReceiptStatus { Ok, Partial, Reject }` enum           | RFC-0014-v2 (canonical substrate enum)               | RFC-0014-v2 accepted                                          |
| `ReceiptSummary` projection struct                     | RFC-0014-v2 (`Receipt` field extensions)             | RFC-0014-v2 accepted + projection defined                     |
| `AuditFilter.subject_did: Option<Did>` ACL             | RFC-0014-v2 (`Receipt.subject_did` field)            | RFC-0014-v2 accepted + RFC-0016-v2 ACL implementation         |
| `AuditFilter.status: Vec<StatusRef>` multi-valued      | RFC-0014-v2 (canonical substrate enum)               | RFC-0014-v2 accepted + RFC-0016-v2 façade field type change   |
| `AuditEventKind::Redaction` variant                    | RFC-0012-v2 (4th enum variant)                       | RFC-0012-v2 accepted + RFC-0016-v2 accepted                   |
...
| Canonical-bytes-on-write invariant                     | RFC-0012-v2 (`AppendOnlyAuditSink::canonical_bytes`) | RFC-0012-v2 accepted + chain-link verification                |
| Single-writer lock + read-stalls-while-write invariant | RFC-0012-v2 (`AppendOnlyAuditSink::append` lock)     | RFC-0012-v2 accepted + concurrent reader stall                |
...
3. **Clock monotonicity for `timestamp_unix`** — RFC-0014 `Receipt::timestamp_unix` is monotonic per RFC-0014 §Data Structures; surface reads assume canonical monotonic ordering
...
4. **Substrate-side scrubber defense-in-depth** — at R2 KEEP, the substrate's only error variant carrying a string payload is `octo_audit_core::AuditError::SinkSpecific(String)`. The substrate-side scrubber applies canonical redaction patterns (private key hex, ed25519 hex, capability-secret base64, BLS12-381 Fr scalar hex, secp256k1 64-char hex, BIP39 mnemonic, JWT three-segment form, WIF base58, filesystem absolute path) before constructing the variant. These patterns are defense-in-depth substrate-side scrubbers; the CLI `OctoCliRedactor` per RFC-0011-a §Redaction performs the canonical second-pass sweep at output time. The patterns do NOT add new `OctoCliRedactor` rules.
...
- **Phase 3 (RFC-0016-a acceptance — paired with RFC-0012-v2 + RFC-0014-v2 + RFC-0011-a)** — restores DEFERRED surface per §6.8
...
- **`AuditFilter.subject_did: Option<Did>` ACL** — RFC-0016-a per §6.8 (requires RFC-0014-v2 substrate field + RFC-0016-v2 façade ACL)
- **`AuditFilter.status: Vec<ReceiptStatus>` multi-valued** — RFC-0016-a per §6.8 (requires RFC-0014-v2 substrate enum + RFC-0016-v2 façade field type change)
- **`AppendOnlyAuditSink::append` write path** — RFC-0016-a per §6.8 (requires RFC-0012-v2 `AuditEventKind` extensions + canonical-bytes-on-write invariant)
- **`ReceiptStatus::Unknown` arm** — Raw escape hatch if RFC-0014-v2 substrate acceptance adds it
...
- RFC-0012 — Audit Substrate (Layer A frozen core; provides `AuditEvent` + `AuditEventKind` + `AppendOnlyAuditSink`); **RFC-0012-v2 required for §6.8 DEFERRED SURFACE** (`append_audit_event` write path)
- RFC-0014 — Settlement Substrate (Layer A frozen core; provides canonical `Receipt` struct per §Data Structures); **RFC-0014-v2 required for §6.8 DEFERRED SURFACE** (`ReceiptStatus` enum + `ReceiptSummary` projection + `AuditFilter.subject_did`)
- RFC-0016-a — Audit Receipt Write-Path Amendment (sibling; DEFERRED surface paired with RFC-0012-v2 + RFC-0014-v2 + RFC-0011-a)
