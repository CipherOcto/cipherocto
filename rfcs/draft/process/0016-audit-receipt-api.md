...
- **RFC-0016-a** (sibling amendment) — Audit Receipt Write-Path: `append_audit_event` + `ReceiptId` + `ChainHash` + `ReceiptStatus` + `ReceiptSummary` + `AuditFilter.subject_did` ACL + CLI-shape error variants + `ScrubbedAuditError`. DEFERRED to paired acceptance with future Layer A substrate amendments adding the `AuditEventKind` extensions (per RFC-0012 (CLAUDE.md §Extension over enumeration pattern)) + `ReceiptStatus` enum + `Receipt` field extensions (per RFC-0959-ask §Data Structures) + RFC-0011-a (CLI-shape error envelope). See `rfcs/draft/process/0016-a-audit-receipt-write-path.md`.
...
- `AuditEventKind` `AgentTransition` extension on `octo-audit-core` (Layer A frozen) per RFC-0012 (CLAUDE.md §Extension over enumeration pattern) (typed-discriminator namespace, NOT central-enum variant addition)
...
- Chain integrity: runs `compute_chain_hash` (RFC-0012 per-façade hash function for canonical bytes) on each row to verify receipt-store integrity before projection; on failure returns `AuditError::SinkSpecific("chain verification failed".into())` (NO row index per position-disclosure mitigation). The substrate's `verify_chain(&[AuditEvent])` operates on audit events per `crates/octo-audit-core/src/chain.rs`; receipt-store integrity uses the same BLAKE3-256 chain-hash primitive via `compute_chain_hash`, applied row-by-row to the canonical `Receipt::settlement_hash` chain.
...
pub fn get_receipt(id: &u64) -> Result<octo_settlement::Receipt, octo_audit_core::AuditError> {
    ...
    // 2. Recompute `compute_chain_hash(&canonical_bytes(&receipt))` (RFC-0012) for the queried row
...
- Chain integrity check at read boundary via per-row `compute_chain_hash` (RFC-0012 chain-hash primitive; `verify_chain(&[AuditEvent])` operates on audit events, not receipt rows, per substrate signature)
...
- UNCONDITIONAL — no `subject_did` field (DEFERRED to RFC-0016-a per future RFC-0014 substrate amendment adding `Receipt.subject_did`)
- UNCONDITIONAL — no `status` field (DEFERRED to RFC-0016-a per future RFC-0014 substrate amendment adding canonical `ReceiptStatus` enum)
- UNCONDITIONAL — no `capability_root` field (DEFERRED to RFC-0016-a per future RFC-0014 substrate amendment adding `Receipt.capability_root`; the KEEP substrate `Receipt` per `crates/octo-settlement-core/src/receipt.rs` §`Receipt` has 6 fields only: `receipt_id`, `ask_id`, `settlement_hash`, `router_id`, `router_sig`, `timestamp_unix`)
- UNCONDITIONAL — no `model` field (DEFERRED to RFC-0016-a per future RFC-0014 substrate amendment adding `Receipt.model`)
...
- **Chain integrity** — chain hashes verified end-to-end via `compute_chain_hash` from RFC-0012 applied to canonical `Receipt::settlement_hash` (the substrate `verify_chain(&[AuditEvent])` operates on audit events per `crates/octo-audit-core/src/chain.rs` §`verify_chain`, NOT on receipt rows); any tampering breaks the chain
...
The following DEFERRED surface ships with `0016-a-audit-receipt-write-path.md`. None of these features is provided by any currently Accepted substrate amendment; the §6.8 column cites the canonical substrate RFC (§Extension over enumeration or §Data Structures) where the future amendment will land:

| Deferred feature                                       | Canonical substrate anchor                                                                                                                                                            | Unblock condition                                                                                            |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `append_audit_event`                                   | RFC-0012 (CLAUDE.md §Extension over enumeration pattern) (`AuditEventKind` typed-discriminator namespace)                                                                             | RFC-0016-a + future substrate amendment adding the `AuditEventKind` extension + RFC-0011-a accepted          |
| `ReceiptId(pub u64)` newtype                           | RFC-0959-ask §Data Structures (`Receipt` field extensions)                                                                                                                            | RFC-0016-a + future substrate amendment adding `Receipt` field extensions + `receipt_id_for_digest` function |
| `ChainHash(pub [u8; 32])` newtype                      | RFC-0012 (parent §Module Layout) `event` (`AuditEvent` `chain_hash` field; already present at `crates/octo-audit-core/src/event.rs`); newtype wrapping is façade-only                 | RFC-0016-a + write path implemented                                                                          |
| `ReceiptStatus { Ok, Partial, Reject }` enum           | RFC-0959-ask §Data Structures (future `ReceiptStatus` enum addition)                                                                                                                  | RFC-0016-a + future substrate amendment adding `ReceiptStatus` enum                                          |
| `ReceiptSummary` projection struct                     | RFC-0959-ask §Data Structures (`Receipt` field extensions)                                                                                                                            | RFC-0016-a + future substrate amendment + projection defined                                                 |
| `AuditFilter.subject_did: Option<Did>` ACL             | RFC-0959-ask §Data Structures (future `Receipt.subject_did` field)                                                                                                                    | RFC-0016-a + future substrate amendment adding `Receipt.subject_did` + ACL implementation                    |
| `AuditFilter.status: Vec<ReceiptStatus>` multi-valued  | RFC-0959-ask §Data Structures (future canonical `ReceiptStatus` enum)                                                                                                                 | RFC-0016-a + future substrate amendment + façade field type change                                           |
| `AuditEventKind` redaction extension                   | RFC-0012 (CLAUDE.md §Extension over enumeration pattern) (`AuditEventKind` typed-discriminator namespace; NO central-enum 4th variant per substrate `#[non_exhaustive]`)              | RFC-0016-a + future substrate amendment adding the redaction extension                                       |
| CLI-shape error variants                               | RFC-0011-a §Error Envelope (canonical `[ADD]` error envelope)                                                                                                                         | RFC-0011-a accepted                                                                                          |
| `ScrubbedAuditError` + `ScrubbedString` newtypes       | none (façade-only; defense-in-depth scrubber at output boundary)                                                                                                                      | RFC-0016-a accepted                                                                                          |
| Canonical-bytes-on-write invariant                     | RFC-0012 (parent §Module Layout) `event` (canonical bytes per `compute_chain_hash`; `verify_chain(&[AuditEvent])` operates on audit events per `crates/octo-audit-core/src/chain.rs`) | RFC-0016-a + write path + chain-link verification                                                            |
| Single-writer lock + read-stalls-while-write invariant | RFC-0012 (parent §Module Layout) `sink` (`AppendOnlyAuditSink` write lock; existing substrate lock primitive)                                                                         | RFC-0016-a + concurrent reader stall implementation                                                          |
...
**Mitigation:** Receipts are append-only; reads surface `octo_audit_core::AuditError::SinkSpecific("chain verification failed".into())` if the receipt store's internal BLAKE3 chain verification fails. Per-row `compute_chain_hash` (RFC-0012 chain-hash primitive) runs on every `get_receipt` and on the `list_receipts` walk; the substrate `verify_chain(&[AuditEvent])` operates on audit events per `crates/octo-audit-core/src/chain.rs` §`verify_chain`, NOT on receipt rows.
...
| ------------------------------------------------ | ------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Receipt-store tampering                          | Operator                                          | Modify persisted store                                          | Hide receipt rows                          | Append-only sink + chain verification + read-time per-row `compute_chain_hash` guard per §6.2.1 — list-time chain-integrity check rejects forged or tampered rows at the read boundary (the substrate `verify_chain(&[AuditEvent])` operates on audit events, not receipt rows, per `crates/octo-audit-core/src/chain.rs` §`verify_chain`) | Disk corruption mitigated                                                                                                                                           |
...
| TV-AUD-3c                  | `list_receipts(&AuditFilter { capability_root: Some(<canonical-32-bytes>), ... })`                                                     | 1000-receipt store (DEFERRED to RFC-0016-a per §6.8 — KEEP `AuditFilter` has no `capability_root` field per §6.2.4) | `Ok(filtered_by_capability_root)`                                                                                         | DEFERRED — server-side 32-byte exact-match filter per §6.2.4 (lands with future RFC-0014 substrate amendment adding `Receipt.capability_root` + RFC-0016-a façade ACL)                                          |
...
Layer direction: `octo-audit` (Layer B) → `octo-settlement` (Layer B) → `octo-settlement-core` (Layer A frozen). `octo-audit` (Layer B) → `octo-audit-core` (Layer A frozen) for the canonical façade-to-substrate hop carrying `AuditError` root re-export + function error types per §6.1 + §6.2.x signatures. These B→A edges are canonical façade-to-substrate hops per CLAUDE.md §Architectural Principles (Layer B → Layer A is the canonical substrate-dep pattern for additive-only surface; no NEW non-canonical B→A edges are introduced by RFC-0016 v2 KEEP). No reverse deps.
...
- **`AppendOnlyAuditSink::append` write path** — RFC-0016-a per §6.8 (requires future substrate amendment adding `AuditEventKind` extensions per RFC-0012 (CLAUDE.md §Extension over enumeration pattern) + canonical-bytes-on-write invariant)
...
- **Redaction via audit append** — redaction extension on `AuditEventKind` per RFC-0012 (CLAUDE.md §Extension over enumeration pattern) (typed-discriminator namespace; NOT central-enum 4th variant); row appended AFTER the target row (Layer A future amendment)
...
pub fn get_receipt(id: &u64) -> Result<octo_settlement::Receipt, octo_audit_core::AuditError> {
    // 1. Point lookup in canonical receipt store via octo-settlement Layer B façade.
    // 2. Recompute `compute_chain_hash(&canonical_bytes(&receipt))` (RFC-0012 per-row chain-hash primitive; `verify_chain(&[AuditEvent])` operates on audit events per `crates/octo-audit-core/src/chain.rs` §`verify_chain`).
    // 3. On chain integrity OK: return canonical Receipt.
