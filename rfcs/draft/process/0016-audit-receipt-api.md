...
This RFC defines the `octo-audit` Layer B façade surface for the audit receipt API at R2 acceptance. The KEEP surface is read-only and substrate-faithful to RFC-0012 (Layer A `octo-audit-core`) and RFC-0014 (Layer A `octo-settlement-core`). See §6.1 §Pre-existing Substrate table for the canonical KEEP item list (4 items + 3 PRE-EXISTING re-exports) and §6.1 §Amendment Surface for items DEFERRED to RFC-0016-a per [[deferred-vs-unspecified]].
...
4. **Canonical projection** — `list_receipts` returns `Vec<u64>` of canonical `Receipt::receipt_id` values (canonical projection at KEEP; the bare `u64` IS the canonical form per RFC-0012/0014 acceptance pattern). Forward-pointers to RFC-0016-a live in §6.7.
...
Seven substrate items shipped on `next` HEAD per the cited commits (4 KEEP additive items + 3 PRE-EXISTING re-exports):
...
| 3   | `pub fn audit_home() -> Result<PathBuf, AuditError>`                               | §audit_home (octo-audit §receipt_read)            | Bare `pub fn`; resolves `<OCTO_HOME>/audit/receipts` when set, default `<HOME>/.config/octo/audit/receipts` when `OCTO_HOME` unset; when both unset, substrate falls back to `./.config/octo/audit/receipts` per §audit_home substrate doc-block                              |
...
| 7   | `pub fn insert_receipt(receipt: Receipt)` (PRE-EXISTING root re-export)            | §octo-audit::lib (root re-export) §insert_receipt | Test-fixture write helper; substrate-canonical signature is bare unit return (substrate swallows mutex failures via `if let Ok(...)` per §insert_receipt substrate doc-block); NOT invoked by KEEP read path (out of KEEP scope; available for tests + future write missions) |
...
- Per §Adversary Analysis row "canonical-path info leak": path-string leak mitigated by facade-side scrubber at §octo-audit::scrub (facade scrubber) (SinkSpecific payload scrubbed defense-in-depth per RFC-0012 §S5.1).
...
- `SinkSpecific(String)` payload is NOT facade-side scrubbed at construction time at KEEP (the `octo-audit` Layer B scrubber at §octo-audit::scrub (facade scrubber) per RFC-0012 §S5.1 canonical scrubber table applies to CLI error redaction at the CLI boundary, NOT to internal `SinkSpecific` construction at the substrate façade). Accepted residual at RFC-0016 KEEP; the CLI redactor downstream scrubs the payload before operator-visible output per the canonical scrubber contract.
...
### §6.7 Forward Pointer to RFC-0016-a

All rows below require RFC-0016-a + paired future Layer A substrate amendments per §Dependencies. The following DEFERRED surface ships with `0016-a-audit-receipt-write-path.md`. None of these features is provided by any currently Accepted substrate amendment; the §6.7 column cites the canonical substrate RFC (RFC-0012 (Extension-over-enumeration pattern) or RFC-0959-ask §Data Structures) where the future amendment will land:
...
| `AuditEventKind` redaction extension                   | RFC-0012 (Extension-over-enumeration pattern) (`AuditEventKind` typed-discriminator namespace; the §6.1 row 7 `AgentTransition` cfg-gated central-enum variant is the transitional paired-acceptance bridge — it becomes a permanent Extension-over-enumeration feature at RFC-0012 paired-acceptance)  |
...
4. **Multi-tenant deployment restriction** — RFC-0016 reads assume per-process trust boundary at R2 KEEP. `AuditFilter.subject_did` ACL DEFERRED to RFC-0016-a per §6.7.
...
| Read-during-write race                           | Concurrent reads                                  | Read inconsistency                                              | Data inconsistency                         | No write path at R2 KEEP (DEFERRED to RFC-0016-a per §6.7); reads today operate against canonical store with no writer present                      | R2 surface is read-only; risk lands with RFC-0016-a acceptance                                                                   |
...
4. **Substrate-side scrubber defense-in-depth** — per §6.2.5 (the `octo-audit/src/scrub.rs` module exists at the façade for future use, but KEEP-time `SinkSpecific` construction is NOT scrubbed at construction; only the CLI redactor downstream scrubs before operator-visible output per the canonical scrubber contract); `ScrubbedAuditError` newtype surface DEFERRED to RFC-0016-a per §6.1 §Amendment Surface.
...
| TV-AUD-11g               | **DELETED VECTOR** — substrate-canonical `audit_home` has no error path (returns `Ok(PathBuf)` directly for both `OCTO_HOME` set and unset cases per §audit_home (octo-audit §receipt_read) substrate verification); the "canonical placeholder substituted" mechanism does NOT exist in substrate. Removed from RFC-0016 KEEP scope. |
...
- `crates/octo-audit/Cargo.toml` — pre-existing `octo-settlement = { path = "../octo-settlement" }` carried for `verify_receipt_chain` re-export only (Layer B settlement façade per RFC-0014; Layer B → Layer B hop, no direct B→A edge per CLAUDE.md §Architectural Principles; the B→A hop lives in `octo-settlement/Cargo.toml`). NO new feature flag — the pre-existing `octo-audit-internal` feature in `Cargo.toml` gates only the DOMAIN tests, NOT the bare `pub fn audit_home` per §6.2.3 substrate-canonical visibility.
...
Layer direction: `octo-audit` (Layer B) → `octo-settlement` (Layer B) → `octo-settlement-core` (Layer A frozen) for the OPT-IN `verify_receipt_chain` re-export path (NOT the KEEP read path; KEEP reads walk `RECEIPT_REGISTRY` static directly per §6.2 Substrate-faithfulness). Pre-existing direct B→A edges from `octo-audit` (Layer B): → `octo-audit-core` (Layer A frozen) for the canonical façade-to-substrate hop carrying `AuditError` root re-export + function error types per §6.1 + §6.2.x signatures; → `octo-storage-core` (Layer A frozen) — storage substrate (Database newtype + TypedStatement + AdapterAllowlist per RFC-0206) consumed by the DOMAIN `StoolapAuditSink` adapter at `crates/octo-audit/src/storage/stoolap.rs`. The §6.3 Cargo.toml proposal adds `octo-settlement` (Layer B) B→B hop on the receipt path; `octo-settlement` (Layer B) → `octo-settlement-core` (Layer A frozen) hop then carries the canonical `Receipt` re-export per §6.1. The `octo-audit` (Layer B) → `octo-settlement-core` (Layer A) hop lives in the `octo-settlement/Cargo.toml` (NOT in `octo-audit/Cargo.toml`) — the canonical B→B→A direction; no NEW non-canonical B→A edges are introduced by RFC-0016 KEEP. All B→A edges are canonical façade-to-substrate hops per CLAUDE.md §Architectural Principles (Layer B → Layer A is the canonical substrate-dep pattern for additive-only surface). No reverse deps.
...
- **No parallel abstractions** — `list_receipts` returns canonical `Receipt::receipt_id` values as `Vec<u64>` directly (no projection newtype at KEEP; `Vec<Receipt>` projection DEFERRED to RFC-0016-a per §6.7 paired-acceptance); bare `u64` for `Receipt::receipt_id` lookup (no newtype)
...
- 2026-09-13 — R2.5 substrate-first restructure per Option C split. §6.1 split into §Pre-existing Substrate (5 items + 2 PRE-EXISTING re-exports with substrate anchors) + §Amendment Surface (DEFERRED to RFC-0016-a per [[deferred-vs-unspecified]]). `audit_home` visibility drift corrected (substrate-canonical `pub fn`, NOT `pub(crate)` + `#[cfg(octo-audit-internal)]`). `AuditFilter` corrected to 5 fields including `cursor`. Validation rules dropped (substrate silently clamps `limit` to 1024). `verify_chain` claim scoped to opt-in (NOT auto-invoked by read path). `octo-settlement` façade walk claim corrected to direct static `RECEIPT_REGISTRY` walk. Multi-tenant restriction restatement collapsed (4× → 1×). RFC-0012 §S5.1 scrubber pin retained. RFC-0011-a version pins dropped.
