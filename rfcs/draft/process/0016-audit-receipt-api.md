...
> **R4.5 scope-cut note (GATED-FEATURE-FLAG KEEP):** KEEP items 1, 4, and 6 reference substrate-faithful types only when the `deferred-rfc-0014-v2` feature flag is active. Without the flag, `StatusRef` / `ReceiptSummary` / `AuditFilter::status` slots are inert. The canonical substrate read path remains `get_receipt(id)` (point lookup, not feature-gated, returns canonical `octo_settlement_core::Receipt` per RFC-0014 §Data Structures).
...
/// RFC-0014 §Data Structures / `crates/octo-settlement-core/src/receipt.rs`
/// §`Receipt` struct); the 32-byte BLAKE3 digest referenced by RFC-0011-a
...
pub fn append_audit_event(event: AuditEvent) -> Result<ReceiptId, AuditError>;
```

- **Append semantics** (planned, post-RFC-0012-v2) — `&mut self` requirement per `AppendOnlyAuditSink`; the chain-hash is computed post-append and returned as `ReceiptId` (the substrate-faithful Layer B newtype per §6.2.6; CLI converts to `Hex32` at the `octo-cli` boundary via `From`) for the caller. Note: `event: AuditEvent` is consumed by value (RFC-0012-v2 contract).
...
- **`limit: Some(n)` where `n > 10000` clamps to 10000** (silently, per RFC-0011-a §7.4 substrate rule).
...
Substrate-faithful to RFC-0014 §Data Structures: the canonical primary key of `octo_settlement_core::Receipt` is `receipt_id: u64` (per `crates/octo-settlement-core/src/receipt.rs` §`Receipt` struct; hard-checked 2026-09-11). The `[u8; 32]` 32-byte BLAKE3 content digest referenced by RFC-0011-a §7.4 is a derived hash via `receipt_id_for(&Receipt) -> [u8; 32]` (RFC-0014 §Data Structures); it is NOT the primary lookup key. `get_receipt(id: &ReceiptId)` performs the canonical u64 primary-key lookup; the 32-byte content hash remains available via the substrate's `receipt_id_for(&Receipt)` function for content addressing but is not used as a reverse-lookup key.
...
pub struct ReceiptSummary {
    ...
    pub executed_at_unix: u64,     // CLI projection from canonical octo_settlement_core::Receipt::timestamp_unix (substrate-faithful per RFC-0014 §Data Structures; see Appendix A step 4 sort key); field name executed_at_unix matches the RFC-0014 §A. Domain extension enum pattern wrapper for wire-form parity.
...
pub use octo_audit_core::{compute_chain_hash, verify_chain, AppendOnlyAuditSink,
                          AuditChainError, AuditError, AuditError as CoreAuditError,
...
#[cfg(feature = "deferred-rfc-0014-v2")]
pub fn list_receipts(filter: &AuditFilter) -> Result<Vec<ReceiptSummary>, AuditError> { ... }
// Without the `deferred-rfc-0014-v2` feature flag active, `list_receipts`
// is NOT exposed at the façade boundary. The canonical substrate read
// path is `octo_settlement_core::Receipt` per RFC-0014 §Data Structures;
...
- **Read determinism** — `list_receipts` returns the same results for the same filter across runs (substrate sorts by the canonical `Receipt::timestamp_unix DESC` (RFC-0014 substrate-faithful); the DEFERRED `ReceiptSummary` projection field `executed_at_unix` (§6.2.8) is the CLI-visible sort key at projection-layer read sites) deterministically, with secondary sort by `ReceiptId` (canonical `u64`) ASC as deterministic tiebreaker per §6.2.1).
...
| `get_receipt` timing oracle on existence         | Compromised CLI                                   | Measure point-lookup latency                                    | Infer whether a specific receipt ID exists | Substrate store-size intuition (large store = flat lookup time); constant-time not guaranteed at façade boundary                               | Substrate-internal trust; size-based padding not yet implemented                                                                                    |
...
| TV-AUD-4c  | `list_receipts(&AuditFilter { limit: Some(20000) })`                                                                        | 20000-receipt store                                                              | `Ok(vec_of_10000_summaries)`                                                                                                                 | limit silent clamp per §6.2.5 (TV-AUD-4c extends §6.2.5 coverage for limit > 10000 silent clamp)                   |
...
| TV-AUD-6   | `get_receipt(&known_id)`                                                                                                    | Known `ReceiptId(u64)`                                                           | `Ok(<full canonical Receipt>)`                                                                                                               | TV-RFC-0011-a-`audit-show-success` (returns `octo_settlement_core::Receipt` per §6.2.2)                            |
| TV-AUD-7   | `append_audit_event(AgentTransition row)`                                                                                   | Valid event                                                                      | `Ok(<ReceiptId>)` + chain-row inserted                                                                                                       | **DEFERRED** per §6.9 — RFC-0012-v2 acceptance required; signature takes `event: AuditEvent` by value              |
...
pub fn append_audit_event(event: AuditEvent) -> Result<ReceiptId, AuditError> {
    ...
    // 6. Return ReceiptId (the chain-hash wrapped in the §6.2.6 Layer B newtype) to caller;
    //    CLI converts to Hex32 at the octo-cli boundary via From.
}
