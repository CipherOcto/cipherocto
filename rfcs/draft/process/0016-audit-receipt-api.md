...
| 7   | `pub fn insert_receipt(receipt: Receipt) -> Result<(), AuditError>` (PRE-EXISTING root re-export) | §octo-audit::lib (root re-export) §insert_receipt | Test-fixture write helper; substrate-canonical signature carries canonical `Receipt` payload; NOT invoked by KEEP read path (out of KEEP scope; available for tests + future write missions) |
...
| TV-AUD-11g               | **DELETED VECTOR** — substrate-canonical `audit_home` has no error path (returns `Ok(PathBuf)` directly for both `OCTO_HOME` set and unset cases per §audit_home (octo-audit §receipt_read) substrate verification at `crates/octo-audit/src/receipt_read.rs:143`); the "canonical placeholder substituted" mechanism does NOT exist in substrate. Removed from RFC-0016 KEEP scope. |
