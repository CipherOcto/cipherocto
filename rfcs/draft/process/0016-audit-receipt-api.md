...
- **Return semantics** — empty `Vec` on no-match (NOT an error); summaries sorted by `executed_at_unix DESC` deterministic across runs, with secondary sort by `ReceiptId` (canonical `u64`) ASC as deterministic tiebreaker for entries sharing the same `executed_at_unix` (substrate-faithful ordering per §6.7 determinism requirement); `ReceiptSummary` is the projection subset (DEFERRED to RFC-0014-v2 acceptance per §6.9).
...
| ----------------------------------- | ------------------------------------- | -------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `AuditError::ReceiptNotFound(hex)`  | `OctoCliError::ReceiptNotFound(hex)`  | 17       | RFC-0011-a §Error Handling | `hex` is canonical decimal `u64` form of the substrate `Receipt::receipt_id` (per §6.2.2 + §6.2.6); redactor-clean (no secret material). |
...
- **Read determinism** — `list_receipts` returns the same results for the same filter across runs (substrate sorts by `executed_at_unix DESC` deterministically, with secondary sort by `ReceiptId` (canonical `u64`) ASC as deterministic tiebreaker per §6.2.1).
...
| TV-AUD-11a | `Internal("BLS12-381 scalar hex: 0x4f6c8b2a9d3e1c5a7f8b6c9d2e4a1f3b8c7d5e9a2f6b4c8d1e3a7f9b2c5d8e4a1f6b9c2d5e8a1f4b7c9d2e5a8f1b4c7d9e2a5f8b1c4d7e9a2f5b8c1d4e7a9f2b5c8d1e4a7f9b2c5d8e4a6f9b3c6d9e2a5f8b1c4d7e9a2f5b8c2e9")` | 96-char hex scalar                                                               | `Err(AuditError::Internal("BLS12-381 scalar hex: <REDACTED>"))` — contains `<REDACTED>`; original 96-char scalar absent                      | Redaction coverage for BLS12-381 scalar hex                                                                |
| TV-AUD-11b | `Internal("secp256k1 privkey: 0xa1b2c3d4e5f60718293a4b5c6d7e8f90a1b2c3d4e5f60718293a4b5c6d7e8f90a1b2")`                                                                                                                     | 64-char hex privkey                                                              | `Err(AuditError::Internal("secp256k1 privkey: <REDACTED>"))` — contains `<REDACTED>`; original 64-char hex absent                            | Redaction coverage for secp256k1 64-char hex                                                               |
