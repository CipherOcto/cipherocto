...
graph TD
    ...
    OwnerCrates["Owner-trait crates<br/>(octo-ident, octo-cap-macaroon,<br/>cipherocto-policy, octo-vault,<br/>octo-reputation,<br/>octo-matrix-session-store)<br/>DEFINE owner traits<br/>consumed by adapters via Facade"]
    Facade -. re-exports adapter impls .-> OwnerCrates
...
graph LR
    ...
    AdapterCrates --> Core["octo-storage-core/<br/>(Layer A substrate<br/>consumed by adapters)"]
    AdapterCrates --> OwnerTraits["Owner-trait crates<br/>(each defines ONE trait<br/>implemented BY the adapter)"]
```
...
- **Cross-adapter data leak** — adapter A reads adapter B's table without permission. **Trust model (v1.7 explicit):** this is process-internal only — no untrusted input ever reaches the `register(Arc<Database>)` constructor; cross-owner table access is controlled by **per-adapter typed-query allowlist** (forward requirement — Phase 2 Task 5/6/7). Until per-adapter RFCs land, every adapter holding `Arc<Database>` has the ability to issue arbitrary SQL against any other adapter's tables (audit doc L97 cross-adapter leak surface caveat). The `register(Arc<Database>) -> Arc<dyn OwnerTrait>` signature may in future accept a `&AdapterCapability` token signed by the per-owner maintainer (RFC-0206 §Future Work); until that lands, document the trust model explicitly and rely on per-adapter code review + TV-0206-A9 runtime guard (typed-query allowlist) to bound cross-adapter blast radius.
...
13. **TV-0206-A6:** Owner-trait crates in migration status (audit `docs/audits/octo-storage-trait-surface-2026-08-19.md` §On-disk migration status at drafting time): **0 of 7** use the substrate today; **2 of 7** are PARTIAL adopters — `octo-vault` (5-7 lines per crate) + `octo-reputation` (5-7 lines per crate) — per the trait-surface audit. The other 5 (`octo-ident`, `octo-cap-macaroon`, `cipherocto-policy`, `octo-cap-macaroon-vault`, `octo-matrix-session-store`) have 0 substrate uses today. Phase 3 Task 10 90-day migration window migrates the 5 unused + completes the 2 partial adopters to consume via `octo_storage_core::Database` exclusively. Implemented as `rg -c 'octo_storage_core::' crates/octo-{ident,cap-macaroon,reputation,cap-macaroon-vault,matrix-session-store}/src/ crates/cipherocto-policy/src/ crates/octo-vault/src/ 2>/dev/null` returns 0 today (target after Phase 3 Task 10: ≥ 1 across all 7 crates). Sanity check that the substrate handle is consumed everywhere.
