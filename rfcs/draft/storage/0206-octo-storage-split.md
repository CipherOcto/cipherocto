...
3. **Adapter crate owners** — one per owner-trait. `HolderRegistry`, `DidRegistry`, `VaultLookup`, `ReputationStore`, `SessionStore` traits ALREADY exist on disk (per workspace audit `docs/audits/octo-storage-trait-surface-2026-08-19.md`): `ReputationStore` lives in `octo-reputation`, `VaultLookup` lives in `octo-cap-macaroon-vault`, `SessionStore` lives in `octo-matrix-session-store`. `PolicyStore` and `VaultStore` are **NEW traits introduced in this restructure cycle** (no `pub trait` declaration in their owner crates today — `crates/cipherocto-policy/src/lib.rs` has 0 traits, `crates/octo-vault/src/lib.rs` has 0 traits). `HolderRegistry` is **moved** from `crates/quota-router-storage/src/holder_registry.rs` to `crates/octo-cap-macaroon/src/` (this is an RFC-major crate-move per §Compatibility Backward; gated on the cap-macaroon adapter RFC). `DidRegistry` is **moved** from `crates/quota-router-storage/src/stoolap_did_registry.rs` to `crates/octo-ident/src/`. Each adapter owner files a separate RFC per §Promotion Path Condition 4.
...
graph LR
    subgraph AdapterCrates["Per-owner adapter crates (Layer B)"]
        ...
        MSS["octo-matrix-session-store/<br/>→ impl SessionStore (NEW)"]
    end
...
6. **TV-0206-06:** CI grep rejects any `stoolap::*` direct construction in owner-trait crates (must go through `octo-storage-core`). Implemented as `! rg '\bstoolap::' crates/octo-{ident,cap-macaroon,reputation,cap-macaroon-vault,matrix-session-store}/src/ crates/cipherocto-policy/src/ crates/octo-vault/src/` in CI. Pattern uses `\b` word boundary so `use stoolap::Database` AND `stoolap::Database::open()` AND `stoolap::Rows` all match; `stoop::stoolap::xxx` would NOT match (correct — that crate is unrelated). Crate list in the pattern matches the seven adapter-supported owner crates per §Adapter Crate List (Initial); extend the pattern when a new adapter crate ships. **`crates/quota-router-storage/src/` is intentionally NOT in the pattern** — it is the Layer B substrate for the quota-router domain (sibling role to `octo-storage-core` but for quota-router), and constructs `stoolap::*` directly by design (same as `octo-storage-core`); the lint exists to catch owner-trait crates bypassing the substrate, not substrate crates themselves. (Note: `crates/octo-market/` does not exist on disk today; `crates/octo-policy/` was renamed to `crates/cipherocto-policy/` per workspace audit — the pattern above matches on-disk crate names. `octo-market-storage/` is deferred per plan §4.2 B.4; once shipped, extend the pattern.)
   **Migration status (audit at drafting time):** of the 7 owner-trait crates in scope, **3 have already migrated** to using the `octo-storage-core` re-exported handle (`octo-ident`, `octo-cap-macaroon`, `cipherocto-policy`); **4 still carry direct `stoolap` deps** (`octo-reputation`, `octo-cap-macaroon-vault`, `octo-matrix-session-store`, `octo-vault`) and will be migrated by Phase 3 Task 10's 90-day window. The CI grep will only enforce once Phase 3 Task 10 lands for the remaining 4 — until then, the lint's role is to prevent new direct `stoolap::*` references, not to force-migrate the legacy 4.
