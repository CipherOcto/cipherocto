#!/usr/bin/env bash
# scripts/persist-contacts-from-csv.sh
#
# Read a CSV file (typically the dump-table output) and call
# contacts.save_contact for each row. Skips rows where the phone
# column matches a configurable admin phone. Pick which columns to
# use via args; the CSV is parsed with python's csv.DictReader so the
# header row drives the mapping.
#
# Usage:
#   scripts/persist-contacts-from-csv.sh /tmp/liberdade_details.csv
#   scripts/persist-contacts-from-csv.sh /tmp/liberdade_details.csv member_phone verified_name
#   scripts/persist-contacts-from-csv.sh /tmp/foo.csv phone_col name_col 5521995544743@s.whatsapp.net
#   scripts/persist-contacts-from-csv.sh --no-strict /tmp/foo.csv phone_col name_col
#
# Args:
...
#                 table would produce — see EXPECTED_DETAILS_COLUMNS
...
# === Expected footprint of a `*_details` table ============================
# The schema is created by persist-member-detail-flex.sh; dump-table.sh
# emits `SELECT *` so the CSV must contain every one of these columns.
# Strict mode (default ON) refuses to run if any are missing — that's a
# sign the CSV came from a different table (e.g. group_members, liberdade)
# and the operator picked the wrong file.
...
ADMIN_PHONE="5521995544743@s.whatsapp.net"
...
if [ "$STRICT" = "1" ]; then
    missing=()
    for col in "${EXPECTED_DETAILS_COLUMNS[@]}"; do
        # Match exact or trailing-comma / leading-comma token
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        echo "  footprint mismatch: CSV is missing ${#missing[@]} expected details columns:" >&2
        for c in "${missing[@]}"; do
            echo "    - $c" >&2
        done
    ...
    echo "  footprint OK: all ${#EXPECTED_DETAILS_COLUMNS[@]} details columns present" >&2
fi
...
echo "→ reading $CSV_FILE (phone=$PHONE_COL name=$NAME_COL admin=$ADMIN_PHONE)" >&2
ROWS_TMP=$(mktemp)
python3 - "$CSV_FILE" "$PHONE_COL" "$NAME_COL" "$ADMIN_PHONE" > "$ROWS_TMP" <<'PY'
import csv, sys
...
with open(csv_path, newline='') as f:
    ...
    for row in rdr:
        ...
        # The CSV usually stores the phone in '@s.whatsapp.net' form;
        # pass through as-is — contacts.save_contact accepts it.
        # Fall back to the digits if the cell is missing the suffix.
        if '@' not in phone:
            phone = f'{phone}@s.whatsapp.net'
        print(f'{phone}\t{name}')
print(f'# raw={raw} unique={len(seen)}', file=sys.stderr)
PY
...
echo "  enumerated $COUNT unique phones" >&2
...
while IFS=$'\t' read -r phone name; do
    ...
    # Skip admin
    if [ "$phone" = "$ADMIN_PHONE" ]; then
        wa_log "[$idx/$COUNT] skip admin phone=$phone"
        skipped=$((skipped+1))
        continue
    fi
...
echo "  admin_phone = $ADMIN_PHONE" >&2
